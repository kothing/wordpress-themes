...
if ( ! class_exists( 'ANGI_Model' ) ) :
  class ANGI_Model {
    ...
    function __construct( $model = array() ) {
          ...
          $_preset = $this -> angi_fn_get_preset_model();
          ...
          $model = $this -> angi_fn_extend_params( $model );


          if ( empty( $model ) ) {
            do_action('angi_dev_notice', 'in ANGI_MODEL construct : a model has no id ');
            return;
          } elseif ( FALSE == $model['id'] ) {
            //if model ID has been set to false => silent exit. Useful in cases when in angi_fn_extend_params the model
            //itself understands that it has to exit its instantiation
            $ANGI -> collection -> angi_fn_delete( $this -> id );
            return;
          }


          //inside will make the equivalent of wp_parse_args() with default model property values
          $this -> angi_fn_update( $model );

          //at this stage the mode must at least have :
          //1) a unique id
          //2) a priority set
          //3) a hook => not anymore since angi_fn_render_template()
          if ( ! $this -> angi_fn_can_model_be_instantiated() ) {
            $ANGI -> collection -> angi_fn_delete( $this -> id );
            return;
          }

          //maybe alter body class
          if ( method_exists( $this, 'angi_fn_body_class' ) )
            add_filter( 'body_class', array( $this, 'angi_fn_body_class' ) );

          //maybe add style
          $this -> angi_fn_maybe_add_style();

          //a way to allow models to act on their view
          $this -> angi_fn_add_view_pre_and_post_actions();

          //Allow models to filter their view visibility
          add_filter( "angi_do_render_view_{$this -> id}", array( $this, 'angi_fn_maybe_render_this_model_view' ), 0 );

          //adds the view instance to the model : DO WE REALLY NEED TO DO THAT ?
          //view instance as param
          add_action( "view_instantiated_{$this -> id}"   , array( $this, 'angi_fn_add_view_to_model'), 10, 1 );

          //takes the view instance as param
          add_action( "view_instantiated_{$this -> id}"   , array( $this, 'angi_fn_maybe_hook_or_render_view'), 20, 1 );


          //Maybe instantiate the model's view
          //listens to 'wp' if not fired yet, or fire the instantiation
          if ( ! did_action('wp') )
            add_action( 'wp'                                , array( $this, 'angi_fn_maybe_instantiate_view' ), 999 );
          else
            $this -> angi_fn_maybe_instantiate_view();
    ...
    function angi_fn_add_view_to_model( $view_instance ) {
          $this -> angi_fn_set_property( 'view_instance', $view_instance );
    }
    ...
    ***********************************************************************************/
    ...
    //@return void()
    public function angi_fn_maybe_instantiate_view() {
          ...
          if ( ! $ANGI -> controllers -> angi_fn_is_possible($this -> angi_fn_get_model_as_array() )  )
            return;

          //instantiate the view with the current model object as param
          $view_instance = new ANGI_View( $this );
    }//fn
    ...
    ***********************************************************************************/
    public function angi_fn_maybe_add_style() {
          //for now just add filter to angi_fn_user_options_style
          if ( method_exists( $this, 'angi_fn_user_options_style_cb' ) )
            add_filter( 'angi_user_options_style', array( $this, 'angi_fn_user_options_style_cb' ) );
    }//fn
    ...
    ***********************************************************************************/
    //hook : 'view_instantiated'
    //@param $instance is the view instance object, can be ANGI_View or a child of ANGI_View
    //hook the rendering method to the hook
    //$this -> view_instance can be used. It can be a child of this class.
    public function angi_fn_maybe_hook_or_render_view( $instance )  {
          if ( empty( $this -> id ) ) {
              do_action('angi_dev_notice', 'In ANGI_Model, a model is missing its id.' );
              return;
          }

          //Are we in angi_fn_render_template case
          ...
          //if not check if template_redirect has already been fired, to see if we are in a angi_fn_render case
          ...
          //? using the angi_fn_set_property ( 'hook' , 'value' ) could also be an option in an extended model ?
          ...
          add_action( $_da_hook, array( $instance , 'angi_fn_maybe_render' ), $_da_priority );
    ...
    //@param instance view
    //@return void()
    public function angi_fn_unhook_view() {
          if ( false == $this -> hook || ! is_object( $this -> view_instance) )
            return;
          remove_action( $this -> hook, array( $instance , 'angi_fn_maybe_render' ), $this -> priority );
          //say it
    ...
    ***********************************************************************************/
    ...
    //@return void()
    public function angi_fn_set_property( $property, $value ) {
          $this -> $property = $value;
    ...
    //normalizes the way we can access and change a single model property
    //@return the property
    public function angi_fn_get_property( $property, $args = array() ) {
          if ( method_exists( $this, "angi_fn_get_{$property}" ) ) {
              return call_user_func_array( array($this, "angi_fn_get_{$property}"), $args );
          }
          return isset ( $this -> $property ) ? $this -> $property : '';
    }

    //@returns the model property as an array of params
    public function angi_fn_get_model_as_array() {
          $model = array();
    ...
    //@return array()
    protected function angi_fn_get_preset_model() {
      return array();
    }
    ...
    //is fired on instantiation
    //@param = array()
    protected function angi_fn_extend_params( $model = array() ) {
    ...
    //is fired on instantiation
    //@param = array()
    public function angi_fn_update( $model = array(), $merge_defaults = true ) {
          /*
    ...
    */
    public function angi_fn_reset_to_defaults () {
          $this -> angi_fn_update( $this->defaults );
    }
    ...
    * => THE POSSIBLE VIEW CLASS IS NOW INSTANCIATED
    ***********************************************************************************/
    //@hook angi_do_render_view_{$this -> id}
    //@return bool
    // Controls the rendering of the model's view
    public function angi_fn_maybe_render_this_model_view() {
    ...
    protected function angi_fn_add_view_pre_and_post_actions() {

          //by default filter this module before rendering (for default properties parsing, e.g. element_class )
          add_action( "pre_rendering_view_{$this -> id}", array($this, "angi_fn_pre_rendering_my_view_cb" ), 9999 );

          //by default filter this module before rendering (for default properties parsing, e.g. element_class )
          add_action( "post_rendering_view_{$this -> id}", array($this, "angi_fn_post_rendering_my_view_cb" ), 9999 );
    }
    ...
    */
    public function angi_fn_pre_rendering_my_view_cb( $model ) {
          if ( method_exists( $this, 'angi_fn_setup_late_properties' ) )
            $this -> angi_fn_setup_late_properties();

          $this -> angi_fn_sanitize_model_properties( $model );
    }

    /*
    * After rendering this model view allow the reset (e.g. in the loops)
    */
    public function angi_fn_post_rendering_my_view_cb( $model ) {
          if ( method_exists( $this, 'angi_fn_reset_late_properties' ) )
            $this -> angi_fn_reset_late_properties();

    }

    protected function angi_fn_sanitize_model_properties( $model ) {
          $this -> element_class  = $this -> angi_fn_stringify_model_property( 'element_class' );
    }
    ...
    * HELPERS
    ***********************************************************************************/
    protected function angi_fn_stringify_model_property( $property ) {
          if ( isset( $this -> $property ) )
           return angi_fn_stringify_array( $this -> $property );
          return '';
    }
    ...
    //1) a unique id
    //2) a priority set
    private function angi_fn_can_model_be_instantiated() {
      //the model must be an array of params
      //the hook is the only mandatory param => not anymore since angi_fn_render_template()
      if ( ! is_numeric( $this -> priority ) || empty($this -> id) ) {
        do_action('angi_dev_notice', "In ANGI_Model class, a model instantiation aborted. Model is not ready for the collection, it won't be registered. at this stage, the model must have an id, a hook and a numeric priority." );
        return;
      }
      return true;
    }
    ...
    //@return bool
    public function angi_fn_has_instantiated_view() {
...
if ( ! class_exists( 'ANGI_Collection' ) ) :
  class ANGI_Collection {
    static $instance;
    ...
    function __construct( $args = array() ) {
      self::$instance =& $this;
      //listens to filter 'angi_prepare_model', takes 1 param : raw model array()
      //makes sure the model has a unique $id set and a proper priority for its rendereing hook
      //model as param
      add_filter( 'angi_prepare_model'            , array( $this, 'angi_fn_set_model_base_properties'), 10, 1 );

      //if 'wp' has not been fired yet, we will pre-register this model for later instantiation
      //2 params :
      //1) model id
      //2) model instance
      add_action ('pre_register_model'          , array( $this, 'angi_fn_pre_register_model'), 10, 2 );
      ...
      //1) model id
      //2) model instance
      add_action( 'model_alive'                 , array( $this, 'angi_fn_update_collection' ), 10, 2 );

      //on 'wp', the pre_registered (if any) are registered
      add_action( 'wp'                          , array($this, 'angi_fn_register_pre_registered') );

      //Reacts on 'angi_delete' event
      //1 param = model id
      add_action( 'angi_delete'                   , array( $this, 'angi_fn_delete' ), 10, 1 );

      //listens to a model changed => update the model collection
      //model_property_changed takes two params :
      //model id
      //model params
      add_action( 'model_property_changed'      , array( $this, 'angi_fn_update_collection' ), 10, 2 );

      //listens to a registered change applied to a model => remove it from the register changes list
      //takes one param : model id
      add_action( 'registered_changed_applied'  , array( $this, 'angi_fn_deregister_change' ), 10, 1 );

      //reacts when a model has been deregistered from the collection
      //=> fire angi_fn_delete()
      //=> take the model id as param
      add_action( 'model_deregistered'          , array( $this , 'angi_fn_delete'), 10, 1 );
    }
    ...
    ***********************************************************************************/
    ...
    public function angi_fn_register( $model = array() ) {
      ...
      if ( ! $this -> angi_fn_is_model_eligible( $model ) )
        return;
      ...
      $model = apply_filters( 'angi_prepare_model' , $model );
      ...
      if ( ! $this -> angi_fn_can_register_model( $model ) )
        return;
      ...
      $model = $this -> angi_fn_instantiate_model( $model );

      //Silent aborting for those models which "decided" in their constructor they're not allowed to be registered
      if ( isset( $model -> id ) && $this -> angi_fn_has_registered_deletion( $model -> id ) )
        return;

      //abort if the model has not been instantiated
      if ( ! is_object( $model ) ) {
        $_model_id = isset( $model -> id ) ? $model -> id : 'undefined';
        do_action('angi_dev_notice', "The model ( " . $_model_id . ") was not instantiated and could not be registered into the collection." );
        return;
      }
      ...
      if ( $this -> angi_fn_is_registered( $model -> id ) ) {
        ...
        do_action('angi_dev_notice', "A model instance ( " . $model -> id . ") was not registered into the collection." );
        return;
    ...
    * BEFORE REGISTRATION
    ***********************************************************************************/
    //hook : 'angi_can_use_model'
    //Check if the model is registered for deletion first
    //the model must be an array of params
    //the hook is the only mandatory param => not anymore since angi_fn_render_template
    //the id is optional => will be set unique on model instantiation
    public function angi_fn_is_model_eligible( $model = array() ) {
      //is model registered for deletion ?
      if ( isset( $model['id'] ) && $this -> angi_fn_has_registered_deletion( $model['id'] ) )
        return;

      if ( ! is_array($model) || empty($model) ) {
        do_action('angi_dev_notice', "ANGI_collection : A model is not eligible for the collection, it won't be registered. The model must be an array of params." );
        return;
      }
      return true;
    }
    ...
    //hook filter 'angi_prepare_model' in angi_fn_register
    //@param model array
    //@return model array() updated
    public function angi_fn_set_model_base_properties( $model = array() ) {
      ...
      if ( isset($model['hook']) && ! empty($model['hook']) && false != $model['hook'] ) {
        ...
        $model['priority']  = $this -> angi_fn_set_priority( $model['hook'] , $priority );
      } else {
        $model['priority'] = 10;
        $model['hook'] = "";
      }
      //check or set the name unicity
      $model['id']        = $this -> angi_fn_set_unique_id( $id , $model['hook'], $model['priority'], $template );

      //don't go further if we still have no id set
      if ( ! $model['id'] ) {
        do_action('angi_dev_notice', "A model has no id set." );
        return;
      }
      ...
      if ( $this -> angi_fn_is_registered( $model['id'] ) ) {
        do_action('angi_dev_notice', "ANGI_Collection. Model : ". $model['id'] ." . The id is still not unique. Not registered." );
        return;
      }
      return $model;
    }
    ...
    private function angi_fn_can_register_model( $model = array() ) {
      $bool = false;
      $ANGI  = ANGI();
      ...
      //if 'wp' has been fired (or is currently being fired) 1) check the controller if set
        ...
        $bool = $ANGI -> controllers -> angi_fn_is_possible( $model );
      }
      return apply_filters('angi_can_register_model', $bool, $model );
    }
    ...
    ***********************************************************************************/
    ...
    //@param id string
    //@param model array
    function angi_fn_pre_register_model( $id, $model = array() ) {
      ...
      if ( isset($pre_registered[$id]) ) {
        do_action('angi_dev_notice', "Model " . $id . " has already been pre-registered." );
        return;
      }
    ...
    function angi_fn_remove_pre_registered($id) {
      $pre_registered = self::$pre_registered;
      if ( isset($pre_registered[$id]) )
    ...
    //hook : wp
    //@return void()
    function angi_fn_register_pre_registered() {
      foreach ( self::$pre_registered as $id => $model ) {
        //removes from the pre_registered list
        $this -> angi_fn_remove_pre_registered($id);
        //registers
        $this -> angi_fn_register($model);
      }
      //say it to the api
    ...
    ***********************************************************************************/
    ...
    public function angi_fn_instantiate_model($model) {
      $instance = null;
      ...
      //b) Also if a whatever model has been instantiated it must be a subclass of ANGI_Model - otherwise a notice will be fired.
      ...
      foreach ( array( 'model_class', 'template' ) as $_model_class ) {
        ...
        if ( 'model_class' == $_model_class && is_array( $model[ 'model_class' ] ) && array_key_exists( 'name', $model['model_class'] ) ) {
          ...
          if ( ! class_exists( sprintf( 'ANGI_%s_model_class', $model['model_class']['parent'] ) ) ) {
            $this -> angi_fn_require_model_class( $model['model_class']['parent'] );
          }
        ...
        $model_class_name     = sprintf( 'ANGI_%s_model_class', basename( $model_class ) );

        if ( ! class_exists($model_class_name) ) {
          $this -> angi_fn_require_model_class( $model_class );
        }
        ...
        if ( ! is_object($instance) && 'model_class' == $_model_class ) {
          do_action('angi_dev_notice', "Model : " . $model['id'] . ". The model has not been instantiated." );
          return;
        }
        //A model must be ANGI_model or a child class of ANGI_model.
        if ( is_object($instance) && ! is_subclass_of($instance, 'ANGI_Model') ) {
          do_action('angi_dev_notice', "Model : " . $model['id'] . ". View Instantiation aborted : the specified model class must be a child of ANGI_Model." );
          return;
        } else break;
      }//end foreach

      if ( ! is_object( $instance ) )
        return new ANGI_Model( $model );
    ...
    //the boolean is required. If false, the theme will fallback on the ANGI_Model base class
    private function angi_fn_require_model_class( $_model_class ) {
        ...
        //this filter is NOT used in the angilla theme
        //=> its intended to be used in plugins / addons, like pro grid for ex.
        $path = apply_filters(
            "angi_model_class_path",
            false,
            $model_class_basename,
            $model_path
        );

        //If not filtered ( normal case in angilla ), use the default path prefix + model path
        if ( ! $path || ! file_exists( $path ) ) {
          return  ANGI() -> angi_fn_require_once(
              ANGI_PHP_FRONT_PATH . $model_path
          );
        }
    ...
    ***********************************************************************************/
    ...
    //=> when a single model property has been changed in ANGI_Model::angi_fn_set_property()
    //@param id string
    //@param $model instance object
    public function angi_fn_update_collection( $id = false, $model ) {
      if ( ! $id || ! is_object($model) )
        return;

      //Check if we have to run a registered deletion here
      if ( $this -> angi_fn_is_registered( $id ) && $this -> angi_fn_has_registered_deletion( $id ) ) {
        do_action( 'angi_delete' , $id );
        return;
      }
    ...
    ***********************************************************************************/
    ...
    public function angi_fn_deregister( $id, $model ) {
      if ( ! is_array($model) )
        return;

      //Removes the previously set action
      if ( ! is_object($model['view_instance']) )
        do_action('angi_dev_notice', 'Attempt to de-register, but no model instance for id : '. $id );
      else if ( ! empty( $model['hook'] ) )
        remove_action( $model['hook'], array( $model['view_instance'], 'angi_fn_maybe_render'), $model['priority'] );
    ...
    ***********************************************************************************/
    ...
    //IMPORTANT : always use the ANGI_Collection::$instance -> _models property to access the model list here
    //=> because it can be accessed from a child class
    public function angi_fn_delete( $id = null ) {
      if ( is_null($id) )
        return;
      ...
      if ( isset($collection[$id]) ) {
        ...
        $this -> angi_fn_deregister_deletion($id);
        //Emit an event on model deleted
        do_action( 'model_deleted' , $id );
      }
      else
        $this -> angi_fn_register_deletion( $id );
      return;
    }


    private function angi_fn_deregister_deletion($id) {
      $to_delete = self::$_delete_candidates;
      if ( $this -> angi_fn_has_registered_deletion($id) )
        unset($to_delete[$id]);
      self::$_delete_candidates = $to_delete;
    }


    private function angi_fn_register_deletion($id) {
      $to_delete = self::$_delete_candidates;
    ...
    private function angi_fn_has_registered_deletion($id) {
    ...
    ***********************************************************************************/
    ...
    public function angi_fn_get_model( $id = null ) {
      $collection = self::$collection;
    ...
    public function angi_fn_get_model_instance( $id = null ) {
      if ( is_null($id) )
        return;
    ...
    public function angi_fn_get_collection() {
    ...
    ***********************************************************************************/
    //@return bool
    private function angi_fn_is_pre_registered($id) {
    ...
    private function angi_fn_set_priority( $hook, $priority ) {
      $priority = empty($priority) ? 10 : (int)$priority;
      $available = true;
      //loop on the existing model object in the collection
      foreach ( $this -> angi_fn_get_collection() as $id => $model) {
        if ( ! isset($model -> hook) )
          continue;
        if ( $hook != $model -> hook )
          continue;
      ...
      return $available ? $priority : $this -> angi_fn_set_priority( $hook, $priority + 1 );
    }


    //Recursively create a unique id when needed
    //@return string id
    private function angi_fn_set_unique_id( $id, $hook, $priority, $template ) {
      //if id not set, then :
      ...
      if ( ! $this -> angi_fn_is_registered($id) && ! $this -> angi_fn_is_pre_registered($id) )
        return apply_filters('angi_set_model_unique_id' , $id, $hook, $priority );
      ...
      //recursive check
      return $this -> angi_fn_set_unique_id( $id, $hook, $priority, $template );
    }
    ...
    //@param string id
    //@return bool
    public function angi_fn_is_registered( $id ) {
...
?><?php
//This is the view class. Each front end part of the Angilla theme is rendered through a view instance.
...
if ( ! class_exists( 'ANGI_View' ) ) :
  class ANGI_View {
    ...
    ***********************************************************************************/
    ...
    //1) in function angi_fn_render_template( $template, $args = array() ) for an already registered model
    //  => Example : header
    //2) in ANGI_Model::angi_fn_maybe_hook_or_render_view, FIRED if did_action('template_redirect') and the 'render' model property is true
    //3) else in ANGI_Model::angi_fn_maybe_hook_or_render_view, SCHEDULED ( hooked )
    public function angi_fn_maybe_render() {
        //this event is used to check for late deletion or change before actually rendering
        //will fire angi_fn_apply_registered_changes_to_instance
        //do_action( 'pre_render_view', $this -> id );

        if ( ! apply_filters( "angi_do_render_view_{$this -> model -> id}", true, $this->model ) )
          return;

        //allow filtering of the model before rendering (the view's model is passed by reference)
        //THIS IS WHERE THE angi_fn_setup_late_properties is fired
        // With this action in the base model class constructor : add_action( "pre_rendering_view_{$this -> id}", array($this, "angi_fn_pre_rendering_my_view_cb" ), 9999 );
        ...
        if ( is_user_logged_in() ) {
            ...
            $this -> model -> element_attributes = join( ' ', array_filter( array_unique( array_merge( $this -> model -> element_attributes, array(
                'data-angi-model_id="'. $this -> model -> id .'"',
                isset( $this -> model -> template ) ? 'data-angi-template="templates/parts/'. $this -> model -> template .'"' : ''
            )))) );
        }
    ...
    public function angi_fn_render() {
        ...
        if ( ! empty( $this -> model -> template ) ) {
            $template_path = trailingslashit( apply_filters( 'angi_template_path', "templates/parts/", $this -> model -> id, $this -> model ) );
            $_template_file_path = apply_filters( 'angi_template_file_path', false, $this -> model -> template );

            if ( false === $_template_file_path || ! file_exists( $_template_file_path ) ) {
                //get the filename
                $_template_file_path = angi_fn_get_theme_file_path( "{$template_path}{$this -> model -> template}.php" );
            }

            if ( false !== $_template_file_path ) {
                angi_fn_set_current_model( $this -> model );
                ...
                angi_fn_reset_current_model();
            }
        }

        if ( ! empty( $this -> model -> callback ) )
            angi_fn_fire_cb( $this -> model -> callback, $this -> model -> cb_params );
    }
    ...
    //@return void()
    private function angi_fn_update_model_instance( $id, $new_params ) {
          //get current params
          $current_params = $this -> angi_fn_get_view($id);
          ...
          foreach ($new_params as $property => $value) {
            if ( $value != $current_params[$property] )
              $current_params['view_instance'] -> angi_fn_set_property( $property, $value );
          }
    }
    ...
    * PUBLIC HELPERS
    ***********************************************************************************/
    public function angi_fn_get_instance() {
          return $this;
    }
...
if ( ! class_exists( 'ANGI_controllers' ) ) :
  class ANGI_controllers {
    static $instance;
    private $controllers;
    ...
    function __construct( $args = array() ) {
          ...
          add_action( 'group_controller_instantiated', array( $this, 'angi_fn_store_controller_instance'),10, 2);
    }//__construct()
    ...
    ***********************************************************************************/
    ...
    public function angi_fn_is_possible( $model ) {
          ...
          $controller_cb = $this -> angi_fn_get_controller( $model );
          //FOR TEST ONLY
          //return true;

          if ( ! empty( $controller_cb ) ) {
            return apply_filters( 'angi_fn_set_control' , (bool) angi_fn_return_cb_result( $controller_cb, $model ) );
          }
          return true;
    }
    ...
    public function angi_fn_has_controller( $model = array() ) {
          $controller = $this -> angi_fn_build_controller( $model );
    ...
    //@param array() or object() model
    private function angi_fn_get_controller( $model = array() ) {
          ...
          //IS THERE A PRE-DEFINED CONTROLLER FOR THE VIEW ?
            ...
            if ( $this -> angi_fn_has_default_controller( $controller_ids ) ) {
              $controller_cb = $this -> angi_fn_get_default_controller( $controller_ids );
              //make sure the default controller is well formed
              //the default controller should look like array( instance, method )
              if ( empty($controller_cb) ) {
                do_action( 'angi_dev_notice', 'View : '.$model['id'].'. The default group controller has not been instantiated');
                return "";
              }//if
    ...
    private function angi_fn_has_default_controller( $controller_ids ) {
    ...
    private function angi_fn_get_default_controller( $controller_ids ) {
          $controller_cb = false;
          foreach ( $this -> controllers as $group => $views_id ) {
            foreach( $controller_ids as $id ) {
              if ( in_array($id, $views_id) ) {
          ...
          if ( ! $controller_cb ) {
            do_action( 'angi_dev_notice', 'View : '.$id.'. No control method found.');
            return array();
          }
          ...
          if ( ! array_key_exists($controller_group, self::$controllers_instances) ) {
            ...
            $_instance = $this -> angi_fn_instantiate_group_controller($controller_group);
          } else {
            $_instance = $this -> angi_fn_get_controller_instance($controller_group);
          }

          //stop here if still nothing is instantiated
          if ( ! isset( $_instance) || ! is_object( $_instance ) ) {
            do_action( 'angi_dev_notice', 'View : '.$id.'. The control class for : ' . $controller_group . ' has not been instantiated.' );
            return array();
          }

          //build the method name
          $method_name = "angi_fn_display_view_{$controller_cb}";//ex : angi_fn_display_view_{favicon_control}()
          ...
          do_action( 'angi_dev_notice', 'model : '.$id.'. The method : ' . $method_name . ' has not been found in group controller : '.$controller_group );
          return array();
    }
    ...
    //@return the $instance
    private function angi_fn_instantiate_group_controller( $group ) {
          $_path     = "controllers/class-controller-{$group}.php";
          $_class    = "ANGI_controller_{$group}";
          $_instance = false;
          $ANGI       = ANGI();

          $ANGI -> angi_fn_require_once( ANGI_PHP_FRONT_PATH . $_path );
    ...
    //@return void()
    public function angi_fn_store_controller_instance( $group , $_instance ) {
    ...
    public function angi_fn_get_controller_instance( $group ) {
          $controller_instances = self::$controllers_instances;
          if ( ! array_key_exists($group, $controller_instances) )
            return;
