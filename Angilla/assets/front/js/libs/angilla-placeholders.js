(function($, angiapp, _ ) {
    ...
    angiapp.ready.then( function() {
          ...
          //<aside class="tc-placeholder-wrap col-12" data-nonce_handle="027a33be64" data-nonce_id="angiHelpBlockNonce" data-dismiss_action="dismiss_widget_notice" data-position="footer">
          ...
          var _placeholder_wrapper_selector = '.tc-placeholder-wrap',
              ...
              _doAjax = function( _query_ ) {
                    var ajaxUrl = angiapp.localized.adminAjaxUrl, dfd = $.Deferred();
                    $.post( ajaxUrl, _query_ )
                          .done( function( _r ) {
                                // Check if the user is logged out.
                                if ( '0' === _r ||  '-1' === _r )
                                  angiapp.errorLog( 'placeHolder dismiss : ajax error for : ', _query_.action, _r );
                          })
                          .fail( function( _r ) {
                                angiapp.errorLog( 'placeHolder dismiss : ajax error for : ', _query_.action, _r );
                          })
              ...
              _ajaxActionDo = function( _what_, _params_ ) {
                    ...
                    if ( ! _.isObject( _params_ ) ) {
                          angiapp.errorLog( 'placeHolder dismiss : wrong params' );
                          return;
                    }
