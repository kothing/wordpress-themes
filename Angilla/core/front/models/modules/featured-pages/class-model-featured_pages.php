<?php
class ANGI_featured_pages_model_class extends ANGI_Model {
  ...
  */
  function angi_fn_get_preset_model() {
        $_preset = array(
          'show_thumb'                => esc_attr( angi_fn_opt( 'tc_show_featured_pages_img' ) ),
          'center_imgs'               => esc_attr( angi_fn_opt( 'tc_center_img' ) ),
          'fp_per_row'                => 3,
          'fps'                       => array(
              esc_attr( angi_fn_opt( 'tc_featured_page_one' ) ),
              esc_attr( angi_fn_opt( 'tc_featured_page_two' ) ),
              esc_attr( angi_fn_opt( 'tc_featured_page_three' ) )
          ),
          'fps_text'                  => array(
              esc_attr( angi_fn_opt( 'tc_featured_text_one' ) ),
              esc_attr( angi_fn_opt( 'tc_featured_text_two' ) ),
              esc_attr( angi_fn_opt( 'tc_featured_text_three' ) )
          ),
          'text_length'               => 200,
          'button_text'               => esc_attr( angi_fn_opt( 'tc_featured_page_button_text') )
        );
  ...
  */
  function angi_fn_extend_params( $model = array() ) {
        ...
        $_skin_color                    = angi_fn_opt( 'tc_skin_color' );
        ...
        $model['fp_ids']                = ANGI___::$instance -> fp_ids;
      ...
      //defines the cols class
        ...
        $this -> featured_pages         = $this -> angi_fn_get_the_featured_pages( $model['fp_nb'], $model['fp_ids'], $model['show_thumb'] );
  ...
  function angi_fn_get_the_featured_pages( $fp_nb, $fp_ids, $show_thumb ) {
        $featured_pages = array();

        foreach ( range(0, $fp_nb - 1 ) as $fp_id )
          $featured_pages[ $fp_id + 1 ] = $this -> angi_fn_get_single_fp_model(
            $fp_ids[$fp_id],
            $show_thumb,
            $fp_id + 1
          );
  ...
  function angi_fn_get_single_fp_model( $fp_single_id, $show_thumb, $fp_index ) {
        $fp_img                         = '';
        $thumb_wrapper_class            = '';
        $edit_enabled                   = false;

        $featured_page_id               = apply_filters( 'angi_fp_id', $this -> fps[$fp_index-1], $fp_single_id );
        ...
        if ( ! $featured_page_id || ! $page = get_post($featured_page_id) ) {
            ...
            if ( ! angi_fn_is_customizing() && is_user_logged_in() && current_user_can('edit_theme_options') ) {
                $angilla_link              = sprintf( '<br/><a href="%1$s" title="%2$s" class="btn btn-edit" style="margin-top: 1em;
        font-weight: bold;"><i class="icn-edit"></i>%3$s</a>',
                    ...
                    __( 'Customize it now' , 'angilla' )
                );
                $featured_page_link           = angi_fn_get_customizer_url( array( 'control' => 'tc_featured_page_'.$fp_single_id, 'section' => 'frontpage_sec') );
            } else if ( angi_fn_is_customizing() ) {
                $angilla_link = sprintf( '<br/>%1$s<a href="%2$s" title="%3$s" class="btn btn-edit" style="margin-top: 1em;
        font-weight: bold;"><i class="icn-edit"></i>%4$s</a>',
                    sprintf( '<div style="position: relative;left: 33px;float: left;">%1$s</div>',
                       angi_fn_get_customizer_focus_icon( array( 'wot' => 'control', 'id' => 'tc_theme_options[tc_featured_page_' . $fp_single_id .']' ) )
                    ),
                    angi_fn_get_customizer_focus_link( array( 'wot' => 'control', 'id' => 'tc_theme_options[tc_featured_page_' . $fp_single_id .']' ) ),
                    __( 'Customizer screen' , 'angilla' ),
                    __( 'Customize it now' , 'angilla' )
                );
                $featured_page_link           = angi_fn_get_customizer_focus_link( array( 'wot' => 'control', 'id' => 'tc_theme_options[tc_featured_page_' . $fp_single_id .']' ) );
            }

            //rendering
            $featured_page_link             = apply_filters( 'angi_fp_link_url', $featured_page_link );
            /*
            ...
            */
            ...
            $featured_page_title            = apply_filters( 'angi_fp_title', __( 'Featured page' , 'angilla' ), $fp_single_id, $featured_page_id);
            $text                           = apply_filters(
                                                    'angi_fp_text',
                                                    sprintf( '%1$s %2$s',
            ...
            $featured_page_link             = esc_url( apply_filters( 'angi_fp_link_url', get_permalink( $featured_page_id ), $fp_single_id ) );
            $featured_page_title            = apply_filters( 'angi_fp_title', get_the_title( $featured_page_id ), $fp_single_id, $featured_page_id );
          ...
          //when are we displaying the edit link?
            ...
            if ( ! angi_fn_is_customizing() ) {
              ...
              add_filter( 'angi_is_one_fp_set', '__return_true' );
            }

            $edit_enabled                   = apply_filters( 'angi_edit_in_fp_title', $edit_enabled );
            $featured_text                  = apply_filters( 'angi_fp_text', $this -> fps_text[$fp_index-1], $fp_single_id, $featured_page_id );
            $featured_text                  = apply_filters( 'angi_fp_text_sanitize', strip_tags( html_entity_decode( $featured_text ) ), $fp_single_id, $featured_page_id );
          ...
          //limit text to 200 car
            $default_fp_text_length         = apply_filters( 'angi_fp_text_length', $this->text_length, $fp_single_id, $featured_page_id );
            ...
            if ( $show_thumb ) {
              ...
              $fp_img_size                    = apply_filters( 'angi_fp_img_size' , 'tc-thumb', $fp_single_id, $featured_page_id );
        ...
        $button_block = $this -> angi_fn_setup_button_block( $fp_single_id );
  ...
  */
  ...
  function angi_fn_get_featured_page( $autoadvance = true ) {
        ...
        * The filter callback is added at wp_head in ANGI___::__construct
        */
        if ( is_array($fp) && isset( $fp[ 'fp_img' ] ) ) {
          //this filter is defined is primarily used in angi_fn_get_thumbnail_model()
          $fp[ 'fp_img' ] = apply_filters( 'angi_thumb_html', $fp[ 'fp_img' ], $requested_size = 'tc-thumb', $post_id = null , $custom_thumb_id = null, $_img_attr = null, $tc_thumb_size = null );
        }
  ...
  function angi_fn_setup_button_block( $fp_single_id ) {
        //button block
        $fp_button_text    = call_user_func( '__', apply_filters( 'angi_fp_button_text' , $this -> button_text , $fp_single_id ) );
        $fp_button_class   = '';

        if ( $fp_button_text || angi_fn_is_customizing() ){
  ...
  * HELPERS
  *******************************/
  function angi_fn_get_fp_img( $fp_img_size, $featured_page_id, $fp_custom_img_id ){
        //try to get "tc_thumb" , "tc_thumb_height" , "tc_thumb_width"
        //angi_fn_get_thumbnail_model defined in core/functions.php
        $_fp_img_model = angi_fn_get_thumbnail_model( array(
