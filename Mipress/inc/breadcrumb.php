...
if ( ! function_exists( 'mipress_breadcrumb' ) ) :
	/**
	 * Display Breadcrumb
	 * @return html Breadcrumb with links
	 */
	function mipress_breadcrumb() {
		$show_on_home = get_theme_mod( 'mipress_breadcrumb_on_homepage' );
		$delimiter    = get_theme_mod( 'mipress_breadcrumb_seperator', '/' );
		...
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'mipress' ); // text for the 'Home' link
		/* translators: 1: before text/html, 2: after text/html. */
		$text['category'] = esc_html__( '%1$s Archive for %2$s', 'mipress' ); // text for a category page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['search']   = esc_html__( '%1$sSearch results for: %2$s', 'mipress' ); // text for a search results page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['tag']      = esc_html__( '%1$sPosts tagged %2$s', 'mipress' ); // text for a tag page
		/* translators: 1: before text/html, 2: after text/html. */
		$text['author']   = esc_html__( '%1$sView all posts by %2$s', 'mipress' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'mipress' ); // text for the 404 page
		...
		if ( is_front_page() ) {
			...
			if ( is_home() ) {
				...
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'mipress' ) ) ) ), esc_html( get_the_time( __( 'Y', 'mipress' ) ) ) ) ; // WPCS: XSS OK.


				echo sprintf( $link, esc_url( get_month_link( get_the_time( __( 'Y', 'mipress' ) ) ) ), esc_html(get_the_time( __( 'F', 'mipress' ) ) ) ); // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'd', 'mipress' ) ) ) . $after; // WPCS: XSS OK.

			} elseif ( is_month() ) {
				echo sprintf( $link, esc_url( get_year_link( get_the_time( __( 'Y', 'mipress' ) ) ) ), esc_html( get_the_time( __( 'Y', 'mipress' ) ) ) ) ; // WPCS: XSS OK.

				echo $before . esc_html( get_the_time( __( 'F', 'mipress' ) ) ) . $after; // WPCS: XSS OK.


			} elseif ( is_year() ) {
				echo $before . esc_html( get_the_time( __( 'Y', 'mipress' ) ) ) . $after; // WPCS: XSS OK.
			...
			if ( get_query_var( 'paged' ) ) {
				...
				echo sprintf( esc_html__( 'Page %s', 'mipress' ), absint( max( $paged, $page ) ) );
