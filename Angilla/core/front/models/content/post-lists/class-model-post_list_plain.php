<?php
class ANGI_post_list_plain_model_class extends ANGI_Model {
  ...
  */
  function angi_fn_get_preset_model() {
    $_preset = array(
      ...
      'contained'                 => false,
      'split_layout'              => true,//0 != esc_attr( angi_fn_opt( 'tc_post_list_plain_split_layout' ) ), //whether display TAX | CONTENT (horiz) or TAX/CONTENT (vertical)
      'wrapped'                   => true,
    );
  ...
  */
  function angi_fn_setup_late_properties() {
      ...
      if ( ! $this -> show_full_content && angi_fn_is_loop_start() )
          $this -> angi_fn_setup_text_hooks();

      $this->post_list_items[] = $this->angi_fn__get_post_list_item();
  }
  ...
  */
  ...
  function angi_fn_get_cat_list() {
    return $this -> angi_fn__get_post_list_item_property( 'cat_list' );
  }

  function angi_fn_get_cat_list_class() {
    return $this -> angi_fn__get_post_list_item_property( 'cat_list_class' );
  }

  function angi_fn_get_entry_header_inner_class() {
    return $this -> angi_fn__get_post_list_item_property( 'entry_header_inner_class' );
  }

  function angi_fn_get_content_inner_class() {
    return $this -> angi_fn__get_post_list_item_property( 'content_inner_class' );
  }
  ...
  */
  protected function angi_fn__get_post_list_item() {
    ...
    $cat_list                    = $this -> angi_fn__get_cat_list();
    ...
    $article_selectors           = $this -> angi_fn__get_article_selectors( $cat_list );

    //add the aspect ratio class for all media types (except audio )
    //$media_class                 = 'audio' == $current_post_format ? '' : 'angi__r-w16by9';
  ...
  */
  protected function angi_fn__get_cat_list() {
    /* Post list plain showing excerpts limits the category to show to 3 */
    $cat_list                  = ! $this -> show_full_content ? angi_fn_get_property( 'cat_list', 'post_metas',  array( 'limit' => 3 ) ) : angi_fn_get_property( 'cat_list', 'post_metas');
    return $cat_list;
  }
  ...
  */
  protected function angi_fn__get_article_selectors( $cat_list ) {
  ...
  */
  function angi_fn_setup_text_hooks() {
    ...
    add_filter( 'excerpt_more'       , array( $this , 'angi_fn_set_excerpt_more') , 99999999 );
  }
  ...
  */
  function angi_fn_reset_text_hooks() {
    ...
    remove_filter( 'excerpt_more'       , array( $this , 'angi_fn_set_excerpt_more') , 99999999 );
  }
  ...
  * @since Angilla 1.0.0
  */
  function angi_fn_set_excerpt_more($more) {
    return $more . angi_fn_readmore_button();
  }
