...
if ( ! class_exists( 'ANGI___' ) ) :
  final class ANGI___ extends ANGI_BASE {
    ...
    function __construct () {
        ...
        if( ! defined( 'ANGI_IS_MODERN_STYLE' ) )            define( 'ANGI_IS_MODERN_STYLE' , false );
        ...
        add_filter( 'tc_get_files_to_load' , array( $this , 'angi_fn_set_files_to_load' ) );


        //theme class groups instanciation
        //$this -> angi_fn__();
        add_action('angi_load', array( $this, 'angi_fn__') );
    ...
    */
    function angi_fn__( $_to_load = array(), $_no_filter = false ) {
        static $instances;
        ...
        foreach ( $_to_load as $group => $files ) {
          foreach ($files as $path_suffix ) {
            ...
            // if ( angi_fn_is_child() && file_exists( TC_BASE_CHILD . $path_suffix[0] . '/class-' . $group . '-' .$path_suffix[1] .'.php') ) {
            ...
            if( ! isset( $instances[ $classname ] ) )  {
              ...
              if ( in_array( $classname, apply_filters( 'tc_dont_instantiate_in_init', array( 'ANGI_nav_walker') ) ) )
                continue;
              //instantiates
    ...
    * HELPERS
    ****************************/
    function angi_fn_req_once( $file_path ) {
        //checks if a child theme is used and if the required file has to be overriden
        if ( angi_fn_is_child() && file_exists( TC_BASE_CHILD . $file_path ) ) {
        ...
        if ( ! angi_fn_is_customizing() )
          {
            if ( is_admin() ) {
              //load
              $this -> angi_fn_req_once( 'core/angi-admin-ccat.php' );

              $_to_load = $this -> angi_fn_unset_core_classes( $_to_load, array( 'header' , 'content' , 'footer' ), array( 'admin|inc/admin|customize' ) );
            }
            else {
              //load
              $this -> angi_fn_req_once( 'inc/angi-front-ccat.php' );

              //Skips all admin classes
              $_to_load = $this -> angi_fn_unset_core_classes( $_to_load, array( 'admin' ), array( 'fire|inc/admin|admin_init', 'fire|inc/admin|admin_page') );
            }
          }
        //Customizing
        else
          {
            //load
            $this -> angi_fn_req_once( 'core/angi-admin-ccat.php' );
            $this -> angi_fn_req_once( 'core/angi-customize-ccat.php' );
  ...
  class ANGI_init {
      ...
      function __construct () {
          ...
          $this -> global_layout      = array(
              'r' => array(
                  'content'       => 'span9',
                  'sidebar'       => 'span3',
                  'customizer'    => __( 'Right sidebar' , 'angilla' ),
                  'metabox'       => __( 'Right sidebar' , 'angilla' ),
              ),
              'l' => array(
                  'content'       => 'span9',
                  'sidebar'       => 'span3',
                  'customizer'    => __( 'Left sidebar' , 'angilla' ),
                  'metabox'       => __( 'Left sidebar' , 'angilla' ),
              ),
              'b' => array(
                  'content'       => 'span6',
                  'sidebar'       => 'span3',
                  'customizer'    => __( '2 sidebars : Right and Left' , 'angilla' ),
                  'metabox'       => __( '2 sidebars : Right and Left' , 'angilla' ),
              ),
              'f' => array(
                  'content'       => 'span12',
                  'sidebar'       => false,
                  'customizer'    => __( 'No sidebars : full width layout', 'angilla' ),
                  'metabox'       => __( 'No sidebars : full width layout' , 'angilla' ),
              ),
          );

          //Default skins array
          $this -> skins              =  array(
              'blue.css'        =>  __( 'Blue' , 'angilla' ),
              'black.css'       =>  __( 'Black' , 'angilla' ),
              'black2.css'      =>  __( 'Flat black' , 'angilla' ),
              'grey.css'        =>  __( 'Grey' , 'angilla' ),
              'grey2.css'       =>  __( 'Light grey' , 'angilla' ),
              'purple2.css'     =>  __( 'Flat purple' , 'angilla' ),
              'purple.css'      =>  __( 'Purple' , 'angilla' ),
              'red2.css'        =>  __( 'Flat red' , 'angilla' ),
              'red.css'         =>  __( 'Red' , 'angilla' ),
              'orange.css'      =>  __( 'Orange' , 'angilla' ),
              'orange2.css'     =>  __( 'Flat orange' , 'angilla'),
              'yellow.css'      =>  __( 'Yellow' , 'angilla' ),
              'yellow2.css'     =>  __( 'Flat yellow' , 'angilla' ),
              'green.css'       =>  __( 'Green' , 'angilla' ),
              'green2.css'      =>  __( 'Light green' , 'angilla'),
              'blue3.css'       =>  __( 'Green blue' , 'angilla'),
              'blue2.css'       =>  __( 'Light blue ' , 'angilla' )
          );
          ...
          $this -> footer_widgets     = array(
              'footer_one'    => array(
                              'name'                 => __( 'Footer Widget Area One' , 'angilla' ),
                              'description'          => __( 'Just use it as you want !' , 'angilla' )
              ),
              'footer_two'    => array(
                              'name'                 => __( 'Footer Widget Area Two' , 'angilla' ),
                              'description'          => __( 'Just use it as you want !' , 'angilla' )
              ),
              'footer_three'   => array(
                              'name'                 => __( 'Footer Widget Area Three' , 'angilla' ),
                              'description'          => __( 'Just use it as you want !' , 'angilla' )
              )
          );//end of array
          ...
          add_filter('body_class'                              , array( $this , 'angi_fn_set_body_classes') );
      }//end of constructor
      ...
      */
      function angi_fn_get_style_src( $_wot = 'skin' ) {
          ...
          $_sheet    = esc_attr( angi_fn_opt( 'tc_minified_skin' ) ) ? str_replace('.css', '.min.css', $_sheet) : $_sheet;

          //Finds the good path : are we in a child theme and is there a skin to override?
          $remote_path    = ( angi_fn_is_child() && file_exists(TC_BASE_CHILD .'inc/assets/css/' . $_sheet) ) ? TC_BASE_URL_CHILD .'inc/assets/css/' : false ;
          ...
          if ( 'skin' != $_wot && ( is_rtl() || ( defined( 'WPLANG' ) && ( 'ar' == WPLANG || 'he_IL' == WPLANG ) ) ) ){
            $remote_rtl_path   = ( angi_fn_is_child() && file_exists(TC_BASE_CHILD .'inc/assets/css/rtl/' . $_sheet) ) ? TC_BASE_URL_CHILD .'inc/assets/css/rtl/' : false ;
      ...
      */
      function angi_fn_set_body_classes( $_classes ) {
          ...
          if ( 0 != esc_attr( angi_fn_opt( 'tc_enable_dropcap' ) ) )
            array_push( $_classes, esc_attr( angi_fn_opt( 'tc_dropcap_design' ) ) );

          //adds the layout
          $_layout = ANGI_utils::angi_fn_get_layout( angi_fn_get_id() , 'sidebar' );
          ...
          //IMAGE CENTERED
          if ( (bool) esc_attr( angi_fn_opt( 'tc_center_img') ) ){
            $_classes = array_merge( $_classes , array( 'tc-center-images' ) );
          }

          //SKIN CLASS
          $_skin = sprintf( 'skin-%s' , basename( $this -> angi_fn_get_style_src() ) );
...
*
*/
if ( ! class_exists( 'ANGI_plugins_compat' ) ) :
  class ANGI_plugins_compat {
    ...
    function __construct () {
      ...
      add_action ('after_setup_theme'          , array( $this , 'angi_fn_set_plugins_supported'), 20 );
      add_action ('after_setup_theme'          , array( $this , 'angi_fn_plugins_compatibility'), 30 );
    ...
    */
    function angi_fn_plugins_compatibility() {
      ...
      if ( current_theme_supports( 'tc-unlimited-featured-pages' ) && angi_fn_is_plugin_active('tc-unlimited-featured-pages/tc_unlimited_featured_pages.php') )
        $this -> angi_fn_set_tc_unlimited_featured_pages_compat();

      /* JETPACK */
      //adds compatibilty with the jetpack image carousel and photon
      if ( current_theme_supports( 'jetpack' ) && angi_fn_is_plugin_active('jetpack/jetpack.php') )
        $this -> angi_fn_set_jetpack_compat();
      ...
      if ( current_theme_supports( 'bbpress' ) && angi_fn_is_plugin_active('bbpress/bbpress.php') )
        $this -> angi_fn_set_bbpress_compat();
      ...
      if ( current_theme_supports( 'buddy-press' ) && angi_fn_is_plugin_active('buddypress/bp-loader.php') )
        $this -> angi_fn_set_buddypress_compat();

      /*
      * QTranslatex
      * Credits : @acub, http://websiter.ro
      */
      if ( current_theme_supports( 'qtranslate-x' ) && angi_fn_is_plugin_active('qtranslate-x/qtranslate.php') )
        $this -> angi_fn_set_qtranslatex_compat();

      /*
      * Polylang
      * Credits : Rocco Aliberti
      */
      if ( current_theme_supports( 'polylang' ) && ( angi_fn_is_plugin_active('polylang/polylang.php') || angi_fn_is_plugin_active('polylang-pro/polylang.php') ) )
        $this -> angi_fn_set_polylang_compat();

      /*
      * WPML
      */
      if ( current_theme_supports( 'wpml' ) && angi_fn_is_plugin_active('sitepress-multilingual-cms/sitepress.php') )
        $this -> angi_fn_set_wpml_compat();

      /* The Events Calendar */
      if ( current_theme_supports( 'the-events-calendar' ) && angi_fn_is_plugin_active('the-events-calendar/the-events-calendar.php') )
        $this -> angi_fn_set_the_events_calendar_compat();

      /* Event Tickets */
      if ( current_theme_supports( 'event-tickets' ) && angi_fn_is_plugin_active('event-tickets/event-tickets.php') )
        $this -> angi_fn_set_event_tickets_compat();

      /* Optimize Press */
      if ( current_theme_supports( 'optimize-press' ) && angi_fn_is_plugin_active('optimizePressPlugin/optimizepress.php') )
        $this -> angi_fn_set_optimizepress_compat();

      /* Woocommerce */
      if ( current_theme_supports( 'woocommerce' ) && angi_fn_is_plugin_active('woocommerce/woocommerce.php') )
        $this -> angi_fn_set_woocomerce_compat();

      /* Sensei woocommerce addon */
      if ( current_theme_supports( 'woo-sensei') && angi_fn_is_plugin_active('woothemes-sensei/woothemes-sensei.php') )
        $this -> angi_fn_set_sensei_compat();

      /* Visual Composer */
      if ( current_theme_supports( 'visual-composer') && angi_fn_is_plugin_active('js_composer/js_composer.php') )
        $this -> angi_fn_set_vc_compat();

      /* Disqus Comment System */
      if ( current_theme_supports( 'disqus') && angi_fn_is_plugin_active('disqus-comment-system/disqus.php') )
        $this -> angi_fn_set_disqus_compat();

      /* Ultimate Responsive Image Slider  */
      if ( current_theme_supports( 'uris' ) && angi_fn_is_plugin_active('ultimate-responsive-image-slider/ultimate-responsive-image-slider.php') )
        $this -> angi_fn_set_uris_compat();

      /* LearnPress  */
      if ( current_theme_supports( 'learnpress' ) && angi_fn_is_plugin_active('learnpress/learnpress.php') )
        $this -> angi_fn_set_lp_compat();

      /* Coauthors-Plus */
      if ( current_theme_supports( 'coauthors' ) && angi_fn_is_plugin_active('co-authors-plus/co-authors-plus.php') )
        $this -> angi_fn_set_coauthors_compat();
    ...
    */
    private function angi_fn_set_jetpack_compat() {
      ...
      if ( class_exists( 'Jetpack' ) && Jetpack::is_module_active( 'photon' ) )
        add_filter( 'angi_img_smartloaded', 'angi_fn_jp_smartload_img');
      function angi_fn_jp_smartload_img( $img ) {
    ...
    */
    private function angi_fn_set_bbpress_compat() {
      if ( ! function_exists( 'angi_fn_bbpress_disable_feature' ) ) {
        function angi_fn_bbpress_disable_feature( $bool ) {
          return ( function_exists('is_bbpress') && is_bbpress() ) ? false : $bool;
        }
      }

      // hide tax archive title
      add_filter( 'tc_show_tax_archive_title', 'angi_fn_bbpress_disable_feature');
      //disables thumbnails and excerpt for post lists
      add_filter( 'tc_opt_tc_post_list_thumb', 'angi_fn_bbpress_disable_feature' );
      //show full content in post lists
      add_filter( 'tc_show_excerpt', 'angi_fn_bbpress_disable_feature' );
      //disables Angilla author infos on forums
      add_filter( 'tc_show_author_metas_in_post', 'angi_fn_bbpress_disable_feature' );
      //disables post navigation
      add_filter( 'tc_show_post_navigation', 'angi_fn_bbpress_disable_feature' );
      //disables post metas
      add_filter( 'tc_show_post_metas', 'angi_fn_bbpress_disable_feature', 100);
      //disable the grid
      add_filter( 'tc_set_grid_hooks' , 'angi_fn_bbpress_disable_feature', 100 );
    }


    /*
    * Same in angi modern except for comments enabled filter prefix (tc_ -> angi_)
    */
    /**
    ...
    */
    private function angi_fn_set_buddypress_compat() {
      ...
      function angi_fn_buddypress_disable_comments($bool){
      ...
      add_action( 'xprofile_screen_change_avatar', 'angi_fn_buddypress_maybe_disable_img_smartload' );
      function angi_fn_buddypress_maybe_disable_img_smartload() {
        add_filter( 'tc_opt_tc_img_smart_load', '__return_false' );
      }
    }

    /*
    * same in angi modern with filter prefixes change ( tc_ -> angi_ )
    */
    /**
    ...
    */
    private function angi_fn_set_qtranslatex_compat() {
      ...
      function angi_fn_apply_qtranslate ($text) {
        return call_user_func(  '__' , $text );
      }
      function angi_fn_remove_char_limit() {
        return 99999;
      }
      function angi_fn_change_transport( $value , $set ) {
      ...
      foreach ( array( 'tc_slide_link_url', 'tc_logo_link_url') as $filter )
        add_filter( $filter, 'angi_fn_url_lang' );
      ...
      foreach ( array( 'tc_slide_title', 'tc_slide_text', 'tc_slide_button_text', 'tc_slide_background_alt' ) as $filter )
        add_filter( $filter, 'angi_fn_apply_qtranslate' );
      ...
      foreach ( array( 'tc_slide_title_length', 'tc_slide_text_length', 'tc_slide_button_length' ) as $filter )
        add_filter( $filter  , 'angi_fn_remove_char_limit');
      ...
      foreach ( $tc_archive_titles as $title )
        add_filter("tc_{$title}_title", 'angi_fn_apply_qtranslate' , 20);
      ...
      if ( ! apply_filters( 'tc_other_plugins_force_fpu_disable', class_exists('TC_fpu') ) && ! class_exists('TC_fpc') ) {
        ...
        add_filter( 'tc_fp_link_url' , 'angi_fn_url_lang');
        //outputs the qtranslate translation for featured pages
        add_filter( 'tc_fp_text', 'angi_fn_apply_qtranslate' );
        add_filter( 'tc_fp_button_text', 'angi_fn_apply_qtranslate' );
        ...
        add_filter( 'tc_featured_page_button_text_customizer_set' , 'angi_fn_change_transport', 20, 2);
        add_filter( 'tc_featured_text_one_customizer_set' , 'angi_fn_change_transport', 20, 2);
        add_filter( 'tc_featured_text_two_customizer_set' , 'angi_fn_change_transport', 20, 2);
        add_filter( 'tc_featured_text_three_customizer_set', 'angi_fn_change_transport', 20, 2);
      }

      //posts slider (this filter is not fired in admin )
      add_filter('tc_posts_slider_pre_model', 'angi_fn_posts_slider_qtranslate');
      function angi_fn_posts_slider_qtranslate( $pre_slides ){
        if ( empty($pre_slides) )
          return $pre_slides;

        // remove useles q-translation of the slider view
        foreach ( array( 'tc_slide_title', 'tc_slide_text', 'tc_slide_button_text', 'tc_slide_background_alt' ) as $filter )
          remove_filter( $filter, 'angi_fn_apply_qtranslate' );
        ...
        $pre_slides['common']['button_text'] = $pre_slides['common']['button_text'] ? ANGI_slider::$instance -> angi_fn_get_post_slide_button_text( $pre_slides['common']['button_text'] ) : '';
        ...
        foreach ($_posts as &$_post) {
          ...
          $_post['title'] = $_post['title'] ? ANGI_slider::$instance -> angi_fn_get_post_slide_title($_p, $ID) : '';
          $_post['text']  = $_post['text'] ? ANGI_slider::$instance -> angi_fn_get_post_slide_excerpt($_p, $ID) : '';
        }
        return $pre_slides;
      }
    }
    ...
    */
    private function angi_fn_set_polylang_compat() {
      ...
      if ( function_exists( 'pll_register_string' ) )
        add_action( 'admin_init', 'angi_fn_pll_strings_setup' );

      function angi_fn_pll_strings_setup() {
        // grab theme options
        $tc_options = angi_fn__f('__options');
        // grab settings map, useful for some options labels
        $tc_settings_map = angi_fn_get_customizer_map( $get_default = true );
        $tc_controls_map = $tc_settings_map['add_setting_control'];
        // set $polylang_group;
        $polylang_group = 'Angilla';

        //get options to translate
        $tc_translatable_raw_options = ANGI_plugins_compat::$instance -> angi_fn_get_string_options_to_translate();
          ...
          if ( isset( $tc_options[$tc_translatable_option] ) ) {
            switch ( $tc_translatable_option ) {
              case 'tc_front_slider': 
                $label = __( 'Front page slider name', 'angilla' );
                break;
              case 'tc_posts_slider_button_text' :
                $label = __( 'Posts slider button text', 'angilla' );
                break;
              default:
      ...
      if ( function_exists( 'pll_get_post' ) && function_exists( 'pll__' ) && ! is_admin() ) {
        ...
        $tc_translatable_options = ANGI_plugins_compat::$instance -> angi_fn_get_string_options_to_translate();
        //translate
        ...
        */
        function angi_fn_pll_translate_tax( $term_ids ){
        ...
        add_filter('tc_opt_tc_blog_restrict_by_cat', 'angi_fn_pll_translate_tax');
        /*end tax filtering*/

        //Featured pages ids "translation"
        // Substitute any page id with the equivalent page in current language (if found)
        add_filter( 'tc_fp_id', 'angi_fn_pll_page_id', 20 );
        function angi_fn_pll_page_id( $fp_page_id ) {
    ...
    */
    private function angi_fn_set_wpml_compat() {
      ...
      //define the CONSTANT wpml context. This means that user have to set the translations again when switching from Angilla.
      //If we don't want to do this, let's go with 'Angilla-option' in any case.
      //Also I choose to use "-option" suffix to avoid confusions as with WPML you can also translate theme's strings ( gettexted -> __() ) and WPML by default assigns to theme the context 'angilla' (textdomain)
      define( 'TC_WPML_CONTEXT' ,  'angilla-option' );
      ...
      function angi_fn_wpml_get_options_names_config() {
      ...
      function angi_fn_wpml_object_id( $object_id, $type ) {
        if ( empty( $object_id ) )
          return $object_id;
        if ( is_array( $object_id ) )
          return array_map( 'angi_fn_wpml_object_id', $object_id, array_fill( 0, sizeof( $object_id ), $type ) );
      ...
      function angi_fn_wpml_pre_update_option_filter( $options ) {
        if ( isset( $options['tc_sliders'] ) ) {
            // Force default language
            $current_language = ANGI_plugins_compat::$instance->current_language;
            ANGI_plugins_compat::$instance->current_language = ANGI_plugins_compat::$instance->default_language;
            $options['tc_sliders'] = angi_fn_wpml_sliders_filter( $options['tc_sliders'] );
            ANGI_plugins_compat::$instance->current_language = $current_language;
        }
        return $options;
      }

      add_action( 'admin_init', 'angi_fn_wpml_admin_setup' );

      function angi_fn_wpml_admin_setup() {
        // If wpml-string-translation is active perform admin pages translation
        if ( function_exists( 'icl_register_string' ) ) {
          $tc_wpml_option_name = angi_fn_wpml_get_options_names_config();
        ...
        if ( angi_fn_is_customize_left_panel() )
          add_filter( 'option_tc_theme_options', 'angi_fn_wpml_customizer_options_transpose' );
        function angi_fn_wpml_customizer_options_transpose( $options ) {
          ...
          foreach ( $options_to_transpose as $type => $option_to_transpose )
            foreach ( $option_to_transpose as $option )
              if ( isset( $options[$option] ) )
                $options[$option] = angi_fn_wpml_object_id( $options[$option], $type);
          return $options;
        }
        ...
        add_action( 'edit_attachment', 'angi_fn_wpml_edit_attachment_action', 99 );
        // Pre-save hook
        add_filter( 'pre_update_option_tc_theme_options', 'angi_fn_wpml_pre_update_option_filter', 99 );
      ...
      if ( ! is_admin() ) {
        ...
        if ( function_exists( 'icl_t') ) {
          ...
          if ( ! function_exists( 'angi_fn_wpml_t_opt' ) ) {
            function angi_fn_wpml_t_opt( $string ) {
              return angi_fn_wpml_t( $string, str_replace('tc_opt_', '', current_filter() ) );
            }
          }

          //define our icl_t wrapper
          if ( ! function_exists( 'angi_fn_wpml_t' ) ) {
            function angi_fn_wpml_t( $string, $opt ) {
              $tc_wpml_options_names = angi_fn_wpml_get_options_names_config();
          ...
          //get the options to translate
          $tc_wpml_options = array_keys( angi_fn_wpml_get_options_names_config() );
        ...
        /*A) FP*/
        // Featured pages ids "translation"
        add_filter( 'tc_fp_id', 'angi_fn_wpml_page_id', 20 );
        function angi_fn_wpml_page_id( $fp_page_id ) {
          return angi_fn_wpml_object_id( $fp_page_id, 'page');
        }
        ...
        * @param array of term ids
        */
        function angi_fn_wpml_translate_cat( $cat_ids ){
          if ( ! ( is_array( $cat_ids ) && ! empty( $cat_ids ) ) )
            return $cat_ids;
          return array_unique( angi_fn_wpml_object_id( $cat_ids, 'category' ) );
        }
        //Translate category ids for the filtered posts in home/blog
        add_filter('tc_opt_tc_blog_restrict_by_cat', 'angi_fn_wpml_translate_cat');
        /*end tax filtering*/
    ...
    */
    private function angi_fn_set_the_events_calendar_compat() {
      ...
      */
      if ( ! ( function_exists( 'angi_fn_is_tec_events_list' ) ) ) {
        function angi_fn_is_tec_events_list() {
      ...
      */
      if ( ! ( function_exists( 'angi_fn_is_tec_single_event' ) ) ) {
        function angi_fn_is_tec_single_event() {
          return function_exists( 'tribe_is_event_query' ) && tribe_is_event_query() && is_single();
        }
      }
      // hide tax archive title
      add_filter( 'tc_show_tax_archive_title', 'angi_fn_tec_disable_tax_archive_title');
      function angi_fn_tec_disable_tax_archive_title( $bool ) {
        return angi_fn_is_tec_events_list() ? false : $bool;
      }

      // Events archive is displayed, wrongly, with our post lists classes, we have to prevent this
      add_filter( 'tc_post_list_controller', 'angi_fn_tec_disable_post_list');
      add_filter( 'tc_is_grid_enabled', 'angi_fn_tec_disable_post_list');
      function angi_fn_tec_disable_post_list( $bool ) {
        return angi_fn_is_tec_events_list() ? false : $bool;
      }

      // Now we have to display a post or page content
      add_filter( 'tc_show_single_post_content', 'angi_fn_tec_show_content' );
      function angi_fn_tec_show_content( $bool ) {
        //2 cases:
        ...
        if ( angi_fn_is_tec_events_list() )
          return true;
        else if( angi_fn_is_tec_single_event() )
          return false;
        return $bool;
      }
      ...
      add_action( 'wp_head', 'angi_fn_tec_allow_display_breadcrumb_in_month_view');
      function angi_fn_tec_allow_display_breadcrumb_in_month_view() {
        if ( ! ( angi_fn_is_tec_events_list() && function_exists( 'tribe_is_month' ) && tribe_is_month() ) )
          return;

        add_filter( 'tc_breadcrumb_trail_args', 'angi_fn_tec_unhook_empty_post_type_archive_title');
        function angi_fn_tec_unhook_empty_post_type_archive_title( $args = null ) {
          remove_filter( 'post_type_archive_title', '__return_false', 10 );
          return $args;
        }
        add_filter( 'tc_breadcrumb_trail_display', 'angi_fn_tec_rehook_empty_post_type_archive_title', PHP_INT_MAX );
        function angi_fn_tec_rehook_empty_post_type_archive_title( $breadcrumb = null ) {
          add_filter( 'post_type_archive_title', '__return_false', 10 );
          return $breadcrumb;
        }
      }
      //disables post navigation in single tec pages
      add_filter( 'tc_show_post_navigation', 'angi_fn_tec_disable_post_navigation' );
      function angi_fn_tec_disable_post_navigation($bool) {
        return ( angi_fn_is_tec_single_event() ) ? false : $bool;
      }

      /*
      * Avoid php smartload image php parsing in events list content
      */
      add_filter( 'angi_disable_img_smart_load', 'angi_fn_tec_disable_img_smart_load_events_list', 999, 2);
      function angi_fn_tec_disable_img_smart_load_events_list( $_bool, $parent_filter ) {
        if ( 'the_content' == $parent_filter && angi_fn_is_tec_events_list() )
          return true;//disable
        return $_bool;
      }
    ...
    * @package Angilla
    */
    private function angi_fn_set_event_tickets_compat() {
      /*
      * Are we in the tickets attendees registration
      */
      if ( ! ( function_exists( 'angi_fn_is_et_attendees_registration' ) ) ) {
        function angi_fn_is_et_attendees_registration() {
          return function_exists( 'tribe' ) && tribe( 'tickets.attendee_registration' )->is_on_page();
        }
      }
      // Workaround because of a bug on tec tickets that makes it require wp-content/themes/angilla/Custom Page Example (localized)
      // in place of wp-content/themes/angilla/custom-page.php
      add_filter( 'tribe_tickets_attendee_registration_page_template', 'angi_fn_et_ticket_fix_custom_page' );
      function angi_fn_et_ticket_fix_custom_page( $what ) {
        return str_replace( __( 'Custom Page Example', 'angilla' ), 'custom-page.php', $what );
      }

      // Attendees registration is displayed, wrongly, with our post lists classes, we have to prevent this
      add_filter( 'tc_post_list_controller', 'angi_fn_et_disable_on_attendees_registration');
      add_filter( 'tc_is_grid_enabled', 'angi_fn_et_disable_on_attendees_registration');
      // hide tax archive title
      add_filter( 'tc_show_tax_archive_title', 'angi_fn_et_disable_on_attendees_registration');
      //hide navigation
      add_filter( 'tc_show_post_navigation', 'angi_fn_et_disable_on_attendees_registration' );
      function angi_fn_et_disable_on_attendees_registration( $bool ) {
        return angi_fn_is_et_attendees_registration() ? false : $bool;
      }

      // Now we have to display a post or page content
      add_filter( 'tc_show_single_post_content', 'angi_fn_et_show_content' );
      function angi_fn_et_show_content( $bool ) {
        return angi_fn_is_et_attendees_registration() ? true : $bool;
      }
    ...
    */
    private function angi_fn_set_optimizepress_compat() {
      ...
      function angi_fn_op_dequeue_fancybox_js(){
        if ( function_exists('is_le_page') ){
          ...
          if ( is_le_page() || defined('OP_LIVEEDITOR') ) {
            ...
            /* Front End: Dequeue Fancybox maybe already embedded in Angilla */
            wp_dequeue_script('tc-fancybox');
      ...
      add_filter('tc_js_params_plugin_compat', 'angi_fn_op_remove_fancyboxloading' );
      //@params = array() of js params
      function angi_fn_op_remove_fancyboxloading( $params ){
    ...
    */
    private function angi_fn_set_sensei_compat() {
      ...
      add_action('sensei_before_main_content', 'angi_fn_sensei_wrappers', 10);
      add_action('sensei_after_main_content', 'angi_fn_sensei_wrappers', 10);


      function angi_fn_sensei_wrappers() {
        switch ( current_filter() ) {
          case 'sensei_before_main_content': ANGI_plugins_compat::$instance -> angi_fn_mainwrapper_start();
                                             break;

          case 'sensei_after_main_content' : ANGI_plugins_compat::$instance -> angi_fn_mainwrapper_end();
                                             break;
        }//end of switch on hook
      }//end of nested function

      // hide tax archive title
      add_filter( 'tc_show_tax_archive_title', 'angi_fn_sensei_disable_tax_archive_title');
      function angi_fn_sensei_disable_tax_archive_title( $bool ){
        return ( function_exists('is_sensei') && ( is_sensei() || is_post_type_archive('sensei_message') ) ) ? false : $bool;
      }

      //disables post navigation
      add_filter( 'tc_show_post_navigation', 'angi_fn_sensei_disable_post_navigation' );
      function angi_fn_sensei_disable_post_navigation($bool) {
      ...
      add_filter( 'tc_show_single_post_content', 'angi_fn_sensei_disable_single_content_in_my_courses');
      function angi_fn_sensei_disable_single_content_in_my_courses( $bool ) {
        global $post;
    ...
    */
    private function angi_fn_set_woocomerce_compat() {
      ...
      add_action('woocommerce_before_main_content', 'angi_fn_woocommerce_wrappers', 10);
      add_action('woocommerce_after_main_content', 'angi_fn_woocommerce_wrappers', 10);

      //disable WooCommerce default breadcrumb
      if ( apply_filters( 'tc_disable_woocommerce_breadcrumb', true ) )
        remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );

      function angi_fn_woocommerce_wrappers() {
        switch ( current_filter() ) {
          case 'woocommerce_before_main_content': ANGI_plugins_compat::$instance -> angi_fn_mainwrapper_start();
                                                  break;

          case 'woocommerce_after_main_content' : ANGI_plugins_compat::$instance -> angi_fn_mainwrapper_end();
                                                  break;
        }//end of switch on hook
      }//end of nested function
      //Helper
      function angi_fn_wc_is_checkout_cart() {
      ...
      //Helper
      function angi_fn_woocommerce_shop_page_id( $id = null ){
      ...
      add_filter( 'angi_id', 'angi_fn_woocommerce_shop_page_id' );

      // use Angilla title
      // initially used to display the edit button
      // doesn't work anymore, lets comment it
      /*
      add_filter( 'the_title', 'angi_fn_woocommerce_the_title' );
      function angi_fn_woocommerce_the_title( $_title ){
      ...
      */
      ...
      add_filter( 'tc_post_list_controller', 'angi_fn_is_woocommerce_disable');
      add_filter( 'tc_set_grid_hooks', 'angi_fn_is_woocommerce_disable');

      // hide tax archive title
      add_filter( 'tc_show_tax_archive_title', 'angi_fn_is_woocommerce_disable' );


      //allow slider in the woocommerce shop page
      add_filter( 'tc_show_slider', 'angi_fn_woocommerce_shop_enable' );

      //allow page layout post meta in 'shop'
      add_filter( 'tc_is_page_layout', 'angi_fn_woocommerce_shop_enable' );
      ...
      //disables post navigation
      add_filter( 'tc_show_post_navigation', 'angi_fn_is_woocommerce_disable' );

      //removes post comment action on after_loop hook
      add_filter( 'tc_are_comments_enabled', 'angi_fn_is_woocommerce_disable' );

      //link smooth scroll: exclude woocommerce tabs
      add_filter( 'tc_anchor_smoothscroll_excl', 'angi_fn_woocommerce_disable_link_scroll' );
      function angi_fn_woocommerce_disable_link_scroll( $excl ){
        if ( false == esc_attr( angi_fn_opt('tc_link_scroll') ) ) return $excl;
      ...
      //changes angilla meta boxes priority (slider and layout not on top) if displaying woocommerce products in admin
      add_filter( 'tc_post_meta_boxes_priority', 'angi_fn_woocommerce_change_meta_boxes_priority' , 2 , 10 );
      function angi_fn_woocommerce_change_meta_boxes_priority($priority , $screen) {
      ...
      add_filter( 'tc_woocommerce_options_enabled', 'angi_fn_woocommerce_options_enabled_cb' );
      function angi_fn_woocommerce_options_enabled_cb() {
        return function_exists( 'WC' ) ? '__return_true' : '__return_false';
      }

      /* rendering the cart icon in the header */
      //narrow the tagline
      add_filter( 'tc_tagline_class', 'angi_fn_woocommerce_force_tagline_width', 100 );
      function angi_fn_woocommerce_force_tagline_width( $_class ) {
        return 1 == esc_attr( angi_fn_opt( 'tc_woocommerce_header_cart' ) ) ? 'span6' : $_class ;
      }

      // Ensure cart contents update when products are added to the cart via AJAX (place the following in functions.php)
      add_filter( 'woocommerce_add_to_cart_fragments', 'angi_fn_woocommerce_add_to_cart_fragment' );
      function angi_fn_woocommerce_add_to_cart_fragment( $fragments ) {
        if ( 1 == esc_attr( angi_fn_opt( 'tc_woocommerce_header_cart' ) ) ) {
      ...
      add_action( '__navbar', 'angi_fn_woocommerce_header_cart', is_rtl() ? 9 : 19 );
      function angi_fn_woocommerce_header_cart() {
        if ( 1 != esc_attr( angi_fn_opt( 'tc_woocommerce_header_cart' ) ) )
          return;
        ...
        if ( angi_fn_wc_is_checkout_cart() ) {
       ...
       <div class="tc-wc-menu tc-open-on-hover span1">
         <ul class="tc-wc-header-cart nav tc-hover-menu">
           <li class="<?php echo esc_attr( $_main_item_class ); ?> menu-item">
             <a class="cart-contents" href="<?php echo $wc_cart_url; ?>" title="<?php _e( 'View your shopping cart', 'angilla' ); ?>">
               <span class="count btn-link tc-wc-count"><?php echo $_cart_count ? $_cart_count : '' ?></span>
            </a>
            <?php
            ?>
            <?php if ( ! angi_fn_wc_is_checkout_cart() ) : //do not display the dropdown in the cart or checkout page ?>
      ...
      add_filter( 'tc_header_classes'   , 'angi_fn_woocommerce_set_header_classes');
      function angi_fn_woocommerce_set_header_classes( $_classes ) {
        if ( 1 == esc_attr( angi_fn_opt( 'tc_woocommerce_header_cart' ) ) )
          $_classes[]          = ( 1 != esc_attr( angi_fn_opt( 'tc_woocommerce_header_cart_sticky' ) ) ) ? 'tc-wccart-off' : 'tc-wccart-on';
        return $_classes;
      }

      //add woocommerce header cart CSS
      add_filter( 'tc_user_options_style', 'angi_fn_woocommerce_header_cart_css');
      function angi_fn_woocommerce_header_cart_css( $_css ) {
        if ( 1 != esc_attr( angi_fn_opt( 'tc_woocommerce_header_cart' ) ) )
          return $_css;
        ...
        */
        $_header_layout      = esc_attr( angi_fn_opt( 'tc_header_layout') );
    ...
    /*
    * same in angi modern except for the filter prefix (tc_ -> angi_)
    */
    /**
    ...
    */
    private function angi_fn_set_vc_compat() {
      ...
      add_filter( 'tc_anchor_smoothscroll_excl', 'angi_fn_vc_disable_link_scroll' );
      function angi_fn_vc_disable_link_scroll( $excl ){
        if ( false == esc_attr( angi_fn_opt('tc_link_scroll') ) ) return $excl;
    ...
    */
    private function angi_fn_set_disqus_compat() {
      if ( ! function_exists( 'angi_fn_disqus_comments_enabled' ) ) {
        function angi_fn_disqus_comments_enabled() {
      ...
      add_filter( 'tc_bubble_comment_anchor', 'angi_fn_disqus_bubble_comment_anchor' );
      function angi_fn_disqus_bubble_comment_anchor( $anchor ) {
        return angi_fn_disqus_comments_enabled() ? '#tc-disqus-comments' : $anchor;
      }
      //wrap disqus comments template in a convenient div
      add_action( 'tc_before_comments_template' , 'angi_fn_disqus_comments_wrapper' );
      add_action( 'tc_after_comments_template'  , 'angi_fn_disqus_comments_wrapper' );
      function angi_fn_disqus_comments_wrapper() {
        if ( ! angi_fn_disqus_comments_enabled() )
          return;
    ...
    */
    private function angi_fn_set_uris_compat() {
      ...
      function angi_fn_uris_disable_img_smartload( $options ){
        if ( ! is_array( $options ) )
          $options = array();
    ...
    */
    private function angi_fn_set_coauthors_compat() {
    ...
    */
    private function angi_fn_set_tc_unlimited_featured_pages_compat() {
      ...
      if ( class_exists( 'TC_fpu' ) &&  version_compare( TC_fpu::$instance -> plug_version, '2.0.24', '<' ) ) {
        ...
        //do not remove angilla free featured pages option panel
    ...
    /**
    * CUSTOMIZR WRAPPERS
    * print the angilla wrappers
    *
    * @since 3.3+
    *
    * originally used for woocommerce compatibility
    */
    function angi_fn_mainwrapper_start() {
      ?>
        ...
        <div class="container" role="main">
          <div class="<?php echo implode(' ', apply_filters( 'tc_column_content_wrapper_classes' , array('row' ,'column-content-wrapper') ) ) ?>">
            ...
            <?php do_action( '__before_article_container'); ##hook of left sidebar?>
              ...
              <div id="content" class="<?php echo implode(' ', apply_filters( 'tc_article_container_class' , array( ANGI_utils::angi_fn_get_layout( angi_fn_get_id() , 'class' ) , 'article-container' ) ) ) ?>">

                <?php do_action ('__before_loop');##hooks the header of the list of post : archive, search... ?>
      <?php
    }

    function angi_fn_mainwrapper_end() {
      ?>
    ...
    /* same in angi modern */
    public function angi_fn_get_string_options_to_translate() {
      ...
      if ( ! apply_filters( 'tc_other_plugins_force_fpu_disable', class_exists('TC_fpu')  ) && ! class_exists('TC_fpc') ) {
        $fp_areas = ANGI___::$instance -> fp_ids;
...
*
*/
if ( ! class_exists( 'ANGI_utils_settings_map' ) ) :
class ANGI_utils_settings_map {
      ...
      */
      public function angi_fn_get_customizer_map( $get_default = null,  $what = null ) {
            ...
            if ( ! empty( ANGI___::$customizer_map ) ) {
                  ...
                  add_filter( 'tc_add_panel_map'            , 'angi_fn_popul_panels_map' );
                  add_filter( 'tc_remove_section_map'       , 'angi_fn_popul_remove_section_map' );
                  //theme switcher's enabled when user opened the customizer from the theme's page
                  add_filter( 'tc_remove_section_map'       , 'angi_fn_set_theme_switcher_visibility' );
                  add_filter( 'tc_add_section_map'          , 'angi_fn_popul_section_map' );
                  //add controls to the map
                  add_filter( 'tc_add_setting_control_map'  , 'angi_fn_popul_setting_control_map', 10, 2 );
                  ...
                  add_filter( 'tc_add_section_map'           , 'angi_fn_force_remove_section_map' );

                  /* ANGI_4 compat */
                  /* ADD SPECIFIC SECTION SETTINGS */
                  //add controls to the map
                  add_filter( 'tc_add_section_map'                , array( $this, 'angi_fn_popul_section_map' ) );
                  add_filter( 'tc_add_setting_control_map'        , array( $this, 'angi_fn_popul_setting_control_map' ), 0, 2 );
      ...
      */
      function angi_fn_popul_setting_control_map( $_map, $get_default = null ) {
            ...
            $_settings_sections = array(
                ...
                'angi_fn_icons_option_map', //Removed in modern style
      ...
      ******************************************************************************************************/
      ...
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_site_identity_option_map( $_map, $get_default = null ) {
            ...
            $_to_add = array(
                  'tc_sticky_logo_upload'  => array(
                                    'control'   =>  version_compare( $wp_version, '4.3', '>=' ) ? 'ANGI_Customize_Cropped_Image_Control' : 'ANGI_Customize_Upload_Control',
                                    'label'     =>  __( 'Sticky Logo Upload (supported formats : .jpg, .png, .gif, svg, svgz)' , 'angilla' ),
                            ...
                            //we can define suggested cropping area and allow it to be flexible (def 150x150 and not flexible)
                                    ...
                                    'dst_width'  => false,
                                    'dst_height'  => false,
                                    'notice'    => __( "Use this upload control to specify a different logo on sticky header mode." , 'angilla')
                  ),
                  //favicon
                  'tc_fav_upload' => array(
                                    'control'   =>  'ANGI_Customize_Upload_Control' ,
                                    'label'       => __( 'Favicon Upload (supported formats : .ico, .png, .gif)' , 'angilla' ),
                                    'title'     => __( 'FAVICON' , 'angilla'),
                                    'section'   =>  'title_tagline' ,
                                    'type'      => 'tc_upload',
                                    'sanitize_callback' => 'angi_fn_sanitize_number',
                                    'priority'  => 25,
      ...
      /*-----------------------------------------------------------------------------------------------------
                                        SKIN SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_skin_option_map( $_map, $get_default = null ) {
            ...
            $_to_add = array(
                  ...
                  'tc_skin'     => array(
                              'default'   => angi_fn_user_started_before_version('1.0.0') ? 'blue3.css' : 'grey.css',
                              'control'   => 'ANGI_controls' ,
                              'label'     =>  __( 'Choose a predefined skin' , 'angilla' ),
                              'section'   =>  'skins_sec' ,
                              'type'      =>  'select' ,
                              'choices'    =>  $get_default ? null : $this -> angi_fn_build_skin_list(),
                              'transport'   =>  'postMessage',
                              'notice'    => __( 'Disabled if the random option is on.' , 'angilla' )
                  ),
                  'tc_skin_random' => array(
                              'default'   => 0,
                              'control'   => 'ANGI_controls',
                              'label'     => __('Randomize the skin', 'angilla'),
                              'section'   => 'skins_sec',
                              'type'      => 'checkbox',
                              'notice'    => __( 'Apply a random color skin on each page load.' , 'angilla' )
                  ),
            );
      ...
      /*-----------------------------------------------------------------------------------------------------
                                     LINKS SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_links_option_map( $_map, $get_default = null ) {
            ...
            $_to_add = array(
                  'tc_link_hover_effect'  =>  array(
                                'default'       => 1,
                                'control'     => 'ANGI_controls' ,
                                'label'         => __( "Fade effect on link hover" , 'angilla' ),
      ...
      function angi_fn_formatting_option_map( $_map, $get_default = null ) {
            ...
            $_to_add = array(
                  'tc_ext_link_style'  =>  array(
                                    'default'       => 0,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Display an icon next to external links" , "angilla" ),
                                    'section'       => 'formatting_sec' ,
                                    'type'          => 'checkbox' ,
                                    'notice'    => __( 'This will be applied to the links included in post or page content only.' , 'angilla' ),
                                    'transport'     => 'postMessage'
                  ),
                  ...
                  'tc_ext_link_target'  =>  array(
                                    'default'       => 0,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Open external links in a new tab" , "angilla" ),
                                    'section'       => 'formatting_sec' ,
                                    'type'          => 'checkbox' ,
                                    'notice'    => __( 'This will be applied to the links included in post or page content only.' , 'angilla' ),
                                    'transport'     => 'postMessage'
                  ...
                  'tc_enable_dropcap'  =>  array(
                                    'default'       => 0,
                                    'title'         => __( 'Drop caps', 'angilla'),
                                    'label'         => __('Enable drop caps' , 'angilla'),
                                    'control'       => 'ANGI_controls' ,
                                    'notice'         => __( "Apply a drop cap to the first paragraph of your post / page content" , "angilla" ),
                                    'section'       => 'formatting_sec' ,
                                    'type'          => 'checkbox',
                  ),
                  'tc_dropcap_minwords'  =>  array(
                                    ...
                                    'sanitize_callback' => 'angi_fn_sanitize_number',
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Apply a drop cap when the paragraph includes at least the following number of words :" , "angilla" ),
                                    'notice'         => __( "(number of words)" , "angilla" ),
                                    'section'       => 'formatting_sec' ,
                                    'type'          => 'number' ,
                                    'step'          => 1,
                                    'min'           => 1,
                  ...
                  'tc_dropcap_design' => array(
                                    'default'     => 'skin-shadow',
                                    'control'     => 'ANGI_controls',
                                    'label'       => __( 'Drop cap style' , 'angilla' ),
                                    'section'     => 'formatting_sec',
                                    'type'      =>  'select' ,
                                    'choices'     => array(
                                            'skin-shadow'    => __( "Primary theme color with a shadow" , 'angilla' ),
                                            'simple-black'   => __( 'Simple black' , 'angilla' ),
                                    ),
                  ),
                  'tc_post_dropcap'  =>  array(
                                    'default'       => 0,
                                    'label'         => __('Enable drop caps in posts' , 'angilla'),
                                    'control'       => 'ANGI_controls' ,
                                    'notice'         => __( "Apply a drop cap to the first paragraph of your single posts content" , "angilla" ),
                                    'section'       => 'formatting_sec' ,
                                    'type'          => 'checkbox',
                  ),
                  'tc_page_dropcap'  =>  array(
                                    'default'       => 0,
                                    'label'         => __('Enable drop caps in pages' , 'angilla'),
                                    'control'       => 'ANGI_controls' ,
                                    'notice'         => __( "Apply a drop cap to the first paragraph of your pages" , "angilla" ),
                                    'section'       => 'formatting_sec' ,
                                    'type'          => 'checkbox',
                  )
            );
      ...
      /*-----------------------------------------------------------------------------------------------------
                                     ICONS SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_icons_option_map( $get_default = null ) {

            return array(
                  'tc_show_title_icon'  =>  array(
                                    'default'       => 1,
                                    'control'     => 'ANGI_controls' ,
                                    'label'         => __( "Display icons next to titles" , 'angilla' ),
                                    'section'       => 'titles_icons_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 10,
                                    'notice'    => __( 'When this option is checked, a contextual icon is displayed next to the titles of pages, posts, archives, and WP built-in widgets.' , 'angilla' ),
                                    'transport'   => 'postMessage'
                  ),
                  'tc_show_page_title_icon'  =>  array(
                                    'default'       => angi_fn_user_started_before_version('1.0.0') ? 1 : 0,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Display a page icon next to the page title" , 'angilla' ),
                                    'section'       => 'titles_icons_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 20,
                                    'transport'   => 'postMessage'
                  ...
                  'tc_show_post_title_icon'  =>  array(
                                    'default'       => angi_fn_user_started_before_version('1.0.0') ? 1 : 0,
                                    'control'     => 'ANGI_controls' ,
                                    'label'         => __( "Display a post icon next to the single post title" , 'angilla' ),
                                    'section'       => 'titles_icons_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 30,
                                    'transport'   => 'postMessage'
                  ...
                  'tc_show_archive_title_icon'  =>  array(
                                    'default'       => 1,
                                    'control'     => 'ANGI_controls' ,
                                    'label'         => __( "Display an icon next to the archive title" , 'angilla' ),
                                    'section'       => 'titles_icons_sec' ,
                                    'type'          => 'checkbox',
                                    'notice'    => __( 'When this option is checked, an archive type icon is displayed in the heading of every types of archives, on the left of the title. An archive page can be : category, tag, author, date archive, custom taxonomies, search results.' , 'angilla' ),
                                    'priority'      => 40,
                                    'transport'   => 'postMessage'
                  ),
                  'tc_show_post_list_title_icon'  =>  array(
                                    'default'       => angi_fn_user_started_before_version('1.0.0') ? 1 : 0,
                                    'control'     => 'ANGI_controls' ,
                                    'label'         => __( "Display an icon next to each post title in an archive page" , 'angilla' ),
                                    'section'       => 'titles_icons_sec' ,
                                    'type'          => 'checkbox',
                                    'notice'    => __( 'When this option is checked, a post type icon is displayed on the left of each post titles in an archive page. An archive page can be : category, tag, author, date archive, custom taxonomies, search results.' , 'angilla' ),
                                    'priority'      => 50,
                                    'transport'   => 'postMessage'
                  ),
                  'tc_show_sidebar_widget_icon'  =>  array(
                                    'default'       => 1,
                                    'control'     => 'ANGI_controls' ,
                                    'label'         => __( "WP sidebar widgets : display icons next to titles" , 'angilla' ),
                                    'section'       => 'titles_icons_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 60,
                                    'transport'   => 'postMessage'
                  ...
                  'tc_show_footer_widget_icon'  =>  array(
                                    'default'       => 1,
                                    'control'     => 'ANGI_controls' ,
                                    'label'         => __( "WP footer widgets : display icons next to titles" , 'angilla' ),
                                    'section'       => 'titles_icons_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 70,
                                    'transport'   => 'postMessage'
      ...
      /*-----------------------------------------------------------------------------------------------------
        IMAGE SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_images_option_map( $_map, $get_default = null ) {
            ...
            $_to_add  = array(
                  ...
                  'tc_fancybox_autoscale' =>  array(
                                    'default'       => 1,
                                    'control'   => 'ANGI_controls' ,
                                    'label'       => __( 'Autoscale images on zoom' , 'angilla' ),
                                    'section'     => 'images_sec' ,
                                    'type'        => 'checkbox' ,
                                    'priority'    => 2,
                                    'notice'    => __( 'If enabled, this option will force images to fit the screen on lightbox zoom.' , 'angilla' ),
                  ),

                'tc_display_slide_loader'  =>  array(
                                    'default'       => 1,
                                    'control'   => 'ANGI_controls' ,
                                    'label'       => __( "Sliders : display on loading icon before rendering the slides" , 'angilla' ),
                                    'section'     => 'images_sec' ,
                                    'type'        => 'checkbox' ,
                                    'priority'    => 15,
                                    'notice'    => __( 'When checked, this option displays a loading icon when the slides are being setup.' , 'angilla' ),
                  ),
            ...
            if ( version_compare( $wp_version, '4.4', '>=' ) ) {
                  $_to_add[ 'tc_resp_slider_img' ] =  array(
                                    'default'     => 0,
                                    'control'     => 'ANGI_controls' ,
                                    'label'       => __( 'Improve your page speed by loading smaller slider images for mobile devices' , 'angilla' ),
                                    'section'     => 'images_sec' ,
                                    'type'        => 'checkbox' ,
                                    'priority'    => 24,
                                    'ubq_section'   => array(
      ...
      ******************************************************************************************************/
      ...
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_header_design_option_map( $_map, $get_default = null ) {
            ...
            $_to_add  = array(
                  'tc_social_in_header' =>  array(
                                    'default'       => 1,
                                    'label'       => __( 'Social links in header' , 'angilla' ),
                                    'control'   =>  'ANGI_controls' ,
                  ...
                  'tc_show_tagline'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Display the tagline in the header" , "angilla" ),
                                    'section'       => 'header_layout_sec' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 15,
                                    'transport'    => angi_fn_is_partial_refreshed_on() ? 'postMessage' : 'refresh',
                  ...
                  'tc_woocommerce_header_cart' => array(
                                  'default'   => 1,
                                  'label'     => sprintf('<span class="dashicons dashicons-cart"></span> %s', __( "Display the shopping cart in the header" , 'angilla' ) ),
                                  'control'   => 'ANGI_controls' ,
                                  'section'   => 'header_layout_sec',
                  ...
                  'tc_display_boxed_navbar'  =>  array(
                                    'default'       => angi_fn_user_started_before_version('1.0.0') ? 1 : 0,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Display menu in a box" , 'angilla' ),
                                    'section'       => 'header_layout_sec' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 25,
                                    'transport'     => 'postMessage',
                                    'notice'        => __( 'If checked, this option wraps the header menu/tagline/social in a light grey box.' , 'angilla' ),
                  ),
                  'tc_sticky_header'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'title'         => __( 'Sticky header settings' , 'angilla'),
                                    'label'         => __( "Sticky on scroll" , 'angilla' ),
                                    'section'       => 'header_layout_sec' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 30,
                                    'transport'     => 'postMessage',
                                    'notice'    => __( 'If checked, this option makes the header stick to the top of the page on scroll down.' , 'angilla' )
                  ),
                  'tc_sticky_show_tagline'  =>  array(
                                    'default'       => 0,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Sticky header : display the tagline" , "angilla" ),
                                    'section'       => 'header_layout_sec' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 40,
                                    'transport'     => 'postMessage',
                  ...
                  'tc_woocommerce_header_cart_sticky' => array(
                                    'default'   => 1,
                                    'label'     => sprintf('<span class="dashicons dashicons-cart"></span> %s', __( "Sticky header: display the shopping cart" , "angilla" ) ),
                                    'control'   => 'ANGI_controls' ,
                                    'section'   => 'header_layout_sec',
                                    'type'      => 'checkbox' ,
                                    'priority'  => 45,
                                    'transport' => 'postMessage',
                                    'active_callback' => apply_filters( 'tc_woocommerce_options_enabled', '__return_false' ),
                                    'notice'    => __( 'WooCommerce: if checked, your WooCommerce cart icon will remain visible when scrolling.' , 'angilla' )
                  ),
                  'tc_sticky_show_title_logo'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Sticky header : display the title / logo" , "angilla" ),
                                    'section'       => 'header_layout_sec' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 50,
                                    'transport'     => 'postMessage',
                  ...
                  'tc_sticky_show_menu'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Sticky header : display the menu" , "angilla" ),
                                    'section'       => 'header_layout_sec' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 60,
                                    'transport'     => 'postMessage',
                                    'notice'        => __('Also applied to the secondary menu if any.' , 'angilla')
                  ),
            );
      ...
      function angi_fn_header_desktop_option_map( $_map, $get_default = null ) {
            if ( !is_array( $_map ) || empty( $_map ) ) {
                  return $_map;
            }
      ...
      function angi_fn_header_mobile_option_map( $_map, $get_default = null ) {
            if ( !is_array( $_map ) || empty( $_map ) ) {
                  return $_map;
            }
      ...
      /*-----------------------------------------------------------------------------------------------------
                          NAVIGATION SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_navigation_option_map( $_map, $get_default = null ) {
            ...
            $_to_add  = array(
                  'tc_menu_resp_dropdown_limit_to_viewport'  =>  array(
                                    'default'       => 0,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => sprintf('<span class="dashicons dashicons-smartphone"></span> %s', __( "For mobile devices (responsive), limit the height of the dropdown menu block to the visible viewport." , 'angilla' ) ),
                                    'section'       => 'nav' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 35,
                                    //'transport'     => 'postMessage',
                  ...
                  'tc_display_menu_label'  =>  array(
                                    'default'       => 0,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( "Display a label next to the menu button." , 'angilla' ),
                                    'section'       => 'nav' ,
                                    'type'          => 'checkbox' ,
                                    'priority'      => 45,
                                    'notice'        => __( 'Note : the label is hidden on mobile devices.' , 'angilla' ),
                  ),
                  //override
                  'tc_menu_position'  =>  array(
                                    ...
                                    'choices'       => array(
                                            ...
                                            'pull-menu-center'    => __( 'Menu centered' , 'angilla' ),
                                            'pull-menu-right'     => __( 'Menu on the right' , 'angilla' )
                                    ),
                                    'priority'      => 50,
                                    'transport'     => 'postMessage',
                                    'notice'        => sprintf( '%1$s <a href="%2$s">%3$s</a>.',
                                        __("Note : the menu centered position is available only when" , 'angilla'),
                                        "javascript:wp.customize.section('header_layout_sec').focus();",
                                        __("the logo is centered", 'angilla')
                                    )
                  ),
                  //override
                  ...
                  'tc_second_menu_resp_setting'  =>  array(
                                    ...
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => sprintf('<span class="dashicons dashicons-smartphone"></span> %s', __( "Choose a mobile devices (responsive) behaviour for the secondary menu." , 'angilla' ) ),
                                    'section'       => 'nav',
                                    'type'      =>  'select',
                                    'choices'     => array(
                                        'in-sn-before'   => __( 'Move before inside the side menu ' , 'angilla'),
                                        'in-sn-after'   => __( 'Move after inside the side menu ' , 'angilla'),
                                        'display-in-header'   => __( 'Display in the header' , 'angilla'),
                                        'hide'   => __( 'Hide' , 'angilla'  ),
                                    ),
                                    'priority'      => 90,
                  ),
      ...
      ******************************************************************************************************/
      ...
      /*-----------------------------------------------------------------------------------------------------
                                    FRONT PAGE SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_front_page_option_map( $_map, $get_default = null ){
      ...
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_layout_option_map( $_map, $get_default = null ){
      ...
      /*-----------------------------------------------------------------------------------------------------
                                    POST METAS SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_post_metas_option_map( $_map, $get_default = null ){
            ...
            $_to_add  = array(
                  ...
                  'tc_post_metas_design'  =>  array(
                                    'default'       => angi_fn_user_started_before_version('1.0.0') ? 'buttons' : 'no-buttons',
                                    'control'       => 'ANGI_controls' ,
                                    'title'         => __( 'Metas Design' , 'angilla' ),
                                    'label'         => __( "Select a design for the post metas" , 'angilla' ),
                                    'section'       => 'post_metas_sec' ,
                                    'type'          =>  'select' ,
                                    'choices'       => array(
                                        'buttons'     => __( 'Buttons and text' , 'angilla' ),
                                        'no-buttons'  => __( 'Text only' , 'angilla' )
                                    ),
                                    'priority'      => 10
                  ),
                  'tc_post_metas_update_date_format'  =>  array(
                                    'default'       => 'days',
                                    'control'       => 'ANGI_controls',
                                    'label'         => __( "Select the last update format" , 'angilla' ),
                                    'section'       => 'post_metas_sec',
                                    'type'          =>  'select' ,
                                    'choices'       => array(
                                            'days'     => __( 'No. of days since last update' , 'angilla' ),
                                            'date'     => __( 'Date of the last update' , 'angilla' )
                                    ),
                                    'priority'      => 55
                  ),
                  ...
                  'tc_post_metas_update_notice_in_title'  =>  array(
                                    'default'       => angi_fn_user_started_before_version('1.0.0') ? 1 : 0,
                                    'control'       => 'ANGI_controls',
                                    'title'         => __( 'Recent update notice after post titles' , 'angilla' ),
                                    'label'         => __( "Display a recent update notice" , 'angilla' ),
                                    'section'       => 'post_metas_sec',
                                    'type'          => 'checkbox',
                                    'priority'      => 65,
                                    'notice'    => __( 'If this option is checked, a customizable recent update notice is displayed next to the post title.' , 'angilla' )
                  ),
                  'tc_post_metas_update_notice_interval'  =>  array(
                                    'default'       => 10,
                                    'control'       => 'ANGI_controls',
                                    'sanitize_callback' => 'angi_fn_sanitize_number',
                                    'label'         => __( "Display the notice if the last update is less (strictly) than n days old" , 'angilla' ),
                                    'section'       => 'post_metas_sec',
                                    'type'          => 'number' ,
                                    'step'          => 1,
                                    'min'           => 0,
                                    'priority'      => 70,
                                    'notice'    => __( 'Set a maximum interval (in days) during which the last update notice will be displayed.' , 'angilla' ),
                  ),
                  'tc_post_metas_update_notice_text'  =>  array(
                                    'default'       => __( "Recently updated !" , 'angilla' ),
                                    'control'       => 'ANGI_controls',
                                    'label'         => __( "Update notice text" , 'angilla' ),
                                    'section'       => 'post_metas_sec',
                                    'type'          => 'text',
                                    'priority'      => 75,
                                    'transport'   => 'postMessage'
                  ...
                  'tc_post_metas_update_notice_format'  =>  array(
                                    ...
                                    'control'       => 'ANGI_controls',
                                    'label'         => __( "Update notice style" , 'angilla' ),
                                    'section'       => 'post_metas_sec',
                                    'type'          =>  'select' ,
                                    'choices'       => array(
                                            'label-default'   => __( 'Default (grey)' , 'angilla' ),
                                            'label-success'   => __( 'Success (green)' , 'angilla' ),
                                            'label-warning'   => __( 'Alert (orange)' , 'angilla' ),
                                            'label-important' => __( 'Important (red)' , 'angilla' ),
                                            'label-info'      => __( 'Info (blue)' , 'angilla' )
                                    ),
                                    'priority'      => 80,
                                    'transport'   => 'postMessage'
                  )
            ...
            if ( isset( $_map[ 'tc_show_post_metas_update_date' ] ) )
                  $_map[ 'tc_show_post_metas_update_date' ]['notice'] = __( 'If this option is checked, additional date informations about the the last post update can be displayed (nothing will show up if the post has never been updated).' , 'angilla' );
      ...
      /*-----------------------------------------------------------------------------------------------------
                                    POST LISTS SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_post_list_option_map( $_map, $get_default = null ) {
            ...
            $_to_add  = array(
                  ...
                  'tc_post_list_length' =>  array(
                                    'default'       => 'excerpt',
                                    'label'         => __( 'Select the length of posts in lists (home, search, archives, ...)' , 'angilla' ),
                                    'section'       => 'post_lists_sec' ,
                                    'type'          => 'select' ,
                                    'choices'       => array(
                                            'excerpt'   => __( 'Display the excerpt' , 'angilla' ),
                                            'full'    => __( 'Display the full content' , 'angilla' )
                                            ),
                                    'priority'       => 20,
                  ),
                  //classic grid
                  'tc_grid_in_blog'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( 'Apply the grid layout to Home/Blog' , "angilla" ),
                                    'section'       => 'post_lists_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 57
                  ),
                  'tc_grid_in_archive'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( 'Apply the grid layout to Archives (archives, categories, author posts)' , "angilla" ),
                                    'section'       => 'post_lists_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 58
                  ),
                  'tc_grid_in_search'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( 'Apply the grid layout to Search results' , "angilla" ),
                                    'section'       => 'post_lists_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 60,
                                    'notice'        => __( 'Unchecked contexts are displayed with the alternate thumbnails layout.' , 'angilla' ),
                  ),
                  'tc_grid_icons'  =>  array(
                                    'default'       => 1,
                                    'control'       => 'ANGI_controls' ,
                                    'label'         => __( 'Display post format icons' , "angilla" ),
                                    'section'       => 'post_lists_sec' ,
                                    'type'          => 'checkbox',
                                    'priority'      => 63,
                                    'transport'     => 'postMessage'
                  ...
                  'tc_post_list_default_thumb'  => array(
                                    'control'   =>  version_compare( $wp_version, '4.3', '>=' ) ? 'ANGI_Customize_Cropped_Image_Control' : 'ANGI_Customize_Upload_Control',
                                    'label'         => __( 'Upload a default thumbnail' , 'angilla' ),
                                    'section'   =>  'post_lists_sec' ,
                                    'sanitize_callback' => 'angi_fn_sanitize_number',
                            //we can define suggested cropping area and allow it to be flexible (def 150x150 and not flexible)
                                    'width'         => 570,
                  ...
                  'tc_grid_thumb_height' => array(
                                    ...
                                    'sanitize_callback' => 'angi_fn_sanitize_number',
                                    'control'       => 'ANGI_controls' ,
                                    'title'         => __( 'Thumbnails max height for the grid layout' , 'angilla' ),
                                    'label'         => __( "Set the post grid thumbnail's max height in pixels" , 'angilla' ),
                                    'section'       => 'post_lists_sec' ,
                                    'type'          => 'number' ,
                                    'step'          => 1,
                                    'min'           => 0,
            ...
            $_map['tc_post_list_thumb_shape']['choices'] = array_merge( $_map['tc_post_list_thumb_shape']['choices'], array(
                                    'squared'               => __( 'Squared, expand on hover' , 'angilla'),
                                    'squared-expanded'      => __( 'Squared, no expansion' , 'angilla'),
                                    'rectangular'           => __( 'Rectangular with no effect' , 'angilla'  ),
                                    'rectangular-blurred'   => __( 'Rectangular with blur effect on hover' , 'angilla'  ),
                                    'rectangular-unblurred' => __( 'Rectangular with unblur effect on hover' , 'angilla')
            ) );

            //Remove only thumb shape
      ...
      /*-----------------------------------------------------------------------------------------------------
                                     COMMENTS SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_comment_option_map( $_map, $get_default = null ) {
            ...
            $_to_add  = array(
                  ...
                  'tc_comment_bubble_shape' => array(
                                  'default'     => 'default',
                                  'control'     => 'ANGI_controls',
                                  'label'       => __( 'Comments bubble shape' , 'angilla' ),
                                  'section'     => 'comments_sec',
                                  'type'      =>  'select' ,
                                  'choices'     => array(
                                          'default'             => __( "Small bubbles" , 'angilla' ),
                                          'custom-bubble-one'   => __( 'Large bubbles' , 'angilla' ),
                                  ),
                                  'priority'    => 10,
                  ),
                  /* Removed in c4 */
                  'tc_comment_bubble_color_type' => array(
                                  'default'     => angi_fn_user_started_before_version('1.0.0') ? 'custom' : 'skin',
                                  'control'     => 'ANGI_controls',
                                  'label'       => __( 'Comments bubble color' , 'angilla' ),
                                  'section'     => 'comments_sec',
                                  'type'      =>  'select' ,
                                  'choices'     => array(
                                          'skin'     => __( "Skin color" , 'angilla' ),
                  ...
                  'tc_comment_bubble_color' => array(
                                  'default'     => angi_fn_user_started_before_version('1.0.0') ? '#F00' : ANGI_utils::$inst->angi_fn_get_skin_color(),
      ...
      /*-----------------------------------------------------------------------------------------------------
        COMMENTS SECTION
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_single_post_option_map( $_map, $get_default = null ) {
      ...
      ******************************************************************************************************/
      ...
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_sidebars_option_map( $_map, $get_default = null ) {
            ...
            $_to_add  = array(
                  ...
                  'tc_social_in_sidebar_title'  =>  array(
                                    'default'       => __( 'Social links' , 'angilla' ),
                                    'label'       => __( 'Social link title in sidebars' , 'angilla' ),
                                    'control'   =>  'ANGI_controls' ,
      ...
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_responsive_option_map( $_map, $get_default = null ) {
            if ( !is_array( $_map ) || empty( $_map ) ) {
                  return $_map;
            }
            ...
            $_to_add  = array(
                  'tc_block_reorder'  =>  array(
                                    'default'       => 1,
                                    'control'   => 'ANGI_controls' ,
                                    'label'         => sprintf('<span class="dashicons dashicons-smartphone"></span> %s', __( 'Dynamic sidebar reordering on small devices' , 'angilla' ) ),
                                    'section'     => 'responsive_sec' ,
                                    'type'        => 'checkbox' ,
                                    'notice'    => __( 'Activate this option to move the sidebars (if any) after the main content block, for smartphones or tablets viewport.' , 'angilla' ),
                  )
            );
      ...
      ******************************************************************************************************/
      ...
      ------------------------------------------------------------------------------------------------------*/
      function angi_fn_footer_global_settings_option_map( $_map, $get_default = null ) {
      ...
      *******************************************************************************************************
      ******************************************************************************************************/
      function angi_fn_woocommerce_option_map( $_map, $get_default = null ) {
            if ( !is_array( $_map ) || empty( $_map ) ) {
                  return $_map;
            }
      ...
      function angi_fn_popul_section_map( $_sections ) {
            ...
            $_old_sections = array(
                  ...
                  'titles_icons_sec'        => array(
                                      'title'     =>  __( 'Titles icons settings' , 'angilla' ),
                                      'priority'    =>  $this->is_wp_version_before_4_0 ? 18 : 40,
                                      'description' =>  __( 'Set up the titles icons options' , 'angilla' ),
                  ...
                  'responsive_sec'           => array(
                                      ...
                                      'priority'    =>  20,
                                      'description' =>  __( 'Various settings for responsive display' , 'angilla' ),
      ...
      */
      ...
      function angi_fn_build_skin_list() {
        $tc_base        = TC_BASE;
        $tc_base_child  = TC_BASE_CHILD;

        $parent_skins   = $this -> angi_fn_get_skins( $tc_base .'inc/assets/css');
        $child_skins    = ( angi_fn_is_child() && file_exists( $tc_base_child .'inc/assets/css') ) ? $this -> angi_fn_get_skins( $tc_base_child .'inc/assets/css') : array();
      ...
      */
      function angi_fn_get_skins($path) {
        ...
        $default_skin_list    = ANGI_init::$instance -> skins;
        ...
        foreach( $files as $file ) {
            ...
            if ( $file[0] != '.' && !is_dir($path.$file) ) {
              if ( substr( $file, -4) == '.css' ) {
                $skin_list[$file] = isset($default_skin_list[$file]) ?  call_user_func( '__' , $default_skin_list[$file] , 'angilla' ) : substr_replace( $file , '' , -4 , 4);
              }
            }
          }//endforeach
        $_to_return = array();
...
*
*/
if ( ! class_exists( 'ANGI_init_retro_compat' ) ) :
  class ANGI_init_retro_compat {
    static $instance;
    ...
    */
    function __construct () {
      ...
      if ( is_user_logged_in() && current_user_can( 'edit_theme_options' ) ) {
        $theme_options            = angi_fn_get_unfiltered_theme_options();
        $_to_update               = false;

        if ( ! empty( $theme_options ) ) {
          //Socials
          $_new_options_w_socials     = $this -> angi_fn_maybe_move_old_socials_to_customizer_fmk( $theme_options );
          ...
          //Custom css
          $_new_options_w_custom_css  = $this -> angi_fn_maybe_move_old_css_to_wp_embed( $theme_options );
          ...
          if ( $_to_update ) {
            update_option( ANGI_THEME_OPTIONS, $theme_options );
          }
        }
      }
    }//construct
    ...
    */
    function angi_fn_maybe_move_old_socials_to_customizer_fmk( $theme_options ) {
      ...
      */
      ...
      $_old_socials          = ANGI___::$instance -> old_socials;
    ...
    * returns array() the new set of options or empty if there's nothing to move
    */
    function angi_fn_maybe_move_old_css_to_wp_embed( $theme_options ) {
      $_options = $theme_options;
...
if ( ! class_exists( 'ANGI_utils' ) ) :
  class ANGI_utils {
      ...
      function __construct () {
        ...
        //title
        add_action( 'wp_head'                 , array( $this , 'angi_fn_wp_filters') );

        //get all options
        add_filter( '__options'               , 'angi_fn_get_theme_options' , 10, 1 );
        //get single option
        add_filter( '__get_option'            , 'angi_fn_opt' , 10, 2 );//deprecated

        //some useful filters
        add_filter( '__ID'                    , 'angi_fn_get_id' );//deprecated
        add_filter( '__screen_layout'         , array( $this , 'angi_fn_get_layout' ) , 10 , 2 );//deprecated
        add_filter( '__is_home'               , 'angi_fn_is_real_home' );
        add_filter( '__is_home_empty'         , 'angi_fn_is_home_empty' );
        add_filter( '__post_type'             , 'angi_fn_get_post_type' );
        add_filter( '__is_no_results'         , 'angi_fn_is_no_results' );
        add_filter( '__article_selectors'     , array( $this , 'angi_fn_article_selectors' ) );

        //social networks
        add_filter( '__get_socials'           , 'angi_fn_get_social_networks', 10, 0 );
      }
      ...
      */
      function angi_fn_wp_filters() {
        ...
        if ( apply_filters( 'tc_enable_fancybox_in_wc_short_description', false  ) ) {
            add_filter( 'woocommerce_short_description'   , array( $this, 'angi_fn_fancybox_content_filter' ) );
        }
        /*
        * Smartload disabled for content retrieved via ajax
        */
        if ( apply_filters( 'tc_globally_enable_img_smart_load', ! angi_fn_is_ajax() && esc_attr( angi_fn_opt( 'tc_img_smart_load' ) ) ) ) {
            add_filter( 'the_content'                       , 'angi_fn_parse_imgs', PHP_INT_MAX );
            add_filter( 'tc_thumb_html'                     , 'angi_fn_parse_imgs' );
            if ( apply_filters( 'tc_enable_img_smart_load_in_wc_short_description', false  ) ) {
                add_filter( 'woocommerce_short_description' , 'angi_fn_parse_imgs' );
            }
        }
        add_filter( 'wp_title'                            , 'angi_fn_wp_title' , 10, 2 );
      }
      ...
      */
      function angi_fn_get_skin_color( $_what = null ) {
          ...
          $_active_skin =  str_replace('.min.', '.', basename( ANGI_init::$instance -> angi_fn_get_style_src() ) );
      ...
      */
      function angi_fn_opt( $option_name , $option_group = null, $use_default = true ) {
          return angi_fn_opt( $option_name , $option_group, $use_default );
      }

      //backward compatibility
      //used until FPU 2.0.33
      function angi_fn_parse_imgs( $_html ) {
        return angi_fn_parse_imgs( $_html );
      }
      ...
      * @since Angilla 1.0
      */
      public static function angi_fn_id()  {
          return angi_fn_get_id();
      }
      ...
      * @since Angilla 1.0
      */
      public static function angi_fn_get_layout( $post_id , $sidebar_or_class = 'class' ) {
          $__options                    = angi_fn__f ( '__options' );
          ...
          if ( apply_filters( 'tc_is_post_layout', is_single( $post_id ), $post_id ) || angi_fn_is_attachment_image() ) {
          ...
          elseif ( $is_singular_layout || is_singular() || angi_fn_is_attachment_image() || $wp_query -> is_posts_page )
            $tc_specific_post_layout  = esc_attr( get_post_meta( $post_id, $key = 'layout_key' , $single = true ) );
      ...
      * @since 3.0.10
      */
      function angi_fn_get_post_class( $class = '', $post_id = null ) {
      ...
      * @since 3.1.0
      */
      function angi_fn_article_selectors() {
        ...
        $selectors                  = $single_post_selector_bool ? apply_filters( 'tc_single_post_selectors' ,'id="post-'.get_the_ID().'" '.$this -> angi_fn_get_post_class('row-fluid') ) : $selectors;

        // POST LIST
        $post_list_selector_bool    = ( isset($post) && !is_singular() && !is_404() && !angi_fn__f( '__is_home_empty') ) || ( is_search() && 0 != $wp_query -> post_count );
        $selectors                  = $post_list_selector_bool ? apply_filters( 'tc_post_list_selectors' , 'id="post-'.get_the_ID().'" '.$this -> angi_fn_get_post_class( 'row-fluid grid-item' ) ) : $selectors;

        // PAGE
        $page_selector_bool         = isset($post) && 'page' == angi_fn__f('__post_type') && is_singular() && !angi_fn__f( '__is_home_empty');
        $selectors                  = $page_selector_bool ? apply_filters( 'tc_page_selectors' , 'id="page-'.get_the_ID().'" '.$this -> angi_fn_get_post_class('row-fluid') ) : $selectors;

        // ATTACHMENT
        //checks if attachement is image and add a selector
        $format_image               = wp_attachment_is_image() ? 'format-image' : '';
        $selectors                  = ( isset($post) && 'attachment' == $post -> post_type && is_singular() ) ? apply_filters( 'tc_attachment_selectors' , 'id="post-'.get_the_ID().'" '.$this -> angi_fn_get_post_class(array('row-fluid', $format_image) ) ) : $selectors;
...
*
*/
if ( ! class_exists( 'ANGI_resources' ) ) :
	class ANGI_resources {
	    //Access any method or var of the class with classname::$instance -> var or method():
	    static $instance;
      ...
      private $_resources_version;
	    ...
	    function __construct () {
          ...
          add_action( 'wp_enqueue_scripts'            , array( $this , 'angi_fn_enqueue_gfonts' ) , 0 );
	        add_action( 'wp_enqueue_scripts'						, array( $this , 'angi_fn_enqueue_front_styles' ) );
          add_action( 'wp_enqueue_scripts'						, array( $this , 'angi_fn_enqueue_front_scripts' ) );

	        //Custom CSS
          add_filter('tc_user_options_style'          , array( $this , 'angi_fn_write_custom_css') , apply_filters( 'tc_custom_css_priority', 9999 ) );
          add_filter('tc_user_options_style'          , array( $this , 'angi_fn_write_fonts_inline_css') );
          add_filter('tc_user_options_style'          , array( $this , 'angi_fn_write_dropcap_inline_css') );

          add_filter('tc_user_options_style'          , array( $this , 'angi_fn_apply_media_upload_front_patch' ) );
          add_filter('tc_user_options_style'          , array( $this , 'angi_fn_maybe_avoid_double_social_icon' ) );

          //set random skin
          add_filter ('tc_opt_tc_skin'                , array( $this, 'angi_fn_set_random_skin' ) );

          add_action( 'wp_ajax_dismiss_style_switcher_note_front',  array( $this , 'angi_fn_dismiss_style_switcher_note_front' ) );
          add_action( 'wp_ajax_nopriv_dismiss_style_switcher_note_front',  array( $this , 'angi_fn_dismiss_style_switcher_note_front' ) );

          //stores the front scripts map in a property
          $this -> tc_script_map = $this -> angi_fn_get_script_map();

          add_filter( 'angi_style_note_content', array( $this,  'angi_fn_get_style_note_content' ) );
	    }//construct
      ...
      function angi_fn_enqueue_front_styles() {
  	      ...
  	      wp_enqueue_style( 'angilla-common', ANGI_init::$instance -> angi_fn_get_style_src( 'common') , array() , $this->_resouces_version, 'all' );
            //Angilla active skin
  	      wp_register_style( 'angilla-skin', ANGI_init::$instance -> angi_fn_get_style_src( 'skin'), array('angilla-common'), $this->_resouces_version, 'all' );
  	      wp_enqueue_style( 'angilla-skin' );
  	      //Angilla stylesheet (style.css)
  	      wp_enqueue_style( 'angilla-style', get_stylesheet_uri(), array( 'angilla-skin' ), $this->_resouces_version , 'all' );

  	      //Customizer user defined style options : the custom CSS is written with a high priority here
  	      wp_add_inline_style( 'angilla-skin', apply_filters( 'tc_user_options_style' , '' ) );
  		}
      ...
      */
      private function angi_fn_get_script_map( $_handles = array() ) {
          ...
          $_libs_path =  ANGI_ASSETS_PREFIX . 'front/js/libs/';
          ...
          $_map = array(
              ...
              'tc-main-front' => array(
                ...
                'dependencies' => $this -> angi_fn_is_fancyboxjs_required() ? array( 'tc-js-arraymap-proto', 'jquery' , 'tc-js-params', 'tc-img-original-sizes', 'tc-bootstrap', 'tc-fancybox' , 'underscore' ) : array( 'jquery' , 'tc-js-params', 'tc-img-original-sizes', 'tc-bootstrap' , 'underscore' )
              ),
              //loaded separately => not included in tc-script.js
              'tc-fancybox' => array(
                'path' => $_libs_path . 'fancybox/',
                'files' => array( 'jquery.fancybox-1.3.4.min.js' ),
                'dependencies' => $this -> angi_fn_load_concatenated_front_scripts() ? array( 'jquery' ) : array( 'tc-js-arraymap-proto', 'jquery' , 'tc-js-params', 'tc-bootstrap' )
              ),
              //concats all scripts except fancybox
              'tc-scripts' => array(
                'path' => $_front_path,
                'files' => array( 'tc-scripts.js' ),
                'dependencies' =>  $this -> angi_fn_is_fancyboxjs_required() ? array( 'jquery', 'tc-fancybox' ) : array( 'jquery' )
              )
          );//end of scripts map
      ...
      *
  		...
  		* @since Angilla 1.0
  		*/
  		function angi_fn_enqueue_front_scripts() {
  	    //wp scripts
        ...
        //angilla scripts and libs
  	   	if ( $this -> angi_fn_load_concatenated_front_scripts() )	{
          if ( $this -> angi_fn_is_fancyboxjs_required() )
            $this -> angi_fn_enqueue_script( 'tc-fancybox' );
          $this -> angi_fn_enqueue_script( 'tc-scripts' );
  			}
  			else {
          wp_enqueue_script( 'underscore' );
          //!!mind the dependencies
          $this -> angi_fn_enqueue_script( array( 'tc-js-params', 'tc-js-arraymap-proto', 'tc-img-original-sizes', 'tc-bootstrap', 'tc-smoothscroll', 'tc-outline', 'tc-waypoints' ) );

          if ( $this -> angi_fn_is_fancyboxjs_required() )
            $this -> angi_fn_enqueue_script( 'tc-fancybox' );

          $this -> angi_fn_enqueue_script( array( 'tc-dropcap' , 'tc-img-smartload', 'tc-ext-links', 'tc-center-images', 'tc-parallax', 'tc-main-front' ) );
  			}//end of load concatenate script if

        //carousel options
        //gets slider options if any for home/front page or for others posts/pages
        $js_slidername      = angi_fn__f('__is_home') ? angi_fn_opt( 'tc_front_slider' ) : get_post_meta( angi_fn_get_id() , $key = 'post_slider_key' , $single = true );
        $js_sliderdelay     = angi_fn__f('__is_home') ? angi_fn_opt( 'tc_slider_delay' ) : get_post_meta( angi_fn_get_id() , $key = 'slider_delay_key' , $single = true );
  			...
  			$anchor_smooth_scroll 		  = ( false != esc_attr( angi_fn_opt( 'tc_link_scroll') ) ) ? 'easeOutExpo' : 'linear';
  			if ( false != esc_attr( angi_fn_opt( 'tc_link_scroll') ) )
        ...
        $smooth_scroll_enabled = apply_filters('tc_enable_smoothscroll', ! wp_is_mobile() && 1 == esc_attr( angi_fn_opt( 'tc_smoothscroll') ) );
        $smooth_scroll_options = apply_filters('tc_smoothscroll_options', array( 'touchpadSupport' => false ) );

        //smart load
        $smart_load_enabled   = esc_attr( angi_fn_opt( 'tc_img_smart_load' ) );
        ...
        //Style switcher note
        $is_style_switch_note_on = ! is_multisite() && angi_fn_user_can_see_customize_notices_on_front() && ! angi_fn_is_customizing() && ! angi_fn_isprevdem();
        $style_note_content = '';
        if ( $is_style_switch_note_on && ! angi_fn_is_ms() && false === angi_fn_opt( 'tc_style', ANGI_THEME_OPTIONS, false ) ) { //false for not default
            $tc_custom_css = esc_html( angi_fn_opt( 'tc_custom_css') );
            ...
            $is_style_switch_note_on = apply_filters(
                'angi_is_style_switch_notification_on',
                $is_style_switch_note_on && ! ANGI_IS_MODERN_STYLE && ! is_child_theme() && 'dismissed' != get_transient( 'angi_style_switch_note_status' )
            );
            if ( $is_style_switch_note_on ) {
                $style_note_content = apply_filters( 'angi_style_note_content', '' );
            }
        }

  			wp_localize_script(
  	        $this -> angi_fn_load_concatenated_front_scripts() ? 'tc-scripts' : 'tc-js-params',
  	        'TCParams',
  	        apply_filters( 'tc_angilla_script_params' , array(
  	          	'_disabled'          => apply_filters( 'tc_disabled_front_js_parts', array() ),
                'FancyBoxState' 		=> $this -> angi_fn_is_fancyboxjs_required(),
  	          	'FancyBoxAutoscale' => ( 1 == angi_fn_opt( 'tc_fancybox_autoscale') ) ? true : false,
  	          	'SliderName' 			  => $js_slidername,
  	          	'SliderDelay' 			=> $js_sliderdelay,
  	          	'SliderHover'			  => apply_filters( 'tc_stop_slider_hover', true ),
  	          	'centerSliderImg'   => esc_attr( angi_fn_opt( 'tc_center_slider_img') ),
                ...
                'anchorSmoothScrollExclude' => $anchor_smooth_scroll_exclude,
  	          	'ReorderBlocks' 		=> esc_attr( angi_fn_opt( 'tc_block_reorder') ),
  	          	'centerAllImg' 			=> esc_attr( angi_fn_opt( 'tc_center_img') ),
  	          	...
  	          	'stickyHeader' 			=> esc_attr( angi_fn_opt( 'tc_sticky_header' ) ),
  	          	'dropdowntoViewport' 	=> esc_attr( angi_fn_opt( 'tc_menu_resp_dropdown_limit_to_viewport') ),
  	          	'timerOnScrollAllBrowsers' => apply_filters( 'tc_timer_on_scroll_for_all_browser' , true), //<= if false, for ie only
                'extLinksStyle'       => esc_attr( angi_fn_opt( 'tc_ext_link_style' ) ),
                'extLinksTargetExt'   => esc_attr( angi_fn_opt( 'tc_ext_link_target' ) ),
                'extLinksSkipSelectors'   => apply_filters( 'tc_ext_links_skip_selectors' , array( 'classes' => array('btn', 'button') , 'ids' => array() ) ),
                'dropcapEnabled'      => esc_attr( angi_fn_opt( 'tc_enable_dropcap' ) ),
                'dropcapWhere'      => array( 'post' => esc_attr( angi_fn_opt( 'tc_post_dropcap' ) ) , 'page' => esc_attr( angi_fn_opt( 'tc_page_dropcap' ) ) ),
                'dropcapMinWords'     => esc_attr( angi_fn_opt( 'tc_dropcap_minwords' ) ),
                ...
                'imgSmartLoadsForSliders' => angi_fn_is_checked( 'tc_slider_img_smart_load' ),

                'goldenRatio'         => apply_filters( 'tc_grid_golden_ratio' , 1.618 ),
                'gridGoldenRatioLimit' => esc_attr( angi_fn_opt( 'tc_grid_thumb_height' ) ),
                'isSecondMenuEnabled'  => angi_fn_is_secondary_menu_enabled(),
                'secondMenuRespSet'   => esc_attr( angi_fn_opt( 'tc_second_menu_resp_setting' ) ),

                'isParallaxOn'        => esc_attr( angi_fn_opt( 'tc_slider_parallax') ),
                ...
                'ajaxUrl'             => add_query_arg(
                      array( 'angiajax' => true ), //to scope our ajax calls
                      set_url_scheme( home_url( '/' ) )
                ),
                'frontNonce'   => array( 'id' => 'ANGIFrontNonce', 'handle' => wp_create_nonce( 'angi-front-nonce' ) ),

                'isDevMode'        => ( defined('WP_DEBUG') && true === WP_DEBUG ) || ( defined('ANGI_DEV') && true === ANGI_DEV ),
                'isModernStyle'    => ANGI_IS_MODERN_STYLE,

                'i18n' => apply_filters( 'angi_front_js_translated_strings',
                    array(
                        'Permanently dismiss' => __('Permanently dismiss', 'angilla')
                    )
                ),
        ...
        $tc_show_featured_pages 	         = class_exists('ANGI_featured_pages') && ANGI_featured_pages::$instance -> angi_fn_show_featured_pages();
      	if ( 0 != $tc_show_featured_pages && $this -> angi_fn_maybe_is_holder_js_required() ) {
  	    	wp_enqueue_script(
  	    		'holder',
  	    ...
  	    if ( apply_filters('tc_load_retinajs', 1 == angi_fn_opt( 'tc_retina_support' ) ) )
      ...
      */
      function angi_fn_write_custom_css( $_css = null ) {
        ...
        $_moved_opts        = angi_fn_opt(  '__moved_opts' ) ;
        ...
        */
        ...
        return apply_filters( 'tc_write_custom_css',
          ...
          $_css,
          angi_fn_opt( 'tc_custom_css')
        );
      }//end of function


      function angi_fn_apply_media_upload_front_patch( $_css ) {
        global $wp_version;
      ...
      */
      function angi_fn_maybe_avoid_double_social_icon( $_css ) {
      ...
      */
      function angi_fn_enqueue_gfonts() {
        ...
        wp_enqueue_style(
          'tc-gfonts',
          sprintf( '//fonts.googleapis.com/css?family=%s', str_replace( '|', '%7C', angi_fn_get_font( 'single' , $_font_pair ) ) ),
          array(),
          null,
          'all'
        );
      ...
      * + Fired in angi_fn_user_defined_tinymce_css => add the user defined font style to the wp editor
      * @return css string
      *
      * @package Angilla
      * @since Angilla 1.0
      */
      function angi_fn_write_fonts_inline_css( $_css = null , $_context = null ) {
        ...
        extract( ANGI_init::$instance -> font_selectors, EXTR_OVERWRITE );
      ...
      */
      function angi_fn_write_dropcap_inline_css( $_css = null , $_context = null ) {
        ...
        $_main_color_pair = ANGI_utils::$inst -> angi_fn_get_skin_color( 'pair' );
      ...
      */
      function angi_fn_set_random_skin ( $_skin ) {
        ...
        $_skins = apply_filters( 'tc_get_skin_color', ANGI___::$instance -> skin_classic_color_map, 'all' );
      ...
      */
      ...
      /**
      * Convenient method to normalize script enqueueing in the Angilla theme
      * @return  void
      * @uses wp_enqueue_script() to manage script dependencies
      * @package Angilla
      * @since Angilla 1.0
      */
      function angi_fn_enqueue_script( $_handles = array() ) {
        ...
        foreach ( $_scripts as $_hand => $_params )
          call_user_func_array( 'wp_enqueue_script',  $this -> angi_fn_normalize_script_args( $_hand, $_params ) );
      ...
      */
      private function angi_fn_normalize_script_args( $_handle, $_params ) {
        ...
        return array(
          ...
          ANGI_DEBUG_MODE || ANGI_DEV_MODE ? CUSTOMIZR_VER . time() : CUSTOMIZR_VER,
      ...
      * @since v3.3+
      */
      function angi_fn_load_concatenated_front_scripts() {
          return apply_filters( 'tc_load_concatenated_front_scripts' , ! defined('ANGI_DEV')  || ( defined('ANGI_DEV') && false == ANGI_DEV ) );
      }
      ...
      * @since v3.3+
      */
      private function angi_fn_is_fancyboxjs_required() {
        return angi_fn_opt( 'tc_fancybox' ) || angi_fn_opt( 'tc_gallery_fancybox');
      }
      ...
      * @since v3.3+
      */
      function angi_fn_maybe_is_holder_js_required(){
        $bool = false;

        if ( ! ( class_exists('ANGI_featured_pages') && ANGI_featured_pages::$instance -> angi_fn_show_featured_pages_img() ) )
          return $bool;

        $fp_ids = apply_filters( 'tc_featured_pages_ids' , ANGI___::$instance -> fp_ids);

        foreach ( $fp_ids as $fp_single_id ){
          $featured_page_id = angi_fn_opt( 'tc_featured_page_'.$fp_single_id );
          if ( null == $featured_page_id || ! $featured_page_id || ! ANGI_featured_pages::$instance -> angi_fn_get_fp_img( null, $featured_page_id, null ) ) {
            $bool = true;
            break;
          }
        }
       ...
       *  STYLE NOTE
      /* ------------------------------------------------------------------------- */
      //hook : 'angi_style_note_content'
      //This function is invoked only when :
      //if the note can be displayed : angi_fn_user_can_see_customize_notices_on_front() && ! angi_fn_is_customizing() && ! angi_fn_isprevdem() && 'dismissed' != get_transient( 'angi_style_switch_note_status' )
      //It returns a welcome note html string that will be localized in the front js
      //@return html string
      function angi_fn_get_style_note_content() {
        ...
        foreach ( array( 'wptexturize', 'convert_smilies', 'wpautop') as $callback ) {
          if ( function_exists( $callback ) )
              add_filter( 'angi_front_style_switch_note_html', $callback );
        }
        ob_start();
          ?>
              <?php
                  printf( '<br/><p>%1$s</p>',
                      sprintf( __('Quick tip : you can choose between two styles for the Angilla theme. Give it a try %s', 'angilla'),
                          sprintf( '<a href="%1$s">%2$s</a>',
                              angi_fn_get_customizer_url( array( 'control' => 'tc_style', 'section' => 'style_sec') ),
                              __('in the live customizer.', 'angilla')
                          )
                      )
                  );
              ?>
        ...
        return $html; //apply_filters('angi_front_style_switch_note_html', $html );
    }


    //hook : angi_ajax_dismiss_style_switcher_note_front
    function angi_fn_dismiss_style_switcher_note_front() {
        set_transient( 'angi_style_switch_note_status', 'dismissed' , 60*60*24*365*20 );//20 years of peace
        wp_send_json_success( array( 'status_note' => 'dismissed' ) );
    }
  }//end of ANGI_ressources
endif;
  ...
  class ANGI_widgets {
    ...
    */
    function angi_fn_widgets_factory() {
      ...
      $sidebar_widgets          = apply_filters( 'tc_sidebar_widgets' , ANGI___::$instance -> sidebar_widgets );
...
* @return  void
*/
if ( ! function_exists( 'angi_fn_render_main_header' ) ) {
  function angi_fn_render_main_header() {
    ANGI_header_main::$instance->angi_fn_set_header_options();
  ?>
...
* @return  void
*/
if ( ! function_exists( 'angi_fn_get_tagline_text' ) ) {
  function angi_fn_get_tagline_text( $echo = true ) {
...
//classic ANGI___ will hook here to instantiate theme class groups
do_action('angi_load');
