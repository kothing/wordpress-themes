...
global $angi_base_fmk_namespace;
$angi_base_fmk_namespace = __NAMESPACE__ . '\\';

////////////////////////////////////////////////////////////////
// ANGI_Fmk_Base
if ( ! class_exists( 'ANGI_Fmk_Base_Construct' ) ) :
    class ANGI_Fmk_Base_Construct {
        static $instance;
        ...
        public $angi_css_attr = array();

        public static function angi_fmk_get_instance( $params ) {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof ANGI_Fmk_Base ) ) {
              self::$instance = new ANGI_Fmk_Base( $params );
            }
        ...
        function __construct( $params = array() ) {
            if ( ! is_array( $params ) || empty( $params ) ) {
                error_log( 'ANGI_Fmk_Base => constructor => missing params');
                return;
            }
            if ( empty( $params['base_url'] ) ) {
                error_log( 'ANGI_Fmk_Base => constructor => wrong params');
                return;
            }
            ...
            $this -> angi_css_attr = $this -> angi_fmk_get_customizer_controls_css_attr();

            // Cache the default dynamic params
            $this -> default_dynamic_setting_params = $this -> angi_fmk_get_default_dynamic_setting_params();
            $this -> default_dynamic_module_params = $this -> angi_fmk_get_default_dynamic_module_params();

            // Enqueue the fmk control js + a module tmpl
            $this -> angi_enqueue_fmk_resources();

            // ajax filters + template generator
            $this -> angi_setup_ajax_tmpl();

            // Dynamic Module Registration
            $this -> angi_setup_dynamic_settings_registration();
            $this -> angi_setup_dynamic_modules_registration();

            // Content picker
            $this -> angi_setup_content_picker_ajax_actions();
        }//__construct


        // fired in the constructor to cache the params in a property
        private function angi_fmk_get_default_dynamic_setting_params() {
          return array(
                'setting_id' => '',
        ...
        // fired in the constructor to cache the params in a property
        private function angi_fmk_get_default_dynamic_module_params() {
          return array(
                'dynamic_registration' => true,
                'module_type' => '',
        ...
        public function angi_fmk_get_customizer_controls_css_attr() {
          return apply_filters('angi_fmk_controls_css_attr',
            array(
              'multi_input_wrapper' => 'angi-multi-input-wrapper',
              'sub_set_wrapper'     => 'angi-sub-set',
              'sub_set_input'       => 'angi-input',
              'img_upload_container' => 'angi-imgup-container',

              'edit_modopt_icon'    => 'angi-toggle-modopt',
              'close_modopt_icon'   => 'angi-close-modopt',
              'mod_opt_wrapper'     => 'angi-mod-opt-wrapper',


              'items_wrapper'     => 'angi-items-wrapper',
              'single_item'        => 'angi-single-item',
              'item_content'      => 'angi-item-content',
              'item_header'       => 'angi-item-header',
              'item_title'        => 'angi-item-title',
              'item_btns'         => 'angi-item-btns',
              'item_sort_handle'   => 'angi-item-sort-handle',

              //remove dialog
              'display_alert_btn' => 'angi-display-alert',
              'remove_alert_wrapper'   => 'angi-remove-alert-wrapper',
              'cancel_alert_btn'  => 'angi-cancel-button',
              'remove_view_btn'        => 'angi-remove-button',

              'edit_view_btn'     => 'angi-edit-view',
              //pre add dialog
              'open_pre_add_btn'      => 'angi-open-pre-add-new',
              'adding_new'        => 'angi-adding-new',
              'pre_add_wrapper'   => 'angi-pre-add-wrapper',
              'pre_add_item_content'   => 'angi-pre-add-view-content',
              'cancel_pre_add_btn'  => 'angi-cancel-add-new',
              'add_new_btn'       => 'angi-add-new',
              'pre_add_success'   => 'angi-add-success'
            )
          );
        }
}//class
...
////////////////////////////////////////////////////////////////
// ANGI_Fmk_Base
if ( ! class_exists( 'ANGI_Fmk_Base_Load_Resources' ) ) :
    class ANGI_Fmk_Base_Load_Resources extends ANGI_Fmk_Base_Construct {

        // fired in the constructor
        function angi_enqueue_fmk_resources() {
            ...
            // adds specific js templates for the angi_module control
        ...
        function ac_load_additional_controls_js() {
            ...
            //'angi-customizer-fmk' will be enqueued as a dependency of 'font-customizer-control' only in plugin mode
            wp_enqueue_script(
                'angi-customizer-fmk',
                //dev / debug mode mode?
                sprintf(
            ...
            // When used with Angilla or Hueman, free and pro, we also need to load the theme js part
            if ( false !== strpos( angi_get_parent_theme_slug(), 'angilla' ) || false !== strpos( angi_get_parent_theme_slug(), 'hueman' ) ) {
                wp_enqueue_script(
                    'angi-theme-customizer-fmk',
                    //dev / debug mode mode?
                    sprintf(
                        '%1$s/assets/js/%2$s',
                        NIMBLE_FMK_BASE_URL,
                        '_1_ccat_angi-theme-fmk.js'
                    ),
                    array( 'angi-customizer-fmk' ),
                    ( defined('WP_DEBUG') && true === WP_DEBUG ) ? time() : NIMBLE_FMK_BASE_VERSION,
                    $in_footer = true
                );
            }
            ...
            wp_localize_script(
                'angi-customizer-fmk',
                'serverControlParams',
                apply_filters( 'angi_js_customizer_control_params' ,
                  array(
                      'css_attr' => $this -> angi_css_attr,
                      'isDevMode' => ( defined('WP_DEBUG') && true === WP_DEBUG ) || ( defined('ANGI_DEV') && true === ANGI_DEV ),
                      'docURL' => esc_url(''),
                      'i18n' => array(
                            'edit' => __('Edit', 'angilla'),
                            'close' => __('Close', 'angilla'),
                            'notset' => __('Not set', 'angilla'),
                            'successMessage' => __('Done !', 'angilla'),

                            'readDocumentation' => __('Learn more about this in the documentation', 'angilla'),
                            'Settings' => __('Settings', 'angilla'),
                            'Options for' => __('Options for', 'angilla'),

                            // img upload translation
                            'select_image'        => __( 'Select Image', 'angilla' ),
                            'change_image'        => __( 'Change Image', 'angilla' ),
                            'remove_image'        => __( 'Remove', 'angilla' ),
                            'default_image'       => __( 'Default', 'angilla'  ),
                            'placeholder_image'   => __( 'No image selected', 'angilla' ),
                            'frame_title_image'   => __( 'Select Image', 'angilla' ),
                            'frame_button_image'  => __( 'Choose Image', 'angilla' ),

                            'Customizing' => __('Customizing', 'angilla'),
                      ),
                      'paramsForDynamicRegistration' => apply_filters( 'angi_fmk_dynamic_setting_js_params', array() )
                  )
                )
            );
        }
        ...
        function ac_load_additional_controls_css() {
            wp_enqueue_style(
                'angi-fmk-controls-style',
                sprintf('%1$s/assets/css/angi-ccat-control-base%2$s.css', NIMBLE_FMK_BASE_URL, ''),
            ...
            //select2 stylesheet
            //overriden by some specific style in angi-control-base.css
            wp_enqueue_style(
                'angi-select2-css',
                 sprintf('%1$s/assets/css/lib/angiSelect2.css', NIMBLE_FMK_BASE_URL, ''),
                array( 'customize-controls' ),
                ( defined('WP_DEBUG') && true === WP_DEBUG ) ? time() : NIMBLE_FMK_BASE_VERSION,
                $media = 'all'
            );
        ...
        function ac_customize_load_preview_js() {
            ...
            wp_enqueue_script(
                'angi-customizer-preview' ,
                  sprintf(
                      '%1$s/assets/js/%2$s',
                      NIMBLE_FMK_BASE_URL,
                      'angi-preview-base.js'
                  ),
            ...
            wp_localize_script(
                  'angi-customizer-preview',
                  'serverPreviewParams',
                  apply_filters('angi_base_fmk_customizer_preview_params' ,
                      array(
        ...
        /////////////////////////////////////////////////////
        //add specific js templates for the angi_module control
        ...
        function ac_print_module_control_templates() {
            ...
            ?>
              ...
              <script type="text/html" id="tmpl-customize-control-angi_module-content">
...
////////////////////////////////////////////////////////////////
// ANGI_Fmk_Base
if ( ! class_exists( 'ANGI_Fmk_Base_Ajax_Filter' ) ) :
    class ANGI_Fmk_Base_Ajax_Filter extends ANGI_Fmk_Base_Load_Resources {

        // fired in the constructor
        function angi_setup_ajax_tmpl() {
            ...
            // $html = apply_filters( "ac_set_ajax_angi_tmpl___{$module_type}", '', $tmpl );
            add_filter( "ac_set_ajax_angi_tmpl___all_modules", array( $this, 'ac_get_all_modules_tmpl' ), 10, 3 );

            // fetch templates
            add_action( 'wp_ajax_ac_get_template', array( $this, 'ac_set_ajax_angi_tmpl' ) );
        }

        // hook : 'wp_ajax_ac_get_template'
        function ac_set_ajax_angi_tmpl() {
            if ( ! is_user_logged_in() ) {
                wp_send_json_error( 'ac_set_ajax_angi_tmpl => unauthenticated' );
            }
            if ( ! current_user_can( 'edit_theme_options' ) ) {
              wp_send_json_error('ac_set_ajax_angi_tmpl => user_cant_edit_theme_options');
            }
            if ( ! current_user_can( 'customize' ) ) {
                status_header( 403 );
                wp_send_json_error( 'ac_set_ajax_angi_tmpl => customize_not_allowed' );
            } else if ( ! isset( $_SERVER['REQUEST_METHOD'] ) || 'POST' !== $_SERVER['REQUEST_METHOD'] ) {
                status_header( 405 );
                wp_send_json_error( 'ac_set_ajax_angi_tmpl => bad_method' );
            }
            $action = 'save-customize_' . get_stylesheet();
            if ( ! check_ajax_referer( $action, 'nonce', false ) ) {
                 wp_send_json_error( array(
                  'code' => 'invalid_nonce',
                  'message' => __( 'ac_set_ajax_angi_tmpl => Security check failed.', 'angilla' ),
                ) );
            }

            if ( ! isset( $_POST['module_type'] ) || empty( $_POST['module_type'] ) ) {
                wp_send_json_error( 'ac_set_ajax_angi_tmpl => missing module_type property in posted data' );
            }
            if ( ! isset( $_POST['tmpl'] ) || empty( $_POST['tmpl'] ) ) {
                wp_send_json_error( 'ac_set_ajax_angi_tmpl => missing tmpl property in posted data' );
            }
            $tmpl = $_POST['tmpl'];
            $module_type = $_POST['module_type'];
            $html = apply_filters( "ac_set_ajax_angi_tmpl___{$module_type}", '', $tmpl, $_POST );

            if ( empty( $html ) ) {
                wp_send_json_error( 'ac_set_ajax_angi_tmpl => module ' . $module_type . ' => template empty for requested tmpl : ' . $tmpl );
            } else {
        ...
        // hook : ac_set_ajax_angi_tmpl___all_modules
        // this dynamic filter is declared on wp_ajax_ac_get_template
        // It allows us to populate the server response with the relevant module html template
        // $html = apply_filters( "ac_set_ajax_angi_tmpl___{$module_type}", '', $tmpl );
        //
        ...
        function ac_get_all_modules_tmpl( $html, $requested_tmpl = '', $posted_params = array() ) {
            ...
            switch ( $requested_tmpl ) {
                case 'crud-module-part' :
                    ?>
                      <button class="<?php echo $css_attr['open_pre_add_btn']; ?>"><?php _e('Add New', 'angilla'); ?> <span class="fas fa-plus-square"></span></button>
                      <div class="<?php echo $css_attr['pre_add_wrapper']; ?>">
                        ...
                        <div class="<?php echo $css_attr['pre_add_item_content']; ?>">
                          ...
                          <span class="<?php echo $css_attr['cancel_pre_add_btn']; ?> button"><?php _e('Cancel', 'angilla'); ?></span> <span class="<?php echo $css_attr['add_new_btn']; ?> button"><?php _e('Add it', 'angilla'); ?></span>
                        </div>
                      </div>
                    <?php
                break;
                case 'rud-item-part' :
                    ?>
                      <div class="<?php echo $css_attr['item_header']; ?> angi-custom-model">
                        ...
                        <# } #>
                        <div class="<?php echo $css_attr['item_btns']; ?>"><a title="<?php _e('Edit', 'angilla'); ?>" href="javascript:void(0);" class="fas fa-pencil-alt <?php echo $css_attr['edit_view_btn']; ?>"></a>&nbsp;<a title="<?php _e('Remove', 'angilla'); ?>" href="javascript:void(0);" class="fas fa-trash <?php echo $css_attr['display_alert_btn']; ?>"></a></div>
                ...
                case 'rud-item-alert-part' :
                    ?>
                      <p class="angi-item-removal-title"><?php _e('Are you sure you want to remove : <strong>{{ data.title }} ?</strong>', 'angilla'); ?></p>
                      <span class="<?php echo $css_attr['remove_view_btn']; ?> button"><?php _e('Yes', 'angilla'); ?></span> <span class="<?php echo $css_attr['cancel_alert_btn']; ?> button"><?php _e('No', 'angilla'); ?></span>
                    <?php
                break;
            ...
            return $html;//will be sent by wp_send_json_success() in ::ac_set_ajax_angi_tmpl()
        }
    }//class
endif;
...
////////////////////////////////////////////////////////////////
// ANGI_Fmk_Base
if ( ! class_exists( 'ANGI_Fmk_Base_Tmpl_Builder' ) ) :
    class ANGI_Fmk_Base_Tmpl_Builder extends ANGI_Fmk_Base_Ajax_Filter {
        /*********************************************************
        ** TMPL BUILDER
        *********************************************************/
        ...
        function ac_generate_angi_tmpl_from_map( $tmpl_map ) {
            $html = '';
            ...
            foreach( $tmpl_map as $input_id => $input_data ) {
                ...
                if ( ! empty( $maybe_diff ) ) {
                    ...
                    error_log('</' . __FUNCTION__ . '>');
                    //wp_send_json_error( 'ac_generate_angi_tmpl_from_map => at least one unknow entry in the input data for input id : ' . $input_id );
                    break;
                }
        ...
        // Fired in ac_generate_angi_tmpl_from_map
        function ac_get_default_input_tmpl( $input_id, $input_data ) {
            ...
            <?php if ( ! empty( $input_data['html_before'] ) ) : ?>
                <div class="angi-html-before"><?php echo $input_data['html_before']; ?></div>
            <?php endif; ?>

            <?php if ( ! empty( $input_data['notice_before_title'] ) ) : ?>
                <span class="angi-notice"><?php echo $input_data['notice_before_title']; ?></span><br/>
            <?php endif; ?>
            ...
            <?php if ( ! empty( $input_data['notice_before'] ) ) : ?>
                <span class="angi-notice"><?php echo $input_data['notice_before']; ?></span>
            <?php endif; ?>

            <?php printf( '<div class="angi-input %1$s">', ! empty( $input_data['input_width'] ) ? $input_data['input_width'] : '' ); ?>
            ...
            ?>
              </div><?php // class="angi-input" ?>
              <?php if ( ! empty( $input_data['notice_after'] ) ) : ?>
                  <span class="angi-notice"><?php echo $input_data['notice_after']; ?></span>
              <?php endif; ?>

              <?php if ( ! empty( $input_data['html_after'] ) ) : ?>
                <div class="angi-html-after"><?php echo $input_data['html_after']; ?></div>
              <?php endif; ?>
            ...
            $tmpl_html = apply_filters( "angi_set_input_tmpl___{$input_type}", ob_get_clean(), $input_id, $input_data );
        ...
        private function ac_set_input_tmpl_content( $input_type, $input_id, $input_data ) {
            ...
            ob_start();
              do_action( 'angi_set_input_tmpl_content', $input_type, $input_id, $input_data );
            ...
            if ( ! empty( $input_tmpl_content ) ) {
                ...
                switch ( $input_type ) {
                    ...
                    case 'hidden':
                      ?>
                        <input data-angitype="<?php echo $input_id; ?>" type="hidden" value=""></input>
                      <?php
                    break;
                     ...
                     *  SELECT
                    /* ------------------------------------------------------------------------- */
                    case 'angi_layouts'://<= specific to the hueman theme
                    case 'select'://<= used in the angilla and hueman theme
                    case 'simpleselect'://<=used in Nimble Builder
                      ?>
                        <select data-angitype="<?php echo $input_id; ?>"></select>
                      <?php
                    break;
                    ...
                    case 'category_picker':
                      ?>
                        <select multiple="multiple" data-angitype="<?php echo $input_id; ?>"></select>
                      <?php
                    break;
                    ...
                    case 'text' :
                      ?>
                        <input data-angitype="<?php echo $input_id; ?>" type="text" value="" placeholder="<?php echo $input_data['placeholder']; ?>"></input>
                      <?php
                    break;
                    ...
                    case 'number' :
                      ?>
                        <?php
                        printf( '<input data-angitype="%4$s" type="number" %1$s %2$s %3$s value="{{ data[\'%4$s\'] }}" />',
                    ...
                    case 'wp_color_alpha' :
                      ?>
                        <input data-angitype="<?php echo $input_id; ?>" class="width-100"  data-alpha="true" type="text" value="{{ data['<?php echo $input_id; ?>'] }}"></input>
                      <?php
                    break;
                    case 'color' :
                      ?>
                        <input data-angitype="<?php echo $input_id; ?>" type="text" value="{{ data['<?php echo $input_id; ?>'] }}"></input>
                      <?php
                    break;
                    ...
                    case 'check' :
                      ?>
                        ...
                        #>
                        <input data-angitype="<?php echo $input_id; ?>" type="checkbox" {{ _checked }}></input>
                      <?php
                    break;
                    ...
                    case 'gutencheck' :
                        ?>
                          <#
                            var _checked = ( false != data['<?php echo $input_id; ?>'] ) ? "checked=checked" : '';
                          #>
                          <span class="angi-toggle-check"><input class="angi-toggle-check__input" data-angitype="<?php echo $input_id; ?>" type="checkbox" {{ _checked }}><span class="angi-toggle-check__track"></span><span class="angi-toggle-check__thumb"></span></span>
                        <?php
                    break;
                    ...
                    /* ------------------------------------------------------------------------- */
                    case 'textarea' :
                      // Added an id attribute for
                      // needed to instantiate wp.editor.initialize(...)
                      ?>
                        <textarea id="textarea-{{ data.id }}" data-angitype="<?php echo $input_id; ?>" class="width-100" name="textarea" rows="10" cols="">{{ data.value }}</textarea>
                      <?php
                    break;
                    ...
                    case 'upload_url' :
                      ?>
                        <input data-angitype="<?php echo $input_id; ?>" type="hidden"/>
                    ...
                    case 'range' :
                      ?>
                        ...
                        <?php
                        printf( '<input data-angitype="%5$s" type="range" %1$s %2$s %3$s %4$s value="{{ data[\'%5$s\'] }}" />',
                    ...
                    case 'content_picker' :
                      ?>
                        <?php
                        printf( '<span data-angitype="%1$s"></span>', $input_id );
                        ?>
                      <?php
                    break;
                     ...
                     *  - the input template has not been populated by the first do_action('angi_set_input_tmpl_content')
...
////////////////////////////////////////////////////////////////
// ANGI_Fmk_Base
if ( ! class_exists( 'ANGI_Fmk_Dyn_Setting_Registration' ) ) :
    class ANGI_Fmk_Dyn_Setting_Registration extends ANGI_Fmk_Base_Tmpl_Builder {

        //fired in the constructor
        function angi_setup_dynamic_settings_registration() {
            add_action( 'customize_register', array( $this, 'angi_setup_dynamic_setting_registration' ), 10 );
            ...
            add_filter( 'angi_fmk_dynamic_setting_js_params', array( $this, 'angi_setup_localized_params_for_dynamic_js_registration' ), 20 );

            // when not dynamically registered
            // TO DEPRECATE ?
            //add_action( 'customize_register', array( $this, 'angi_register_not_dynamic_settings' ), 20 );
        }
        ...
        //     )
        // )
        function angi_pre_register_dynamic_setting( $setting_params ) {
            if ( ! is_array( $setting_params ) || empty( $setting_params ) ) {
                error_log( 'angi_pre_register_dynamic_setting => empty $setting_params submitted' );
                return;
            }
            if ( ! array_key_exists( 'setting_id', $setting_params ) || empty( $setting_params['setting_id'] ) ) {
                error_log( 'angi_pre_register_dynamic_setting => missing setting id' );
                return;
            }
            ...
            if ( array_key_exists( $setting_id_candidate, $registered ) ) {
                error_log( 'angi_pre_register_dynamic_setting => setting id already registered => ' . $setting_id_candidate );
                return;
            }
        ...
        function angi_setup_dynamic_setting_registration( $wp_customize ) {
            add_filter( 'customize_dynamic_setting_args', array( $this, 'angi_setup_customizer_dynamic_setting_args' ), 10, 2  );
            add_filter( 'customize_dynamic_setting_class', array( $this, 'angi_setup_customizer_dynamic_setting_class' ), 10, 3 );
        }

        // hook : 'customize_dynamic_setting_args'
        function angi_setup_customizer_dynamic_setting_args( $setting_args, $setting_id ) {
            ...
            foreach ( $this->registered_settings as $registerered_setting_id => $params ) {
                ...
                $module_params = $this -> angi_get_registered_dynamic_module( $params[ 'module_type' ] );
        ...
        function angi_setup_customizer_dynamic_setting_class( $class, $setting_id, $args ) {
            if ( ! is_array( $this->registered_settings ) || empty( $this->registered_settings ) )
              return $class;
        ...
        // hook : angi_fmk_dynamic_setting_js_params'
        // @param js_param = array()
        function angi_setup_localized_params_for_dynamic_js_registration( $js_params ) {
            ...
            foreach ( $this->registered_settings as $registerered_setting_id => $params ) {
                ...
                if ( array_key_exists( $registerered_setting_id, $params ) ) {
                    error_log( 'angi_setup_localized_params_for_dynamic_js_registration => js_params already setup for setting : ' . $registerered_setting_id );
                }

                $js_params[ $registerered_setting_id ] = array(
                    'setting_id' => $registerered_setting_id,
                    'module_type' => $params[ 'module_type' ],
                    'module_registration_params' => $this -> angi_get_registered_dynamic_module( $params[ 'module_type' ] ),
        ...
        // hook : customize_register
        function angi_register_not_dynamic_settings( $wp_customize ) {
        ...
        }//angi_register_not_dynamic_settings
...
////////////////////////////////////////////////////////////////
// ANGI_Fmk_Base
if ( ! class_exists( 'ANGI_Fmk_Dyn_Module_Registration' ) ) :
    class ANGI_Fmk_Dyn_Module_Registration extends ANGI_Fmk_Dyn_Setting_Registration {

        //fired in the constructor
        function angi_setup_dynamic_modules_registration() {
            // Dynamic Module Registration
            add_action( 'init', array( $this, 'angi_schedule_ajax_tmpl' ) );
            // Enqueue the module customizer control assets
            add_action( 'customize_controls_enqueue_scripts' , array( $this, 'angi_register_dynamic_modules_customizer_control_assets' ) );
        }
        ...
        //     'tmpl' => array()
        // )
        function angi_pre_register_dynamic_module( $module_params ) {
            // error_log( '<angi_pre_register_dynamic_module>' );
            // error_log( print_r( $module_params, true ) );
            // error_log( '</angi_pre_register_dynamic_module>' );

            if ( ! is_array( $module_params ) || empty( $module_params ) ) {
                error_log( 'angi_pre_register_dynamic_module => empty $module_params submitted' );
                return;
            }
            if ( ! array_key_exists( 'module_type', $module_params ) || empty( $module_params['module_type'] ) ) {
                error_log( 'angi_pre_register_dynamic_module => missing module_type' );
                return;
            }
            ...
            if ( array_key_exists( $module_type_candidate, $registered ) ) {
                error_log( 'angi_pre_register_dynamic_module => module type already registered => ' . $module_type_candidate );
                return;
            }
        ...
        function angi_get_registered_dynamic_module( $module_type = '' ) {
            $registered = $this->registered_modules;
        ...
        // @return bool
        function angi_is_module_registered( $module_type = '' ) {
            $registered = $this->registered_modules;
            if ( empty( $module_type ) || ! is_array( $registered ) || empty( $registered ) )
              return;
        ...
        //     )
        // ),
        function angi_register_dynamic_modules_customizer_control_assets() {
        ...
        // hook : init
        function angi_schedule_ajax_tmpl() {
            if ( ! is_array( $this->registered_modules ) || empty( $this->registered_modules ) )
              return;
            ...
            foreach ( $this->registered_modules as $module_type => $params ) {
                ...
                if ( ! empty( $params['tmpl'] ) ) {
                    $module_type = $params['module_type'];
                    // filter declared with $html = apply_filters( "ac_set_ajax_angi_tmpl___{$module_type}", '', $tmpl, $_POST );
                    add_filter( "ac_set_ajax_angi_tmpl___{$module_type}", array( $this, 'ac_get_ajax_module_tmpl'), 10, 3 );
                }
            }//foreach
        }
        ...
        // hook : ac_set_ajax_angi_tmpl___{$module_type}
        function ac_get_ajax_module_tmpl( $html, $requested_tmpl = '', $posted_params = array() ) {
            ...
            if ( array_key_exists( 'tabs', $tmpl_map ) ) {
                    ...
                    <?php
                      foreach ( $tmpl_map['tabs'] as $_key => $tab ) {
                        printf( '<section id="section-topline-%1$s">%2$s</section>',
                            $_key + 1,
                            $this -> ac_generate_angi_tmpl_from_map( $tab['inputs'] )
                        );
                      }//foreach
                    ?>
                ...
                return $this -> ac_generate_angi_tmpl_from_map( $tmpl_map );
            }
        }
...
*/
// ANGI_Fmk_Base
if ( ! class_exists( 'ANGI_Fmk_Base' ) ) :
    class ANGI_Fmk_Base extends ANGI_Fmk_Dyn_Module_Registration {

      //fired in the constructor
      public function angi_setup_content_picker_ajax_actions() {
          if ( current_user_can( 'edit_theme_options' ) ) {
            ...
            //add_filter( 'content_picker_ajax_items', array( $this, 'angi_add_custom_item_to_ajax_results' ), 10, 3 );
      }

      // hook : 'content_picker_ajax_items'
      function angi_add_custom_item_to_ajax_results( $items, $page, $context ) {
          if ( is_numeric( $page ) && $page < 1 ) {
              return array_merge(
                  array(
                      array(
                         'title'      => sprintf( '<span style="font-weight:bold">%1$s</span>', __('Set a custom url', 'angilla') ),
                         'type'       => '',
                         'type_label' => '',
                         'object'     => '',
                         'id'         => '_custom_',
      ...
      public function ajax_load_available_items() {
            ...
            if ( ! check_ajax_referer( $action, 'nonce', false ) ) {
                 wp_send_json_error( array(
                  'code' => 'invalid_nonce',
                  'message' => __( 'ajax_load_available_items => Security check failed.', 'angilla' ),
                ) );
            }
            ...
            if ( ! isset( $_POST['wp_object_types'] ) || empty( $_POST['wp_object_types'] ) ) {
                wp_send_json_error( 'angi_ajax_content_picker_missing_type_or_object_parameter' );
            }

            if ( ! isset( $_POST['page'] ) ) {
                wp_send_json_error( 'angi_ajax_content_picker_missing_pagination_param' );
            }
            ...
            if ( ! is_array( $wp_object_types ) || empty( $wp_object_types ) ) {
              wp_send_json_error( 'angi_ajax_content_picker_missing_object_types' );
            }
      ...
      public function load_available_items_query( $args ) {
            ...
            if ( 'post' === $type ) {
                  ...
                  if ( ! $post_types || ! is_array( $post_types ) || empty( $post_types ) ) {
                      return new \WP_Error( 'angi_contents_invalid_post_type' );
                  }
                  ...
                  foreach ( $posts as $post ) {
                        ...
                        if ( '' === $post_title ) {
                          ...
                          $post_title = sprintf( __( '#%d (no title)', 'angilla' ), $post->ID );
                        }
                        $items[] = array(
             ...
             */
            $items = apply_filters( 'angi_customize_content_picker_available_items', $items, $type, $object_types, $page );
            return $items;
      }
      ...
      public function ajax_search_available_items() {
            ...
            if ( empty( $_POST['search'] ) ) {
                wp_send_json_error( 'angi_contents_missing_search_parameter' );
            }
            ...
            if ( empty( $items ) ) {
                wp_send_json_success( array( 'message' => __( 'No results found.', 'angilla') ) );
            } else {
                wp_send_json_success( array(
      ...
      public function search_available_items_query( $args = array() ) {
            ...
            if ( 'post' === $args['type'] ) {
                  ...
                  * Filters the available menu items during a search request.
                   ...
                   */
                  $items = apply_filters( 'angi_customize_content_picker_searched_items', $items, $args );
            }
            return $items;
      }
}
...
*/
function angi_get_parent_theme_slug() {
...
//@return boolean
function angi_is_multi_item_module( $module_type ) {
    $is_multi_item = false;
    $module_params = ANGI_Fmk_Base() -> angi_get_registered_dynamic_module( $module_type );
...
//@params ex :
//array(
//    'base_url' => NIMBLE_BASE_URL . '/inc/angi-base-fmk'
// )
function ANGI_Fmk_Base( $params = array() ) {
    return ANGI_Fmk_Base::angi_fmk_get_instance( $params );
}
