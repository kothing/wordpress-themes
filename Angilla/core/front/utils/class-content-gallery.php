...
class ANGI_gallery {
      ...
      function __construct () {
            ...
            */
            add_filter( 'post_gallery'                            , array( $this, 'angi_fn_angi_gallery' ), 10, 3 );

            //add data attribute
            add_filter( 'angi_gallery_image_linking_media'         , array( $this, 'angi_fn_maybe_lighbox_attachment_link' ), 10, 3 );
            add_filter( 'angi_gallery_image_linking_no_media'      , array( $this, 'angi_fn_maybe_add_lighbox_button' ), 10, 3 );
      ...
      */
      function angi_fn_angi_gallery( $gallery, $attr, $instance ) {
            ...
            //do nothing if the angilla gallery is not enabled and type different than "attachments-only"
            //we use "attachments-only" when retrieving the first post gallery in some lists of posts
            //see core/front/models/content/common/media/class-model-gallery.php::angi_fn__get_post_gallery()
            if ( 'attachments-only' != $atts[ 'type' ] && ! $this -> angi_fn_is_gallery_enabled() ) {
                  return $gallery;
            }
            ...
            $gallery_class = implode( ' ', array_filter( array(
                  'angi-gallery',
                   'row',
                   'flex-row',
                  1 == esc_attr( angi_fn_opt( 'tc_gallery_style' ) ) ? 'angi-gallery-style' : ''
                  )
            ) );
            ...
            $image_attr[ 'sizes' ] = $this -> angi_fn_generate_gallery_img_sizes( $columns );
            ...
            foreach ( $attachments as $id => $attachment ) {
                  ...
                  if ( ! empty( $atts['link'] ) && 'file' === $atts['link'] ) {
                        ...
                        $image_output = apply_filters( 'angi_gallery_image_linking_media', $image_output, $id, $attachment );

                  } elseif ( ! empty( $atts['link'] ) && 'none' === $atts['link'] ) {

                        //no link
                        //maybe add expand img button
                        $image_output = apply_filters( 'angi_gallery_image_linking_no_media', $image_output, $id, $attachment );
      ...
      function angi_fn_maybe_lighbox_attachment_link( $link_markup, $id, $attachment ) {

            if ( ! apply_filters( 'tc_gallery_fancybox', esc_attr( angi_fn_opt( 'tc_gallery_fancybox' ) ) , $id ) ) {
                  return $link_markup;
            }
      ...
      function angi_fn_maybe_add_lighbox_button( $markup, $id, $attachment ) {
            ...
            return $markup . angi_fn_post_action( $link, $class = '', $attr,  $echo = false );
      ...
      * @return string.
      */
      function angi_fn_generate_gallery_img_sizes( $gallery_columns ) {

            //get content breadth, depends on the global layout, can be: 'full' (no sidebars), 'semi-narrow' (1 sidebar), 'narrow' (2 sidebars)
            $content_breadth        = angi_fn_get_content_breadth();
            //starting from md (tablets, 768px and up)
            //this actually depends on ANGI_init::$instnace->global_layout
            ...
            */
            ...
            return apply_filters( 'angi_fn_gallery_img_sizes',  join( ', ', $sizes ), $sizes, $gallery_columns );
      }



    function angi_fn_is_gallery_enabled(){
        return apply_filters('angi_enable_gallery', esc_attr( angi_fn_opt('tc_enable_gallery') ) );
    }
