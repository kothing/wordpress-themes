...
if ( ! function_exists( 'mipress_featured_slider' ) ) :
	/**
	 * Add slider.
	 *
	 * @uses action hook mipress_before_content.
	 *
	 * @since Mipress 0.1
	 */
	function mipress_featured_slider() {
		$enable_slider = get_theme_mod( 'mipress_slider_option', 'disabled' );

		if ( mipress_check_section( $enable_slider ) ) {
			$type       = get_theme_mod( 'mipress_slider_type', 'category' );
			$transition_effect = get_theme_mod( 'mipress_slider_transition_effect', 'fade' );
			$transition_length = get_theme_mod( 'mipress_slider_transition_length', 1 );
			$transition_delay  = get_theme_mod( 'mipress_slider_transition_delay', 4 );
			$image_loader      = get_theme_mod( 'mipress_slider_image_loader', true );
				...
				<div id="feature-slider-section" class="section">
					<div class="wrapper">
						<div class="cycle-slideshow"
							...
							<!-- prev/next links -->
							<button class="cycle-prev" aria-label="Previous"><span class="screen-reader-text">' . esc_html__( 'Previous Slide', 'mipress' ) . '</span>' . mipress_get_svg( array( 'icon' => 'angle-down' ) ) . '</button>
							<button class="cycle-next" aria-label="Next"><span class="screen-reader-text">' . esc_html__( 'Next Slide', 'mipress' ) . '</span>' . mipress_get_svg( array( 'icon' => 'angle-down' ) ) . '</button>


							<!-- empty element for pager links -->
							<div class="cycle-pager"></div>';
			$output .= mipress_post_page_category_slider();
			$output .= '
						</div><!-- .cycle-slideshow -->
					</div><!-- .wrapper -->
				</div><!-- #feature-slider -->';
...
add_action( 'mipress_slider', 'mipress_featured_slider', 10 );


if ( ! function_exists( 'mipress_post_page_category_slider' ) ) :
	/**
	 * This function to display featured posts/page/category slider
	 *
	 * @param $options: mipress_theme_options from customizer
	 *
	 * @since Mipress 1.0
	 */
	function mipress_post_page_category_slider() {
		$quantity     = get_theme_mod( 'mipress_slider_number', 4 );
		$no_of_post   = 0; // for number of posts
		$post_list    = array();// list of valid post/page ids
		//$type         = get_theme_mod( 'mipress_slider_type', 'category' );
		$show_content = get_theme_mod( 'mipress_slider_content_show', 'hide-content' );
		//$show_meta    = get_theme_mod( 'mipress_slider_meta_show', 'show-meta' );
		$output       = '';
		...
		$args = array(
			...
			for ( $i = 1; $i <= $quantity; $i++ ) {
				$post_id = '';
					...
					$post_id = get_theme_mod( 'mipress_slider_page_' . $i );
		...
		while ( $loop->have_posts() ) :
			...
			if ( has_post_thumbnail() ) {
				$image_url = get_the_post_thumbnail_url( get_the_ID(), 'mipress-slider' );
			} else {
				// Get the first image in page, returns false if there is no image.
				$first_image_url = mipress_get_first_image( get_the_ID(), 'mipress-slider', '', true );
...
endif; // mipress_post_page_category_slider.


if ( ! function_exists( 'mipress_image_slider' ) ) :
	/**
	 ...
	 *
	 */
	function mipress_image_slider() {
		$quantity = get_theme_mod( 'mipress_slider_number', 4 );

		$output = '';

		for ( $i = 1; $i <= $quantity; $i++ ) {
			$image = get_theme_mod( 'mipress_slider_image_' . $i );

			// Check Image Not Empty to add in the slides.
			if ( $image ) {
				$imagetitle = get_theme_mod( 'mipress_featured_title_' . $i ) ? get_theme_mod( 'mipress_featured_title_' . $i ) : 'Featured Image-' . $i;

				$title  = '';
				$link   = get_theme_mod( 'mipress_featured_link_' . $i );
				$target = get_theme_mod( 'mipress_featured_target_' . $i ) ? '_blank' : '_self';
				...
				$content = get_theme_mod( 'mipress_featured_content_' . $i ) ? '<div class="entry-summary"><p>' . get_theme_mod( 'mipress_featured_content_' . $i ) . '</p></div><!-- .entry-summary -->' : '';
