...
*
*/
if ( ! class_exists( 'ANGI_customize' ) ) :
  class ANGI_customize {
    static $instance;
    ...
    function __construct () {
      ...
      if ( ! version_compare( $wp_version, '3.4' , '>=' ) ) {
        add_action( 'admin_menu'                             , array( $this , 'angi_fn_add_fallback_page' ));
        return;
      }
      ...
      //add control class
      add_action( 'customize_register'                       , array( $this, 'angi_fn_augment_customizer' ),10, 1);

      //Partial refreshs
      add_action( 'customize_register'                       , array( $this, 'angi_fn_register_partials' ) );

      //add the customizer built with the builder below
      add_action( 'customize_register'                       , array( $this, 'angi_fn_customize_register' ), 20, 1 );

      //modify some WP built-in settings / controls / sections
      add_action( 'customize_register'                       , array( $this, 'angi_fn_alter_wp_customizer_settings' ), 1000, 1 );

      //add grid/post list buttons in the control views
      add_action( '__before_setting_control'                 , array( $this, 'angi_fn_render_grid_control_link' ) );

      //remove old logo settong if the wp custom logo option has been set
      add_action( 'customize_save_custom_logo'               , array( $this, 'angi_fn_remove_old_tc_logo_upload' ) );

      //load resources class
      $this -> angi_fn_fire_angi_resources();
    }




    function angi_fn_fire_angi_resources() {
      if (  ! is_object(ANGI_customize_resources::$instance) )
        new ANGI_customize_resources();
    }
    ...
    * @since Angilla 1.0
    */
    function angi_fn_augment_customizer( $manager ) {
      /* Concatenated with grunt  */

      //Registered types are eligible to be rendered via JS and created dynamically.
      if ( class_exists('ANGI_Customize_Cropped_Image_Control') )
        $manager -> register_control_type( 'ANGI_Customize_Cropped_Image_Control' );

      if ( class_exists('ANGI_Customize_Code_Editor_Control') )
        $manager -> register_control_type( 'ANGI_Customize_Code_Editor_Control' );

      if ( class_exists('ANGI_Customize_Panels') )
        $manager -> register_panel_type( 'ANGI_Customize_Panels');

      if ( class_exists('ANGI_Customize_Sections') )
        $manager -> register_panel_type( 'ANGI_Customize_Sections');
    }
    ...
    function angi_fn_register_partials( WP_Customize_Manager $wp_customize ) {

        //Bail if selective refresh is not available (old versions) or disabled (for skope for example)
        if ( ! isset( $wp_customize->selective_refresh ) || ! angi_fn_is_partial_refreshed_on() ) {
            return;
        }
        ...
        $wp_customize->selective_refresh->add_partial( 'social_links', array(
            ...
            'settings'            => array( ANGI_THEME_OPTIONS . '[tc_social_links]' ),
            'render_callback'     => 'angi_fn_print_social_links',
            'fallback_refresh'    => false,
        ) );

        //ONLY FOR OLD ANGI at the moment
        if ( ! angi_fn_is_ms() ) {
            /* Header */
            $wp_customize->selective_refresh->add_partial( 'main_header', array(
                'selector'            => 'header.tc-header',
                'settings'            => array(
                  ANGI_THEME_OPTIONS . '[tc_header_layout]',
                  ANGI_THEME_OPTIONS . '[tc_show_tagline]',
                  ANGI_THEME_OPTIONS . '[tc_social_in_header]',
                ),
                'container_inclusive' => true,
                'render_callback'     => 'angi_fn_render_main_header',
                'fallback_refresh'    => false,
            ) );
            /* Tagline text */
            $wp_customize->selective_refresh->add_partial( 'blogdescription', array(
                ...
                'render_callback'     => 'angi_fn_get_tagline_text',
                'fallback_refresh'    => false,
            ) );
        }
    }
    ...
    * @return void()
    */
    function angi_fn_alter_wp_customizer_settings( $wp_customize ) {
      ...
      if ( version_compare( $wp_version, '4.3', '>=' ) && is_object( $wp_customize -> get_control( 'site_icon' ) ) ) {
        $tc_option_group = ANGI_THEME_OPTIONS;
        ...
        //note : the setting is kept because used in the customizer js api to handle the transition between Angilla favicon to WP site icon.
      ...
      // $choices      = array( '' => __( '&mdash; Select &mdash;', 'angilla' ) );
      ...
      //     'label'   => $description,
      //     'section' => 'nav',
      //     'title'   => "main" == $location ? __( 'Assign menus to locations' , 'angilla') : false,
      //     'type'    => 'select',
      //     'choices' => $choices,
      //     'priority' => isset($_priorities[$location]) ? $_priorities[$location] : $_priority,
      //     'notice' => __('If your freshly created menu is not listed, please refresh the customizer panel.', 'angilla')
      //   );
      ...
      //     $_control_properties['notice'] = sprintf( __("You haven't created any menu yet. %s or check the %s to learn more about menus.", "angilla"),
      //       sprintf( '<strong><a href="%1$s" title="%2$s">%2$s</a></strong>', $_create_menu_link, __("Create a new menu now" , "angilla") ),
      //       sprintf( '<a href="%1$s" title="%2$s" target="_blank">%2$s</a>', esc_url('codex.wordpress.org/WordPress_Menu_User_Guide'),  __("WordPress documentation" , "angilla") )
      //     );
      //   }

      //   $wp_customize -> add_control( new ANGI_controls( $wp_customize, $menu_setting_id, $_control_properties ) );
    ...
    */
    function angi_fn_customize_register( $wp_customize) {
      return $this -> angi_fn_customize_factory (
        $wp_customize,
        $this -> angi_fn_customize_arguments(),
        angi_fn_get_customizer_map()
      );
    }
    ...
    function angi_fn_customize_factory ( $wp_customize , $args, $setup ) {
      ...
      if ( isset( $setup['add_panel']) && version_compare( $wp_version, '4.0', '>=' ) ) {
        foreach ( $setup['add_panel'] as $p_key => $p_options ) {
          ...
          if ( class_exists( 'ANGI_Customize_Panels' ) ) {
            $wp_customize -> add_panel( new ANGI_Customize_Panels( $wp_customize, $p_key, $panel_options ) );
          } else {
      ...
      if ( isset( $setup['add_setting'])) {
        ...
        foreach ( $setup['add_setting'] as $key => $options) {
          ...
          $angi_option_group = ANGI_THEME_OPTIONS;

          $_opt_name = "{$angi_option_group}[{$key}]";
      ...
      if ( isset( $setup['add_setting_control'])) {
        ...
        foreach ( $setup['add_setting_control'] as $key => $options) {
          ...
          //all angilla theme options start by "tc_" by convention
          ...
          if ( angi_fn_is_angilla_option( $key ) )
            $add_prefix = true;
          $_opt_name = $add_prefix ? "{$angi_option_group}[{$key}]" : $key;
          ...
          //$option_settings['transport'] = ANGI_IS_MODERN_STYLE ? 'refresh' : $option_settings['transport'];
    ...
    function angi_fn_get_wp_builtin_settings() {
      return array(
        'blogname',
        'blogdescription',
        'site_icon',
    ...
    function angi_fn_get_grid_design_controls() {
    ...
    * @echo clickable text
    */
    function angi_fn_render_grid_control_link( $set_id ) {
      if ( false !== strpos( $set_id, 'tc_post_list_show_thumb' ) ) {
        printf('<span class="tc-grid-toggle-controls" title="%1$s">%1$s</span>' , __('More grid design options' , 'angilla'));
      }
    }
    ...
    function angi_fn_remove_old_tc_logo_upload( $setting ) {
    ...
    */
    function angi_fn_add_favicon_title($set_id) {
      if ( false !== strpos( $set_id, 'tc_sticky_logo_upload' ) )
        printf( '<h3 class="angi-angilla-title">%s</h3>', __( 'SITE ICON' , 'angilla') );
    }
    ...
    */
    function angi_fn_add_fallback_page() {
        $theme_page = add_theme_page(
            __( 'Upgrade WP' , 'angilla' ),   // Name of page
            __( 'Upgrade WP' , 'angilla' ),   // Label in menu
            'edit_theme_options' ,          // Capability required
            'upgrade_wp.php' ,             // Menu slug, used to uniquely identify the page
            array( $this , 'angi_fn_fallback_admin_page' )         //function to be called to output the content of this page
        );
    }
    ...
    function angi_fn_fallback_admin_page() {
      ?>
        <div class="wrap upgrade_wordpress">
          ...
          <h2><?php _e( 'This theme requires WordPress 3.4+' , 'angilla' ) ?> </h2>
          <br />
          <p style="text-align:center">
            <a style="padding: 8px" class="button-primary" href="<?php echo admin_url().'update-core.php' ?>" title="<?php _e( 'Upgrade Wordpress Now' , 'angilla' ) ?>">
            <?php _e( 'Upgrade Wordpress Now' , 'angilla' ) ?></a>
            <br /><br />
          <img src="<?php echo ANGI_BASE_URL . 'screenshot.png' ?>" alt="Angilla" />
          </p>
        </div>
      <?php
    }
...
*
*/
if ( ! class_exists( 'ANGI_customize_resources' ) ) :
  class ANGI_customize_resources {
    static $instance;
    ...
    function __construct () {
      ...
      $this->_is_dev_mode           = ( defined('ANGI_DEV') && true === ANGI_DEV );
      $this->_style_version_suffix  = defined( 'ANGI_IS_MODERN_STYLE' ) && ANGI_IS_MODERN_STYLE ? '-modern' : '';

      //control scripts and style
      add_action( 'customize_controls_enqueue_scripts'        , array( $this, 'angi_fn_customize_controls_js_css' ), 20 );

      //preview scripts
      //set with priority 20 to be fired after angi_fn_customize_store_db_opt
      add_action( 'customize_preview_init'                    , array( $this, 'angi_fn_customize_preview_js_css' ), 20 );

      //exports some wp_query informations. Updated on each preview refresh.
      add_action( 'customize_preview_init'                    , array( $this, 'angi_fn_add_preview_footer_action' ), 20 );
    ...
    function angi_fn_write_preview_style_classic( $_css ) {
  ...
  left: 0
    ...
    function angi_fn_customize_controls_js_css() {
        ...
        wp_enqueue_style(
            ...
            sprintf('%1$sassets/angi/css/angi-control-theme.css', ANGI_BASE_URL),
            array( 'angi-fmk-controls-style' ),
            $this->_is_debug_mode ? time() : CUSTOMIZR_VER,
            $media = 'all'
        );


        // angi-control-deps-modern.js / angi-control-deps.js
        wp_enqueue_script(
            'tc-customizer-controls-deps',
            sprintf('%1$sassets/angi/js/angi-control-deps%2$s.js' , ANGI_BASE_URL, $this->_style_version_suffix ),
            array( 'angi-theme-customizer-fmk' ),
            $this->_is_debug_mode ? time() : CUSTOMIZR_VER,
            true
        );
        ...
        wp_enqueue_script(
            ...
            sprintf('%1$sassets/angi/js/angi-control-dom_ready.js', ANGI_BASE_URL ),
            array( 'angi-theme-customizer-fmk' ),
            $this->_is_debug_mode ? time() : CUSTOMIZR_VER,
            true
        );

        $this->angi_fn_customize_controls_localize();
    }



    //shared
    function angi_fn_customize_controls_localize() {
      ...
      $fp_controls      = array(
        ANGI_THEME_OPTIONS.'[tc_show_featured_pages_img]',
        ANGI_THEME_OPTIONS.'[tc_featured_page_button_text]'
      );
      ...
      foreach ( $fp_ids as $id ) {
        $page_dropdowns[]   = ANGI_THEME_OPTIONS.'[tc_featured_page_'. $id.']';
        $text_fields[]    = ANGI_THEME_OPTIONS.'[tc_featured_text_'. $id.']';
      }
      ...
      wp_localize_script(
        ...
        array(
            ...
            'wpBuiltinSettings'=> ANGI_customize::$instance -> angi_fn_get_wp_builtin_settings(),
            'isThemeSwitchOn'  => false,
            'themeSettingList' => ANGI_BASE::$theme_setting_list,
            'themeOptions'     => ANGI_THEME_OPTIONS,

            // Angilla theme specifics
            ...
            'i18n'   => $this -> angi_fn_get_translated_strings(),

            //not used by the new
            'faviconOptionName' => 'tc_fav_upload',

            'gridDesignControls' => ANGI_customize::$instance -> angi_fn_get_grid_design_controls(),
            'isRTL'           => is_rtl(),
            'isChildTheme'    => is_child_theme(),
            'isModernStyle'   => angi_fn_is_ms()
        )
      );
    ...
    function angi_fn_customize_preview_js_css() {
        global $wp_version;

        // loads angi-preview-post_message.js / angi-preview-post_message-modern.js
        wp_enqueue_script(
            'angi-angilla-theme-preview-js' ,
            sprintf('%1$s/assets/angi/js/angi-preview-post_message%2$s.js' , ANGI_BASE_URL, $this->_style_version_suffix ),
            array( 'angi-customizer-preview' ),//<= angi-preview-base.js, loaded from the angi-base-fmk
            $this->_is_debug_mode ? time() : CUSTOMIZR_VER,
            true
        );
        ...
        wp_localize_script(
              'angi-angilla-theme-preview-js',
              'themeServerPreviewParams',// 'ANGIPreviewParams',
              apply_filters('tc_js_customizer_preview_params' ,
                  array(
                      //angi4 won't use this
                      'customSkin'      => apply_filters( 'tc_custom_skin_preview_params' , array( 'skinName' => '', 'fullPath' => '' ) ),
                      'fontPairs'       => angi_fn_get_font( 'list' ),
                      'fontSelectors'   => ANGI_init::$instance -> font_selectors,

                      'wpBuiltinSettings' => ANGI_customize::$instance -> angi_fn_get_wp_builtin_settings(),
                      'themeOptionsPrefix'  => ANGI_THEME_OPTIONS,
                  )
              )
        );

        if ( 'modern' != $this->_style_version_suffix ) {
            add_filter( 'tc_user_options_style', array( $this, 'angi_fn_write_preview_style_classic' ) );
        }
    }


    //hook : customize_preview_init
    function angi_fn_add_preview_footer_action() {
      add_action( 'wp_footer', array( $this, 'angi_fn_add_customize_preview_data' ) , 20 );
    }


    //hook : wp_footer in the preview
    function angi_fn_add_customize_preview_data() {
      ...
      foreach( (array)$wp_query as $prop => $val ) {
        if (  false === strpos($prop, 'is_') )
          continue;
        if ( 'is_home' == $prop )
          $val = angi_fn_is_real_home();

        $_wp_conditionals[$prop] = $val;
      }

      ?>
        <script type="text/javascript" id="angi-customizer-data">
          (function ( _export ){
            _export.angi_wp_conditionals = <?php echo wp_json_encode( $_wp_conditionals ) ?>;
    ...
    function angi_fn_get_translated_strings() {
      return apply_filters('controls_translated_strings',
          array(
                ...
                'rss' => __('Rss', 'angilla'),
                'selectSocialIcon' => __('Select a social icon', 'angilla'),
                'followUs' => __('Follow us on', 'angilla'),
                'successMessage' => __('Done !', 'angilla'),
                'socialLinkAdded' => __('New Social Link created ! Scroll down to edit it.', 'angilla'),
                'readDocumentation' => __('Learn more about this in the documentation', 'angilla'),
                //WP TEXT EDITOR MODULE
                'textEditorOpen' => __('Edit', 'angilla'),
                'textEditorClose' => __('Close Editor', 'angilla'),
                //SLIDER MODULE
                'slideAdded'   => __('New Slide created ! Scroll down to edit it.', 'angilla'),
                'slideTitle'   => __( 'Slide', 'angilla'),
                'postSliderNote' => __( "This option generates a home page slider based on your last posts, starting from the most recent or the featured (sticky) post(s) if any.", "angilla" ),
                'sidenavNote'  => sprintf( '%1$s<br/>%2$s',
                                    __( 'The side on which the menu is revealed depends on the choosen header layout.', 'angilla'),
                                    sprintf( __("To change the global header layout, %s" , "angilla"),
                                      sprintf( '<a href="%1$s" title="%3$s">%2$s &raquo;</a>',
                                        "javascript:wp.customize.section('header_layout_sec').focus();",
                                        __("jump to the Design and Layout section" , "angilla"),
                                        __("Change the header layout", "angilla")
                                      )
                                    )
                                  ),
...
*
*/
if ( ! class_exists( 'ANGI_controls' ) ) :
  class ANGI_controls extends WP_Customize_Control  {
      public $type;
      public $link;
      public $title;
      public $label;
      ...
      public function render_content()  {
        ...
        switch ( $this->type) {
            case 'hr':
              echo '<hr class="angi-customizer-separator" />';
            break;
            ...
            case 'title' :
              ...
              <h3 class="angi-angilla-title"><?php echo esc_html( $this->title); ?></h3>
              <?php endif; ?>
              <?php if (isset( $this->notice)) : ?>
              <i class="angi-notice"><?php echo $this -> notice ?></i>
             <?php endif; ?>
            ...
            case 'select':
              ...
              <label>
                ...
                <?php $this -> angi_fn_print_select_control( in_array( $this->id, array( ANGI_THEME_OPTIONS.'[tc_fonts]', ANGI_THEME_OPTIONS.'[tc_skin]' ) ) ? 'angiSelect2 no-selecter-js' : '' ) ?>
                <?php if(!empty( $this -> notice)) : ?>
                  <span class="angi-notice"><?php echo $this -> notice ?></span>
                <?php endif; ?>
              </label>
              <?php
              if ( ANGI_THEME_OPTIONS.'[tc_front_slider]' == $this -> id ) {
                //retrieve all sliders in option array
                $sliders          = angi_fn_opt( 'tc_sliders' );

                if ( empty( $sliders ) ) {
                  printf('<div class="angi-notice" style="width:99%; padding: 5px;"><span class="angi-notice">%1$s<br/><a class="button-primary" href="%2$s" target="_blank">%3$s</a><br/></span>',
                    __("You haven't create any slider yet. Go to the media library, edit your images and add them to your sliders.", "angilla" ),
                    admin_url( 'upload.php?mode=list' ),
                    __( 'Create a slider' , 'angilla' )
                  );
                }
              }
            break;
            ...
            case 'number':
              ...
              <?php endif; ?>
              <label>
                <span class="angi-number-label customize-control-title"><?php echo $this->label ?></span>
                <input <?php $this->link() ?> type="number" step="<?php echo $this-> step ?>" min="<?php echo $this-> min ?>" id="posts_per_page" value="<?php echo $this->value() ?>" class="angi-number-input small-text">
                <?php if(!empty( $this -> notice)) : ?>
                  <span class="angi-notice"><?php echo $this-> notice ?></span>
                <?php endif; ?>
              </label>
            ...
            case 'nimblecheck':
              ...
              <?php if ( 'checkbox' === $this->type ) : ?>
                <?php
                    printf('<div class="angi-check-label"><label><span class="customize-control-title">%1$s</span></label></div>',
                      $this->label
                    );
                ?>
                <input <?php $this->link(); ?> type="checkbox" value="<?php echo esc_attr( $this->value() ); ?>"  <?php angi_fn_checked( $this->value() ); ?> />
              <?php elseif ( 'nimblecheck' === $this->type ) : ?>
                <div class="angi-control-nimblecheck">
                  <?php
                    printf('<div class="angi-check-label"><label><span class="customize-control-title">%1$s</span></label></div>',
                      $this->label
                    );
                  ...
                  <div class="nimblecheck-wrap">
                    <input id="nimblecheck-<?php echo $this -> id; ?>" <?php $this->link(); ?> type="checkbox" value="<?php echo esc_attr( $this->value() ); ?>"  <?php angi_fn_checked( $this->value() ); ?> class="nimblecheck-input">
            ...
            case 'email':
              ...
              printf('<label><span class="customize-control-title %1$s">%2$s</span><input type="text" value="%3$s" %4$s /></label>',
                ...
                $this->label,
                call_user_func( 'angi_fn_sanitize_' . $this -> type, $this->value() ),
    ...
    private function angi_fn_print_select_control($class) {
      printf('<select %1$s class="%2$s">%3$s</select>',
        call_user_func( array( $this, 'get'.'_'.'link' ) ),
        $class,
        $this -> angi_fn_get_select_options()
      );
    }


    private function angi_fn_get_select_options() {
      $_options_html = '';
      switch ( $this -> id ) {
        case ANGI_THEME_OPTIONS.'[tc_fonts]':
        ...
        case ANGI_THEME_OPTIONS.'[tc_skin]':
          $_data_hex  = '';
          //only for angi3
          if ( defined( 'ANGI_IS_MODERN_STYLE' ) && ANGI_IS_MODERN_STYLE )
            return;

          $_color_map = ANGI_utils::$inst->angi_fn_get_skin_color( 'all' );
...
/*
*/
if ( class_exists('WP_Customize_Cropped_Image_Control') && ! class_exists( 'ANGI_Customize_Cropped_Image_Control' ) ) :
  class ANGI_Customize_Cropped_Image_Control extends WP_Customize_Cropped_Image_Control {
    public $type = 'angi_cropped_image';
    public $title;
    public $notice;
    public $dst_width;
    public $dst_height;
    ...
    */
    public function to_json() {
        ...
        //we need to re-build the absolute url of the logo src set in old Angilla
        $value = $this->value();
        if ( $value ) {
    ...
    */
    public function content_template() {
      ?>
      <# if ( data.title ) { #>
          <h3 class="angi-angilla-title">{{{ data.title }}}</h3>
        <# } #>
          <?php parent::content_template(); ?>
        <# if ( data.notice ) { #>
          <span class="angi-notice">{{{ data.notice }}}</span>
        <# } #>
      <?php
    }
  }//end class
...
/*
*/
if ( class_exists('WP_Customize_Code_Editor_Control') && ! class_exists( 'ANGI_Customize_Code_Editor_Control' ) ) :
  class ANGI_Customize_Code_Editor_Control extends WP_Customize_Code_Editor_Control {

    public $type = 'angi_code_editor';
    public $title;
    public $notice;
...
* MULTIPICKER CLASSES
***************************************************************************************************/
if ( ! class_exists( 'ANGI_Customize_Multipicker_Control' ) ) :
  /**
  ...
  * @since 3.4.10
  */
  abstract class ANGI_Customize_Multipicker_Control extends ANGI_controls {
    ...
    public function render_content() {
      ...
      $dropdown = $this -> angi_fn_get_dropdown_multipicker();
    ...
    abstract public function angi_fn_get_dropdown_multipicker();
  }//end class
endif;

if ( ! class_exists( 'ANGI_Customize_Multipicker_Categories_Control' ) ) :
  class ANGI_Customize_Multipicker_Categories_Control extends ANGI_Customize_Multipicker_Control {

    public function angi_fn_get_dropdown_multipicker() {
      $cats_dropdown = wp_dropdown_categories(
          array(
              ...
              'hide_empty'         => 0 ,
              'echo'               => 0 ,
              'walker'             => new ANGI_Walker_CategoryDropdown_Multipicker(),
              'hierarchical'       => 1,
              'class'              => 'angiSelect2 no-selecter-js '.$this->type,
...
if ( ! class_exists( 'ANGI_Walker_CategoryDropdown_Multipicker' ) ) :
  class ANGI_Walker_CategoryDropdown_Multipicker extends Walker_CategoryDropdown {
    /**
...
**********************************************************************************/
if ( ! class_exists( 'ANGI_Customize_Upload_Control' ) ) :
  /**
   ...
   * @since 3.4.0
   */
  class ANGI_Customize_Upload_Control extends WP_Customize_Control {
    public $type    = 'angi_upload';
    public $removed = '';
    public $context;
    ...
    public function render_content() {
      ...
      <label>
        ...
        <?php endif; ?>
        <div>
          <a href="#" class="button-secondary angi-upload"><?php _e( 'Upload' , 'angilla'  ); ?></a>
          <a href="#" class="remove"><?php _e( 'Remove' , 'angilla'  ); ?></a>
        </div>
        <?php if(!empty( $this -> notice)) : ?>
          <span class="angi-notice"><?php echo $this -> notice; ?></span>
        <?php endif; ?>
...
***************************************************/
if ( ! class_exists( 'ANGI_Customize_Panels') ) :
  class ANGI_Customize_Panels extends WP_Customize_Panel {
    public $angi_subtitle = '';
    ...
    public function json() {
      $array = parent::json();
      $array['angi_subtitle'] = html_entity_decode( $this->angi_subtitle, ENT_QUOTES, get_bloginfo( 'charset' ) );
      return $array;
    }
    ...
    public function print_template() {
      ?>
      <script type="text/html" id="tmpl-customize-panel-angi_panel">
        <?php $this->angi_fn_render_template(); ?>
      </script>
          <?php
    }
     ...
     * @access protected
     */
    protected function angi_fn_render_template() {
      ?>
      <li id="accordion-panel-{{ data.id }}" class="accordion-section control-section control-panel control-panel-{{ data.type }}">
        <h3 class="accordion-section-title" tabindex="0">
          {{ data.title }}
          <span class="angi-panel-subtitle">{{ data.angi_subtitle }}</span>
          <span class="screen-reader-text"><?php _e( 'Press return or enter to open this panel', 'angilla' ); ?></span>
        </h3>
 ...
 */
class ANGI_Customize_Sections extends WP_Customize_Section {
     ...
     * @var    string
     */
    public $type = 'angi-customize-sections';
