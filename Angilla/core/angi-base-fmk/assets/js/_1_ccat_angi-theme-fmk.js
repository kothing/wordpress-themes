(function (api, $, _) {
    ...
    if ('function' === typeof api.Section) {
        ...
        //=> implemented in the angilla theme for the social links boolean visibility controls ( socials in header, sidebar, footer )
    ...
    var _closeModOpt = function () {
        if (!_.has(api, 'angi_ModOptVisible'))
            return;
        api.angi_ModOptVisible(false);
    };
    api.bind('ready', function () {
        api.angi_activeSectionId.bind(_closeModOpt);
        api.angi_activePanelId.bind(_closeModOpt);
    });
})(wp.customize, jQuery, _);

//named angi_multiple_picker in the php setting map
var ANGIMultiplePickerMths = ANGIMultiplePickerMths || {};
/* Multiple Picker */
/**
 * @constructor
...
(function (api, $, _) {
    $.extend(ANGIMultiplePickerMths, {
        ready: function () {
            ...
            _select.angiSelect2({
                closeOnSelect: false,
                templateSelection: angiEscapeMarkup
            });

            function angiEscapeMarkup(obj) {
                //trim dashes
...
//named angi_cropped_image in the php setting map
var ANGICroppedImageMths = ANGICroppedImageMths || {};
...
(function (api, $, _) {
     ...
     */
    wp.media.controller.ANGICustomizeImageCropper = wp.media.controller.Cropper.extend({
    ...
    /* ANGICroppedImageControl */
    $.extend(ANGICroppedImageMths, {
        /**
         * Create a media modal select frame, and store it so the instance can be reused when needed.
         * ANGI: We don't want to crop svg (cropping fails), gif (animated gifs become static )
         * @Override
        ...
        initFrame: function () {
            ...
            this.frame = wp.media({
                ...
                states: [
                    ...
                    new wp.media.controller.ANGICustomizeImageCropper({
...
//named angi_upload in the php setting map
var ANGIUploadMths = ANGIUploadMths || {};
(function (api, $, _) {
    /**
     * @constructor
     * @augments wp.customize.Control
    ...
    $.extend(ANGIUploadMths, {
        ready: function () {
            ...
            this.uploader = $.extend({
                ...
                browser: this.container.find('.angi-upload'),
...
//named angi_layouts in the php setting map
var ANGILayoutSelectMths = ANGILayoutSelectMths || {};
(function (api, $, _) {
    $.extend(ANGILayoutSelectMths, {
        ready: function () {
            this.setupSelect();
        },
        ...
        setupSelect: function (obj) {
            ...
            function addImg(state) {
                ...
                var _layout_data = control.params.layouts[state.element.value],
                    ...
                    $state = $(
                        '<img src="' + _src + '" class="angi-layout-img" title="' + _title + '" /><span class="angi-layout-title">' + _title + '</span>'
                    );
                return $state;
            }
...
(function (api, $, _) {
    //THEME CONTROLS
    //api.ANGIBackgroundControl     = api.ANGIItemControl.extend( ANGIBackgroundMths );

    //api.ANGIWidgetAreasControl    = api.ANGIDynModule.extend( ANGIWidgetAreasMths );

    api.ANGIUploadControl = api.Control.extend(ANGIUploadMths);
    api.ANGILayoutControl = api.Control.extend(ANGILayoutSelectMths);
    api.ANGIMultiplePickerControl = api.Control.extend(ANGIMultiplePickerMths);
    api.ANGIColorAlpha = api.Control.extend({
        ready: api.ColorControl.prototype.ready
    }); //api.ANGIColorAlpha

    $.extend(api.controlConstructor, {
        angi_upload: api.ANGIUploadControl,
        //angi_sidebars   : api.ANGIWidgetAreasControl,
        //angi_socials    : api.ANGISocialControl,
        angi_multiple_picker: api.ANGIMultiplePickerControl,
        angi_layouts: api.ANGILayoutControl,
        wp_color_alpha: api.ANGIColorAlpha,
        //angi_background : api.ANGIBackgroundControl
    });

    if ('function' == typeof api.CroppedImageControl) {
        api.ANGICroppedImageControl = api.CroppedImageControl.extend(ANGICroppedImageMths);

        $.extend(api.controlConstructor, {
            angi_cropped_image: api.ANGICroppedImageControl
        });
    }

    if ('function' == typeof api.CodeEditorControl) {
        $.extend(api.controlConstructor, {
            angi_code_editor: api.CodeEditorControl
        });
    }
...
(function (api, $, _) {
    ...
    api.angi_CrtlDependenciesReady = $.Deferred();

    api.bind('ready', function () {
        if (_.has(api, 'angi_ctrlDependencies'))
            return;

        api.angi_ctrlDependencies = new api.ANGI_ctrlDependencies();
        api.angi_CrtlDependenciesReady.resolve();
    });


    api.ANGI_ctrlDependencies = api.Class.extend({
        dominiDeps: [],
        initialize: function () {
            var self = this;
            ...
            api.angi_activeSectionId.bind(function (section_id) {
                if (!_.isEmpty(section_id) && api.section.has(section_id)) {
                    try {
                        self.setServiDependencies(section_id);
                    } catch (er) {
                        api.errorLog('In api.ANGI_ctrlDependencies : ' + er);
                    }
                }
            });
        ...
        setServiDependencies: function (targetSectionId, sourceSectionId, refresh) {
            ...
            api.section(targetSectionId).angi_ctrlDependenciesReady = api.section(targetSectionId).angi_ctrlDependenciesReady || $.Deferred();

            //Bail here if this section has already been setup for ctrl dependencies
            if (!refresh && 'resolved' == api.section(targetSectionId).angi_ctrlDependenciesReady.state())
            ...
            _.each(self.dominiDeps, function (params) {
                ...
                var wpDominusId = api.ANGI_Helpers.build_setId(params.dominus);
            ...
            var _secCtrls = api.ANGI_Helpers.getSectionControlIds(targetSectionId),
            ...
            _.each(_servusDominiIds, function (shortDominusId) {
                ...
                var wpDominusId = api.ANGI_Helpers.build_setId(shortDominusId);
            ...
            dfd.always(function () {
                api.section(targetSectionId).angi_ctrlDependenciesReady.resolve();
            });
            return dfd.promise();
        },
        ...
        _processDominusCallbacks: function (shortDominusId, dominusParams, refresh) {
            var self = this,
                wpDominusId = api.ANGI_Helpers.build_setId(shortDominusId),
            ...
            _.each(dominusParams.servi, function (servusShortSetId) {
                if (!api.control.has(api.ANGI_Helpers.build_setId(servusShortSetId))) {
                    return;
                }
                ...
                var _fireDominusCallbacks = function (dominusSetVal, servusShortSetId, dominusParams, refresh) {
                    ...
                    _deferCallbacks = function (dominusSetVal) {
                        ...
                        var wpServusSetId = api.ANGI_Helpers.build_setId(servusShortSetId);
                ...
                if (!_.has(dominusSetInst, 'angi_visibilityServi'))
                    dominusSetInst.angi_visibilityServi = new api.Value([]);

                //Maybe bind to react on setting _dirty change
                var _currentDependantBound = dominusSetInst.angi_visibilityServi();
                ...
                if (!_.contains(_currentDependantBound, servusShortSetId)) {
                    ...
                    dominusSetInst.angi_visibilityServi(_.union(_currentDependantBound, [servusShortSetId]));
                }
            }); //_.each
            if (!hasProcessed)
                return dfd.resolve().promise();
        ...
        _prepareDominusParams: function (params_candidate) {
            ...
            var wpDominusId = api.ANGI_Helpers.build_setId(params_candidate.dominus);
            if (!api.control.has(wpDominusId)) {
        ...
        _handleFaviconNote: function () {
            var self = this,
                _fav_setId = api.ANGI_Helpers.build_setId(serverControlParams.faviconOptionName);
            //do nothing if (||)
    ...
    }); //api.Class.extend() //api.ANGI_ctrlDependencies
})(wp.customize, jQuery, _);
//DOM READY :
...
(function (wp, $) {
    $(function ($) {
        ...
        /* CHECKBOXES */
        api.angiSetupCheckbox = function (controlId, refresh) {
        ...
        }; //api.angiSetupCheckbox()

        /* SELECT INPUT */
        api.angiSetupSelect = function (controlId, refresh) {
        ...
        /* NUMBER INPUT */
        api.angiSetupStepper = function (controlId, refresh) {
            var _ctrl = api.control(controlId);
            $('input[type="number"]', _ctrl.container).each(function () {
                $(this).stepper();
            });
        }; //api.angiSetupStepper()
        ...
        api.control.each(function (control) {
            ...
            if (0 < control.container.find('input[type="number"]').length && control.params && control.params.section && !_.contains(_exclude, control.params.section)) {
                api.angiSetupStepper(control.id);
            }
        });
