...
!function ($) {
  ...
  , show: function () {
      ...
      this.transition('addClass', $.Event('show.angiCollapse'), 'shown.angiCollapse')
      ...
      if ( TCParams && 1 == TCParams.dropdowntoViewport )
      {
        var winHeight = 'undefined' === typeof window.innerHeight ? window.innerHeight : angiapp.$_window.height(),
            tcVisible = winHeight - this.$element.offset().top + angiapp.$_window.scrollTop();
  ...
  , hide: function () {
      ...
      this.transition('removeClass', $.Event('hide.angiCollapse'), 'hidden.angiCollapse')
      this.$element[dimension](0)
      this._collapsed = true;
      if ( ! this.$element.hasClass('nav-collapse') )
          return;
...
!function ($) {
  ...
  , getActiveIndex: function () {
      this.$active = this.$element.find('.angi-item.active')
  ...
  , to: function (pos) {
      ...
      if (this.sliding) {
        return this.$element.one('angilla.slid', function () {
          that.to(pos)
        })
      }
  ...
  , slide: function (type, next) {
      if(!$.support.transition && this.$element.hasClass('angilla-slide')) {
         this.$element.find('.angi-item').stop(true, true); //Finish animation and jump to end.
      }
      var $active = this.$element.find('.angi-item.active')
      ...
      $next = $next.length ? $next : this.$element.find('.angi-item')[fallback]()

      e = $.Event('angilla.slide', {
      ...
      if (this.$indicators.length) {
        ...
        this.$element.one('angilla.slid', function () {
      ...
      if ($.support.transition && this.$element.hasClass('angilla-slide')) {
        ...
        if ( 0 !== $next.find('img').length )
          $next.find('img').trigger('angilla.slide');
        $next.addClass(type)
        ...
        this.$element.one($.support.transition.end, function () {
          ...
          that.sliding = false
          setTimeout(function () {
            that.$element.trigger('angilla.slid');
            if ( 0 !== $next.find('img').length )
              $next.find('img').trigger('angilla.slid');
          }, 0)
        })
      } else if(!$.support.transition && this.$element.hasClass('angilla-slide')) {
          this.$element.trigger(e)
          if (e.isDefaultPrevented()) return
          $active.animate({left: (direction == 'right' ? '100%' : '-100%')}, 600, function(){
             $active.removeClass('active')
              that.sliding = false
              setTimeout(function () { that.$element.trigger('angilla.slid') }, 0)
            })
        ...
        this.sliding = false
        this.$element.trigger('angilla.slid')
      }
  ...
  var old = $.fn.angiCarousel

  $.fn.angiCarousel = function (option) {
    return this.each(function () {
      var $this = $(this)
        , data = $this.data('angi-carousel')
        , options = $.extend({}, $.fn.angiCarousel.defaults, typeof option == 'object' && option)
        , action = typeof option == 'string' ? option : options.slide
      if (!data) $this.data('angi-carousel', (data = new Carousel(this, options)))
  ...
  $.fn.angiCarousel.defaults = {
    interval: 5000
  , pause: 'hover'
  }

  $.fn.angiCarousel.Constructor = Carousel

  $.fn.angiCarousel.noConflict = function () {
    $.fn.angiCarousel = old
    return this
  }

  $(document).on('click.angi-carousel.data-api', '.angilla-slide [data-slide], .angilla-slide [data-slide-to]', function (e) {
    var $this = $(this), href
      , $target = $($this.attr('data-target') || (href = $this.attr('href')) && href.replace(/.*(?=#[^\s]+$)/, '')) //strip for ie7
      , options = $.extend({}, $target.data(), $this.data())
      , slideIndex

    $target.angiCarousel(options);

    if (slideIndex = $this.attr('data-slide-to')) {
      $target.data('angi-carousel').pause().to(slideIndex).cycle()
    }
...
(function ( $, window ) {
      ...
      Plugin.prototype._load_img = function( _img ) {
            ...
            $_img.unbind('load_img')
                  ...
                  .attr( 'src', _src )
                  .load( function () {
                        if ( ! $_img.hasClass('angi-smart-loaded') ) {
                              $_img.fadeIn(self.options.fadeIn_options).addClass('angi-smart-loaded');
                        }
                        ...
                        $_img.data('angi-smart-loaded', true );
                  });//<= create a load() fn
            if ( $_img[0].complete ) {
                  $_img.load();
            }
...
(function ( $ ) {
    ...
    Plugin.prototype._is_selector_allowed = function( requested_sel_type ) {
      if ( angiapp && angiapp.userXP && angiapp.userXP.isSelectorAllowed )
        return angiapp.userXP.isSelectorAllowed( this.$_el, this.options.skipSelectors, requested_sel_type);
...
(function ( $, window ) {
      var pluginName = 'centerImages',
          defaults = {
                ...
                useImgAttr:false,//uses the img height and width attributes if not visible (typically used for the angilla slider hidden images)
...
(function ( $, window, _ ) {
        ...
        Plugin.prototype.init = function () {
              ...
              this.$_window     = angiapp ? angiapp.$_window : $(window);
...
}());// Angilla version of Galambosi's SmoothScroll
...
smoothScroll = function ( _options ) {
  ...
  _maybeInit() && angiapp.$_body.addClass('tc-smoothscroll');
}
smoothScroll._cleanUp = function(){
  cleanup();    
  angiapp.$_body.removeClass('tc-smoothscroll');
}
smoothScroll._maybeFire = function(){
  _maybeInit(true) && angiapp.$_body.addClass('tc-smoothscroll');
}
