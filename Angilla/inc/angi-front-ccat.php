...
*
*/
if ( ! class_exists( 'ANGI_header_main' ) ) :
	class ANGI_header_main {
    static $instance;
    function __construct () {
      ...
      add_action ( 'wp'                    , array( $this , 'angi_fn_set_header_hooks' ) );

      //Set header options
      add_action ( 'wp'                    , array( $this , 'angi_fn_set_header_options' ) );
      ...
      add_filter( 'tc_user_options_style'  , array( $this , 'angi_fn_write_header_inline_css') );
    }
    ...
    ****************************/
    ...
    function angi_fn_set_header_hooks() {
      ...
      add_action ( 'wp_head'     		  , array( $this , 'angi_fn_favicon_display' ));
      ...
      add_action ( '__header' 				, array( $this , 'angi_fn_prepare_logo_title_display' ) , 10 );
      add_action ( '__header' 				, array( $this , 'angi_fn_tagline_display' ) , 20, 1 );
      add_action ( '__header' 				, array( $this , 'angi_fn_navbar_display' ) , 30 );

      //New menu view (since 3.2.0)
      add_filter ( 'tc_navbar_display', array( $this , 'angi_fn_new_menu_view'), 10, 2);

      //body > header > navbar actions ordered by priority
  	  // GY : switch order for RTL sites
  	  if (is_rtl()) {
        add_action ( '__navbar' 				, array( $this , 'angi_fn_social_in_header' ) , 20, 2 );
        add_action ( '__navbar' 				, array( $this , 'angi_fn_tagline_display' ) , 10, 1 );
  	  }
  	  else {
        add_action ( '__navbar' 				, array( $this , 'angi_fn_social_in_header' ) , 10, 2 );
        add_action ( '__navbar' 				, array( $this , 'angi_fn_tagline_display' ) , 20, 1 );
  	  }

      //add a 100% wide container just after the sticky header to reset margin top
      if ( 1 == esc_attr( angi_fn_opt( 'tc_sticky_header' ) ) || angi_fn_is_customizing() )
        add_action( '__after_header'              , array( $this, 'angi_fn_reset_margin_top_after_sticky_header'), 0 );
    ...
    */
    function angi_fn_set_header_options() {
      ...
      add_filter( 'body_class'               , array( $this , 'angi_fn_add_body_classes') );
      //Set header classes from options
      add_filter( 'tc_header_classes'        , array( $this , 'angi_fn_set_header_classes') );
      //Set logo layout with a customizer option (since 3.2.0)
      add_filter( 'tc_logo_class'            , array( $this , 'angi_fn_set_logo_title_layout') );
    }
    ...
    */
    function angi_fn_favicon_display() {
      ...
      $_fav_option  			= esc_attr( angi_fn_opt( 'tc_fav_upload') );
     	...
     	if ( is_numeric($_fav_option) ) {
     		...
     		$_saved_path 		= esc_url ( angi_fn_opt( 'tc_fav_upload') );
      ...
      if( null == $_fav_src || !$_fav_src )
    	...
    	echo apply_filters( 'tc_favicon_display',
      		sprintf('<link id="angi-favicon" rel="shortcut icon" href="%1$s" type="%2$s">' ,
      			$_fav_src,
      			$type
      		)
    	);
    ...
    /**
      ...
      foreach ( $logos_type as $logo_type ){
          ...
          if ( '_sticky_' == $logo_type && ! $this -> angi_fn_use_sticky_logo() )
              continue;
          ...
          //for the standard logo use the wp custom logo feature if set, otherwise fall back on the angilla custom logo
          $_logo_option       = '_' === $logo_type ? get_theme_mod( 'custom_logo', '' ) : '';
          $_logo_option       = $_logo_option ? $_logo_option : esc_attr( angi_fn_opt( "tc{$logo_type}logo_upload") );
          //check if option is an attachement id or a path (for backward compatibility)
          if ( is_numeric($_logo_option) ) {
              ...
              $upload_dir 			= wp_upload_dir();
              $_saved_path 			= esc_url ( angi_fn_opt( "tc{$logo_type}logo_upload") );
          ...
          $logo_resize 			= ( $logo_type == '_' ) ? esc_attr( angi_fn_opt( 'tc_logo_resize') ) : '';
          $filetype 				= angi_fn_check_filetype($_logo_src);
          if( ! empty($_logo_src) && in_array( $filetype['ext'], $accepted_formats ) ) {
              ...
              $logos_img[] = $this -> angi_fn_logo_img_view($_args);
          }
      }//end foreach
      //render
      if ( count($logos_img) == 0 )
          $this -> angi_fn_title_view($logo_classes);
      else
          $this -> angi_fn_logo_view( array (
    ...
    */
    function angi_fn_logo_img_view( $_args ){
      ...
      $_html = sprintf( '<img src="%1$s" alt="%2$s" %3$s %4$s %5$s %6$s class="%7$s %8$s"/>',
        $logo_src,
      	apply_filters( 'tc_logo_alt', __( 'Back Home' , 'angilla' ) ),
        ...
        implode(' ' , apply_filters('tc_logo_other_attributes' , ( 0 == angi_fn_opt( 'tc_retina_support' ) ) ? array('data-no-retina') : array() ) ),
        $logo_type,
    ...
    */
    ...
    function angi_fn_social_in_header($resp = null) {
        ...
        $_nothing_to_render         = 0 == esc_attr( angi_fn_opt( 'tc_social_in_header' ) ) || ! ( $_socials = angi_fn__f( '__get_socials' ) );
      ...
      if ( 0 == esc_attr( angi_fn_opt( 'tc_show_tagline' ) ) )
        return;

      $_tagline_text  = angi_fn_get_tagline_text( $echo = false );
    ...
    */
      ...
      //TOP BORDER
			if ( 1 != esc_attr( angi_fn_opt( 'tc_top_border') ) ) {
  			$_css = sprintf("%s\n%s",
  				$_css,
      ...
      //STICKY HEADER
	    if ( 0 != esc_attr( angi_fn_opt( 'tc_sticky_shrink_title_logo') ) || angi_fn_is_customizing() ) {
      ...
      //STICKY LOGO
      if ( $this -> angi_fn_use_sticky_logo() ) {
        $_css = sprintf( "%s\n%s",
            $_css,
			...
			//HEADER Z-INDEX
	    if ( 100 != esc_attr( angi_fn_opt( 'tc_sticky_z_index') ) ) {
	    	$_custom_z_index 	= esc_attr( angi_fn_opt( 'tc_sticky_z_index') );
		    $_css = sprintf("%s\n%s",
  		      $_css,
    ...
    */
    function angi_fn_add_body_classes($_classes) {
      //STICKY HEADER
    	if ( 1 == esc_attr( angi_fn_opt( 'tc_sticky_header' ) ) ) {
     		$_classes = array_merge( $_classes, array('tc-sticky-header', 'sticky-disabled') );
     		//STICKY TRANSPARENT ON SCROLL
       	if ( 1 == esc_attr( angi_fn_opt( 'tc_sticky_transparent_on_scroll' ) ) )
      ...
      //No navbar box
      if ( 1 != esc_attr( angi_fn_opt( 'tc_display_boxed_navbar') ) )
   	...
   	*/
		function angi_fn_set_header_classes( $_classes ) {
			...
			$_show_tagline 			= 0 != esc_attr( angi_fn_opt( 'tc_sticky_show_tagline') );
      $_show_title_logo 		= 0 != esc_attr( angi_fn_opt( 'tc_sticky_show_title_logo') );
      $_use_sticky_logo 		= $this -> angi_fn_use_sticky_logo();
			$_shrink_title_logo 	= 0 != esc_attr( angi_fn_opt( 'tc_sticky_shrink_title_logo') );
			$_show_menu 			  = 0 != esc_attr( angi_fn_opt( 'tc_sticky_show_menu') );
			$_header_layout 		= "logo-" . esc_attr( angi_fn_opt( 'tc_header_layout' ) );
			$_add_classes 			= array(
    ...
    */
    function angi_fn_use_sticky_logo(){
        if ( ! esc_attr( angi_fn_opt( "tc_sticky_logo_upload") ) )
            return false;
        if ( ! ( esc_attr( angi_fn_opt( "tc_sticky_header") ) &&
                     esc_attr( angi_fn_opt( 'tc_sticky_show_title_logo') )
               )
        )
            return false;
        return true;
   	...
   	*/
		function angi_fn_set_logo_title_layout( $_classes ) {
			...
			$_layout = esc_attr( angi_fn_opt( 'tc_header_layout') );
			switch ($_layout) {
				case 'left':
...
*
*/
if ( ! class_exists( 'ANGI_menu' ) ) :
  class ANGI_menu {
    static $instance;
    function __construct () {
      self::$instance =& $this;
      //Set menu customizer options (since 3.2.0)
      add_action( 'wp'             , array( $this, 'angi_fn_set_menu_hooks') );
    }
    ...
    */
    function angi_fn_set_menu_hooks() {
      ...
      add_filter( 'tc_header_classes'             , array( $this , 'angi_fn_set_header_classes') );
      add_filter( 'tc_social_header_block_class'  , array( $this, 'angi_fn_set_social_header_class') );

      //! tc_user_options_style filter is shared by several classes => must always check the local context inside the callback before appending new css
      //fired on hook : wp_enqueue_scripts
      //set second menu specific style including @media rules
      add_filter( 'tc_user_options_style'         , array( $this , 'angi_fn_add_second_menu_inline_style') );

      //SIDE MENU HOOKS SINCE v3.3+
      if ( $this -> angi_fn_is_sidenav_enabled() ){
        add_action( 'wp_head'                     , array( $this , 'angi_fn_set_sidenav_hooks') );
        add_filter( 'tc_user_options_style'       , array( $this , 'angi_fn_set_sidenav_style') );
      }
      ...
      if ( ! $this -> angi_fn_is_sidenav_enabled() || angi_fn_is_secondary_menu_enabled() ) {
        add_filter( 'tc_navbar_wrapper_class'     , array( $this, 'angi_fn_set_menu_style_options'), 0 );
      }

      //body > header > navbar action ordered by priority
      add_action ( '__navbar'                     , array( $this , 'angi_fn_menu_display' ), 30 );
      //adds class
      add_filter ( 'wp_page_menu'                 , array( $this , 'angi_fn_add_menuclass' ));
    }
    ...
    * @return void
    */
    function angi_fn_set_sidenav_hooks() {
      add_filter( 'body_class'              , array( $this, 'angi_fn_sidenav_body_class') );

      // disable dropdown on click
      add_filter( 'tc_menu_open_on_click'   , array( $this, 'angi_fn_disable_dropdown_on_click'), 10, 3 );

      // add side menu before the page wrapper
      add_action( '__before_page_wrapper'   , array( $this, 'angi_fn_sidenav_display'), 0 );
      // add menu button to the sidebar
      add_action( '__sidenav'               , array( $this, 'angi_fn_sidenav_toggle_button_display'), 5 );
      // add menu
      add_action( '__sidenav'               , array( $this, 'angi_fn_sidenav_display_menu_customizer'), 10 );
    }
    ...
    */
    function angi_fn_menu_display() {
      ob_start();
        ...
        if ( ! $this -> angi_fn_is_sidenav_enabled() ) {
          $this -> angi_fn_regular_menu_display( 'main' );
        } else {
          $this -> angi_fn_sidenav_toggle_button_display();
          if ( $this -> angi_fn_is_second_menu_enabled() )
            $this -> angi_fn_regular_menu_display( 'secondary' );
        }
    ...
    *
    */
    function angi_fn_menu_button_view( $args ) {
      //extracts : 'type', 'button_class', 'button_attr'
      extract( $args );

      $_button_label = sprintf( '<span class="menu-label">%s</span>',
        '__sidenav' == current_filter() ? __('Close', 'angilla') : __('Menu' , 'angilla')
      );
      $_button = sprintf( '<div class="%1$s"><button type="button" class="btn menu-btn" %2$s title="%5$s" aria-label="%5$s">%3$s%3$s%3$s </button>%4$s</div>',
        ...
        (bool)esc_attr( angi_fn_opt('tc_display_menu_label') ) ? $_button_label : '',
        '__sidenav' == current_filter() ? __('Close', 'angilla') : __('Open the menu' , 'angilla')
      );
    ...
    * @since Angilla 1.0
    */
    function angi_fn_link_to_menu_editor( $args ) {
    ...
    *
    */
    function angi_fn_regular_menu_display( $_location = 'main' ){
      $type               = 'regular';
      $button_where       = 'right' != esc_attr( angi_fn_opt( 'tc_header_layout') ) ? 'pull-right' : 'pull-left';
      $button_class       = array( 'btn-toggle-nav', $button_where );
      $button_attr        = 'data-toggle="collapse" data-target=".nav-collapse"';

      $menu_class         = ( ! wp_is_mobile() && 'hover' == esc_attr( angi_fn_opt( 'tc_menu_type' ) ) ) ? array( 'nav tc-hover-menu' ) : array( 'nav' ) ;
      $menu_wrapper_class = ( ! wp_is_mobile() && 'hover' == esc_attr( angi_fn_opt( 'tc_menu_type' ) ) ) ? array( 'nav-collapse collapse', 'tc-hover-menu-wrapper' ) : array( 'nav-collapse', 'collapse' );

      $menu_view = $this -> angi_fn_wp_nav_menu_view( compact( '_location', 'type', 'menu_class', 'menu_wrapper_class' ) );

      if ( $menu_view && 'main' == $_location )
        $menu_view = $menu_view . $this -> angi_fn_menu_button_view( compact( 'type', 'button_class', 'button_attr') );
    ...
    */
    function angi_fn_sidenav_display() {
      ob_start();
    ...
    * hook: __sidenav
    */
    function angi_fn_sidenav_display_menu_customizer(){
       //menu setup
       $type               = 'sidenav';
       ...
       echo $this -> angi_fn_wp_nav_menu_view( compact( '_location', 'type', 'menu_class', 'menu_wrapper_class') );
    }
    ...
    */
    function angi_fn_sidenav_toggle_button_display() {
      $type          = 'sidenav';
      $where         = 'right' != esc_attr( angi_fn_opt( 'tc_header_layout') ) ? 'pull-right' : 'pull-left';
      $button_class  = array( 'btn-toggle-nav', 'sn-toggle', $where );
      $button_attr   = '';

      echo $this -> angi_fn_menu_button_view( compact( 'type', 'button_class', 'button_attr') );
    }
    ...
    function angi_fn_wp_nav_menu_view( $args ) {
      ...
      $menu_args = apply_filters( "tc_{$type}_menu_args",
          array(
            ...
            'fallback_cb'     => array( $this, 'angi_fn_page_menu' ),
            ...
            'walker'          => ! angi_fn_has_location_menu($_location) ? '' : new ANGI_nav_walker($_location),
            'echo'            => false,
        )
      );
    ...
    */
    function angi_fn_set_menu_style_options( $_classes ) {
      ...
      $_classes = ( 0 != esc_attr( angi_fn_opt( 'tc_menu_submenu_item_move_effect') ) ) ? array_merge( $_classes, array( 'tc-submenu-move' ) ) : $_classes;
      $_classes = ( ! wp_is_mobile() && 'hover' == esc_attr( angi_fn_opt( 'tc_menu_type' ) ) ) ? array_merge( $_classes, array( 'tc-open-on-hover' ) ) : array_merge( $_classes, array( 'tc-open-on-click' ) );
      ...
      $_menu_position = '';
      if ( ! $this -> angi_fn_is_sidenav_enabled() )
        $_menu_position = $_classes[] = esc_attr( angi_fn_opt( 'tc_menu_position') );
      if ( angi_fn_is_secondary_menu_enabled() )
        $_menu_position = $_classes[] = esc_attr( angi_fn_opt( 'tc_second_menu_position') );



      if ( 'pull-menu-center' == $_menu_position ) {
        //pull-menu-center is possible only when logo-centered
        if ( 'centered' != esc_attr( angi_fn_opt( 'tc_header_layout' ) ) )
          array_pop($_classes);
    ...
    */
    function angi_fn_add_body_classes($_classes) {
      //menu type class
      $_menu_type = $this -> angi_fn_is_sidenav_enabled() ? 'tc-side-menu' : 'tc-regular-menu';
    ...
    */
    function angi_fn_set_header_classes( $_classes ) {
      ...
      array_push( $_classes, 'tc-second-menu-' . esc_attr( angi_fn_opt( 'tc_second_menu_resp_setting' ) . '-when-mobile' ) );
    ...
    */
    function angi_fn_add_second_menu_inline_style( $_css ) {
      if ( ! angi_fn_is_secondary_menu_enabled() )
        return $_css;
    ...
    */
     ...
     */
    function angi_fn_disable_dropdown_on_click( $replace, $search, $_location = null ) {
    ...
    * @return bool
    */
    function angi_fn_is_sidenav_enabled() {
      return apply_filters( 'tc_is_sidenav_enabled', 'aside' == esc_attr( angi_fn_opt( 'tc_menu_style' ) ) );
    }
    ...
    * @return bool
    */
    function angi_fn_is_second_menu_enabled() {
      return apply_filters( 'tc_is_second_menu_enabled', (bool)esc_attr( angi_fn_opt( 'tc_display_second_menu' ) ) );
    }
     ...
     */
    function angi_fn_page_menu( $args = array() ) {
      ...
      $list_args['title_li'] = '';
      $menu .= str_replace( array( "\r", "\n", "\t" ), '', $this -> angi_fn_list_pages($list_args) );
     ...
     */
    function angi_fn_list_pages( $args = '' ) {
      $defaults = array(
        'depth' => 0, 'show_date' => '',
        'date_format' => get_option( 'date_format' ),
        'child_of' => 0, 'exclude' => '',
        'title_li' => __( 'Pages', 'angilla' ), 'echo' => 1,
      ...
      if ( ! empty( $pages ) ) {
        ...
        $output .= $this -> angi_fn_walk_page_tree( $pages, $r['depth'], $current_page, $r );
     ...
     */
    function angi_fn_walk_page_tree($pages, $depth, $current_page, $r) {
      // if ( empty($r['walker']) )
      //   $walker = new Walker_Page;
      // else
      //   $walker = $r['walker'];
      $walker = new ANGI_nav_walker_page;
...
*/
if ( ! class_exists( 'ANGI_nav_walker' ) ) :
  class ANGI_nav_walker extends Walker_Nav_Menu {
    static $instance;
    public $tc_location;
    function __construct($_location) {
      self::$instance =& $this;
      $this -> tc_location = $_location;
      add_filter( 'tc_nav_menu_css_class' , array($this, 'angi_fn_add_bootstrap_classes'), 10, 4 );
    }
    ...
    */
    function angi_fn_add_bootstrap_classes($classes, $item, $args, $depth ) {
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
      ...
      if ( $item->is_dropdown ) {
        ...
        $search         = '<a';
        $replace        = ( ! wp_is_mobile() && 'hover' == esc_attr( angi_fn_opt( 'tc_menu_type' ) ) ) ? '<a' : '<a class="dropdown-toggle" data-toggle="dropdown" data-target="#"';
...
/**
* Replace the walker for angi_fn_page_menu()
...
*/
if ( ! class_exists( 'ANGI_nav_walker_page' ) ) :
  class ANGI_nav_walker_page extends Walker_Page {
    function __construct() {
      add_filter('page_css_class' , array($this, 'angi_fn_add_bootstrap_classes'), 10, 5 );
    }
    ...
    * hook : page_css_class
    */
    function angi_fn_add_bootstrap_classes($css_class, $page = null, $depth = 0, $args = array(), $current_page = 0) {
...
if ( ! class_exists( 'ANGI_404' ) ) :
  class ANGI_404 {
      ...
      function __construct () {
          self::$instance =& $this;
          //404 content
          add_action  ( '__loop'                      , array( $this , 'angi_fn_404_content' ));
      }
      ...
      function angi_fn_404_content() {
          ...
          echo apply_filters( 'tc_404_content',
              sprintf('<div class="%1$s"><div class="entry-content"><p>%2$s</p> %3$s</div>%4$s</div>',
                  'tc-content span12',
                  __( 'Sorry, but the requested page is not found. You might try a search below.' , 'angilla' ),
...
*
*/
if ( ! class_exists( 'ANGI_attachment' ) ) :
    class ANGI_attachment {
        static $instance;
        function __construct () {
            self::$instance =& $this;
            add_action  ( '__loop'			              , array( $this , 'angi_fn_attachment_content' ));
        }
        ...
        function angi_fn_attachment_content() {
            ...
            <nav id="image-navigation" class="navigation" role="navigation">
                <span class="previous-image"><?php previous_image_link( false, __( '&larr; Previous' , 'angilla' ) ); ?></span>
                <span class="next-image"><?php next_image_link( false, __( 'Next &rarr;' , 'angilla' ) ); ?></span>
                    ...
                    <div class="attachment">
                        ...
                        <?php if ( $tc_fancybox == 0 ) : //fancy box not checked! ?>
                            ...
                            */
                            ...
                            $attachment_size = apply_filters( 'tc_angilla_attachment_size' , array( 960, 960 ) );
...
?><?php
/**
* Breadcrumb for Angilla
*
*/

class ANGI_breadcrumb {
    ...
    function __construct () {
        self::$instance =& $this;
        add_action( '__before_main_container'			, array( $this , 'angi_fn_breadcrumb_display' ), 20 );
        //since v3.2.0, customizer option
        add_filter( 'tc_show_breadcrumb_in_context' 	, array( $this , 'angi_fn_set_breadcrumb_display_in_context' ) );
    }
    ...
    function angi_fn_set_breadcrumb_display_in_context( $_bool ) {
    	if ( angi_fn__f('__is_home') )
	  		return 1 != esc_attr( angi_fn_opt( 'tc_show_breadcrumb_home' ) ) ? false : true;
	  	else {
		  	if ( is_page() && 1 != esc_attr( angi_fn_opt( 'tc_show_breadcrumb_in_pages' ) ) )
		  		return false;
		  	if ( is_single() && 1 != esc_attr( angi_fn_opt( 'tc_show_breadcrumb_in_single_posts' ) ) )
		  		return false;
		  	if ( ! is_page() && ! is_single() && 1 != esc_attr( angi_fn_opt( 'tc_show_breadcrumb_in_post_lists' ) ) )
		  		return false;
		}
		return $_bool;
    }
    ...
    * @since Angilla 1.0
    */
    function angi_fn_breadcrumb_display() {
	  	if ( ! apply_filters( 'tc_show_breadcrumb' , 1 == esc_attr( angi_fn_opt( 'tc_breadcrumb') ) ) )
	      return;

	  	if ( ! apply_filters( 'tc_show_breadcrumb_in_context' , true ) )
	      return;

	  	if ( angi_fn__f('__is_home')  && 1 != esc_attr( angi_fn_opt( 'tc_show_breadcrumb_home' ) ) )
	  		return;
	  	...
	  	*/
	  	...
	  	$breadcrumbs  = $breadcrumbs ? $breadcrumbs : $this -> angi_fn_breadcrumb_trail( $this -> args );
	...
	function angi_fn_breadcrumb_trail( $args = array() ) {

		/* Create an empty variable for the breadcrumb. */
		$breadcrumb = '';

		/* Get the trail items. */
		$trail = apply_filters( 'tc_breadcrumb_trail' , $this -> angi_fn_breadcrumb_trail_get_items( $args ) );
	 ...
	 */
	function angi_fn_breadcrumb_trail_get_items( $args = array() ) {
		global $wp_rewrite;
		...
		if ( function_exists( 'is_bbpress' ) && is_bbpress() ) {
			$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_bbpress_items() );
		}
		/* If WooCommerce is installed and we're on a WooCommerce page. */
		elseif ( function_exists( 'is_woocommerce' ) && is_woocommerce() ) {
			$trail 			 = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_woocommerce_items() );
			$maybe_paged = false; //angi_fn_breadcrumb_trail_get_woocommerce_items already gives us the page
		}
		...
		elseif ( is_home() ) {
			...
			$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( $home_page->post_parent, '' ) );
		...
		elseif ( is_singular() ) {
			...
			if ( 'post' == $post_type ) {
				...
				if ( !empty( $path ) && !$page_for_posts )
					$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( '' , $path ) );
				else if ( $page_for_posts )
					$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( $page_for_posts , $path ) );

				/* Map the permalink structure tags to actual links. */
				/*$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_map_rewrite_tags( $post_id, get_option( 'permalink_structure' ), $args ) );*/
			}
			...
			elseif ( 'attachment' == $post_type ) {
				...
				if ( 0 < $parent_id ) {
					...
					if ( 'post' == $parent_post_type ) {
						...
						$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_map_rewrite_tags( $post->post_parent, get_option( 'permalink_structure' ), $args ) );
					}
			...
			if ( ( empty( $path ) && 0 !== $parent ) || ( 'attachment' == $post_type ) )
				$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( $parent, '' ) );
			...
			if (  isset($args["singular_breadcrumb_taxonomy"]) && $args["singular_breadcrumb_taxonomy"] )
				...
				$trail 	= ( 1 < count($this -> angi_fn_breadcrumb_trail_get_parents($post_id) ) ) ? $trail : $this -> angi_fn_add_first_term_from_hierarchical_taxinomy( $trail , $post_id );
		...
		elseif ( is_archive() ) {
			...
			if ( is_tax() || is_category() || is_tag() ) {
				...
				if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
					$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_term_parents( $term->parent, $term->taxonomy ) );
			...
			elseif ( is_time() ) {
				...
				if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
					$trail[] = get_the_time( __( 'g:i a' , 'angilla' ) );

				elseif ( get_query_var( 'minute' ) )
					$trail[] = sprintf( __( 'Minute %1$s' , 'angilla' ), get_the_time( __( 'i' , 'angilla' ) ) );

				elseif ( get_query_var( 'hour' ) )
					$trail[] = get_the_time( __( 'g a' , 'angilla' ) );
			}
			...
			elseif ( is_date() ) {
				...
				if ( $wp_rewrite->front )
					$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( '' , $wp_rewrite->front ) );

				if ( is_day() ) {
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y' , 'angilla' ) ) . '">' . get_the_time( __( 'Y' , 'angilla' ) ) . '</a>';
					$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F' , 'angilla' ) ) . '">' . get_the_time( __( 'F' , 'angilla' ) ) . '</a>';

					if ( is_paged() )
						$trail[] = '<a href="' . get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ) . '" title="' . get_the_time( esc_attr__( 'd' , 'angilla' ) ) . '">' . get_the_time( __( 'd' , 'angilla' ) ) . '</a>';
					else
						$trail[] = get_the_time( __( 'd' , 'angilla' ) );
				}
				...
				elseif ( get_query_var( 'w' ) ) {
					...
					if ( is_paged() )
						$trail[] = get_archives_link( add_query_arg( array( 'm' => get_the_time( 'Y' ), 'w' => get_the_time( 'W' ) ), esc_url(home_url()) ), sprintf( __( 'Week %1$s' , 'angilla' ), get_the_time( esc_attr__( 'W' , 'angilla' ) ) ), false );
					else
						$trail[] = sprintf( __( 'Week %1$s' , 'angilla' ), get_the_time( esc_attr__( 'W' , 'angilla' ) ) );
				}
				...
				elseif ( is_month() ) {
					...
					if ( is_paged() )
						...
						$trail[] = get_the_time( __( 'F' , 'angilla' ) );
				}

				elseif ( is_year() ) {

					if ( is_paged() )
						$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( get_the_time( __( 'Y' , 'angilla' ) ) ) . '">' . get_the_time( __( 'Y' , 'angilla' ) ) . '</a>';
					else
						$trail[] = get_the_time( __( 'Y' , 'angilla' ) );
				}
			}
		}
		...
		elseif ( is_search() ) {
			...
			if ( is_paged() )
				$trail[] = '<a href="' . get_search_link() . '" title="' . sprintf( esc_attr__( 'Search results for &quot;%1$s&quot;' , 'angilla' ), esc_attr( get_search_query() ) ) . '">' . sprintf( __( 'Search results for &quot;%1$s&quot;' , 'angilla' ), esc_attr( get_search_query() ) ) . '</a>';
			else
				$trail[] = sprintf( __( 'Search results for &quot;%1$s&quot;' , 'angilla' ), esc_attr( get_search_query() ) );
		}

		/* If viewing a 404 error page. */
		elseif ( is_404() ) {
			$trail[] = __( '404 Not Found' , 'angilla' );
		}

		/* Check for pagination. */
		if ( $maybe_paged ) {
			if ( is_paged() )
				$trail[] = sprintf( __( 'Page %d' , 'angilla' ), absint( get_query_var( 'paged' ) ) );
			elseif ( is_singular() && 1 < get_query_var( 'page' ) )
				$trail[] = sprintf( __( 'Page %d' , 'angilla' ), absint( get_query_var( 'page' ) ) );
		}
	 ...
	 * @return array List of items to be shown in the trail.
	 */
	function angi_fn_breadcrumb_trail_get_bbpress_items( $args = array() ) {
		...
		elseif ( bbp_is_single_topic() ) {
			...
			$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( bbp_get_topic_forum_id( $topic_id ) ) );
			...
			if ( bbp_is_topic_split() )
				$trail[] = __( 'Split' , 'angilla' );

			/* If viewing a topic merge page. */
			elseif ( bbp_is_topic_merge() )
				$trail[] = __( 'Merge' , 'angilla' );
		...
		elseif ( bbp_is_single_reply() ) {
			...
			$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( bbp_get_reply_topic_id( $reply_id ) ) );
		...
		elseif ( bbp_is_single_forum() ) {
			...
			if ( 0 !== $forum_parent_id)
				$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_parents( $forum_parent_id ) );
	...
	* @return array List of items to be shown in the trail.
	*/
    function angi_fn_breadcrumb_trail_get_woocommerce_items( $args = array() ) {
      $trail = array();
	 ...
	 */
	function angi_fn_breadcrumb_trail_map_rewrite_tags( $post_id = '' , $path = '' , $args = array() ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' , $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y' , 'angilla' ), $post_id ) . '">' . get_the_time( __( 'Y' , 'angilla' ), $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' , $post_id ), get_the_time( 'm' , $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y' , 'angilla' ), $post_id ) . '">' . get_the_time( __( 'F' , 'angilla' ), $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$trail[] = '<a href="' . get_day_link( get_the_time( 'Y' , $post_id ), get_the_time( 'm' , $post_id ), get_the_time( 'd' , $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y' , 'angilla' ), $post_id ) . '">' . get_the_time( __( 'd' , 'angilla' ), $post_id ) . '</a>';
				...
				/*elseif ( '%category%' == $tag && isset($args["singular_breadcrumb_taxonomy"]) && $args["singular_breadcrumb_taxonomy"] ) {
					...
					$trail 	= $this -> angi_fn_add_first_term_from_hierarchical_taxinomy( $trail , $post_id );
				}*/
			}
		}
	 ...
	 */
	function angi_fn_breadcrumb_trail_get_parents( $post_id = '' , $path = '' ) {
		/* Set up an empty trail array. */
		$trail = array();
		...
		/*if (  isset($args["singular_breadcrumb_taxonomy"]) && $args["singular_breadcrumb_taxonomy"] )
			$trail 	= $this -> angi_fn_add_first_term_from_hierarchical_taxinomy( $trail , $parent_key );*/
	 ...
	 */
	function angi_fn_breadcrumb_trail_get_term_parents( $parent_id = '' , $taxonomy = '' ) {
	...
	function angi_fn_add_first_term_from_hierarchical_taxinomy( $trail , $post_id ) {
		// get post by post id
	  	$post = get_post( $post_id );
		...
		if ( 0 !== $first_term -> parent )
			$trail = array_merge( $trail, $this -> angi_fn_breadcrumb_trail_get_term_parents( $first_term -> parent , $first_hierarchical_tax -> name ) );
...
*
*/
if ( ! class_exists( 'ANGI_comments' ) ) :
  class ANGI_comments {
      static $instance;
      function __construct () {
        self::$instance =& $this;
        //wp hook => wp_query is built
        add_action ( 'wp'                     , array( $this , 'angi_fn_comments_set_hooks' ) );

        //! tc_user_options_style filter is shared by several classes => must always check the local context inside the callback before appending new css
        //fired on hook : wp_enqueue_scripts
        //Set thumbnail specific design based on user options
        //Set user defined various inline stylings
        add_filter( 'tc_user_options_style'   , array( $this , 'angi_fn_comment_bubble_inline_css' ) );
      }
      ...
      */
      function angi_fn_comments_set_hooks() {
        ...
        add_action ( '__after_loop'           , array( $this , 'angi_fn_comments' ), 10 );
        ...
        add_filter( 'tc_display_comment_list' , array( $this , 'angi_fn_set_comment_list_display' ) );

        //Add actions in the comment's template
        add_action ( '__comment'              , array( $this , 'angi_fn_comment_title' ), 10 );
        add_action ( '__comment'              , array( $this , 'angi_fn_comment_list' ), 20 );
        add_action ( '__comment'              , array( $this , 'angi_fn_comment_navigation' ), 30 );
        add_action ( '__comment'              , array( $this , 'angi_fn_comment_close' ), 40 );
        add_filter ( 'comment_form_defaults'  , array( $this , 'angi_fn_set_comment_title') );

        //Add comment bubble
        add_filter( 'tc_the_title'            , array( $this , 'angi_fn_display_comment_bubble' ), 1 );
        //Custom Bubble comment since 3.2.6
        add_filter( 'tc_bubble_comment'       , array( $this , 'angi_fn_custom_bubble_comment'), 10, 2 );
      }
     ...
     */
       ...
       */
        function angi_fn_comment_title() {
          if ( 1 == get_comments_number() ) {
            $_title = __( 'One thought on', 'angilla' );
          } else {
            $_title = sprintf( '%1$s %2$s', number_format_i18n( get_comments_number(), 'angilla' ) , __( 'thoughts on', 'angilla' ) );
          }
       ...
       */
        ...
        * @since Angilla 1.0
        */
       function angi_fn_comment_callback( $comment, $args, $depth ) {
        ...
        <li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
          <article id="comment-<?php comment_ID(); ?>" class="comment">
            <p><?php _e( 'Pingback:' , 'angilla' ); ?> <?php comment_author_link(); ?>
                <?php if ( ! angi_fn_is_customizing() )  edit_comment_link( __( '(Edit)' , 'angilla' ), '<span class="edit-link btn btn-success btn-mini">' , '</span>' ); ?>
            </p>
          </article>
        <?php
            break;
              ...
              printf('<article id="comment-%9$s" class="comment"><div class="%1$s"><div class="%2$s">%3$s</div><div class="%4$s">%5$s %6$s %7$s %8$s</div></div></article>',
                  ...
                  $tc_show_comment_content ? sprintf('<div class="%1$s">%2$s</div>',
                                            ...
                                            get_comment_reply_link( array_merge(
                                                                        $args,
                                                                        array(  'reply_text' => __( 'Reply' , 'angilla' ).' <span>&darr;</span>',
                                                                                'depth' => $depth,
                  ...
                  sprintf('<header class="comment-meta comment-author vcard">%1$s %2$s</header>',
                        sprintf( '<cite class="fn">%1$s %2$s %3$s</cite>' ,
                            ...
                            ( $comment->user_id === $post->post_author ) ? '<span> ' . __( 'Post author' , 'angilla' ) . '</span>' : '' ,
                            ! angi_fn_is_customizing() && current_user_can( 'edit_comment', $comment->comment_ID ) ? '<p class="edit-link btn btn-success btn-mini"><a class="comment-edit-link" href="' . get_edit_comment_link( $comment->comment_ID ) . '">' . __( 'Edit' , 'angilla' ) . '</a></p>' : ''
                        ),
                        sprintf( '<a class="comment-date" href="%1$s"><time datetime="%2$s">%3$s</time></a>' ,
                            esc_url( get_comment_link( $comment->comment_ID ) ),
                            get_comment_time( 'c' ),
                            /* translators: 1: date, 2: time */
                            sprintf( __( '%1$s at %2$s' , 'angilla' ), get_comment_date(), get_comment_time() )
                        )
                  ),

                  ( '0' == $comment->comment_approved ) ? sprintf('<p class="comment-awaiting-moderation">%1$s</p>',
                    __( 'Your comment is awaiting moderation.' , 'angilla' )
                    ) : '',
   ...
   */
    function angi_fn_comment_navigation () {
      if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) : // are there comments to navigate through
        ...
        <nav id="comment-nav-below" class="navigation" role="navigation">
          <h3 class="assistive-text section-heading"><?php _e( 'Comment navigation' , 'angilla' ); ?></h3>
          <ul class="pager">

            <?php if(get_previous_comments_link() != null) : ?>

              <li class="previous">
                <span class="nav-previous"><?php previous_comments_link( __( '<span class="meta-nav">&larr;</span> Older Comments' , 'angilla' ) ); ?></span>
              </li>
            ...
            <?php if(get_next_comments_link() != null) : ?>
              ...
              <li class="next">
                <span class="nav-next"><?php next_comments_link( __( 'Newer Comments <span class="meta-nav">&rarr;</span>' , 'angilla' ) ); ?></span>
              </li>
      ...
      if ( ! comments_open() && get_comments_number() ) :
        echo apply_filters( 'tc_comment_close' ,
          sprintf('<p class="nocomments">%1$s</p>',
            __( 'Comments are closed.' , 'angilla' )
          )
        );
    ...
    */
    function angi_fn_display_comment_bubble( $_title = null ) {
      ...
      return sprintf('%1$s <span class="comments-link"><a href="%2$s%3$s" title="%4$s %5$s" data-disqus-identifier="javascript:this.page.identifier">%6$s</a></span>',
        ...
        sprintf( '%1$s %2$s' , get_comments_number(), __( 'Comment(s) on' , 'angilla' ) ),
        is_null($_title) ? esc_attr( strip_tags( $post -> post_title ) ) : esc_attr( strip_tags( $_title ) ),
        0 != get_comments_number() ? apply_filters( 'tc_bubble_comment' , '' , esc_attr( angi_fn_opt( 'tc_comment_bubble_shape' ) ) ) : ''
      );
    }
    ...
    */
    function angi_fn_custom_bubble_comment( $_html , $_opt ) {
      return sprintf('%4$s<span class="tc-comment-bubble %1$s">%2$s %3$s</span>',
        ...
        get_comments_number(),
        'default' == $_opt ? '' : sprintf( _n( 'comment' , 'comments' , get_comments_number(), 'angilla' ),
          number_format_i18n( get_comments_number(), 'angilla' )
        ),
        $_html
      );
    }
    ...
    */
    function angi_fn_comment_bubble_inline_css( $_css ) {
      ...
      $_bubble_color_type   = esc_attr( angi_fn_opt( 'tc_comment_bubble_color_type' ) );
      $_custom_bubble_color = esc_attr( angi_fn_opt( 'tc_comment_bubble_color' ) );

      $_bubble_shape        = esc_attr( angi_fn_opt( 'tc_comment_bubble_shape' ) );
    ...
    */
    private function angi_fn_are_comments_enabled() {
      ...
      if ( isset( $post ) ) {
        $_bool = ( post_password_required() || angi_fn__f( '__is_home' ) || ! is_singular() )  ? false : true;
    ...
    */
    private function angi_fn_is_bubble_enabled() {
      $_bool_arr = array(
        in_the_loop(),
        (bool) esc_attr( angi_fn_opt( 'tc_comment_show_bubble' ) ),
        $this -> angi_fn_are_comments_enabled(),
        get_comments_number() != 0,
        (bool) esc_attr( angi_fn_opt( 'tc_show_comment_list' ) ),
...
*
*/
if ( ! class_exists( 'ANGI_featured_pages' ) ) :
  class ANGI_featured_pages {
    static $instance;
    function __construct () {
        self::$instance =& $this;
        add_action( '__before_main_container'     , array( $this , 'angi_fn_fp_block_display'), 10 );
    }
    ...
    */
    function angi_fn_fp_block_display() {
      ...
      if ( ! $this -> angi_fn_show_featured_pages()  )
        return;

      $tc_show_featured_pages_img     = $this -> angi_fn_show_featured_pages_img();
        ...
        <?php
          ...
          for ($i = 1; $i <= $fp_nb ; $i++ ) {
                printf('%1$s<div class="span%2$s fp-%3$s">%4$s</div>%5$s',
                    ...
                    $span_value,
                    $fp_ids[$i - 1],
                    $this -> angi_fn_fp_single_display( $fp_ids[$i - 1] , $tc_show_featured_pages_img ),
      ...
      <?php  echo ! angi_fn__f( '__is_home_empty') ? apply_filters( 'tc_after_fp_separator', '<hr class="featurette-divider '.current_filter().'">' ) : ''; ?>
      ...
      */
      function angi_fn_fp_single_display( $fp_single_id,$show_img) {
        $_skin_color                        = ANGI_utils::$inst -> angi_fn_get_skin_color();
        ...
        //if fps are not set
        if ( null == angi_fn_opt( 'tc_featured_page_'.$fp_single_id ) || ! angi_fn_opt( 'tc_featured_page_'.$fp_single_id ) ) {
            //admin link if user logged in
            $featured_page_link             = '';
            $angilla_link                 = '';
            if ( ! angi_fn_is_customizing() && is_user_logged_in() && current_user_can('edit_theme_options') ) {
              $angilla_link              = sprintf( '<a href="%1$s" title="%2$s">%3$s</a>',
                angi_fn_get_customizer_url( array( 'control' => 'tc_featured_text_'.$fp_single_id, 'section' => 'frontpage_sec') ),
                __( 'Customizer screen' , 'angilla' ),
                __( 'Edit now.' , 'angilla' )
              );
            }
            $featured_page_link          = apply_filters( 'tc_fp_link_url', angi_fn_get_customizer_url( array( 'control' => 'tc_featured_page_'.$fp_single_id, 'section' => 'frontpage_sec') ) );

            //rendering
            $featured_page_id               =  null;
            $featured_page_title            =  apply_filters( 'tc_fp_title', __( 'Featured page' , 'angilla' ), $fp_single_id, $featured_page_id);
            $text                           =  apply_filters(
                                                'tc_fp_text',
                                                sprintf( '%1$s %2$s',
                                                  __( 'Featured page description text : use the page excerpt or set your own custom text in the customizer screen.' , 'angilla' ),
                                                  $angilla_link
                                                ),
                                                $fp_single_id,
                                                $featured_page_id
            ...
            $featured_page_id               = apply_filters( 'tc_fp_id', esc_attr( angi_fn_opt( 'tc_featured_page_'.$fp_single_id) ), $fp_single_id );
            ...
            $featured_text                  = apply_filters( 'tc_fp_text', angi_fn_opt( 'tc_featured_text_'.$fp_single_id ), $fp_single_id, $featured_page_id );
            ...
            $text                           = angi_fn_text_truncate( $text, $default_fp_text_length, $more = '...', $strip_tags = false ); //tags already stripped
            ...
            $fp_img                         = $this -> angi_fn_get_fp_img( $fp_img_size, $featured_page_id, $fp_custom_img_id);
          ...
          <div class="widget-front">
            <?php
              ...
              //button block
              $tc_fp_button_text = apply_filters( 'tc_fp_button_text' , esc_attr( angi_fn_opt( 'tc_featured_page_button_text') ) , $fp_single_id );

              if ( $tc_fp_button_text || angi_fn_is_customizing() ){
    ...
    * HELPERS
    *******************************/
    function angi_fn_get_fp_img( $fp_img_size, $featured_page_id = null , $fp_custom_img_id = null ){
      //try to get "tc_thumb" , "tc_thumb_height" , "tc_thumb_width"
      //angi_fn_get_thumbnail_model( $requested_size = null, $_post_id = null , $_thumb_id = null )
      $_fp_img_model = ANGI_post_thumbnails::$instance -> angi_fn_get_thumbnail_model( $fp_img_size, $featured_page_id, $fp_custom_img_id );
    ...
    function angi_fn_show_featured_pages() {
      //gets display fp option
      $tc_show_featured_pages         = esc_attr( angi_fn_opt( 'tc_show_featured_pages' ) );

      return apply_filters( 'tc_show_fp', 0 != $tc_show_featured_pages && angi_fn__f('__is_home') );
    }


    function angi_fn_show_featured_pages_img() {
      //gets  display img option
      return apply_filters( 'tc_show_featured_pages_img', esc_attr( angi_fn_opt( 'tc_show_featured_pages_img' ) ) );
    }
...
*
*/
if ( ! class_exists( 'ANGI_gallery' ) ) :
  class ANGI_gallery {
      static $instance;
      function __construct () {
        self::$instance =& $this;

        add_filter ( 'tc_article_container_class' , array( $this, 'angi_fn_add_gallery_class' ), 20 );
        //adds a filter for link markup (allow lightbox)
        add_filter ( 'wp_get_attachment_link'     , array( $this, 'angi_fn_modify_attachment_link') , 20, 6 );
      }
      ...
      function angi_fn_add_gallery_class( $_classes ){
        if (  $this -> angi_fn_is_gallery_enabled() && apply_filters( 'tc_gallery_style', esc_attr( angi_fn_opt( 'tc_gallery_style' ) ) ) )
      ...
      function angi_fn_modify_attachment_link( $markup, $id, $size, $permalink, $icon, $text ) {
        ...
        if ( ! $this -> angi_fn_is_gallery_enabled() )
          return $markup;

        $tc_gallery_fancybox = apply_filters( 'tc_gallery_fancybox', esc_attr( angi_fn_opt( 'tc_gallery_fancybox' ) ) , $id );
        ...
        if ( $tc_gallery_fancybox == 1 && $permalink == false ) //add the filter only if link to the attachment file/image
          {
              ...
              if ( empty( $_post ) || ( 'attachment' != $_post->post_type ) || ! $url = wp_get_attachment_url( $_post->ID ) )
                return __( 'Missing Attachment' , 'angilla');
       ...
       * HELPERS
       */
      function angi_fn_is_gallery_enabled(){
        return apply_filters('tc_enable_gallery', esc_attr( angi_fn_opt('tc_enable_gallery') ) );
      }
  }//end of class
endif;
...
*
*/
if ( ! class_exists( 'ANGI_headings' ) ) :
  class ANGI_headings {
      static $instance;
      function __construct () {
        self::$instance =& $this;
        //set actions and filters for posts and page headings
        add_action( 'template_redirect'                            , array( $this , 'angi_fn_set_post_page_heading_hooks') );
        //set actions and filters for archives headings
        add_action( 'template_redirect'                            , array( $this , 'angi_fn_set_archives_heading_hooks') );
        //Set headings user options
        add_action( 'template_redirect'                            , array( $this , 'angi_fn_set_headings_options') );
      }
      ...
      */
      function angi_fn_set_archives_heading_hooks() {
        ...
        if ( apply_filters('tc_display_angilla_headings',  ! $this -> angi_fn_archive_title_and_class_callback() || is_feed() ) )
          return;

        //Headings for archives, authors, search, 404
        add_action ( '__before_loop'                  , array( $this , 'angi_fn_render_headings_view' ) );
        //Set archive icon with customizer options (since 3.2.0)
        add_filter ( 'tc_archive_icon'                , array( $this , 'angi_fn_set_archive_icon' ) );

        add_filter( 'tc_archive_header_class'         , array( $this , 'angi_fn_archive_title_and_class_callback'), 10, 2 );
        add_filter( 'tc_headings_archive_html'        , array( $this , 'angi_fn_archive_title_and_class_callback'), 10, 1 );
        global $wp_query;
        if ( angi_fn__f('__is_home') )
      ...
      */
      function angi_fn_set_post_page_heading_hooks() {
        ...
        //by default don't display the Angilla title of the front page and in feeds
        if ( apply_filters('tc_display_angilla_headings', ( is_front_page() && 'page' == get_option( 'show_on_front' ) ) ) || is_feed() )
          return;

        //Set single post/page icon with customizer options (since 3.2.0)
        add_filter ( 'tc_content_title_icon'    , array( $this , 'angi_fn_set_post_page_icon' ) );
        //Prepare the headings for post, page, attachment
        add_action ( '__before_content'         , array( $this , 'angi_fn_render_headings_view' ) );
        //Populate heading with default content
        add_filter ( 'tc_headings_content_html' , array( $this , 'angi_fn_post_page_title_callback'), 10 );
        //Create the Angilla title
        add_filter( 'tc_the_title'              , array( $this , 'angi_fn_content_heading_title' ) , 0 );
        //Add edit link
        add_filter( 'tc_the_title'              , array( $this , 'angi_fn_add_edit_link_after_title' ), 2 );
        //Set user defined archive titles
        add_filter( 'tc_category_archive_title' , array( $this , 'angi_fn_set_archive_custom_title' ) );
        add_filter( 'tc_tag_archive_title'      , array( $this , 'angi_fn_set_archive_custom_title' ) );
        add_filter( 'tc_search_results_title'   , array( $this , 'angi_fn_set_archive_custom_title' ) );
        add_filter( 'tc_author_archive_title'   , array( $this , 'angi_fn_set_archive_custom_title' ) );
        ...
        add_filter( 'tc_headings_content_html'  , array( $this, 'angi_fn_post_formats_heading') , 100 );
      ...
      */
      function angi_fn_content_heading_title( $_title ) {
        ...
        if ( is_singular() || ! apply_filters('tc_display_link_for_post_titles' , true ) )
          return is_null($_title) ? apply_filters( 'tc_no_title_post', __( '{no title} Read the post &raquo;' , 'angilla' ) )  : $_title;
        else
          return sprintf('<a href="%1$s" title="%2$s" rel="bookmark">%3$s</a>',
            get_permalink(),
            sprintf( apply_filters( 'tc_post_link_title' , __( 'Permalink to %s' , 'angilla' ) ) , esc_attr( strip_tags( get_the_title() ) ) ),
            is_null($_title) ? apply_filters( 'tc_no_title_post', __( '{no title} Read the post &raquo;' , 'angilla' ) )  : $_title
          );//end sprintf
      }
      ...
      */
      function angi_fn_add_edit_link_after_title( $_title ) {
        ...
        if ( ! apply_filters( 'tc_edit_in_title', $this -> angi_fn_is_edit_enabled() ) )
          return $_title;

        return sprintf('%1$s %2$s',
          $_title,
          $this -> angi_fn_render_edit_link_view( $_echo = false )
        );
      ...
      */
      function angi_fn_set_post_page_icon( $_bool ) {
          if ( is_page() )
            $_bool = ( 0 == esc_attr( angi_fn_opt( 'tc_show_page_title_icon' ) ) ) ? false : $_bool;
          if ( is_single() && ! is_page() )
            $_bool = ( 0 == esc_attr( angi_fn_opt( 'tc_show_post_title_icon' ) ) ) ? false : $_bool;
          if ( ! is_single() )
            $_bool = ( 0 == esc_attr( angi_fn_opt( 'tc_show_post_list_title_icon' ) ) ) ? false : $_bool;
          //last condition
          return ( 0 == esc_attr( angi_fn_opt( 'tc_show_title_icon' ) ) ) ? false : $_bool;
      }
      ...
      */
      function angi_fn_set_archive_icon( $_class ) {
          ...
          //last condition
          return 0 == esc_attr( angi_fn_opt( 'tc_show_title_icon' ) ) ? '' : $_class;
      }
      ...
      * hook : tc_display_angilla_headings
      * @return  boolean
      *
      * @package Angilla
      * @since Angilla 1.0
      ...
      function angi_fn_archive_title_and_class_callback( $_title = null, $_return_class = false ) {
        ...
        //404
          ...
          $content        = sprintf('<h1 class="entry-title %1$s">%2$s</h1>',
                ...
                apply_filters( 'tc_404_title' , __( 'Ooops, page not found' , 'angilla' ) )
           );
        ...
        //search results
          ...
          $content        = sprintf( '<div class="row-fluid"><div class="%1$s"><h1 class="%2$s">%3$s%4$s %5$s </h1></div><div class="%6$s">%7$s</div></div>',
                ...
                have_posts() ? '' :  __( 'No' , 'angilla' ).'&nbsp;' ,
                apply_filters( 'tc_search_results_title' , __( 'Search Results for :' , 'angilla' ) ),
        ...
        // all archives
            ...
            if ( apply_filters ( 'tc_show_author_meta' , get_the_author_meta( 'description', $user_id  ) ) ) {
              $content    .= sprintf('%1$s<div class="author-info"><div class="%2$s">%3$s</div></div>',
                  ...
                  sprintf('<div class="%1$s">%2$s</div><div class="%3$s"><h2>%4$s</h2><div>%5$s</div></div>',
                      ...
                      sprintf( __( 'About %s' , 'angilla' ), get_the_author() ),
          ...
          //time archives
            ...
            $archive_type   = is_day() ? sprintf( __( 'Daily Archives: %s' , 'angilla' ), '<span>' . get_the_date() . '</span>' ) : __( 'Archives' , 'angilla' );
            $archive_type   = is_month() ? sprintf( __( 'Monthly Archives: %s' , 'angilla' ), '<span>' . get_the_date( _x( 'F Y' , 'monthly archives date format' , 'angilla' ) ) . '</span>' ) : $archive_type;
            $archive_type   = is_year() ? sprintf( __( 'Yearly Archives: %s' , 'angilla' ), '<span>' . get_the_date( _x( 'Y' , 'yearly archives date format' , 'angilla' ) ) . '</span>' ) : $archive_type;
      ...
      */
      function angi_fn_set_headings_options() {
        //by default don't display the Angilla title in feeds
        if ( apply_filters('tc_display_angilla_headings',  is_feed() ) )
          return;

        //Add update status next to the title (@since 3.2.6)
        add_filter( 'tc_the_title'                  , array( $this , 'angi_fn_add_update_notice_in_title'), 20);
      }
      ...
      */
      function angi_fn_add_update_notice_in_title($html) {
          ...
          if ( 0 == esc_attr( angi_fn_opt( 'tc_post_metas_update_notice_in_title' ) ) || ! in_array( get_post_type(), apply_filters('tc_show_update_notice_for_post_types' , array( 'post') ) ) )
              return $html;
          ...
          $_interval = esc_attr( angi_fn_opt( 'tc_post_metas_update_notice_interval' ) );
          $_interval = ( 0 != $_interval ) ? $_interval : 30;

          //Check if the last update is less than n days old. (replace n by your own value)
          $has_recent_update = ( angi_fn_post_has_update( true ) && angi_fn_post_has_update( 'in_days') < $_interval ) ? true : false;
          ...
          return apply_filters(
              ...
              sprintf('%1$s &nbsp; <span class="tc-update-notice label %3$s">%2$s</span>',
                  $html,
                  esc_attr( angi_fn_opt( 'tc_post_metas_update_notice_text' ) ),
                  esc_attr( angi_fn_opt( 'tc_post_metas_update_notice_format' ) )
              )
          );
      }
      ...
      function angi_fn_set_archive_custom_title( $_title ) {
        switch ( current_filter() ) {
          case 'tc_category_archive_title' :
            return esc_attr( angi_fn_opt( 'tc_cat_title' ) );
          break;

          case 'tc_tag_archive_title' :
            return esc_attr( angi_fn_opt( 'tc_tag_title' ) );
          break;

          case 'tc_search_results_title' :
            return esc_attr( angi_fn_opt( 'tc_search_title' ) );
          break;

          case 'tc_author_archive_title' :
            return esc_attr( angi_fn_opt( 'tc_author_title' ) );
          break;
        }
        return $_title;
      }
...
*
*/
if ( ! class_exists( 'ANGI_no_results' ) ) :
  class ANGI_no_results {
      static $instance;
      function __construct () {
          self::$instance =& $this;
          add_action  ( '__loop'                        , array( $this , 'angi_fn_no_result_content' ));
      }
      ...
      function angi_fn_no_result_content() {
          ...
          echo apply_filters( 'tc_no_result_content',
              sprintf('<div class="%1$s"><div class="entry-content"><p>%2$s</p> %3$s</div>%4$s</div>',
                  'tc-content span12',
                  __( 'Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'angilla' ),
                  get_search_form( $echo = false ),
                  '<hr class="featurette-divider '.current_filter().'">'
              )//end sprintf
          );//end filter
...
*
*/
if ( ! class_exists( 'ANGI_page' ) ) :
  class ANGI_page {
    static $instance;
    function __construct () {
      self::$instance =& $this;
      add_action( 'wp'                , array( $this , 'angi_fn_set_page_hooks' ) );
      //Set single post thumbnail with customizer options (since 3.5+)
      add_action( 'wp'                , array( $this , 'angi_fn_set_single_page_thumbnail_hooks' ));

      //append inline style to the custom stylesheet
      //! tc_user_options_style filter is shared by several classes => must always check the local context inside the callback before appending new css
      //fired on hook : wp_enqueue_scripts
      //Set thumbnail specific design based on user options
      add_filter( 'tc_user_options_style'    , array( $this , 'angi_fn_write_thumbnail_inline_css') );
    }
    ...
    */
    function angi_fn_set_page_hooks() {
      ...
      add_action( '__loop'              , array( $this , 'angi_fn_page_content' ) );
    }
    ...
    */
    function angi_fn_set_single_page_thumbnail_hooks() {
      ...
      //Hook post view
      add_action( $_hook, array($this , 'angi_fn_single_page_prepare_thumb') , $_priority );
      //Set thumb shape with customizer options (since 3.2.0)
      add_filter( 'tc_post_thumb_wrapper'      , array( $this , 'angi_fn_set_thumb_shape'), 10 , 2 );
    }
    ...
    function angi_fn_page_content() {
      ...
      ob_start();
        ...
        <div class="entry-content">
          <?php
            the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>' , 'angilla' ) );
            wp_link_pages( array(
                'before'        => '<div class="btn-toolbar page-links"><div class="btn-group">' . __( 'Pages:' , 'angilla' ),
                'after'         => '</div></div>',
    ...
    */
    function angi_fn_single_page_prepare_thumb() {
      ...
      if ( 0 != did_action('__after_carousel_inner') && '__before_main_wrapper' == current_filter() )
        return;
      $_size_to_request = apply_filters( 'tc_single_page_thumb_size' , $this -> angi_fn_get_current_thumb_size() );
      //get the thumbnail data (src, width, height) if any
      //array( "tc_thumb" , "tc_thumb_height" , "tc_thumb_width" )
      $_thumb_model   = ANGI_post_thumbnails::$instance -> angi_fn_get_thumbnail_model( $_size_to_request ) ;
      //may be render
      if ( ANGI_post_thumbnails::$instance -> angi_fn_has_thumb() ) {
        ...
        $this -> angi_fn_render_single_page_thumb_view( $_thumb_model , $_thumb_class );
      }
    }
    ...
    private function angi_fn_render_single_page_thumb_view( $_thumb_model , $_thumb_class ) {
      echo apply_filters( 'tc_render_single_page_thumb_view',
        sprintf( '<div class="%1$s">%2$s</div>' ,
          $_thumb_class,
          ANGI_post_thumbnails::$instance -> angi_fn_render_thumb_view( $_thumb_model, 'span12', false )
        )
      );
    }
    ...
    */
    function angi_fn_write_thumbnail_inline_css( $_css ) {
      ...
      $_single_thumb_height   = apply_filters('tc_single_page_thumb_height', esc_attr( angi_fn_opt( 'tc_single_page_thumb_height' ) ) );
      $_single_thumb_height   = (! $_single_thumb_height || ! is_numeric($_single_thumb_height) ) ? 250 : $_single_thumb_height;

      $_single_thumb_smartphone_height   = apply_filters('tc_single_page_thumb_smartphone_height', esc_attr( angi_fn_opt( 'tc_single_page_thumb_smartphone_height' ) ) );
...
*
*/
if ( ! class_exists( 'ANGI_post' ) ) :
  class ANGI_post {
    static $instance;
    function __construct () {
      self::$instance =& $this;
      add_action( 'wp'                , array( $this , 'angi_fn_set_single_post_hooks' ));
      //Set single post thumbnail with customizer options (since 3.2.0)
      add_action( 'wp'                , array( $this , 'angi_fn_set_single_post_thumbnail_hooks' ));
    ...
    */
    function angi_fn_set_single_post_hooks() {
      ...
      add_action( '__loop'              , array( $this , 'angi_fn_post_content' ));
      //posts parts actions
      add_action( '__after_content'     , array( $this , 'angi_fn_post_footer' ));
    }
    ...
    */
    function angi_fn_set_single_post_thumbnail_hooks() {
      ...
      //Hook post view
      add_action( $_hook, array($this , 'angi_fn_single_post_prepare_thumb') , $_priority );
      //Set thumb shape with customizer options (since 3.2.0)
      add_filter( 'tc_post_thumb_wrapper'      , array( $this , 'angi_fn_set_thumb_shape'), 10 , 2 );
    }
    ...
    ****************************/
    ...
    function angi_fn_post_content() {
      ...
      if ( ! $this -> angi_fn_single_post_display_controller() )
          return;
      ...
      do_action( '__before_content' );
        ?>
          <section class="<?php echo implode( ' ', apply_filters( 'tc_single_post_section_class', array( 'entry-content' ) ) ); ?> <?php echo $icon_class ?>">
              ...
              <?php wp_link_pages( array( 'before' => '<div class="pagination pagination-centered">' . __( 'Pages:' , 'angilla' ), 'after' => '</div>' ) ); ?>
    ...
    */
    function angi_fn_post_footer() {
      //check conditional tags : we want to show single post or single custom post types
      if ( ! $this -> angi_fn_single_post_display_controller() || ! apply_filters( 'tc_show_single_post_footer', true ) )
          return;

      //@todo check if some conditions below not redundant?
      if ( ! is_singular() || ! apply_filters( 'tc_show_author_metas_in_post', true ) || ! esc_attr( angi_fn_opt( 'tc_show_author_info' ) ) ) {
        return;
      }
    ...
    */
    function angi_fn_single_post_prepare_thumb() {
      ...
      if ( 0 != did_action('__after_carousel_inner') && '__before_main_wrapper' == current_filter() )
        return;
      $_size_to_request = apply_filters( 'tc_single_post_thumb_size' , $this -> angi_fn_get_current_thumb_size() );
      //get the thumbnail data (src, width, height) if any
      //array( "tc_thumb" , "tc_thumb_height" , "tc_thumb_width" )
      $_thumb_model   = ANGI_post_thumbnails::$instance -> angi_fn_get_thumbnail_model( $_size_to_request ) ;
      //may be render
      if ( ANGI_post_thumbnails::$instance -> angi_fn_has_thumb() ) {
        ...
        $this -> angi_fn_render_single_post_thumb_view( $_thumb_model , $_thumb_class );
      }
    }
    ...
    */
    function angi_fn_write_thumbnail_inline_css( $_css ) {
      ...
      $_single_thumb_height   = apply_filters('tc_single_post_thumb_height', esc_attr( angi_fn_opt( 'tc_single_post_thumb_height' ) ) );
      $_single_thumb_height   = (! $_single_thumb_height || ! is_numeric($_single_thumb_height) ) ? 250 : $_single_thumb_height;

      $_single_thumb_smartphone_height   = apply_filters('tc_single_post_thumb_smartphone_height', esc_attr( angi_fn_opt( 'tc_single_post_thumb_smartphone_height' ) ) );
      $_single_thumb_smartphone_height   = (! $_single_thumb_smartphone_height || ! is_numeric($_single_thumb_smartphone_height) ) ? 200 : $_single_thumb_smartphone_height;
...
*
*/
if ( ! class_exists( 'ANGI_post_list' ) ) :
class ANGI_post_list {
  static $instance;
  function __construct () {
    self::$instance =& $this;
    //Set new image size can be set here ( => wp hook would be too late) (since 3.2.0)
    add_action( 'init'                    , array( $this, 'angi_fn_set_thumb_early_options') );
    //Set __loop hooks and customizer options (since 3.2.0)
    add_action( 'wp_head'                 , array( $this, 'angi_fn_set_post_list_hooks'));
    //append inline style to the custom stylesheet
    //! tc_user_options_style filter is shared by several classes => must always check the local context inside the callback before appending new css
    //fired on hook : wp_enqueue_scripts
    //Set thumbnail specific design based on user options
  ...
  */
  function angi_fn_set_thumb_early_options() {
    ...
    add_filter ( 'tc_thumb_size_name'     , array( $this , 'angi_fn_set_thumb_size') );
  }
  ...
  */
  function angi_fn_set_post_list_hooks() {
    ...
    add_action ( '__loop'                    , array( $this , 'angi_fn_prepare_section_view') );

    //ARTICLE CONTAINER CSS CLASSES
    add_filter( 'tc_article_container_class' , array( $this, 'angi_fn_article_container_set_classes' ) );

    //based on customizer user options
    add_filter( 'tc_post_list_layout'        , array( $this , 'angi_fn_set_post_list_layout') );
    add_filter( 'post_class'                 , array( $this , 'angi_fn_set_content_class') );
    add_filter( 'excerpt_length'             , array( $this , 'angi_fn_set_excerpt_length') , 999 );
    add_filter( 'post_class'                 , array( $this , 'angi_fn_add_thumb_shape_name') );

    //add current context to the body class
    add_filter( 'body_class'                 , array( $this , 'angi_fn_add_post_list_context') );
    //Set thumb shape with customizer options (since 3.2.0)
    add_filter( 'tc_post_thumb_wrapper'      , array( $this , 'angi_fn_set_thumb_shape'), 10 , 2 );

    add_filter( 'tc_the_content'             , array( $this , 'angi_fn_add_support_for_shortcode_special_chars') );
    ...
    add_filter( 'tc_post_thumb_inline_style'  , array( $this , 'angi_fn_change_thumbnail_inline_css_width'), 20, 3 );
  }
  ...
  */
  function angi_fn_prepare_section_view() {
    global $post;
    if ( ! isset( $post ) || empty( $post ) || ! apply_filters( 'tc_show_post_in_post_list', $this -> angi_fn_post_list_controller() , $post ) )
      return;

    //get the filtered post list layout
    $_layout        = apply_filters( 'tc_post_list_layout', ANGI_init::$instance -> post_list_layout );
    $_content_model = $this -> angi_fn_get_content_model( $_layout );
    $_thumb_model   = $this -> angi_fn_show_thumb() ? ANGI_post_thumbnails::$instance -> angi_fn_get_thumbnail_model() : array();

    $this -> angi_fn_render_section_view( $_layout, $_content_model, $_thumb_model );
  }
  ...
  */
  private function angi_fn_get_content_model($_layout) {
    ...
    $_layout_class = $this -> angi_fn_show_thumb() ? $_layout['content'] : 'span12';
    $_layout_class = implode( " " , apply_filters( 'tc_post_list_content_class', array($_layout_class) , $this -> angi_fn_show_thumb() , $_layout ) );
  ...
  */
  private function angi_fn_show_excerpt() {
    ...
    return (bool) apply_filters( 'tc_show_excerpt', 'full' != esc_attr( angi_fn_opt( 'tc_post_list_length' ) ) );
  }
  ...
  */
  private function angi_fn_show_thumb() {
    ...
    return apply_filters( 'tc_show_thumb', array_product(
        array(
          $this -> angi_fn_show_excerpt(),
          ANGI_post_thumbnails::$instance -> angi_fn_has_thumb(),
          0 != esc_attr( angi_fn_opt( 'tc_post_list_show_thumb' ) )
        )
      )
    );
  }
  ...
  private function angi_fn_render_section_view( $_layout, $_content_model, $_thumb_model ) {
    ...
    if ( isset($_layout['alternate']) && $_layout['alternate'] ) {
      if ( 0 == $wp_query->current_post % 2 ) {
        $this -> angi_fn_render_content_view( $_content_model ) ;
        ANGI_post_thumbnails::$instance -> angi_fn_render_thumb_view( $_thumb_model , $_layout['thumb'] );
      }
      else {
        ANGI_post_thumbnails::$instance -> angi_fn_render_thumb_view( $_thumb_model , $_layout['thumb'] );
        $this -> angi_fn_render_content_view( $_content_model );
  ...
  private function angi_fn_render_content_view( $_content_model ) {
    ...
    elseif ( in_array( get_post_format(), array( 'quote', 'status', 'link', 'aside', 'video' ) ) )
    {
      ...
      $_content   = sprintf( '%1$s%2$s',
        apply_filters( 'tc_the_content', get_the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>' , 'angilla' ) ) ),
        wp_link_pages( array(
          'before'  => '<div class="pagination pagination-centered">' . __( 'Pages:' , 'angilla' ),
          'after'   => '</div>',
          'echo'    => 0
          ) )
      );
  ...
  */
  public function angi_fn_article_container_set_classes( $_classes ) {
  ...
  */
  function angi_fn_set_post_list_layout( $_layout ) {
    ...
    $_layout['alternate']        = ( 0 == esc_attr( angi_fn_opt( 'tc_post_list_thumb_alternate' ) )
  ...
  */
  ...
  /**
  * hook tc_post_thumb_inline_style (declared in ANGI_post_thumbnails)
  ...
  */
  function angi_fn_change_thumbnail_inline_css_width( $_style,  $image, $_filtered_thumb_size) {
    ...
    $_bool = array_product(
      array(
        ! esc_attr( angi_fn_opt( 'tc_center_img') ),
        false != $image,
        ! empty($image),
  ...
  */
  function angi_fn_write_thumbnail_inline_css( $_css ) {
    if ( ! $this -> angi_fn_post_list_controller() )
      return $_css;
    $_list_thumb_height     = esc_attr( angi_fn_opt( 'tc_post_list_thumb_height' ) );
...
*
*/
if ( ! class_exists( 'ANGI_post_list_grid' ) ) :
    class ANGI_post_list_grid {
        static $instance;
        ...
        function __construct () {
          ...
          add_action ( 'pre_get_posts'              , array( $this , 'angi_fn_maybe_excl_first_sticky') );
          add_action ( 'wp_head'                    , array( $this , 'angi_fn_set_grid_hooks') );

          //Font size filter
          //Updates the array of font sizes for a given sidebar layout
          add_filter( 'tc_get_grid_font_sizes'      , array( $this , 'angi_fn_set_layout_font_size' ), 10, 4 );


          //Layout filter (must filter also the grid cols used in the inline style)
          add_filter( 'tc_get_grid_cols'            , array( $this, 'angi_fn_set_grid_section_cols'), 20 , 2 );

          //append inline style to the custom stylesheet
          //! tc_user_options_style filter is shared by several classes => must always check the local context inside the callback before appending new css
          //fired on hook : wp_enqueue_scripts
          add_filter( 'tc_user_options_style'       , array( $this , 'angi_fn_grid_write_inline_css'), 100 );
        }
        ...
        * hook : wp
        */
        function angi_fn_set_grid_hooks(){
          if ( ! apply_filters( 'tc_set_grid_hooks' , $this -> angi_fn_is_grid_enabled() ) )
              return;

          $this -> post_id = angi_fn_get_id();
          ...
          add_filter( 'tc_edit_in_title'            , array( $this, 'angi_fn_grid_disable_edit_in_title_expanded' ) );

          add_filter( 'tc_content_title_icon'       , '__return_false', 50 );
          //icon option
          add_filter( 'tc-grid-thumb-html'          , array( $this, 'angi_fn_set_grid_icon_visibility') );

          //pre loop hooks
          add_action( '__before_article_container'  , array( $this, 'angi_fn_set_grid_before_loop_hooks'), 5 );
          //loop hooks
          add_action( '__before_loop'               , array( $this, 'angi_fn_set_grid_loop_hooks'), 0 );
        }
        ...
        * before loop
        */
        function angi_fn_set_grid_before_loop_hooks(){
          // LAYOUT
          add_filter( 'tc_post_list_layout'         , array( $this, 'angi_fn_grid_set_content_layout') );
          add_filter( 'tc_post_list_selectors'      , array( $this, 'angi_fn_grid_set_article_selectors') );
          add_action( '__before_article_container'  , array( $this, 'angi_fn_grid_prepare_expand_sticky' ) );

          // THUMBNAILS
          remove_filter( 'post_class'               , array( ANGI_post_list::$instance , 'angi_fn_add_thumb_shape_name'));
          remove_filter( 'tc_thumb_size_name'       , array( ANGI_post_thumbnails::$instance, 'angi_fn_set_thumb_size') );
          add_filter( 'tc_thumb_size_name'          , array( $this, 'angi_fn_set_thumb_size_name') );
          add_filter( 'tc_thumb_size'               , array( $this, 'angi_fn_set_thumb_size') );

          // SINGLE POST CONTENT IN GRID
          $_content_priorities = apply_filters('tc_grid_post_content_priorities' , array( 'content' => 20, 'link' =>30 ));
          add_action( '__grid_single_post_content'  , array( $this, 'angi_fn_grid_display_figcaption_content') , $_content_priorities['content'] );
          add_action( '__grid_single_post_content'  , array( $this, 'angi_fn_grid_display_post_link'), $_content_priorities['link'] );
          add_action( '__grid_single_post_content'  , array( $this, 'angi_fn_grid_display_fade_excerpt'), 100 );
          //expanded sticky post : filter the figcaption content to include the post title
          add_filter( 'tc_grid_display_figcaption_content' , array( $this, 'angi_fn_grid_set_expanded_post_title') );

          //ARTICLE CONTAINER CSS CLASSES TO HANDLE EFFECT LIKE SHADOWS
          add_filter( 'tc_article_container_class'  , array( $this, 'angi_fn_grid_container_set_classes' ) );

          //COMMENT BUBBLE
          remove_filter( 'tc_the_title'             , array( ANGI_comments::$instance, 'angi_fn_display_comment_bubble' ) , 1 );
          add_filter( 'tc_grid_get_single_post_html'  , array( $this, 'angi_fn_grid_display_comment_bubble' ) );

          //POST METAS
          remove_filter( 'tc_meta_utility_text'     , array( ANGI_post_metas::$instance , 'angi_fn_add_link_to_post_after_metas'), 20 );

          //TITLE LENGTH
          add_filter( 'tc_title_text'               , array( $this, 'angi_fn_grid_set_title_length' ) );
        }
        ...
        * @return  void
        */
        function angi_fn_set_grid_loop_hooks() {
          add_action( '__before_article'            , array( $this, 'angi_fn_print_row_fluid_section_wrapper' ), 1 );
          add_action( '__after_article'             , array( $this, 'angi_fn_print_article_sep' ), 0 );
          add_action( '__after_article'             , array( $this, 'angi_fn_print_row_fluid_section_wrapper' ), 1 );

          remove_action( '__loop'                   , array( ANGI_post_list::$instance, 'angi_fn_prepare_section_view') );
          add_action( '__loop'                      , array( $this, 'angi_fn_grid_prepare_single_post') );

          if ( ANGI_headings::$instance -> angi_fn_is_edit_enabled() && apply_filters( 'tc_grid_render_expanded_edit_link', true ) )
            add_filter( 'tc_grid_get_single_post_html' , array( $this, 'angi_fn_grid_render_expanded_edit_link' ), 50 );
        }
        ...
        */
        function angi_fn_print_row_fluid_section_wrapper(){
          global $wp_query;
          $current_post   = $wp_query -> current_post;
          $start_post     = $this -> expanded_sticky ? 1 : 0;
          $cols           = $this -> angi_fn_get_grid_section_cols();
        ...
        * inside loop
        */
        function angi_fn_grid_prepare_single_post() {
          global $post;
          if ( ! isset($post) || empty($post) || ! apply_filters( 'tc_show_post_in_post_list', $this -> angi_fn_is_grid_context_matching() , $post ) )
            return;
          ...
          if ( apply_filters( 'tc_render_grid_headings_view' , ! $this -> angi_fn_force_current_post_expansion() ) ) {
              $hook_prefix = '__before';
              if ( $_layout['show_thumb_first'] )
                  $hook_prefix = '__after';

              add_action( "{$hook_prefix}_grid_single_post",  array( ANGI_headings::$instance, 'angi_fn_render_headings_view' ) );
          }

          // THUMBNAIL : cache the post format icon first
          //add thumbnail html (src, width, height) if any
          $_thumb_html = '';
          if ( $this -> angi_fn_grid_show_thumb() ) {
            //return an array( $tc_thumb(image object), $tc_thumb_width(string), $tc_thumb_height(string) )
            $_thumb_model = ANGI_post_thumbnails::$instance -> angi_fn_get_thumbnail_model();
          ...
          $_post_content_html               = $this -> angi_fn_grid_get_single_post_html( implode( ' ', $post_list_content_class ) );
          ...
          if ( ( '1' == $this -> angi_fn_get_grid_cols() || $this -> angi_fn_force_current_post_expansion() ) && ! wp_is_mobile() )
            array_push( $_classes, 'no-gold-ratio' );

          $_classes  = implode( ' ' , apply_filters('tc_single_grid_post_wrapper_class', $_classes ) );

          //RENDER VIEW
          $this -> angi_fn_grid_render_single_post( $_classes, $_thumb_html, $_post_content_html );
        ...
        *
        */
        private function angi_fn_grid_render_single_post( $_classes, $_thumb_html, $_post_content_html ) {
          ob_start();
            echo '<div class="grid__item">';
        ...
        */
        function angi_fn_grid_display_post_link(){
        ...
        * hook : __grid_single_post_content
        */
        function angi_fn_grid_display_fade_excerpt(){
          if ( ! apply_filters( 'tc_grid_fade_excerpt' , ! $this -> angi_fn_force_current_post_expansion() ) )
            return;
        ...
        * hook : __grid_single_post_content
        */
        function angi_fn_grid_display_figcaption_content() {
          ?>
        ...
        */
        function angi_fn_print_article_sep() {
          //renders the hr separator after each article
        ...
        * @return string
        */
        function angi_fn_grid_set_title_length( $_title ) {
          $_max = esc_attr( angi_fn_opt( 'tc_grid_num_words') );
        ...
        */
        function angi_fn_maybe_excl_first_sticky( $query ){
          if ( $this -> angi_fn_is_grid_enabled() && $this -> angi_fn_is_sticky_expanded( $query ) )
        ...
        */
        function angi_fn_grid_set_content_layout( $_layout ){
        ...
        */
        function angi_fn_set_grid_section_cols( $_col_nb, $_current_layout ) {
          $_map = apply_filters(
        ...
        */
        function angi_fn_grid_set_article_selectors($selectors){
          $_class = sprintf( '%1$stc-grid span%2$s',
            apply_filters( 'tc_grid_add_expanded_class', $this -> angi_fn_force_current_post_expansion() ) ? 'expanded ' : '',
            is_numeric( $this -> angi_fn_get_grid_section_cols() ) ? 12 / $this -> angi_fn_get_grid_section_cols() : 6
          );
        ...
        * hook : __before_article_container
        */
        function angi_fn_grid_prepare_expand_sticky(){
          global $wp_query;
          if ( ! ( $this -> angi_fn_is_sticky_expanded() &&
        ...
        */
        function angi_fn_set_thumb_size_name(){
          return ( $this -> angi_fn_get_grid_section_cols() == '1' ) ? 'tc-grid-full' : 'tc-grid';
        }
        ...
        * hook : tc_thumb_size
        */
        function angi_fn_set_thumb_size(){
          $thumb = ( $this -> angi_fn_get_grid_section_cols() == '1' ) ? 'tc_grid_full_size' : 'tc_grid_size';
          return ANGI___::$instance -> $thumb;
        }
        ...
        */
        function angi_fn_grid_container_set_classes( $_classes ) {
          array_push( $_classes, 'tc-post-list-grid' );
          if ( esc_attr( angi_fn_opt( 'tc_grid_shadow') ) )
            array_push( $_classes, 'tc-grid-shadow' );
          if ( esc_attr( angi_fn_opt( 'tc_grid_bottom_border') ) )
        ...
        * inside loop
        */
        private function angi_fn_grid_get_single_post_html( $post_list_content_class ) {
          global $post;
          ob_start();
            ?>
        ...
        * inside loop
        */
        function angi_fn_grid_display_comment_bubble( $_html ) {
          return ANGI_comments::$instance -> angi_fn_display_comment_bubble() . $_html;
        }
        ...
        */
        function angi_fn_grid_set_expanded_post_title( $_html ){
        ...
        */
        function angi_fn_grid_render_expanded_edit_link( $_html ) {
          if ( $this -> angi_fn_force_current_post_expansion() )
            $_html .= ANGI_headings::$instance -> angi_fn_render_edit_link_view( $_echo = false );
          return $_html;
        }
        ...
        */
        function angi_fn_grid_write_inline_css( $_css ){
          ...
          $_col_nb  = $this -> angi_fn_get_grid_cols();

          //GENERATE THE FIGURE HEIGHT CSS
          $_current_col_figure_css  = $this -> angi_fn_grid_get_figure_css( $_col_nb );

          //GENERATE THE MEDIA QUERY CSS FOR FONT-SIZES
          $_current_col_media_css   = $this -> angi_fn_get_grid_font_css( $_col_nb );

          $_grid_border_skinned_css = $this -> angi_fn_get_grid_boder_skinned_css();
        ...
        */
        function angi_fn_set_grid_icon_visibility( $_html ) {
          $_icon_enabled = (bool) esc_attr( angi_fn_opt( 'tc_grid_icons') );
          if ( angi_fn_is_customizing() )
        ...
        */
        private function angi_fn_get_grid_font_css( $_col_nb = '3' ) {
          $_media_queries     = $this -> angi_fn_get_grid_media_queries();//returns the simple array of media queries
          $_grid_font_sizes = $this -> angi_fn_get_grid_font_sizes( $_col_nb );//return the array of sizes (ordered by @media queries) for a given column layout
          $_col_rules         = array();
          $_media_queries_css = '';
          ...
          foreach ($_media_queries as $key => $_med_query_sizes ) {
            ...
            $_css_prop = array(
              'h' => $this -> angi_fn_grid_build_css_rules( $_size , 'h' ),
              'p' => $this -> angi_fn_grid_build_css_rules( $_size , 'p' )
            );
            $_rules = $this -> angi_fn_grid_assign_css_rules_to_selectors( $_med_query_sizes , $_css_prop, $_col_nb );
            $_media_queries_css .= "
        ...
        */
        private function angi_fn_get_grid_media_queries() {
        ...
        */
        private function angi_fn_get_grid_font_sizes( $_col_nb = '3', $_requested_media_size = null ) {
          ...
          if ( ! is_null($_requested_media_size) ) {
            $_media_queries = $this -> angi_fn_get_grid_media_queries();
          ...
          return apply_filters(
            ...
            $_col_nb,
            $_col_media_matrix,
            ANGI_utils::angi_fn_get_layout( $this -> post_id , 'class' )
          );
        }
        ...
        * @return array()
        */
        function angi_fn_set_layout_font_size( $_sizes, $_col_nb, $_col_media_matrix, $_current_layout ) {
        ...
        */
        private function angi_fn_get_grid_font_ratios( $_size = 'xl' , $_sel = 'h' ) {
        ...
        */
        private function angi_fn_grid_assign_css_rules_to_selectors( $_media_query, $_css_prop, $_col_nb ) {
          $_css = '';
          //Add one column font rules if there's a sticky post
          if ( $this -> angi_fn_is_sticky_expanded() || '1' == $_col_nb ) {
            $_size      = $this -> angi_fn_get_grid_font_sizes( $_col_nb = '1', $_media_query );//size like xxl
            $_h_one_col = $this -> angi_fn_grid_build_css_rules( $_size , 'h' );
            $_p_one_col = $this -> angi_fn_grid_build_css_rules( $_size , 'p' );
            $_css .= "
        ...
        * 2) user layout is one column
        */
        private function angi_fn_grid_get_figure_css( $_col_nb = '3' ) {
          $_height = $this -> angi_fn_get_grid_column_height( $_col_nb );
          $_cols_class      = sprintf( 'grid-cols-%s' , $_col_nb );
          $_css = '';
          //Add one column height if there's a sticky post
          if ( $this -> angi_fn_is_sticky_expanded() && '1' != $_col_nb ) {
            $_height_col_one = $this -> angi_fn_get_grid_column_height( '1' );
        ...
        */
        private function angi_fn_grid_build_css_rules( $_size = 'xl', $_wot = 'h' ) {
          $_lh_ratio = apply_filters( 'tc_grid_line_height_ratio' , 1.55 ); //line-height / font-size
          $_ratio = $this -> angi_fn_get_grid_font_ratios( $_size , $_wot );
          //body font size
          $_bs = esc_attr( angi_fn_opt( 'tc_body_font_size') );
        ...
        */
        private function angi_fn_get_grid_boder_skinned_css() {
          //add this reset rule for custom skin users
          $_reset_old_skinned_border = '.tc-grid-border .tc-grid { border-bottom: none }';
          $_skin_main_color          = ANGI_utils::$instance -> angi_fn_get_skin_color();
        ...
        *
        */
        private function angi_fn_get_grid_column_height( $_cols_nb = '3' ) {
          $_h               = $this -> angi_fn_grid_get_thumb_height();
          $_current_layout  = ANGI_utils::angi_fn_get_layout( $this -> post_id , 'sidebar' );
          ...
          foreach ( $_grid_col_height_map as $_c => $_heights ) {
            $_grid_col_height_map[$_c] = $this -> angi_fn_set_max_col_height ( $_heights ,$_h );
          }
        ...
        *
        */
        private function angi_fn_set_max_col_height( $_heights ,$_h ) {
          $_return = array();
        ...
        */
        private function angi_fn_grid_get_thumb_height() {
          $_opt = esc_attr( angi_fn_opt( 'tc_grid_thumb_height') );
        ...
        */
        private function angi_fn_is_sticky_expanded( $query = null ){
          ...
          $_expand_feat_post_opt = apply_filters( 'tc_grid_expand_featured', esc_attr( angi_fn_opt( 'tc_grid_expand_featured') ) );
        ...
        */
        private function angi_fn_force_current_post_expansion(){
          global $wp_query;
        ...
        * @return bool
        */
        public function angi_fn_is_grid_enabled() {
          return apply_filters( 'tc_is_grid_enabled', 'grid' == esc_attr( angi_fn_opt( 'tc_post_list_grid') ) && $this -> angi_fn_is_grid_context_matching() );
        }


        /* retrieves number of cols option, and wrap it into a filter */
        private function angi_fn_get_grid_cols() {
          return apply_filters( 'tc_get_grid_cols',
            esc_attr( angi_fn_opt( 'tc_grid_columns') ),
            ANGI_utils::angi_fn_get_layout( $this -> post_id , 'class' )
          );
        }
        ...
        private function angi_fn_get_grid_section_cols() {
          return apply_filters( 'tc_grid_section_cols',
            $this -> angi_fn_force_current_post_expansion() ? '1' : $this -> angi_fn_get_grid_cols()
          );
        }
        ...
        private function angi_fn_get_grid_context() {
          global $wp_query;
        ...
        private function angi_fn_is_grid_context_matching() {
          $_type = $this -> angi_fn_get_grid_context();
          $_apply_grid_to_post_type = apply_filters( 'tc_grid_in_' . $_type, esc_attr( angi_fn_opt( 'tc_grid_in_' . $_type ) ) );
        ...
        * @return  boolean
        */
        private function angi_fn_grid_show_thumb() {
          return ANGI_post_thumbnails::$instance -> angi_fn_has_thumb() && 0 != esc_attr( angi_fn_opt( 'tc_post_list_show_thumb' ) );
        }
  }//end of class
endif;
...
*
*/
if ( ! class_exists( 'ANGI_post_metas' ) ) :
    class ANGI_post_metas {
        static $instance;
        function __construct () {
          self::$instance =& $this;
          //Show / hide metas based on customizer user options (@since 3.2.0)
          add_action( 'template_redirect'                            , array( $this , 'angi_fn_set_visibility_options' ) , 10 );
           //Show / hide metas based on customizer user options (@since 3.2.0)
          add_action( 'template_redirect'                            , array( $this , 'angi_fn_set_design_options' ) , 20 );
          //Show / hide metas based on customizer user options (@since 3.2.0)
          add_action( '__after_content_title'         , array( $this , 'angi_fn_set_post_metas_hooks' ), 20 );
        ...
        */
        function angi_fn_set_visibility_options() {
          ...
          if ( 0 == esc_attr( angi_fn_opt( 'tc_show_post_metas' ) ) ) {
            if ( angi_fn_is_customizing() )
              add_filter( 'body_class' , array( $this , 'angi_fn_hide_all_post_metas') );
            else{
              add_filter( 'tc_show_post_metas' , '__return_false' );
              return;
            }
          }
          if ( is_singular() && ! is_page() && ! angi_fn__f('__is_home') ) {
              if ( 0 != esc_attr( angi_fn_opt( 'tc_show_post_metas_single_post' ) ) ) {
                  add_filter( 'tc_show_post_metas' , '__return_true' );
                  return;
              }

              if ( angi_fn_is_customizing() ) {
                  add_filter( 'body_class' , array( $this , 'angi_fn_hide_post_metas') );
                  add_filter( 'tc_show_post_metas' , '__return_true' );
              }
              else
                  add_filter( 'tc_show_post_metas' , '__return_false' );
          ...
          if ( ! is_singular() && ! angi_fn__f('__is_home') && ! is_page() ) {
              if ( 0 != esc_attr( angi_fn_opt( 'tc_show_post_metas_post_lists' ) ) ) {
                  add_filter( 'tc_show_post_metas' , '__return_true' );
                  return;
              }
          ...
          if ( angi_fn__f('__is_home') ) {
              if ( 0 != esc_attr( angi_fn_opt( 'tc_show_post_metas_home' ) ) ) {
                  add_filter( 'tc_show_post_metas' , '__return_true' );
                  return;
              }
              if ( angi_fn_is_customizing() ) {
        ...
        * DESIGN HOOK SETUP
        ***********************/
        function angi_fn_set_design_options() {
          if ( 'buttons' == esc_attr( angi_fn_opt( 'tc_post_metas_design' ) ) )
            return;

          add_filter( 'tc_meta_terms_glue'           , array( $this, 'angi_fn_set_term_meta_glue' ) );
          add_filter( 'tc_meta_tax_class'            , '__return_empty_array' );

          add_filter( 'tc_post_tax_metas_html'       , array( $this, 'angi_fn_set_tax_metas' ), 10, 2 );
          add_filter( 'tc_post_date_metas_html'      , array( $this, 'angi_fn_set_date_metas' ), 10, 2 );
          add_filter( 'tc_post_author_metas_html'    , array( $this, 'angi_fn_set_author_metas' ), 10 , 2 );
          add_filter( 'tc_set_metas_content'         , array( $this, 'angi_fn_set_metas' ), 10, 2 );
        }
        ...
        */
        function angi_fn_set_post_metas_hooks() {
          ...
          if ( 'attachment' == $post -> post_type ) {
            $_model = $this -> angi_fn_build_attachment_post_metas_model();
          } else {
            $_model = $this -> angi_fn_build_post_post_metas_model();
            //Set metas content based on customizer user options (@since 3.2.6)
            add_filter( 'tc_meta_utility_text'      , array( $this , 'angi_fn_set_post_metas_elements'), 10 , 2 );
            //filter metas content with default theme settings
            add_filter( 'tc_meta_utility_text'      , array( $this , 'angi_fn_add_link_to_post_after_metas'), 20 );
          }

          //RENDER VIEW
          $this -> angi_fn_render_metas_view( $_model );
        }
        ...
        */
        private function angi_fn_build_post_post_metas_model() {
          ...
          $tag_list   = $this -> angi_fn_meta_generate_tax_list( false );
          $pub_date   = $this -> angi_fn_get_meta_date( 'publication' );
          $auth       = $this -> angi_fn_get_meta_author();
          $upd_date   = $this -> angi_fn_get_meta_date( 'update' );
          ...
          */
          $_html      = wp_sprintf( __( 'This entry was posted on %1$s<span class="by-author"> by %2$s</span>.' , 'angilla' ),
              $pub_date,
              $auth
          );
        ...
        */
        private function angi_fn_build_attachment_post_metas_model() {
          ...
          $_html = sprintf( '%1$s <span class="entry-date"><time class="entry-date updated" datetime="%2$s">%3$s</time></span> %4$s %5$s',
              '<span class="meta-prep meta-prep-entry-date">'.__('Published' , 'angilla').'</span>',
              apply_filters('tc_use_the_post_modified_date' , false ) ? esc_attr( get_the_date( 'c' ) ) : esc_attr( get_the_modified_date('c') ),
              esc_html( get_the_date() ),
              ( isset($metadata['width']) && isset($metadata['height']) ) ? __('at dimensions' , 'angilla').'<a href="'.esc_url( wp_get_attachment_url() ).'" title="'.__('Link to full-size image' , 'angilla').'"> '.$metadata['width'].' &times; '.$metadata['height'].'</a>' : '',
              __('in' , 'angilla').'<a href="'.esc_url( get_permalink( $post->post_parent ) ).'" title="'.__('Return to ' , 'angilla').esc_attr( strip_tags( get_the_title( $post->post_parent ) ) ).'" rel="gallery"> '.get_the_title( $post->post_parent ).'</a>.'
          );
        ...
        */
        function angi_fn_set_post_metas_elements( $_default , $_args = array() ) {
            ...
            $_show_tags         = 0 != esc_attr( angi_fn_opt( 'tc_show_post_metas_tags' ) ) && false != $this -> angi_fn_meta_generate_tax_list( false );
            $_show_pub_date     = 0 != esc_attr( angi_fn_opt( 'tc_show_post_metas_publication_date' ) );
            $_show_upd_date     = 0 != esc_attr( angi_fn_opt( 'tc_show_post_metas_update_date' ) ) && false !== angi_fn_post_has_update();
            $_show_upd_in_days  = 'days' == esc_attr( angi_fn_opt( 'tc_post_metas_update_date_format' ) );
            $_show_date         = $_show_pub_date || $_show_upd_date;
            $_show_author       = 0 != esc_attr( angi_fn_opt( 'tc_show_post_metas_author' ) );
            ...
            if ( $_show_cats && $_show_tags )
                $_tax_text   .= __( 'This entry was posted in %1$s and tagged %2$s' , 'angilla' );
            if ( $_show_cats && ! $_show_tags )
                $_tax_text   .= __( 'This entry was posted in %1$s' , 'angilla' );
            if ( ! $_show_cats && $_show_tags )
                $_tax_text   .= __( 'This entry was tagged %2$s' , 'angilla' );
            ...
            $_date_text = '';
            if ( $_show_pub_date ) {
              $_date_text        = empty($_tax_text) ? __( 'This entry was posted on %1$s' , 'angilla' ) : $_date_text;
              if ( $_show_cats )
                $_date_text   .= __( 'on %1$s' , 'angilla' );
              if ( ! $_show_cats && $_show_tags )
                $_date_text   .= __( 'and posted on %1$s' , 'angilla' );
            ...
            if ( $_show_author ) {
              if ( empty($_tax_text) && empty($_date_text) ) {
                  $_author_text = sprintf( '%s <span class="by-author">%s</span>' , __( 'This entry was posted', 'angilla' ), __('by %1$s' , 'angilla') );
              } else {
                  $_author_text = sprintf( '<span class="by-author">%s</span>' , __('by %1$s' , 'angilla') );
              }
            ...
            if ( $_show_upd_date ) {
              if ( $_show_upd_in_days ) {
                $_update_days = angi_fn_post_has_update();
                $_update_text = ( 0 == $_update_days ) ? __( '(updated today)' , 'angilla' ) : sprintf( __( '(updated %s days ago)' , 'angilla' ), $_update_days );
                $_update_text = ( 1 == $_update_days ) ? __( '(updated 1 day ago)' , 'angilla' ) : $_update_text;
              }
              else {
                $_update_text = __( '(updated on %1$s)' , 'angilla' );
              }
        ...
        */
        public function angi_fn_meta_generate_tax_list( $hierarchical ) {
          ...
          $_terms_html_array  = array_map( array( $this , 'angi_fn_meta_term_view' ), $post_terms );
        ...
        */
        private function angi_fn_meta_term_view( $term ) {
          ...
          // (Rocco's PR Comment) : following to this https://wordpress.org/support/topic/empty-articles-when-upgrading-to-angilla-version-332
          ...
          return apply_filters( 'tc_meta_term_view' , sprintf($_to_return,
              $_classes,
              $_term_link,
              esc_attr( sprintf( __( "View all posts in %s", 'angilla' ), $term -> name ) ),
              $term -> name
            )
          );
        }
        ...
        */
        public function angi_fn_get_term_of_tax_type( $hierarchical = true ) {
          //var declaration
          $post_type              = get_post_type( angi_fn_get_id() );
          ...
          while ( $_tax_object = current($tax_list) ) {
            ...
            if ( ! $this -> angi_fn_is_tax_authorized( $_tax_object, $post_type ) ) {
              next($tax_list);
              continue;
            }
          ...
          foreach ($_tax_type_list as $tax_name => $data ) {
              $_current_tax_terms = get_the_terms( angi_fn_get_id() , $tax_name );
        ...
        */
        public function angi_fn_is_tax_authorized( $_tax_object , $post_type ) {
          $_in_exclude_list = in_array(
            $_tax_object['name'],
            apply_filters_ref_array ( 'tc_exclude_taxonomies_from_metas' , array( array('post_format') , $post_type , angi_fn_get_id() ) )
          );

          $_is_private = false === (bool) $_tax_object['public'] && apply_filters_ref_array( 'tc_exclude_private_taxonomies', array( true, $_tax_object['public'], angi_fn_get_id() ) );
        ...
        private function angi_fn_get_meta_author() {
            ...
            foreach ( $author_id_array as $author_id ) {
                ...
                $_html      .= sprintf( '<span class="author vcard author_name"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span>' ,
                      esc_url( get_author_posts_url( $author_id ) ),
                      esc_attr( sprintf( __( 'View all posts by %s' , 'angilla' ), $author_name ) ),
                      $author_name
                );
            }
        ...
        */
        function angi_fn_add_link_to_post_after_metas( $_metas_html ) {
          ...
          if ( apply_filters( 'tc_show_link_after_post_metas' , true )
            && in_array( get_post_format(), apply_filters( 'tc_post_formats_with_no_heading', ANGI_init::$instance -> post_formats_with_no_heading ) )
            && ! is_singular() ) {
            return apply_filters('tc_add_link_to_post_after_metas',
              sprintf('%1$s | <a href="%2$s" title="%3$s">%3$s &raquo;</a>', $_metas_html, get_permalink(), __('Open' , 'angilla') )
            );
          }
          return $_metas_html;
        }
        ...
        * @return  string
        */
        public function angi_fn_set_term_meta_glue() {
          return ' / ';
        }
        ...
        * @return  string
        */
        function angi_fn_set_tax_metas( $_html , $_tax = array() ) {
          if ( empty($_tax) )
            return $_html;
          //extract "_show_cats" , "_show_tags" , "cat_list", "tag_list"
          extract($_tax);
          $cat_list = ! empty($cat_list) && $_show_cats ? sprintf( '&nbsp;%s %s' , __('in' , 'angilla') , $cat_list ) : '';
          $tag_list = ! empty($tag_list) && $_show_tags ? sprintf( '&nbsp;%s %s' , __('tagged' , 'angilla') , $tag_list ) : '';
        ...
        * @return  string
        */
        function angi_fn_set_date_metas( $_html, $_pubdate = '' ) {
          if ( empty($_pubdate))
            return $_html;
          return ANGI_post_metas::$instance -> angi_fn_get_meta_date( 'publication' , 'short' );
        }
        ...
        * @return  string
        */
        function angi_fn_set_author_metas( $_html , $_auth = '' ) {
          if ( empty($_auth) )
            return $_html;

          return sprintf( '<span class="by-author"> %s %s</span>' , __('by' , 'angilla'), $_auth );
        }
        ...
        * @return  string
        */
        function angi_fn_set_metas( $_html, $_parts = array() ) {
          if ( empty($_parts) )
            return $_html;
...
*
*/
if ( ! class_exists( 'ANGI_post_navigation' ) ) :
  class ANGI_post_navigation {
      static  $instance;

      function __construct () {
        self::$instance =& $this;

        add_action ( '__after_loop'             , array( $this , 'angi_fn_post_nav' ), 20 );
      ...
      */
      function angi_fn_set_visibility_options(){
        ...
        $_context                  = $this -> angi_fn_get_context();
        $_post_nav_enabled         = $this -> angi_fn_is_post_navigation_enabled();
        $_post_nav_context_enabled = $this -> angi_fn_is_post_navigation_context_enabled( $_context );

        $_is_customizing           = angi_fn_is_customizing() ;
      ...
      function angi_fn_post_nav() {
        ...
        list( $post_navigation_bool, $post_nav_class, $_context) = $this -> angi_fn_set_visibility_options();
        ...
        <nav id="<?php echo $html_id; ?>" class="<?php echo $post_nav_class; ?>" role="navigation">
              ...
              <h3 class="assistive-text">
                <?php echo apply_filters( 'tc_singular_nav_title', __( 'Post navigation' , 'angilla' ) ) ; ?>
              </h3>
              ...
              <ul class="pager">
                <?php if ( get_previous_post() != null ) : ?>
                  <li class="previous">
                    <span class="nav-previous">
                      <?php
                        $singular_nav_previous_text   = apply_filters( 'tc_singular_nav_previous_text', call_user_func( '_x',  $prev_arrow , 'Previous post link' , 'angilla' ) );
                ...
                <?php if ( get_next_post() != null ) : ?>
                  <li class="next">
                    <span class="nav-next">
                        <?php
                        $singular_nav_next_text       = apply_filters( 'tc_singular_nav_next_text', call_user_func( '_x', $next_arrow , 'Next post link' , 'angilla' ) );
        ...
        <?php elseif ( $wp_query->max_num_pages > 1 && in_array($_context, array('archive', 'home') ) ) : ?>
          ...
          <nav id="<?php echo $html_id; ?>" class="<?php echo $post_nav_class; ?>" role="navigation">
            ...
            <h3 class="assistive-text">
              <?php echo apply_filters( 'tc_list_nav_title', __( 'Post navigation' , 'angilla' ) ) ; ?>
            </h3>
                    ...
                    <span class="nav-previous">
                      <?php
                        $next_posts_link_args      = apply_filters(
                          ...
                          array(
                            'label'        => apply_filters( 'tc_list_nav_next_text', __( '<span class="meta-nav">&larr;</span> Older posts' , 'angilla' ) ),
                            'max_pages'    => 0
                          )
                        );
                    ...
                    <span class="nav-next">
                      <?php
                        $previous_posts_link_args      = apply_filters(
                          ...
                          array(
                            'label'        => apply_filters( 'tc_list_nav_previous_text', __( 'Newer posts <span class="meta-nav">&rarr;</span>' , 'angilla' ) ),
                            'max_pages'    => 0
                          )
                        );
      ...
      */
      function angi_fn_get_context(){
        ...
        if ( is_home() && 'posts' == get_option('show_on_front') )
          return 'home';
        if ( !is_404() && !angi_fn__f( '__is_home_empty') )
          return 'archive';
      ...
      * @return bool
      */
      function angi_fn_is_post_navigation_context_enabled( $_context ) {
      ...
      * @return bool
      */
      function angi_fn_is_post_navigation_enabled(){
        return 1 == esc_attr( angi_fn_opt( 'tc_show_post_navigation' ) ) ;
      }
...
*
*/
if ( ! class_exists( 'ANGI_post_thumbnails' ) ) :
class ANGI_post_thumbnails {
    static $instance;
    function __construct () {
      self::$instance =& $this;
      //may be filter the thumbnail inline style
      add_filter( 'tc_post_thumb_inline_style'  , array( $this , 'angi_fn_change_thumb_inline_css' ), 10, 3 );
    }
    ...
    * @since Angilla 1.0
    */
    function angi_fn_get_thumbnail_model( $requested_size = null, $_post_id = null , $_custom_thumb_id = null, $_enable_wp_responsive_imgs = null ) {
      if ( ! $this -> angi_fn_has_thumb( $_post_id, $_custom_thumb_id ) )
        return array();
      ...
      $_filtered_thumb_size       = apply_filters( 'tc_thumb_size' , ANGI___::$instance -> tc_thumb_size );
      $_model                     = array();
      $_img_attr                  = array();
      $tc_thumb_height            = '';
      $tc_thumb_width             = '';
      ...
      if ( version_compare( $GLOBALS['wp_version'], '4.4', '>=' ) )
        $_enable_wp_responsive_imgs = is_null( $_enable_wp_responsive_imgs ) ? 1 == angi_fn_opt('tc_resp_thumbs_img') : $_enable_wp_responsive_imgs;

      //try to extract $_thumb_id and $_thumb_type
      extract( $this -> angi_fn_maybe_set_and_get_thumb_info( $_post_id, $_custom_thumb_id ) );
      ...
      if ( false === $_enable_wp_responsive_imgs ) {
        ...
        $_img_attr['srcset']  = " ";
        add_filter( 'wp_get_attachment_image_attributes', array( $this, 'angi_fn_remove_srcset_attr' ) );
      }
      //get the thumb html
    ...
    */
    private function angi_fn_maybe_set_and_get_thumb_info( $_post_id = null, $_thumb_id = null ) {
      $_post_id     = is_null($_post_id) ? get_the_ID() : $_post_id;
      ...
      return $this -> angi_fn_set_thumb_info( $_post_id , $_thumb_id, true );
    }
    ...
    * @return bool
    */
    public function angi_fn_has_thumb( $_post_id = null , $_thumb_id = null ) {
      $_post_id  = is_null($_post_id) ? get_the_ID() : $_post_id;
      //try to extract (OVERWRITE) $_thumb_id and $_thumb_type
      extract( $this -> angi_fn_maybe_set_and_get_thumb_info( $_post_id, $_thumb_id ) );
    ...
    */
    public function angi_fn_set_thumb_info( $post_id = null , $_thumb_id = null, $_return = false ) {
      ...
      //3) default thumb
        ...
        if ( has_post_thumbnail( $post_id ) ) {
          ...
          $_thumb_id    = $this -> angi_fn_get_id_from_attachment( $post_id );
          $_thumb_type  = false !== $_thumb_id ? 'attachment' : $_thumb_type;
        }
        if ( ! $_thumb_id || empty( $_thumb_id ) ) {
          $_thumb_id    = esc_attr( angi_fn_opt( 'tc_post_list_default_thumb' ) );
    ...
    private function angi_fn_get_id_from_attachment( $post_id ) {
      ...
      $_bool = 0 != esc_attr( angi_fn_opt( 'tc_post_list_use_attachment_as_thumb' ) );
      if ( ! is_admin() ) {
        $_bool = !ANGI_post::$instance -> angi_fn_single_post_display_controller() && $_bool;
        $_bool = !ANGI_page::$instance -> angi_fn_page_display_controller() && $_bool;
      }
    ...
    */
    function angi_fn_render_thumb_view( $_thumb_model , $layout = 'span3', $_echo = true ) {
      ...
      $thumb_img        = apply_filters( 'tc_post_thumb_img', $thumb_img, angi_fn_get_id() );
      if ( ! $thumb_img )
        return;

      //handles the case when the image dimensions are too small
      $thumb_size       = apply_filters( 'tc_thumb_size' , ANGI___::$instance -> tc_thumb_size, angi_fn_get_id()  );
      $no_effect_class  = ( isset($tc_thumb) && isset($tc_thumb_height) && ( $tc_thumb_height < $thumb_size['height']) ) ? 'no-effect' : '';
      $no_effect_class  = ( esc_attr( angi_fn_opt( 'tc_center_img') ) || ! isset($tc_thumb) || empty($tc_thumb_height) || empty($tc_thumb_width) ) ? '' : $no_effect_class;
      ...
      $thumb_wrapper    = apply_filters_ref_array( 'tc_post_thumb_wrapper', array( $thumb_wrapper, $thumb_img, angi_fn_get_id() ) );
...
/**
* Sidebar actions
* The default widgets areas are defined as properties of the ANGI_utils class in class-fire-utils.php
* ANGI_utils::$inst -> sidebar_widgets for left and right sidebars
* ANGI_utils::$inst -> footer_widgets for the footer
...
*/
if ( ! class_exists( 'ANGI_sidebar' ) ) :
  class ANGI_sidebar {
      static $instance;
      function __construct () {
        self::$instance =& $this;
        add_action ( 'wp'       , array( $this , 'angi_fn_set_sidebar_hooks' ) );
      }
      ...
      */
      function angi_fn_set_sidebar_hooks() {
        //displays left sidebar
    		add_action ( '__before_article_container'  , array( $this , 'angi_fn_render_sidebar' ) );
    		add_action ( '__before_left_sidebar'       , array( $this , 'angi_fn_social_in_sidebar' ) );

        //displays right sidebar
    		add_action ( '__after_article_container'   , array( $this , 'angi_fn_render_sidebar' ) );
    		add_action ( '__before_right_sidebar'      , array( $this , 'angi_fn_social_in_sidebar' ) );

        //since 3.2.0 show/hide the WP built-in widget icons
        add_filter ( 'tc_left_sidebar_class'       , array( $this , 'angi_fn_set_sidebar_wrapper_widget_class' ) );
        add_filter ( 'tc_right_sidebar_class'      , array( $this , 'angi_fn_set_sidebar_wrapper_widget_class' ) );
      }
      ...
      * @since Angilla 1.0
      */
      function angi_fn_render_sidebar() {
        //first check if home and no content option is choosen
        if ( angi_fn__f( '__is_home_empty') )
          return;
        //gets current screen layout
        $screen_layout        = ANGI_utils::angi_fn_get_layout( angi_fn_get_id() , 'sidebar'  );
      ...
      * @since Angilla 1.0
      */
      function angi_fn_social_in_sidebar() {
        ...
        //1) if customizing: must be enabled
        //2) if not customizing : must be enabled and have social networks.
        $_nothing_to_render         = 0 == esc_attr( angi_fn_opt( $option ) );

        $_nothing_to_render_front   = $_nothing_to_render || ! ( $_socials = angi_fn__f( '__get_socials' ) ) ? true : $_nothing_to_render;

        //only when partial refresh enabled, otherwise we fall back on refresh
        $_nothing_to_render         = angi_fn_is_customizing() && angi_fn_is_partial_refreshed_on() ? $_nothing_to_render : $_nothing_to_render_front;

        if ( $_nothing_to_render )
          return;

        $_title = esc_attr( angi_fn_opt( 'tc_social_in_sidebar_title') );
      ...
      */
      function angi_fn_set_sidebar_wrapper_widget_class($_original_classes) {
        ...
        if ( 1 == esc_attr( angi_fn_opt('tc_show_sidebar_widget_icon' ) ) )
          return ( 0 == esc_attr( angi_fn_opt('tc_show_title_icon' ) ) ) ? $_no_icons_classes : $_original_classes;
         //last condition
...
class ANGI_slider {
  ...
  function __construct () {
    self::$instance =& $this;
    add_action( 'tc_set_slider_hooks_done' , array( $this, 'angi_fn_maybe_setup_parallax' ) );
    add_action( 'template_redirect'        , array( $this, 'angi_fn_set_slider_hooks' ) );
    //set user customizer options. @since v3.2.0
    add_filter( 'tc_slider_layout_class'   , array( $this , 'angi_fn_set_slider_wrapper_class' ) );
    //! tc_user_options_style filter is shared by several classes => must always check the local context inside the callback before appending new css
    //fired on hook : wp_enqueue_scripts
    //Set thumbnail specific design based on user options
    //Set user defined height
    add_filter( 'tc_user_options_style'    , array( $this , 'angi_fn_write_slider_inline_css' ) );
    //tc_slider_height is fired in ANGI_slider::angi_fn_write_slider_inline_css()
    add_filter( 'tc_slider_height'         , array( $this, 'angi_fn_set_demo_slider_height') );
  }//end of construct
  ...
  * @return  void
  */
  function angi_fn_set_slider_hooks() {
    //get slides model
    //extract $slider_name_id, $slides, $layout_class, $img_size
    extract( $this -> angi_fn_get_slider_model() );
    //returns nothing if no slides to display
    if ( ! isset($slides) || ! $slides )
      return;

    add_action( '__after_header'            , array( $this , 'angi_fn_slider_display' ) );
    add_action( '__after_carousel_inner'    , array( $this , 'angi_fn_slider_control_view' ) );

    //adds the center-slides-enabled css class
    add_filter( 'tc_carousel_inner_classes' , array( $this, 'angi_fn_set_inner_class') );

    //wrap the slide into a link
    add_filter( 'tc_slide_background'       , array( $this, 'angi_fn_link_whole_slide'), 5, 5 );

    //display an edit deep link to the Slider section in the Customize or post/page
    add_action( '__after_carousel_inner'    , array( $this, 'angi_fn_render_slider_edit_link_view'), 10, 2 );
  ...
  */
  private function angi_fn_get_single_slide_model( $slider_name_id, $_loop_index , $id , $img_size ) {
    ...
    $button_text            = angi_fn_text_truncate( $button_text, $default_button_length, '...' );
    ...
    if ( version_compare( $GLOBALS['wp_version'], '4.4', '>=' ) )
      if ( 0 == esc_attr( angi_fn_opt('tc_resp_slider_img') ) ) {
        ...
        //to avoid this we filter the attributes getting rid of the srcset if any.
        //Basically this trick, even if ugly, will avoid the srcset attr computation
        add_filter( 'wp_get_attachment_image_attributes', array( ANGI_post_thumbnails::$instance, 'angi_fn_remove_srcset_attr' ) );
      }
    $slide_background       = wp_get_attachment_image( $id, $img_size, false, $slide_background_attr );

    if ( angi_fn_is_checked( 'tc_slider_img_smart_load' ) ) {
        $slide_background = angi_fn_parse_imgs( $slide_background ); //<- to prepare the img smartload
    }
  ...
  */
  function angi_fn_get_single_post_slide_pre_model( $_post , $img_size, $args ){
      ...
      //attachment image
      $thumb                  = ANGI_post_thumbnails::$instance -> angi_fn_get_thumbnail_model(
      ...
      //title
      $title                  = ( isset( $args['show_title'] ) && $args['show_title'] ) ? $this -> angi_fn_get_post_slide_title( $_post, $ID) : '';

      //lead text
      $text                   = ( isset( $args['show_excerpt'] ) && $args['show_excerpt'] ) ? $this -> angi_fn_get_post_slide_excerpt( $_post, $ID) : '';
  ...
  */
  private function angi_fn_get_the_slides( $slider_name_id, $img_size ) {
    ...
    if ( 'demo' == $slider_name_id )
      return apply_filters( 'tc_default_slides', ANGI___::$instance -> default_slides );
    else if ( 'tc_posts_slider' == $slider_name_id ) {
      return $this -> angi_fn_get_the_posts_slides( $slider_name_id, $img_size );
    }
    //if not demo or tc_posts_slider, we get slides from options
    $all_sliders    = angi_fn_opt( 'tc_sliders');
    ...
    if ( ! $this -> angi_fn_slider_exists( $saved_slides) )
      return;
    ...
    foreach ( $saved_slides as $s ) {
      ...
      $slide_model = $this -> angi_fn_get_single_slide_model( $slider_name_id, $_loop_index, $id, $img_size);
  ...
  */
  private function angi_fn_get_the_posts_slides( $slider_name_id, $img_size ) {


    $pre_slides      = $this -> angi_fn_get_pre_posts_slides( array( 'img_size' => $img_size ) );
    ...
    if ( ! $this -> angi_fn_slider_exists( $pre_slides ) )
      return false;
    ...
    foreach ( $posts as $_post_slide ) {
      $slide_model = $this -> angi_fn_get_single_post_slide_model( $slider_name_id, $_loop_index, $_post_slide, $common, $img_size);
  ...
  */
  private function angi_fn_get_pre_posts_slides( $args ){
    ...
    $defaults       = array(
      'img_size'            => null,
      //options
      'stickies_only'       => esc_attr( angi_fn_opt( 'tc_posts_slider_stickies' ) ),
      'show_title'          => esc_attr( angi_fn_opt( 'tc_posts_slider_title' ) ),
      'show_excerpt'        => esc_attr( angi_fn_opt( 'tc_posts_slider_text' ) ),
      'button_text'         => esc_attr( angi_fn_opt( 'tc_posts_slider_button_text' ) ),
      'posts_per_page'      => esc_attr( angi_fn_opt( 'tc_posts_slider_number' ) ), //limit
      'link_type'           => esc_attr( angi_fn_opt( 'tc_posts_slider_link') ),
    );
    ...
    $queried_posts    = $this -> angi_fn_query_posts_slider( $args );
    ...
    $smart_load_enabled = 1 == esc_attr( angi_fn_opt( 'tc_img_smart_load' ) );
    if ( $smart_load_enabled )
      remove_filter( 'tc_thumb_html', 'angi_fn_parse_imgs' );
    ...
    //allow responsive images?
    if ( version_compare( $GLOBALS['wp_version'], '4.4', '>=' ) )
      $args['slider_responsive_images'] = 0 == esc_attr( angi_fn_opt('tc_resp_slider_img') ) ? false : true ;

    /* Get the pre_model */
    $pre_slides = $pre_slides_posts = array();

    foreach ( $queried_posts as $_post ) {
      $pre_slide_model = $this ->  angi_fn_get_single_post_slide_pre_model( $_post , $img_size, $args );
    ...
    if ( $smart_load_enabled )
      add_filter('tc_thumb_html', 'angi_fn_parse_imgs' );
    ...
    if ( ! empty( $pre_slides_posts ) ) {
      ...
      if ( strstr($link_type, 'cta') )
        $button_text            = $this -> angi_fn_get_post_slide_button_text( $button_text );
      else
        $button_text            = '';
  ...
  */
  private function angi_fn_get_slider_model() {
    ...
    if ( ! $this -> angi_fn_is_slider_possible() )
      return array();

    //gets the actual page id if we are displaying the posts page
    $queried_id                   = $this -> angi_fn_get_real_id();

    $slider_name_id               = $this -> angi_fn_get_current_slider( $queried_id );

    if ( ! $this -> angi_fn_is_slider_active( $queried_id) )
      return array();
    ...
    $layout_value                 = angi_fn__f('__is_home') ? angi_fn_opt( 'tc_slider_width' ) : esc_attr(get_post_meta( $queried_id, $key = 'slider_layout_key' , $single = true ));
    $layout_value                 = apply_filters( 'tc_slider_layout', $layout_value, $queried_id );

    //declares the layout vars
    $layout_class                 = ( 0 == $layout_value ) ? array('container', 'carousel', 'angilla-slide', $slider_name_id ) : array('carousel', 'angilla-slide', $slider_name_id);
    $img_size                     = apply_filters( 'tc_slider_img_size' , ( 0 == $layout_value ) ? 'slider' : 'slider-full');

    //get slides
    $slides                       = $this -> angi_fn_get_the_slides( $slider_name_id , $img_size );
  ...
  *
  */
  function angi_fn_slider_display() {
    //get slides model
    //extract $slider_name_id, $slides, $layout_class, $img_size
    extract( $this -> angi_fn_get_slider_model() );
    //returns nothing if no slides to display
    ...
    ob_start();
    ?>
    <div id="angilla-slider-<?php echo self::$rendered_sliders ?>" class="<?php echo $_layout_classes ?>">

      <?php $this -> angi_fn_render_slider_loader_view( $slider_name_id ); ?>
      ...
      <div class="<?php echo $_inner_classes?>" <?php echo $_inner_attributes ?>>
        <?php
          foreach ($slides as $id => $data) {
            ...
            $this -> angi_fn_render_single_slide_view( $_view_model );
          }
        ?>
      </div><!-- /.carousel-inner -->
  ...
  */
  function angi_fn_render_single_slide_view( $_view_model ) {
    ...
    $slide_classes = implode( ' ', apply_filters( 'tc_single_slide_item_classes', array( 'angi-item', $data['active'], "slide-{$id}" ) ) );
    ?>
    <div class="<?php echo $slide_classes; ?>">
      <?php
        $this -> angi_fn_render_slide_background_view( $_view_model );
        $this -> angi_fn_render_slide_caption_view( $_view_model );
        $this -> angi_fn_render_slide_edit_link_view( $_view_model );
      ?>
    </div><!-- /.angi-item -->
    <?php
  }
  ...
  */
  function angi_fn_render_slider_loader_view( $slider_name_id ) {
    if ( ! $this -> angi_fn_is_slider_loader_active( $slider_name_id ) )
      return;
  ...
  */
  ...
  function angi_fn_render_slider_edit_link_view( $slides, $slider_name_id ) {
    //never display when customizing
    if ( angi_fn_is_customizing() )
      return;
    if ( 'demo' == $slider_name_id )
    ...
    if ( angi_fn__f('__is_home') ){
      ...
      $_edit_link            = angi_fn_get_customizer_url( array( 'control' => 'tc_front_slider', 'section' => 'frontpage_sec') );
    ...
    $_text                   = sprintf( __( 'Customize or remove %s' , 'angilla' ),
                                  ( 'tc_posts_slider' == $slider_name_id ) ?__('the posts slider', 'angilla') : __('this slider', 'angilla' )
                              );
  ...
  *
  */
  function angi_fn_slider_control_view( $_slides ) {
    if ( count( $_slides ) <= 1 )
      return;
    ...
    $_html .= sprintf('<div class="tc-slider-controls %1$s">%2$s</div>',
      ! is_rtl() ? 'left' : 'right',
      sprintf('<a class="tc-carousel-control" href="#angilla-slider-%2$s" data-slide="prev">%1$s</a>',
    ...
    $_html .= sprintf('<div class="tc-slider-controls %1$s">%2$s</div>',
      ! is_rtl() ? 'right' : 'left',
      sprintf('<a class="tc-carousel-control" href="#angilla-slider-%2$s" data-slide="next">%1$s</a>',
  ...
  *******************************/
  ...
  function angi_fn_maybe_setup_parallax() {
    if ( 1 != esc_attr( angi_fn_opt( 'tc_slider_parallax') ) )
      return;

    add_filter( 'tc_slider_layout_class'       , array( $this, 'angi_fn_add_parallax_wrapper_class' ) );
    add_filter( 'tc_carousel_inner_classes'    , array( $this, 'angi_fn_add_parallax_item_class' ) );
    add_filter( 'tc_carousel_inner_attributes' , array( $this, 'angi_fn_add_parallax_item_data_attributes' ) );
  }

  //hook : tc_carousel_inner_classes
  function angi_fn_add_parallax_item_class ( $classes ) {
    array_push($classes, 'angi-parallax-slider' );
    return $classes;
  }

  //hook : tc_slider_layout_class
  function angi_fn_add_parallax_wrapper_class( $classes ) {
    array_push($classes, 'parallax-wrapper' );
    return $classes;
  }


  function angi_fn_add_parallax_item_data_attributes ( $attributes ) {
  ...
  */
  private function angi_fn_is_slider_possible() {
    ...
    $tc_front_slider              = esc_attr(angi_fn_opt( 'tc_front_slider' ) );
    //when do we display a slider? By default only for home (if a slider is defined), pages and posts (including custom post types)
    $_show_slider = angi_fn__f('__is_home') ? ! empty( $tc_front_slider ) : ! is_404() && ! is_archive() && ! is_search();
  ...
  *
  */
  private function angi_fn_get_current_slider($queried_id) {
    //gets the current slider id
    $_home_slider     = angi_fn_opt( 'tc_front_slider' );
    $slider_name_id   = ( angi_fn__f('__is_home') && $_home_slider ) ? $_home_slider : esc_attr( get_post_meta( $queried_id, $key = 'post_slider_key' , $single = true ) );
  ...
  *
  */
  private function angi_fn_get_real_id() {
    global $wp_query;
    $queried_id                   = angi_fn_get_id();

    return apply_filters( 'tc_slider_get_real_id', ( ! angi_fn__f('__is_home') && ! empty($queried_id) ) ?  $queried_id : get_the_ID() );
  }
  ...
  *
  */
  private function angi_fn_is_slider_active( $queried_id ) {
    //is the slider set to on for the queried id?
    if ( angi_fn__f('__is_home') && angi_fn_opt( 'tc_front_slider' ) )
  ...
  *
  */
  private function angi_fn_is_slider_loader_active( $slider_name_id ) {
    ...
    return ( 'demo' == $slider_name_id
        || apply_filters( 'tc_display_slider_loader', 1 == esc_attr( angi_fn_opt( 'tc_display_slide_loader') ), $slider_name_id )
    );
  }
  ...
  */
  function angi_fn_set_demo_slider_height( $_h ) {
    ...
    if ( 'demo' != $this -> angi_fn_get_current_slider( $this -> angi_fn_get_real_id() ) )
      return $_h;

    //2) height option has not been changed by user yet
    //the possible customization context must be taken into account here
    if ( angi_fn_is_customizing() ) {
      if ( 500 != esc_attr( angi_fn_opt( 'tc_slider_default_height') ) )
        return $_h;
    } else {
      if ( false !== (bool) esc_attr( angi_fn_opt( 'tc_slider_default_height', ANGI___::$tc_option_group, $use_default = false ) ) )
        return $_h;
    }
  ...
  */
  function angi_fn_write_slider_inline_css( $_css ) {
    ...
    if ( $this -> angi_fn_is_slider_loader_active( $this -> angi_fn_get_current_slider( $this -> angi_fn_get_real_id() ) ) ) {
      ...
      if ( ! apply_filters( 'tc_slider_loader_gif_only', false ) ) {
        ...
        $_current_skin_colors      = ANGI_utils::$inst -> angi_fn_get_skin_color( 'pair' );
    ...
    $_custom_height     = apply_filters( 'tc_slider_height' , esc_attr( angi_fn_opt( 'tc_slider_default_height') ) );
    $_slider_inline_css = "";

    //When shall we append custom slider style to the global custom inline stylesheet?
    $_bool = 500 != $_custom_height;
    $_bool = $_bool && ( angi_fn__f('__is_home') || 0 != esc_attr( angi_fn_opt( 'tc_slider_default_height_apply_all') ) );
  ...
  */
  function angi_fn_set_slider_wrapper_class($_classes) {
    if ( ! is_array($_classes) || 500 == esc_attr( angi_fn_opt( 'tc_slider_default_height') ) )
      return $_classes;
  ...
  */
  function angi_fn_get_post_slide_title( $_post, $ID ) {
    ...
    return $this -> angi_fn_get_post_title( $_post, $title_length, $more );
  }
  ...
  */
  function angi_fn_get_post_slide_excerpt( $_post, $ID ) {
    ...
    $more            = apply_filters( 'tc_post_slide_more', '...', $ID );
    return $this -> angi_fn_get_post_excerpt( $_post, $excerpt_length, $more );
  }
  ...
  */
  function angi_fn_get_post_slide_button_text( $button_text ) {
    ...
    return angi_fn_text_truncate( $button_text, $button_text_length, $more );
  }
  ...
  */
  ...
  function angi_fn_get_post_title( $_post, $default_title_length, $more ) {
    $title = $_post->post_title;
    if ( ! empty( $_post->post_password ) ) {
      $protected_title_format = apply_filters( 'protected_title_format', __( 'Protected: %s', 'angilla' ), $_post);
      $title = sprintf( $protected_title_format, $title );
    }

    $title = apply_filters( 'tc_post_title_pre_trim' , $title );
    return angi_fn_text_truncate( $title, $default_title_length, $more);
  }
  ...
  */
  // move this into ANGI_utils?
  function angi_fn_get_post_excerpt( $_post, $default_text_length, $more ) {
    if ( ! empty( $_post->post_password) )
      return __( 'There is no excerpt because this is a protected post.', 'angilla' );
    ...
    return angi_fn_text_truncate( $excerpt, $default_text_length, $more);
  }
...
*
*/
if ( ! class_exists( 'ANGI_footer_main' ) ) :
	class ANGI_footer_main {
    static $instance;
    function __construct () {
      self::$instance =& $this;
      //All footer hooks setup
      add_action( 'wp_head'                   , array( $this , 'angi_fn_footer_hook_setup') );

      // Sticky footer style
      add_filter( 'tc_user_options_style' , array( $this , 'angi_fn_write_sticky_footer_inline_css' ) );
    }
    ...
    */
    function angi_fn_footer_hook_setup() {
      ...
      add_filter ( 'body_class' , array( $this, 'angi_fn_add_sticky_footer_body_class' ) );

      //print the sticky_footer push div
      add_action ( '__after_main_container' , array( $this, 'angi_fn_sticky_footer_push'), 100 );
      ...
      //footer actions
      add_action ( '__footer'         , array( $this , 'angi_fn_widgets_footer' ), 10 );
      add_action ( '__footer'         , array( $this , 'angi_fn_colophon_display' ), 20 );

      //colophon actions => some priorities are rtl dependants
      add_action ( '__colophon'       , array( $this , 'angi_fn_colophon_left_block' ), 10 );
      add_action ( '__colophon'       , array( $this , 'angi_fn_colophon_center_block' ), 20 );
      add_action ( '__colophon'       , array( $this , 'angi_fn_colophon_right_block' ), 30 );

      //since v3.2.0, Show back to top from the Customizer option panel
      add_action ( '__after_footer'       , array( $this , 'angi_fn_render_back_to_top') );
      //since v3.2.0, set no widget icons from the Customizer option panel
      add_filter ( 'tc_footer_widget_wrapper_class' , array( $this , 'angi_fn_set_widget_wrapper_class') );
    }
    ...
    *******************************/
	  ...
	  function angi_fn_widgets_footer() {
    	...
    	$status 					= false;
    	$footer_widgets 			= apply_filters( 'tc_footer_widgets', ANGI_init::$instance -> footer_widgets );
      ...
      //if no active widget area yet, return
        ...
        return;
			...
			$skin_class 					= ( in_array( angi_fn_opt( 'tc_skin') , array('grey.css' , 'black.css', 'black2.css')) ) ? 'white-icons' : '';
	   ...
	   /**
        ...
        //the whole block will be always displayed for a matter of structure (columns)
	    	$_hide_socials = ( 0 == esc_attr( angi_fn_opt( 'tc_social_in_footer') ) );
	      	...
	      	echo apply_filters(
	      		...
	      		sprintf('<div class="%1$s">%2$s</div>',
	      			...
	      			( ! $_hide_socials ) ? sprintf('<span class="social-links">%1$s</span>',	angi_fn__f( '__get_socials' ) ) : ''
            )
	      	);
	    }
  		    		...
  		    		sprintf( '<p>%1$s %2$s %3$s</p>',
                              ...
                              apply_filters( 'tc_wp_powered',
                                  sprintf( '&middot; <span class="tc-wp-powered-text">%1$s</span> <a class="icon-wordpress" target="_blank" href="https://wordpress.org" title="%2$s"></a> &middot;',
                                      __('Powered by', 'angilla'),
                                      __('Powered by WordPress', 'angilla')
                                  )
                              ),
      ...
      if ( ! apply_filters('tc_show_text_btt', 0 == esc_attr( angi_fn_opt( 'tc_show_back_to_top' ) ) ) )
        return;
    ...
    *
    */
    function angi_fn_sticky_footer_push() {
      if ( ! ( $this -> is_sticky_footer_enabled() || angi_fn_is_customizing() ) )
        return;
      ...
      echo '<div id="tc-push-footer"></div>';
		...
		*/
		function angi_fn_render_back_to_top() {
            ...
            printf('<div id="tc-footer-btt-wrapper" class="tc-btt-wrapper %1$s"><i class="btt-arrow"></i></div>',
                esc_attr( angi_fn_opt( 'tc_back_to_top_position' ) )
            );
		}
		...
		*/
		function angi_fn_set_widget_wrapper_class( $_original_classes ) {
			...
			if ( 1 == esc_attr( angi_fn_opt( 'tc_show_footer_widget_icon' ) ) )
				return ( 0 == esc_attr( angi_fn_opt( 'tc_show_title_icon' ) ) ) ? $_no_icons_classes : $_original_classes;
			 //last condition
          	return $_no_icons_classes;
        }
    ...
    */
    function is_sticky_footer_enabled() {
      return 1 == esc_attr( angi_fn_opt( 'tc_sticky_footer') );
    }
  }//end of class
endif;
