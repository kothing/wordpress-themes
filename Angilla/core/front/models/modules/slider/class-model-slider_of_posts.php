<?php
class ANGI_slider_of_posts_model_class extends ANGI_slider_model_class {
  ...
  */
  protected function angi_fn_get_the_slides( $slider_name_id, $img_size = 'slider-full' ) {
    return apply_filters( 'angi_the_slides', $this -> angi_fn_get_the_posts_slides( $slider_name_id, $img_size ) );
  }
  ...
  */
  function angi_fn_get_slider_edit_link_text() {
    return __( 'Customize or remove the posts slider', 'angilla' );
  }
  ...
  */
  private function angi_fn_get_the_posts_slides( $slider_name_id, $img_size ) {
    ...
    $pre_slides      = apply_filters( 'angi_posts_slider_pre_model', $pre_slides, $this );
    ...
    foreach ( $posts as $_post_slide ) {
      $slide_model = $this -> angi_fn_get_single_post_slide_model( $slider_name_id, $_post_slide, $common, $img_size);
      if ( ! $slide_model )
        continue;
      $slides[ $_post_slide['ID'] ] = $slide_model;
    }//end of slides loop
    //returns the slides or false if nothing
    return apply_filters('angi_the_posts_slides', ! empty($slides) ? $slides : false );
  }
  ...
  */
  private function angi_fn_get_pre_posts_slides( $args ){
    $defaults       = array(
      ...
      'posts_per_page'           => esc_attr( angi_fn_opt( 'tc_posts_slider_number' ) ),
      'link_type'                => esc_attr( angi_fn_opt( 'tc_posts_slider_link') )
    );

    $args         = apply_filters( 'angi_get_pre_posts_slides_args', wp_parse_args( $args, $defaults ) );
    extract( $args );
    ...
    if ( $smart_load_enabled )
      remove_filter( 'angi_thumb_html', 'angi_fn_parse_imgs' );

    // prevent adding thumb inline style when no center img is added
    add_filter( 'angi_post_thumb_inline_style', '__return_empty_string', 100 );

    //Allow retrieving first attachment as thumb
    add_filter( 'angi_use_attachment_as_thumb', '__return_true', 100 );
    /*** end tc_thumb setup ***/
    ...
    remove_filter( 'angi_post_thumb_inline_style', '__return_empty_string', 100 );

    // remove forced retrieval first attachment as thumb;
    remove_filter( 'angi_use_attachment_as_thumb', '__return_true', 100 );
  ...
  */
  private function angi_fn_query_posts_slider( $args = array() ) {
    ...
    $args           = apply_filters( 'angi_query_posts_slider_args', wp_parse_args( $args, $defaults ) );
    $_posts         = false;
    ...
    return apply_filters( 'angi_query_posts_slider', $_posts, $args );
  }
  ...
  */
  function angi_fn_get_single_post_slide_pre_model( $_post , $img_size, $args ){
    ...
    //attachment image
    $thumb                  = angi_fn_get_thumbnail_model( array(
    ...
    if ( ! $slide_background ) {
        $placeholder_src = angi_fn_get_theme_file_url( ANGI_ASSETS_PREFIX . 'front/img/slide-placeholder.png' );
        if ( $placeholder_src ) {
  ...
  */
  function angi_fn_get_single_post_slide_model( $slider_name_id, $_post_slide , $common, $img_size ) {
    ...
    //background image
    $slide_background       = apply_filters( 'angi_posts_slide_background', $slide_background, $ID );
    // we don't want to show slides with no image
    if ( ! $slide_background )
      return false;
    $title                  = apply_filters('angi_posts_slider_title', $title, $ID );
    //lead text
    $text                   = apply_filters('angi_posts_slider_text', $text, $ID );
    //button
    $button_text            = apply_filters('angi_posts_slider_button_text', $button_text, $ID );
    //link
    $link_id                = apply_filters( 'angi_posts_slide_link_id', $ID );
    $link_url               = apply_filters( 'angi_posts_slide_link_url', $link_id ? get_permalink( $link_id ) : '', $ID );
    $link_target            = apply_filters( 'angi_posts_slide_link_target', '_self', $ID );
    $link_whole_slide       = apply_filters( 'angi_posts_slide_link_whole_slide', $link_whole_slide, $ID );
    $color_style            = apply_filters( 'angi_posts_slide_color_style', '', $ID );

    return apply_filters( 'angi_single_post_slide_model', compact(
        'title',
        'text',
        'button_text',
        'link_id',
  ...
  */
  function angi_fn_get_post_slide_title( $_post, $ID ) {
    $title_length   = apply_filters('angi_post_slide_title_length', 80, $ID );
    $more           = apply_filters('angi_post_slide_more', '...', $ID );
  ...
  */
  function angi_fn_get_post_slide_excerpt( $_post, $ID ) {
    $excerpt_length  = apply_filters( 'angi_post_slide_text_length', 80, $ID );
    $more            = apply_filters( 'angi_post_slide_more', '...', $ID );
  ...
  */
  function angi_fn_get_post_slide_button_text( $button_text ) {
    $button_text_length  = apply_filters( 'angi_posts_slider_button_text_length', 80 );
    $more                = apply_filters( 'angi_post_slide_more', '...');
    $button_text         = apply_filters( 'angi_posts_slider_button_text_pre_trim' , $button_text );
  ...
  */
  ...
  function angi_fn_get_post_title( $_post, $default_title_length, $more ) {
    ...
    $title = apply_filters( 'angi_post_title_pre_trim' , $title );
  ...
  */
  ...
  function angi_fn_get_post_excerpt( $_post, $default_text_length, $more ) {
    ...
    $excerpt = apply_filters( 'angi_post_excerpt_pre_sanitize' , $excerpt );
    ...
    $excerpt = apply_filters( 'angi_post_excerpt_pre_trim' , $excerpt );
