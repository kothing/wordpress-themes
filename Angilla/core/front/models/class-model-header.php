<?php
class ANGI_header_model_class extends ANGI_Model {
  ...
  function __construct( $model = array() ) {
    ...
    $children = array(
      ...
      array(
        ...
        'args' => array(
          'angi_menu_location' => 'primary_navbar'
        )
      ),
      ...
      array(
        ...
        'model_class' => 'header/parts/menu',
        'args' => array(
          'angi_menu_location' => 'secondary_navbar'
        ),
      ),
    ...
    if ( apply_filters( 'tc_woocommerce_options_enabled', false )  ) {
        ...
        if ( 'none' != esc_attr( angi_fn_opt( 'tc_header_desktop_wc_cart' ) ) || esc_attr( angi_fn_opt( 'tc_header_mobile_wc_cart' ) ) ) {
            $children[] =  array(
    ...
    $children = apply_filters( 'angi_header_children_models', $children );
    ...
    add_filter( 'pc_mc_open_effect', array( $this,  'angi_fn_maybe_force_mc_open_with_slide_top' ) );
  ...
  */
  function angi_fn_extend_params( $model = array() ) {
    ...
    */
    //if ( !is_404() && ( 'absolute' == esc_attr( angi_fn_opt( 'tc_header_type' ) ) /*||
    //    ( 'full' == esc_attr( angi_fn_opt( 'tc_heading' ) ) && ! angi_fn_is_real_home() ) */ ) )
    ...
    array_push( $element_class,
        0 != esc_attr( angi_fn_opt( 'tc_sticky_shrink_title_logo') ) ? 'sticky-brand-shrink-on' : '',
        0 != esc_attr( angi_fn_opt( 'tc_sticky_transparent_on_scroll') ) ? 'sticky-transparent' : ''
    );
    ...
    */
    /*
      ...
      *meaning that you need to exactly know how to display it in lg, either flex
      ...
      d-lg-none
    */
    $_topbar_visibility = esc_attr( angi_fn_opt( 'tc_header_show_topbar' ) );
    ...
    */
    if ( angi_fn_is_registered_or_possible( 'navbar_primary_menu' ) && has_nav_menu( 'main') ) {
        $_desktop_primary_navbar_class[] = 'has-horizontal-menu';
    }
    if ( has_nav_menu( 'secondary') && angi_fn_is_registered_or_possible( 'navbar_secondary_menu' ) ) {
        $_desktop_primary_navbar_class[] = 'has-horizontal-menu';
    }
    if ( angi_fn_is_registered_or_possible( 'branding_tagline_aside' ) || angi_fn_is_registered_or_possible( 'title_next_logo' ) ) {
        $_desktop_primary_navbar_class[] = 'has-branding-aside';
    }
    /*
    * Desktop sticky header
    */
    if ( 'no_stick' != esc_attr( angi_fn_opt( 'tc_header_desktop_sticky' ) ) ) {
      if (  'topbar' == esc_attr( angi_fn_opt( 'tc_header_desktop_to_stick' ) ) ) {
    ...
    */
    if ( 'no_stick' != esc_attr( angi_fn_opt( 'tc_header_mobile_sticky' ) ) ) {
    ...
    if ( angi_fn_is_home_and_header_transparent_set() ) {
      ...
      if ( 1 == esc_attr( angi_fn_opt( 'tc_header_no_borders' ) ) ) {
    ...
    /* Submenus effect */
    if ( 0 != esc_attr( angi_fn_opt( 'tc_menu_submenu_fade_effect' ) ) ) {
      $element_class[] = 'angi-submenu-fade';
    }

    if ( 0 != esc_attr( angi_fn_opt( 'tc_menu_submenu_item_move_effect' ) ) ) {
      $element_class[] = 'angi-submenu-move';
    }

    if ( 0 != esc_attr( angi_fn_opt( 'tc_highlight_contextually_active_menu_items' ) ) ) {
      $element_class[] = 'angi-highlight-contextual-menu-items';
    }
    ...
    if ( 'boxed' == esc_attr( angi_fn_opt( 'tc_site_layout') ) ) {
      ...
      $_desktop_primary_navbar_container_class = $_mobile_navbar_container_class = 'boxed' == esc_attr( angi_fn_opt( 'tc_header_navbar_layout' ) ) ? 'container' :  'container-fluid';

      $_topbar_navbar_container_class = 'boxed' == esc_attr( angi_fn_opt( 'tc_header_topbar_layout' ) ) ? 'container' : 'container-fluid';

    }

    return array_merge( $model, array(
        'element_class'                      => array_filter( apply_filters( 'angi_header_class', $element_class ) ),
  ...
  public function angi_fn_maybe_force_mc_open_with_slide_top( $effect ) {
      if ( 'boxed' == esc_attr( angi_fn_opt( 'tc_site_layout') ) ) {
          return 'mc_slide_top';
      }
