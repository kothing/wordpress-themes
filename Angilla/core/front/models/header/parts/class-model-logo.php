<?php
class ANGI_logo_model_class extends ANGI_Model {
  public $logo_type = '';
  ...
  */
  function angi_fn_extend_params( $model = array() ) {
    $atts = angi_fn_get_logo_atts( $this -> logo_type );
    ...
    $logo_resize              = ( 'sticky' != $this->logo_type ) ? esc_attr( angi_fn_opt( 'tc_logo_resize') ) : '';
    //build other attrs
    $model[ 'element_attributes' ] = trim( sprintf('%1$s %2$s %3$s %4$s',
        ...
        ( 1 == $logo_resize ) ? sprintf( 'style="max-width:%1$spx;max-height:%2$spx"',
                                apply_filters( 'angi_logo_max_width', 250 ),
                                apply_filters( 'angi_logo_max_height', 100 )
                                ) : '',
        implode(' ' , apply_filters('angi_logo_other_attributes' , ( 0 == angi_fn_opt( 'tc_retina_support' ) ) ? array('data-no-retina') : array() ) )
    ));
  ...
  */
  function angi_fn_user_options_style_cb( $_css ) {
    ...
    if ( angi_fn_is_customizing() ||
        ( 0 != esc_attr( angi_fn_opt( 'tc_sticky_header') ) && 0 != esc_attr( angi_fn_opt( 'tc_sticky_shrink_title_logo') ) ) ) {
      $_logo_shrink  = implode (';' , apply_filters('angi_logo_shrink_css' , array("height:60px!important","width:auto!important") ) );
      $_css = sprintf("%s%s",
          $_css,
          "
      .sticky-enabled .angi-shrink-on .branding__container img {
        {$_logo_shrink}
      }"
      );
    }
