...
*
*/
class ANGI_slider_model_class extends ANGI_Model {
  public $inner_class;
  public $inner_attrs;
  ...
  */
  function angi_fn_extend_params( $model = array() ) {
    ...
    $this -> queried_id = $queried_id = $this -> angi_fn_get_real_id();

    $this -> allow_resp_images = 1 == esc_attr( angi_fn_opt('tc_resp_thumbs_img') );

    $slider_name_id     = angi_fn_get_current_slider( $queried_id );

    $layout             = $this -> angi_fn_get__slider_layout( $queried_id, $slider_name_id );

    $img_size           = apply_filters( 'angi_slider_img_size' , ( 'boxed' == $layout ) ? 'slider' : 'slider-full');


    $slides             = $this -> angi_fn_get_slides( $slider_name_id, $img_size );
    ...
    $element_class      = $this -> angi_fn_get_slider_element_class( $queried_id, $slider_name_id, $layout );
    $inner_class        = $this -> angi_fn_get_slider_inner_class( $queried_id );

    //set-up inner attrs
    $inner_attrs        = $this -> angi_fn_get_slider_inner_attrs();

    //set-up controls
    if ( apply_filters( 'angi_show_slider_controls' , count( $slides ) > 1 ) ) {
    ...
    if ( $this -> angi_fn_is_slider_loader_active( $slider_name_id ) ) {
      ...
      if ( ! apply_filters( 'angi_slider_loader_gif_only', false ) )
        $pure_css_loader = sprintf( '<div class="angi-css-loader %1$s">%2$s</div>',
            implode( ' ', apply_filters( 'angi_pure_css_loader_add_classes', array( 'angi-mr-loader') ) ),
            apply_filters( 'angi_pure_css_loader_inner', '<div></div><div></div><div></div>')
        );
    } else {
      $has_loader      = false;
  ...
  protected function angi_fn_get_slides( $slider_name_id, $img_size ) {
    //returns the default slider if requested
    if ( 'demo' == $slider_name_id )
      return apply_filters( 'tc_default_slides', $this -> angi_fn_get_default_slides() );
    else {
      ...
      if ( $this -> allow_resp_images )
        add_filter( 'wp_calculate_image_srcset', array( $this, 'angi_maybe_add_resp_slide_img_size_to_the_srcset' ), 10, 5 );

      $the_slides = $this -> angi_fn_get_the_slides( $slider_name_id, $img_size );

      //remove the filter
      if ( $this -> allow_resp_images )
        remove_filter( 'wp_calculate_image_srcset', array( $this, 'angi_maybe_add_resp_slide_img_size_to_the_srcset' ), 10 );
  ...
  */
  protected function angi_fn_get_the_slides( $slider_name_id, $img_size ) {
    ...
    return apply_filters('angi_the_slides', ! empty($slides) ? $slides : false );
  }
  ...
  */
  protected function angi_fn_get_single_slide_model( $slider_name_id, $_loop_index , $id , $img_size = 'slider-full' ) {
    ...
    $default_title_length   = apply_filters( 'angi_slide_title_length', 80 );
    $title                  = angi_fn_text_truncate( $title, $default_title_length, '...' );
    //lead text
    $text                   = get_post_meta( $id, $key = 'slide_text_key' , $single = true );
    $default_text_length    = apply_filters( 'angi_slide_text_length', 250 );
    ...
    $default_button_length  = apply_filters( 'angi_slide_button_length', 80 );
    $button_text            = angi_fn_text_truncate( $button_text, $default_button_length, '...' );
    //link post id
    $link_id                = apply_filters( 'angi_slide_link_id', esc_attr(get_post_meta( $id, $key = 'slide_link_key' , $single = true )), $id, $slider_name_id );
    //link
    $link_url               = esc_url( get_post_meta( $id, $key = 'slide_custom_link_key', $single = true ) );
    if ( ! $link_url )
      $link_url = $link_id ? get_permalink( $link_id ) : $link_url;
    $link_url               = apply_filters( 'angi_slide_link_url', $link_url, $id, $slider_name_id );
    //link target
    $link_target_bool       = esc_attr(get_post_meta( $id, $key= 'slide_link_target_key', $single = true ));
    $link_target            = apply_filters( 'angi_slide_link_target', $link_target_bool ? '_blank' : '_self', $id, $slider_name_id );
    //link the whole slide?
    $link_whole_slide       = apply_filters( 'angi_slide_link_whole_slide', esc_attr(get_post_meta( $id, $key= 'slide_link_whole_slide_key', $single = true )), $id, $slider_name_id );
    ...
    //attachment image
    $alt                    = apply_filters( 'angi_slide_background_alt' , trim(strip_tags(get_post_meta( $id, '_wp_attachment_image_alt' , true))) );

    $slide_background_attr  = array_filter( array_merge( array( 'class' => 'slide' , 'alt' => $alt ), $this -> angi_fn_set_wp_responsive_slide_img_attr() ) );

    $slide_background       = wp_get_attachment_image( $id, $img_size, false, $slide_background_attr );

    //Parse img for smartload
    //Should normally be done with apply_filters( 'angi_thumb_html', $html ), but this filter is added later (  on 'wp_head' ) for plugin compatibility.
    //=> that's why we invoke angi_fn_parse_imgs() directly here
    //@see angi_fn_wp_filters
  ...
  * return array()
  */
  protected function angi_fn_set_wp_responsive_slide_img_attr() {
  ...
  function angi_maybe_add_resp_slide_img_size_to_the_srcset( $sources, $size_array, $image_src, $image_meta, $attachment_id ) {
  ...
  */
  ...
  function angi_fn_get_the_slide( $autoadvance = true ) {
  ...
  */
  function angi_fn_sanitize_model_properties( $model ) {
    parent::angi_fn_sanitize_model_properties( $model );
    $model -> inner_class = $this -> angi_fn_stringify_model_property( 'inner_class' );
  }
  ...
  * for them
  */
  protected function angi_fn_get_default_slides() {
    //Default slides content
    $demo_slides = ANGI___::$instance -> default_slides;
  ...
  */
  function angi_fn_get_slider_inner_class( $queried_id ) {
    ...
    $class[] = (bool) esc_attr( angi_fn_opt( 'tc_center_slider_img') ) ? 'center-slides-enabled' : 'center-slides-disabled';

    if ( (bool) esc_attr( angi_fn_opt( 'tc_slider_parallax') ) )
      $class[] = 'angi-parallax-slider';

    //Is the overlay checked for this slider ?
    //gets slider options if any
    $overlay_value                 = angi_fn_is_real_home() ? angi_fn_opt( 'tc_home_slider_overlay' ) : esc_attr( get_post_meta( $queried_id, $key = 'slider_overlay_key' , $single = true ) );
    $overlay_value                 = apply_filters( 'angi_slider_overlay', $overlay_value, $queried_id );
    //if the option is unchecked OR has never been set
    if ( 'off' == $overlay_value ) {
      $class[] = 'angi-has-no-dark-overlay';
    }

    return apply_filters( 'angi_carousel_inner_classes', $class );
  }
  ...
  * @return array()
  */
  protected function angi_fn_get_slider_element_class( $queried_id, $slider_name_id, $layout ) {
    $class        = array( 'angi-carousel', $slider_name_id );
    ...
    //layout
    $layout_class = apply_filters( 'angi_slider_layout_class', $layout_class );

    $class        = array_merge( $class, $layout_class );

    //custom height
    if ( ! in_array( 'full-page', $class ) && 500 != esc_attr( angi_fn_opt( 'tc_slider_default_height') ) )
  ...
  * @return string
  */
  protected function angi_fn_get_slider_inner_attrs() {
    $atts = array();
    ...
    $atts[] = sprintf( 'data-slider-delay="%s"',
        angi_fn_is_real_home() ? angi_fn_opt( 'tc_slider_delay' ) : get_post_meta( angi_fn_get_id() , $key = 'slider_delay_key' , $single = true )
    );


    $has_dots  =  angi_fn_is_real_home() ? angi_fn_opt( 'tc_home_slider_dots' ) : get_post_meta( angi_fn_get_id() , $key = 'slider_dots_key' , $single = true );
    ...
    return angi_fn_stringify_array( $atts );
  }
  ...
  * @return bool
  */
  protected function angi_fn_get__slider_layout( $queried_id, $slider_name_id ) {
    //gets slider options if any
    $layout_value                 = angi_fn_is_real_home() ? angi_fn_opt( 'tc_slider_width' ) : esc_attr( get_post_meta( $queried_id, $key = 'slider_layout_key' , $single = true ) );
    $layout_value                 = apply_filters( 'angi_slider_layout', $layout_value, $queried_id );
  ...
  *
  */
  function angi_fn_get_has_slider_edit_link() {
    ...
    return apply_filters( 'angi_show_slider_edit_link' , $show_slider_edit_link, $this -> slider_name_id );
  }
  ...
  *
  */
  function angi_fn_get_slider_edit_link() {
    if ( angi_fn_is_real_home() )
      $slider_edit_link            = angi_fn_get_customizer_url( array( 'control' => 'tc_front_slider', 'section' => 'frontpage_sec') );
    elseif ( is_singular() ) {
      global $post;
  ...
  *
  */
  function angi_fn_get_slider_edit_link_text() {
    return __( 'Customize or remove this slider', 'angilla' );
  }
  ...
  *
  */
  protected function angi_fn_get_real_id() {
    return apply_filters( 'angi_slider_get_real_id', angi_fn_get_real_id(), $this );
  }
  ...
  */
  private function angi_fn_is_slider_loader_active( $slider_name_id ) {
    ...
    return ( 'demo' == $slider_name_id
        || apply_filters( 'angi_display_slider_loader', true , $slider_name_id, $this -> id )
    );
  }
  ...
  /**
  * hook : tc_slider_height, fired in angi_fn_user_options_style
  ...
  */
  function angi_fn_set_demo_slider_height( $_h ) {
    ...
    if ( 'demo' != angi_fn_get_current_slider( $this -> angi_fn_get_real_id() ) )
      return $_h;
    ...
    if ( angi_fn_is_customizing() ) {
      ...
      if ( false !== (bool) esc_attr( angi_fn_opt( 'tc_slider_default_height', ANGI_THEME_OPTIONS, $use_default = false ) ) )
        return $_h;
    }
    return apply_filters( 'angi_set_demo_slider_height' , 750 );
  }
  ...
  *
  */
  function angi_fn_get_slider_loader_css( $slider_name_id ) {
    $slider_name_id =  $slider_name_id ? $slider_name_id : angi_fn_get_current_slider( $this -> angi_fn_get_real_id() );
    $_css           = '';

    //custom css for the slider loader
    if ( $this -> angi_fn_is_slider_loader_active( $slider_name_id ) ) {

      $_slider_loader_src = apply_filters( 'angi_slider_loader_src' , sprintf( '%1$s%2$s' , ANGI_FRONT_ASSETS_URL, 'img/slider-loader.gif') );
      ...
      $_slider_loader_gif_css     = $_slider_loader_src ? sprintf('
                                        %1$s .angi-slider-loader-wrapper .angi-img-gif-loader {
  ...
  *
  */
  function angi_fn_get_slider_custom_height_css( $slider_name_id ) {
    ...
    $_custom_height     = esc_attr( angi_fn_opt( 'tc_slider_default_height') );
    $_custom_height     = ( !angi_fn_is_real_home() && 0 == esc_attr( angi_fn_opt( 'tc_slider_default_height_apply_all') ) ) ? 500 : $_custom_height;

    $_custom_height     = apply_filters( 'angi_slider_height' , 'demo' != $slider_name_id ? $_custom_height : $this -> angi_fn_set_demo_slider_height( $_custom_height ) );

    $_slider_inline_css = "";

    $_resp_shrink_ratios = ANGI()->slider_resp_shrink_ratio;

    //this slider element id;
    $slider_html_element_id = "#angilla-slider-{$this->id}";

    $_slider_inline_css = "
      $slider_html_element_id.angi-carousel {
        height:{$_custom_height}px;

      }
      $slider_html_element_id .angi-slider-loader-wrapper {
  ...
  */
  function angi_fn_user_options_style_cb( $_css ) {
    ...
    return sprintf("%s\n%s\n%s",
              $_css,
              $this -> angi_fn_get_slider_loader_css( $slider_name_id ),
              $this -> angi_fn_get_slider_custom_height_css( $slider_name_id )
    );
  }
}//end class