...
if ( ! class_exists( 'ANGI_BASE' ) ) :
  ...
  class ANGI_BASE {
        ...
        function __construct( $_args = array()) {
            //init properties
            add_action( 'after_setup_theme'       , array( $this , 'angi_fn_init_properties') );

            //Set image options set by user @since v3.2.0
            //! must be available in admin for plugins like regenerate thumbnails
            add_action( 'after_setup_theme'       , array( $this, 'angi_fn_set_user_defined_settings'));


            //add the text domain, various theme supports : editor style, automatic-feed-links, post formats, post-thumbnails
            add_action( 'after_setup_theme'       , array( $this , 'angi_fn_base_angilla_setup' ) );

            //IMPORTANT : this callback needs to be ran AFTER angi_fn_init_properties.
            add_action( 'after_setup_theme'       , array( $this , 'angi_fn_cache_theme_setting_list' ), 100 );

            //refresh the theme options right after the _preview_filter when previewing
            add_action( 'customize_preview_init'  , array( $this , 'angi_fn_customize_refresh_db_opt' ) );
            ...
            add_action( 'wp_loaded'               , array( $this, 'angi_fn_set_early_hooks') );

            //Javascript detection
            add_action( 'wp_head'                 , array( $this, 'angi_fn_javascript_detection'), 0 );


            //registers the menus
            add_action( 'after_setup_theme'       , array( $this, 'angi_fn_register_menus'));

            //add retina support for high resolution devices
            add_filter( 'wp_generate_attachment_metadata'        , array( $this , 'angi_fn_add_retina_support') , 10 , 2 );
            add_action( 'delete_attachment'                      , array( $this , 'angi_fn_clean_retina_images') );

            //prevent rendering the comments template more than once
            add_filter( 'tc_render_comments_template'            , array( $this,  'angi_fn_control_coments_template_rendering' ) );


            //remove hentry class when the current $post type is a page
            add_filter( 'post_class'                             , array( $this, 'angi_fn_maybe_remove_hentry_class' ), 20 );
            ...
            add_action( 'customize_save_validation_before'       , array( $this, 'angi_fn_remove_callback_wp_targeted_link_rel' ) );
            ...
            $this -> slider_size        = array( 'width' => ANGI_IS_MODERN_STYLE ? 1110 : 1170 , 'height' => 500, 'crop' => true ); //size name : slider

            $this -> tc_grid_size       = array( 'width' => 570 , 'height' => 350, 'crop' => true ); //size name : tc-grid
            //Default images sizes
            $this -> tc_grid_full_size  = array( 'width' => ANGI_IS_MODERN_STYLE ? 1110 : 1170 , 'height' => ANGI_IS_MODERN_STYLE ? 444 : 350, 'crop' => true ); //size name : tc-grid-full
            ...
            $this -> sidebar_widgets    = array(
              'left'          => array(
                              'name'                 => __( 'Left Sidebar' , 'angilla' ),
                              'description'          => __( 'Appears on posts, static pages, archives and search pages' , 'angilla' )
              ),
              'right'         => array(
                              'name'                 => __( 'Right Sidebar' , 'angilla' ),
                              'description'          => __( 'Appears on posts, static pages, archives and search pages' , 'angilla' )
              )
            );//end of array
            ...
            $this -> old_socials            = array(
              'tc_rss'            => array(
                                      'link_title'    => __( 'Subscribe to my rss feed' , 'angilla' ),
                                      'default'       => get_bloginfo( 'rss_url' ) //kept as it's the only one used in the transition
                                  ),
              'tc_email'          => array(
                                      'link_title'    => __( 'E-mail' , 'angilla' ),
                                    ),
              'tc_twitter'        => array(
                                      'link_title'    => __( 'Follow me on Twitter' , 'angilla' ),
                                    ),
              'tc_facebook'       => array(
                                      'link_title'    => __( 'Follow me on Facebook' , 'angilla' ),
                                    ),
              'tc_google'         => array(
                                      'link_title'    => __( 'Follow me on Google+' , 'angilla' ),
                                    ),
              'tc_instagram'      => array(
                                      'link_title'    => __( 'Follow me on Instagram' , 'angilla' ),
                                    ),
              'tc_tumblr'       => array(
                                      'link_title'    => __( 'Follow me on Tumblr' , 'angilla' ),
                                    ),
              'tc_flickr'       => array(
                                      'link_title'    => __( 'Follow me on Flickr' , 'angilla' ),
                                    ),
              'tc_wordpress'      => array(
                                      'link_title'    => __( 'Follow me on WordPress' , 'angilla' ),
                                    ),
              'tc_youtube'        => array(
                                      'link_title'    => __( 'Follow me on Youtube' , 'angilla' ),
                                    ),
              'tc_pinterest'      => array(
                                      'link_title'    => __( 'Pin me on Pinterest' , 'angilla' ),
                                    ),
              'tc_github'         => array(
                                      'link_title'    => __( 'Follow me on Github' , 'angilla' ),
                                    ),
              'tc_dribbble'       => array(
                                      'link_title'    => __( 'Follow me on Dribbble' , 'angilla' ),
                                    ),
              'tc_linkedin'       => array(
                                      'link_title'    => __( 'Follow me on LinkedIn' , 'angilla' ),
                                    ),
              'tc_vk'             => array(
                                      'link_title'    => __( 'Follow me on VKontakte' , 'angilla' ),
                                    ),
              'tc_yelp'           => array(
                                      'link_title'    => __( 'Follow me on Yelp' , 'angilla' ),
                                    ),
              'tc_xing'           => array(
                                      'link_title'    => __( 'Follow me on Xing' , 'angilla' ),
                                    ),
              'tc_snapchat'       => array(
                                      'link_title'    => __( 'Contact me on Snapchat' , 'angilla' ),
                                    )
            );//end of social array
            ...
            $this -> font_pairs             = array(
              'gfont' => array(
                'name'  => __('Google fonts pairs' , 'angilla'),
              ...
              'wsfont' => array(
                'name'  => __('Web safe fonts pairs' , 'angilla'),
            ...
            $this -> default_slides     = array(
                1 => array(
                  ...
                  'slide_background'       =>  sprintf('<img width="1910" height="750" src="%1$s" class="" alt="%2$s" />',
                                              TC_BASE_URL.'assets/front/img/angilla-demo-slider-1.jpg',
                                              __( 'Angilla is a clean responsive theme' , 'angilla' )
                                      )
                ),
                ...
                2 => array(
                  ...
                  'color_style'   =>  '',
                  'slide_background'       =>  sprintf('<img width="1910" height="750" src="%1$s" class="" alt="%2$s" />',
                                              TC_BASE_URL.'assets/front/img/angilla-demo-slider-2.jpg',
                                              __( 'Many layout and design options are available from the WordPress customizer screen : see your changes live !' , 'angilla' )
                                      )
                )
            );///end of slides array
        }//construct
         ...
         *
         */
        function angi_fn_base_angilla_setup() {
            /* Set default content width for post images and media. */
            global $content_width;
            if (! isset( $content_width ) ) {
                $content_width = apply_filters( 'angi_content_width' , ANGI_IS_MODERN_STYLE ? 1140 : 1170 );
            }

            /*
             * Makes Angilla available for translation.
             * Translations can be added to the /inc/lang/ directory.
             */
            load_theme_textdomain( 'angilla' , TC_BASE . '/inc/lang' );
            ...
            if ( ANGI_IS_MODERN_STYLE ) {
              ...
              */
              ...
              $tc_sq_thumb_size = apply_filters( 'tc_square_thumb_size' , ANGI() -> tc_sq_thumb_size );
              ...
              $tc_ws_thumb_size = apply_filters( 'tc_ws_thumb_size' , ANGI() -> tc_ws_thumb_size );
              ...
              $tc_ws_small_thumb_size = apply_filters( 'tc_ws_small_thumb_size' , ANGI() -> tc_ws_small_thumb_size );
              ...
              $tc_slider_small_size = apply_filters( 'tc_slider_small_size' , ANGI() -> tc_slider_small_size  );
            ...
            add_filter( 'upload_mimes'                        , array( $this , 'angi_fn_custom_mtypes' ) );
        ...
        */
        function angi_fn_register_menus() {
            /* This theme uses wp_nav_menu() in one location. */
            register_nav_menu( 'main' , __( 'Main Menu' , 'angilla' ) );
            register_nav_menu( 'secondary' , __( 'Secondary (horizontal) Menu' , 'angilla' ) );
            if ( ANGI_IS_MODERN_STYLE ) {
              register_nav_menu( 'topbar' , __( 'Horizontal Top Bar Menu. (make sure that the topbar is displayed in the header settings ).' , 'angilla' ) );
              register_nav_menu( 'mobile' , __( 'Mobile Menu' , 'angilla' ) );
            }
        }
      ...
      *
      */
      function angi_fn_custom_mtypes( $mimes ) {
       ...
       */
        function angi_fn_add_retina_support( $metadata, $attachment_id ) {
          ...
          if ( 0 == angi_fn_opt( 'tc_retina_support' ) )
            return $metadata;
          ...
          */
          if ( apply_filters( 'angi_build_retina_files_for_pdf_thumbnails', 'application/pdf' == get_post_mime_type( $attachment_id ) ) ) {
          ...
          if ( isset($metadata['width']) && isset($metadata['height']) )
            $this -> angi_fn_create_retina_images( get_attached_file( $attachment_id ), $metadata['width'], $metadata['height'] , false, $_is_intermediate = false );
          ...
          if ( is_array( $sizes ) ) {
              foreach ( $sizes as $_size_name => $_attr ) {
                  if ( is_array( $_attr ) && isset($_attr['width']) && isset($_attr['height']) ) {
                      $this -> angi_fn_create_retina_images(
                        $full_image_file,
                        $_attr['width'],
                        $_attr['height'],
       ...
       */
        ...
        *
        */
        function angi_fn_control_coments_template_rendering( $bool ) {
        ...
        *
        */
        function angi_fn_maybe_remove_hentry_class( $class ) {
            $remove_bool = 'page' == angi_fn_get_post_type();

            if ( apply_filters( 'angi_post_class_remove_hentry_class', $remove_bool ) ) {
        ...
        *
        */
        function angi_fn_set_early_hooks() {
            //Filter home/blog postsa (priority 9 is to make it act before the grid hook for expanded post)
            add_action ( 'pre_get_posts'                , array( $this , 'angi_fn_filter_home_blog_posts_by_tax' ), 9);
            // Make sure the infinite scroll query object is filtered as well
            add_filter ( 'infinite_scroll_query_object' , array( $this , 'angi_fn_filter_home_blog_infinite_posts_by_tax' ) );
            //Include attachments in search results
            add_action ( 'pre_get_posts'                , array( $this , 'angi_fn_include_attachments_in_search' ));
            //Include all post types in archive pages
            add_action ( 'pre_get_posts'                , array( $this , 'angi_fn_include_cpt_in_lists' ));
        }
        ...
        * @package Angilla
        */
        function angi_fn_filter_home_blog_infinite_posts_by_tax( $query ) {
          return $this->_angi_fn_filter_home_blog_posts_by_tax( $query, $reset_cat_category_name = true );
        }

        private function _angi_fn_filter_home_blog_posts_by_tax( $query, $reset_cat_category_name = false ) {
            ...
            if ( ! ANGI_IS_MODERN_STYLE &&
              'grid'== esc_attr( angi_fn_opt( 'tc_post_list_grid' ) ) &&
                class_exists( 'PC_init_infinite' ) && esc_attr( angi_fn_opt( 'tc_infinite_scroll' ) ) && esc_attr( angi_fn_opt( 'tc_infinite_scroll_in_home' ) ) ) {
                return $query;
            }
            ...
            $cats = angi_fn_opt('tc_blog_restrict_by_cat');
            $cats = array_filter( $cats, 'angi_fn_category_id_exists' );
        ...
        *
        */
        function angi_fn_set_user_defined_settings() {
            ...
            if ( isset ( $_options['tc_slider_change_default_img_size'] ) && 0 != esc_attr( $_options['tc_slider_change_default_img_size'] ) && isset ( $_options['tc_slider_default_height'] ) && 500 != esc_attr( $_options['tc_slider_default_height'] ) ) {
                add_filter( 'tc_slider_full_size'          , array($this,  'angi_fn_set_slider_img_height') );
                add_filter( 'tc_slider_size'               , array($this,  'angi_fn_set_slider_img_height') );

                //ONLY FOR MODERN STYLE
                if ( ANGI_IS_MODERN_STYLE ) {
                    add_filter( 'tc_slider_small_size'         , array($this,  'angi_fn_set_slider_small_img_height') );
                }
            }
            ...
            if ( ! ANGI_IS_MODERN_STYLE ) {
                ...
                ***********/
                ...
                if ( $_user_grid_height != $tc_grid_full_size['height'] )
                  add_filter( 'tc_grid_full_size', array( $this,  'angi_fn_set_grid_img_height') );
                if ( $_user_grid_height != $tc_grid_size['height'] )
                  add_filter( 'tc_grid_size'     , array( $this,  'angi_fn_set_grid_img_height') );
            }
            else {
        ...
        */
        ...
        function angi_fn_set_slider_small_img_height( $_default_size ) {

            $_options                     = get_option('tc_theme_options');

            //original slider size
            $_slider_size                 = ANGI() -> slider_size;
        ...
        /**
        * Init ANGI_utils class properties after_setup_theme
        ...
        *
        * ANGI_THEME_OPTIONS, CUSTOMIZR_VER is defined by the child classes before "after_setup_theme"
        *
        * hook : after_setup_theme
        *
        * @package Angilla
        ...
        */
        function angi_fn_init_properties() {
              ...
              // db_options
              // default_options
              do_action( 'angi_before_caching_options' );

              self::$theme_name         = ANGI_SANITIZED_THEMENAME;
              self::$db_options         = false === get_option( ANGI_THEME_OPTIONS ) ? array() : (array)get_option( ANGI_THEME_OPTIONS );
              self::$default_options    = angi_fn_get_default_options();

              //fire an action hook after theme main properties have been set up
              // theme_name
              // db_options
              // default_options
              do_action( 'angi_after_caching_options' );
        }
        ...
        function angi_fn_cache_theme_setting_list() {
          if ( is_array(self::$theme_setting_list) && ! empty( self::$theme_setting_list ) )
            return;

          //fire an action hook before caching theme settomgs list
          do_action( 'angi_before_caching_theme_settings_list' );
          self::$theme_setting_list = angi_fn_generate_theme_setting_list();
          //fire an action hook after caching theme settomgs list
          do_action( 'angi_after_caching_theme_settings_list' );
        }
        ...
        * @return  void
        */
        function angi_fn_customize_refresh_db_opt(){
          ANGI___::$db_options = false === get_option( ANGI_THEME_OPTIONS ) ? array() : (array)get_option( ANGI_THEME_OPTIONS );
        }
         ...
         */
        function angi_fn_javascript_detection() {
         ...
         */
        function angi_fn_remove_callback_wp_targeted_link_rel() {
...
//setup constants
angi_fn_setup_constants();

//setup started using theme option ( before checking angi_fn_is_ms() that uses the user_started_before_Version function to determine it )
angi_fn_setup_started_using_theme_option_and_constants();


add_action( 'after_setup_theme', 'angi_fn_load_angi_base_fmk', 15 );
function angi_fn_load_angi_base_fmk() {
    // load the angi-base-fmk
    if ( ! isset( $GLOBALS['angi_base_fmk_namespace'] ) ) {
        require_once(  dirname( __FILE__ ) . '/angi-base-fmk/angi-base-fmk.php' );
        \angi_fn\ANGI_Fmk_Base( array(
          'text_domain' => 'angilla',
          'base_url' => ANGI_BASE_URL . 'core/angi-base-fmk',
          'version' => CUSTOMIZR_VER
        ) );
    } else {
        //error_log('Warning => the angi_base_fmk should be loaded and instantiated by the theme.');
    }
}

add_action( 'after_setup_theme', 'angi_fn_load_social_links_module', 20 );
function angi_fn_load_social_links_module() {
    // load the social links module
    require_once( ANGI_BASE . ANGI_CORE_PATH . 'angi-modules/social-links/social_links_module.php' );
    angi_fn_register_social_links_module(
        array(
            'setting_id' => 'tc_theme_options[tc_social_links]',

            'base_url_path' => ANGI_BASE_URL . '/core/angi-modules/social-links',
            'version' => CUSTOMIZR_VER,

            'option_value' => angi_fn_opt( 'tc_social_links' ), // for dynamic registration
            'setting' => array(
                'type' => 'option',
                'default'  => array(),
                'transport' => angi_fn_is_partial_refreshed_on() ? 'postMessage' : 'refresh',
                'sanitize_callback' => 'angi_fn_sanitize_callback__angi_social_module',
                'validate_callback' => ''//angi_fn_validate_callback__angi_social_module'
            ),
...
require_once( get_template_directory() . ( angi_fn_is_ms() ? '/core/init.php' : '/inc/angi-init-ccat.php' ) );