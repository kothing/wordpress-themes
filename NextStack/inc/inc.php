 ...
 * 禁止WordPress自动生成缩略图
 */
function thumb_remove_image_size($sizes) {
    unset( $sizes['small'] );
    unset( $sizes['medium'] );
    unset( $sizes['large'] );
    return $sizes;
}
add_filter('image_size_names_choose', 'thumb_remove_image_size');
...
function left_admin_footer_text($text) {
    $text = '<span id="footer-thankyou">感谢您使用 Lyove 的 WordPress主题</a></span>';
    return $text;
}
...
function searchfilter($query) {
    ...
    if ($query->is_search && !is_admin() ) {
        $query->set('post_type',array('site','post'));
    }
    return $query;
}
