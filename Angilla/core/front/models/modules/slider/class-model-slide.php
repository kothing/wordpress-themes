<?php
class ANGI_slide_model_class extends ANGI_Model {
  ...
  function angi_fn_setup_late_properties() {
    ...
    $slider_name_id = angi_fn_get_property( 'slider_name_id' );
    ...
    //caption elements
    $caption           = $this -> angi_fn_get_slide_caption_model( $slide, $slider_name_id, $slide_id );
    $has_caption       = ! empty( $caption );

    //img elements
    $img_wrapper_class = apply_filters( 'angi_slide_content_class', 'carousel-image', $slide_id );

    $edit_url          = $this -> angi_fn_get_the_edit_url( $slide, $slide_id, $slider_name_id );
  ...
  function angi_fn_get_the_edit_url( $slide, $slide_id, $slider_name_id ) {
    if ( ! $slide_id  || 'demo' == $slider_name_id )
      return '';

    $show_slide_edit_link  = current_user_can( 'edit_post', $slide_id ) ? true : false;

    if ( ! apply_filters('angi_show_slide_edit_link' , $show_slide_edit_link && ! is_null( $slide['link_id'] ), $slide_id  ) )
      return '';
  ...
  */
  function angi_fn_get_slide_caption_model( $slide, $slider_name_id, $id ) {
    //filters the data before (=> used for demo for example )
    $data                   = apply_filters( 'angi_slide_caption_data', $slide, $slider_name_id, $id );
    ...
    if ( ! apply_filters( 'angi_slide_show_caption', $show_caption , $slider_name_id ) )
      return array();


    //apply filters first (Lang plugins)
    $_title                  = isset($title) ? apply_filters( 'angi_slide_title', $title , $id, $slider_name_id ) : '';
    $_subtitle               = isset($subtitle) ? esc_html( apply_filters( 'angi_slide_text', $subtitle, $id, $slider_name_id ) ) : '';
    $_button_text            = isset($button_text) ? apply_filters( 'angi_slide_button_text', $button_text, $id, $slider_name_id ) : '';

    // title elements
    if ( apply_filters( 'angi_slide_show_title', $_title != null, $slider_name_id ) )
      $title        = $_title;

    // text elements
    if (  apply_filters( 'angi_slide_show_text', $_subtitle != null, $slider_name_id ) )
      $subtitle         = $_subtitle;

    // button elements
    if ( apply_filters( 'angi_slide_show_button', $_button_text != null, $slider_name_id ) )
