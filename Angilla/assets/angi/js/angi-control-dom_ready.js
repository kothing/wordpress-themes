...
(function (wp, $) {
        ...
        $( function($) {
              ...
              /*
              * Override angiSelect2 Results Adapter in order to select on highlight
              ...
              */
              ...
              if ( 'undefined' !== typeof $.fn.angiSelect2 && 'undefined' !== typeof $.fn.angiSelect2.amd && 'function' === typeof $.fn.angiSelect2.amd.require ) {
                    $.fn.angiSelect2.amd.require(['angiSelect2/results', 'angiSelect2/utils'], function (Result, Utils) {
              ...
              $.when( selectFocusResults ).done( function( customResultsAdapter ) {
                      ...
                      */
                      if ( customResultsAdapter ) {
                            $.extend( _skin_angiSelect2_params, {
                                  resultsAdapter: customResultsAdapter,
                                  closeOnSelect: false,
                            } );
                            $.extend( _fonts_angiSelect2_params, {
                                  resultsAdapter: customResultsAdapter,
                                  closeOnSelect: false,
                            } );
                      }

                      //http://ivaynberg.github.io/angiSelect2/#documentation
                      $('select[data-customize-setting-link="tc_theme_options[tc_skin]"]').angiSelect2( _skin_angiSelect2_params );

                      //Skins handled with angiSelect2
                      function paintSkinOptionElement(state) {
                              if (!state.id) return state.text; // optgroup
                              return '<span class="tc-angiSelect2-skin-color" style="background:' + $(state.element).data('hex') + '">' + $(state.element).data('hex') + '<span>';
                      }


                      //FONTS
                      $('select[data-customize-setting-link="tc_theme_options[tc_fonts]"]').angiSelect2( _fonts_angiSelect2_params );

                      function paintFontOptionElement(state) {
                            if ( ! state.id && ( -1 != state.text.indexOf('Google') ) )
                              return 'Font pairs'; // google font optgroup
                            else if ( ! state.id )
                              return state.text;// optgroup different than google font
                            return '<span class="tc-angiSelect2-font">' + state.text + '</span>';
                      }
        ...
        api.bind( 'ready', function() {
            _.each( [ 'panel', 'section', 'control' ], function( wot ) {
                api.previewer.bind( 'angi-' + wot + '-focus', function( _id_ ) {
                    var _do = function() {
                        if ( api[ wot ].has( _id_ ) ) {
                            api[ wot ]( _id_ ).focus();
                        }
