...
<?php
    ...
    if(get_post_meta($post->ID, '_wechat_qr', true)){
        ...
        switch(io_get_option('po_prompt')) {
            ...
            case 'url': 
                ...
                break;
            case 'summary':
                $title = get_post_meta($post->ID, '_site_sescribe', true);
                break;
            case 'qr':
                if($link_url=="")
                    $title = __('地址错误！','i_theme');
    ...
    ?>
    <a href="<?php echo $url ?>" target="<?php echo $blank ?>" class="xe-widget xe-conversations site-card" <?php echo $tooltip . ' ' . $is_html ?> title="<?php echo $title ?>">
        <div class="xe-comment-entry">
            ...
            <div class="xe-comment">
                ...
                <p class="overflowClip_1"><?php echo get_post_meta($post->ID, '_site_sescribe', true) ?: preg_replace("/(\s|\&nbsp\;|　|\xc2\xa0)/","",get_the_excerpt($post->ID)); ?></p>
            </div>
        </div>
    </a>
