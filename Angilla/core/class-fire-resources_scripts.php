...
* @package            Angilla
*/
if ( ! class_exists( 'ANGI_resources_scripts' ) ) :
   class ANGI_resources_scripts {
         ...
         function __construct () {
              ...
              $this->_resouces_version        = ANGI_DEBUG_MODE || ANGI_DEV_MODE || ANGI_REFRESH_ASSETS ? CUSTOMIZR_VER . time() : CUSTOMIZR_VER;
              $this->_minify_js               = ANGI_DEBUG_MODE || ANGI_DEV_MODE ? false : true ;


              add_action( 'wp_enqueue_scripts'                    , array( $this , 'angi_fn_enqueue_front_scripts' ) );
              add_action( 'angi_ajax_dismiss_welcome_note_front'   , array( $this , 'angi_fn_dismiss_welcome_note_front' ) );
         ...
         */
         ...
         function angi_fn_enqueue_front_scripts() {
              if ( angi_fn_is_full_nimble_tmpl() )
               ...
               wp_enqueue_script(
                   'modernizr',
                   ANGI_BASE_URL . ANGI_ASSETS_PREFIX . 'front/js/libs/modernizr.min.js',
                   array(),
               ...
               if ( $this -> angi_fn_load_concatenated_front_scripts() ) {
                  ...
                  $this -> angi_fn_enqueue_script( array(
                     'tc-js-params',
                     'tc-js-arraymap-proto',
                     //libs
                     'tc-bootstrap',
                     'tc-smoothscroll',//fired if  $('body').hasClass( 'angi-infinite-scroll-on' ) || ( ANGIParams.SmoothScroll && ANGIParams.SmoothScroll.Enabled && ! angiapp.base.matchMedia( 1024 )
                     'tc-outline',
                     'tc-waypoints',
                  ...
                  // if ( $this -> angi_fn_is_lightbox_required() )
                  //       $this -> angi_fn_enqueue_script( 'tc-mfp' );
               ...
               $anchor_smooth_scroll_exclude =  apply_filters( 'angi_anchor_smoothscroll_excl' , array(
                   'simple' => array( '[class*=edd]' , '.carousel-control', '[data-toggle="modal"]', '[data-toggle="dropdown"]', '[data-toggle="angi-dropdown"]', '[data-toggle="tooltip"]', '[data-toggle="popover"]', '[data-toggle="collapse"]', '[data-toggle="angi-collapse"]', '[data-toggle="tab"]', '[data-toggle="pill"]', '[data-toggle="angi-pill"]', '[class*=upme]', '[class*=um-]' ),
                   'deep'   => array(
                     'classes' => array(),
                     'ids'     => array()
                   )
               ...
               $smooth_scroll_enabled = apply_filters('tc_enable_smoothscroll', angi_fn_is_checked( 'tc_smoothscroll') );
               ...
               if ( angi_fn_user_started_with_current_version() ) {
                    $is_welcome_note_on = apply_filters(
                         'angi_is_welcome_front_notification_on',
                         false
                         //angi_fn_user_can_see_customize_notices_on_front() && ! angi_fn_is_customizing() && ! angi_fn_isprevdem() && 'dismissed' != get_transient( 'angi_welcome_note_status' )
                    );
                    if ( $is_welcome_note_on ) {
                         $welcome_note_content =  $this -> angi_fn_get_welcome_note_content();
                    }
               }
               ...
               wp_localize_script(
                    ...
                    apply_filters( 'tc_angilla_script_params' , array(
                         ...
                         'assetsPath'      => angi_fn_get_theme_file_url( ANGI_ASSETS_PREFIX . 'front/' ),

                         '_disabled'          => apply_filters( 'angi_disabled_front_js_parts', array() ),
                         'centerSliderImg'   => esc_attr( angi_fn_opt( 'tc_center_slider_img') ),

                         'isLightBoxEnabled'  => $this -> angi_fn_is_lightbox_required(),
                      ...
                      'isAnchorScrollEnabled'     => angi_fn_is_checked( 'tc_link_scroll' ), // also adds the jquery effect library if smooth scroll is enabled => easeOutExpo effect
                      ...
                      'stickyHeader'          => angi_fn_opt( 'tc_sticky_header' ),
                      ...
                      'menuStickyUserSettings' => array(
                          'desktop' => angi_fn_opt( 'tc_header_desktop_sticky' ),
                          'mobile'  => angi_fn_opt( 'tc_header_mobile_sticky' )
                      ),
                      ...
                      'isDevMode'        => ANGI_DEBUG_MODE || ANGI_DEV_MODE,
              ...
              if ( apply_filters( 'tc_holder_js_required', false ) || angi_fn_is_customizing() ) {
                  wp_enqueue_script(
                     'holder-js',
                     ANGI_FRONT_ASSETS_URL . 'js/libs/holder.min.js',
                     array(),
                     $this-> _resouces_version,
                     $in_footer = true
                  );
              ...
              if ( apply_filters('tc_load_retinajs', 1 == angi_fn_opt( 'tc_retina_support' ) ) ) {
                  wp_enqueue_script( 'retinajs', ANGI_FRONT_ASSETS_URL . 'js/libs/retina.min.js', array(), $this-> _resouces_version, $in_footer = true);
              }
              ...
              if ( apply_filters(  'angi_enqueue_placeholders_resources', false ) ) {
                  ...
                  wp_enqueue_script( 'angilla-front-placholders', ANGI_FRONT_ASSETS_URL . 'js/libs/angilla-placeholders.js', array(), $this-> _resouces_version, $in_footer = true );
              }
         }
         ...
         */
         private function angi_fn_normalize_script_args( $_handle, $_params ) {
               ...
               return array(
                     $_handle,
                     sprintf( '%1$s%2$s%3$s', ANGI_BASE_URL , $_params['path'], $_filename ),
         ...
         * @since v3.3+
         */
         private function angi_fn_is_lightbox_required() {
        ...
        function angi_fn_load_concatenated_front_scripts() {
            if ( defined( 'ANGI_DEBUG_MODE' ) && true === ANGI_DEBUG_MODE )
                return false;
        ...
        //1) angi_fn_user_started_with_current_version()
        //2) AND if the welcome note can be displayed : angi_fn_user_can_see_customize_notices_on_front() && ! angi_fn_is_customizing() && ! angi_fn_isprevdem() && 'dismissed' != get_transient( 'angi_welcome_note_status' )
        //It returns a welcome note html string that will be localized in the front js
        //@return html string
        function angi_fn_get_welcome_note_content() {
            ...
            foreach ( array( 'wptexturize', 'convert_smilies', 'wpautop') as $callback ) {
              if ( function_exists( $callback ) )
                  add_filter( 'angi_front_welcome_note_html', $callback );
            }
            ob_start();
              ?>
              <h2><?php printf( '%1$s :D' , __('Welcome in the Angilla theme', 'angilla' ) ); ?></h2>
                <?php
                    printf('<p>%1$s <a href="%2$s" target="_blank">%3$s</a> %4$s</p>',
                        __('The theme offers a wide range', 'angilla'),
                         admin_url( 'customize.php'),
                        __('of customization options', 'angilla'),
                        __('to let you create the best possible websites.', 'angilla' )
                    );
                    printf('<p>%1$s : <a href="%2$s" title="%3$s" target="_blank">%3$s <i class="fas fa-external-link-alt" aria-hidden="true"></i></a>&nbsp;,<a href="%4$s" title="%5$s" target="_blank">%5$s <i class="fas fa-external-link-alt" aria-hidden="true"></i></a></p>',
                        __("If you need inspiration, you can visit our online demos", 'angilla'),
                        esc_url(''),
                        __('Angilla Demo 1', 'angilla'),
                        esc_url('demo.angilla.com/'),
                        __('Angilla Demo 2', 'angilla')
                    );
                    printf( '<br/><br/><p>%1$s <a href="%2$s" target="_blank">%3$s <i class="fas fa-external-link-alt" aria-hidden="true"></i></a></p>',
                        __('To help you getting started with Angilla, we have published', 'angilla'),
                        esc_url(''),
                        __('a short guide here.', 'angilla')
                    );
                ?>
              <?php
            $html = ob_get_contents();
            if ($html) ob_end_clean();
            return apply_filters('angi_front_welcome_note_html', $html );
        }

        //hook : angi_ajax_dismiss_welcome_note_front
        function angi_fn_dismiss_welcome_note_front() {
            set_transient( 'angi_welcome_note_status', 'dismissed' , 60*60*24*365*20 );//20 years of peace
            wp_send_json_success( array( 'status_note' => 'dismissed' ) );
        }

   }//end of ANGI_resources_js
endif;
