...
// 网址
add_action( 'init', 'post_type_site' );
function post_type_site() {
	$labels = array(
...
// 网址分类
add_action( 'init', 'create_site_taxonomies', 0 );
function create_site_taxonomies() {
	$labels = array(
	...
	register_taxonomy( 'favorites', array( 'site' ), $args );
}
...
/**
 * 设置 site 这种自定义文章类型的固定链接结构为 ID.html 
...
add_action( 'init', 'custom_site_rewrites_init' );
function custom_site_rewrites_init(){
    add_rewrite_rule(
        'site/([0-9]+)?.html$',
...
function io_post_type_filter($post_type, $which){
    if('site' !== $post_type){ //这里为自定义文章类型，需修改
 ...
 * 文章列表添加自定义字段
 */
add_filter('manage_edit-site_columns', 'io_ordinal_manage_posts_columns');
...
function io_ordinal_manage_posts_custom_column($column_name,$id){ 
	switch( $column_name ) :
		case 'link': {
			echo get_post_meta($id, '_site_link', true);
			break;
		}
		case 'ordinal': {
			echo get_post_meta($id, '_site_order', true);
			break;
		}
		case 'visible': {
...
//文章列表添加排序规则
add_filter('manage_edit-site_sortable_columns', 'sort_site_order_column');
//add_filter('manage_edit-favorites_sortable_columns', 'sort_favorites_order_column');
add_action('pre_get_posts', 'sort_site_order');
function sort_site_order_column($defaults)
{
...
function sort_site_order($query) {
    if(!is_admin())
		return;
    $orderby = $query->get('orderby');
    if('ordinal' == $orderby) {
        $query->set('meta_key', '_site_order');
...
function io_save_quick_edit_data($post_id) {
    ...
    if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE )
        return $post_id;
    // 验证权限，'site' 为文章类型，默认为 'post' ,这里为我自定义的文章类型'site'
    ...
    if (isset($_POST['ordinal']) && ($post->post_type != 'revision')) {
        ...
        if ($left_menu_id)
			update_post_meta( $post_id, '_site_order', $left_menu_id);// ‘_site_order’为自定义字段
    } 
}
...
function ashuwp_quick_edit_javascript() {
    ...
    if (!is_object($current_screen) || ($current_screen->post_type != 'site'))return;
	if($current_screen->id == 'edit-site'){
 	echo"
