...
?>
<div class="post-navigation row <?php angi_fn_echo( 'element_class' ) ?>" <?php angi_fn_echo('element_attributes') ?>>
  <nav id="nav-below" class="col-12" role="navigation">
    <h2 class="sr-only"><?php _e('Post navigation', 'angilla') ?></h2>
    <ul class="angi_pager row flex-row flex-no-wrap">
      <li class="previous col-5 <?php echo $tprev_align_class ?>">
      <?php if ( null != $prev_link ) : ?>
        <span class="sr-only"><?php _e('Previous post', 'angilla') ?></span>
        <span class="nav-previous nav-dir"><?php echo $prev_link ?></span>
      <?php endif; ?>
      </li>
      <li class="nav-back col-2 text-center">
        <?php if ( is_single() ) : ?>
        <a href="<?php echo esc_url( ( 'page' != get_option( 'show_on_front' ) || ! get_option( 'page_for_posts' ) ) ? home_url( '/' ) : get_permalink( get_option( 'page_for_posts' ) ) ) ?>" title="<?php _e( 'Back to post list', 'angilla') ?>">
          <span><i class="icn-grid-empty"></i></span>
          <span class="sr-only"><?php _e( 'Back to post list', 'angilla') ?></span>
        </a>
        <?php endif ?>
      </li>
      <li class="next col-5 <?php echo $tnext_align_class ?>">
      <?php if ( null != $next_link ) : ?>
        <span class="sr-only"><?php _e('Next post', 'angilla') ?></span>
