...
if ( ! class_exists( 'ANGI_nav_walker' ) ) :
  class ANGI_nav_walker extends Walker_Nav_Menu {
    static $instance;
    public $angi_location;
    function __construct($_location) {
      self::$instance =& $this;
      $this -> angi_location = $_location;

      add_filter( 'angi_nav_menu_css_class' , array($this, 'angi_fn_add_bootstrap_classes'), 10, 4 );

    }


    /**
    * hook : angi_nav_menu_css_class
    */
    function angi_fn_add_bootstrap_classes($classes, $item, $args, $depth ) {
      ...
      if ( $item -> is_dropdown ) {
        if ( $depth === 0 ) {
          if ( ! in_array( 'angi-dropdown', $classes ) )
            $classes[] = 'angi-dropdown';
        } elseif ( $depth > 0 ) {
          if ( ! in_array( 'angi-dropdown-submenu', $classes ) )
            $classes[] = 'angi-dropdown-submenu';
        }
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
      ...
      //since WP 3.6.0
      add_filter( 'nav_menu_link_attributes' , 'angi_fn_add_nav_link_class', 10, 4 );
      ...
      //since WP 3.6.0
      remove_filter( 'nav_menu_link_attributes' , 'angi_fn_add_nav_link_class', 10, 4 );

      if ( $item->is_dropdown ) {
        $item_html = angi_fn_maybe_add_dropdown_html( $item_html, $args->dropdown_on );
    ...
    function display_element( $element, &$children_elements, $max_depth, $depth = 0, $args, &$output) {
      ...
      $element->classes = apply_filters( 'angi_nav_menu_css_class', array_filter( empty( $element->classes) ? array() : (array)$element->classes ), $element, $args, $depth );
...
if ( ! class_exists( 'ANGI_nav_walker_page' ) ) :
  class ANGI_nav_walker_page extends Walker_Page {
    ...
    function __construct() {
      add_filter( 'page_css_class' , array($this, 'angi_fn_add_bootstrap_classes'), 10, 4 );
    ...
    * hook : page_css_class
    */
    function angi_fn_add_bootstrap_classes($classes, $page = null, $depth = 0, $args = array() ) {

      /* Scope the changes only to Angilla fallback page menu!! */
      if ( ! ( isset( $args['fallback_cb'] ) && isset( $args['fallback_cb'][1] ) && 'angi_fn_page_menu' == $args['fallback_cb'][1] ) )
        return $classes;
    ...
    function start_el(&$output, $page, $depth = 0, $args = array(), $current_page = 0) {
      ...
      //since WP 4.8.0
      add_filter( 'page_menu_link_attributes' , 'angi_fn_add_nav_link_class', 10, 4 );
      ...
      remove_filter( 'page_menu_link_attributes' , 'angi_fn_add_nav_link_class', 10, 4 );
...
function angi_fn_maybe_add_dropdown_html( $item_html, $dropdown_on ) {
    ...
    switch( $dropdown_on ) {
      ...
      case 'link-action' :
                $item_html = str_replace(
                  array( '<a', '</a>' ),
                  array(
                    '<a data-toggle="angi-dropdown" aria-haspopup="true" aria-expanded="false"',
      ...
      case 'caret-click' :
                $item_html = str_replace(
                  ...
                  array(
                    ...
                    '</a><button data-toggle="angi-dropdown" aria-haspopup="true" aria-expanded="false" class="caret__dropdown-toggler angi-btn-link"><i class="icn-down-small"></i></button></span>'
                  ),
                  $item_html
                );
                break;
...
function angi_fn_add_nav_link_class( $atts, $item, $args, $depth) {
    if ( !is_array( $atts ) )
      return $atts;
