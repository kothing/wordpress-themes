...
function io_contribute(){  
	...
	//表单变量初始化
	$site_link = isset( $_POST['tougao_site_link'] ) ? trim(htmlspecialchars($_POST['tougao_site_link'], ENT_QUOTES)) : '';
	$site_sescribe = isset( $_POST['tougao_site_sescribe'] ) ? trim(htmlspecialchars($_POST['tougao_site_sescribe'], ENT_QUOTES)) : '';
	$title = isset( $_POST['tougao_title'] ) ? trim(htmlspecialchars($_POST['tougao_title'], ENT_QUOTES)) : '';
	$category = isset( $_POST['tougao_cat'] ) ? $_POST['tougao_cat'] : '0';
	$site_ico = isset( $_POST['tougao_site_ico'] ) ? trim(htmlspecialchars($_POST['tougao_site_ico'], ENT_QUOTES)) : '';
	...
	if ( empty($site_sescribe) || mb_strlen($site_sescribe) > 50 ) {
		error('{"status":4,"msg":"'.__('网站描述必须填写，且长度不得超过50字。','i_theme').'"}');
	}
	if ( empty($site_link) && empty($wechat_qr) ){
		error('{"status":3,"msg":"'.__('网站链接和公众号二维码至少填一项。','i_theme').'"}');
	}
	elseif ( !empty($site_link) && !preg_match('/http(s)?:\/\/[\w.]+[\w\/]*[\w.]*\??[\w=&\+\%]*/is', $site_link)) {
	...
	if ($status != 0){
		global $wpdb;
		add_post_meta($status, '_site_sescribe', $site_sescribe);
		add_post_meta($status, '_site_link', $site_link);
		add_post_meta($status, '_site_order', '0');
		if( !empty($site_ico))
			add_post_meta($status, '_thumbnail', $site_ico); 
		if( !empty($wechat_qr))
