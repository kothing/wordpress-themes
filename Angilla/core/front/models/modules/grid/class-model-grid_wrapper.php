<?php
class ANGI_grid_wrapper_model_class extends ANGI_Model {
  public $expanded_sticky;
  public $id_base = 'angi_grid';
  ...
  */
  function angi_fn_get_preset_model() {
    ...
    $_preset = array(
      'grid_columns'             => esc_attr( angi_fn_opt( 'tc_grid_columns') ),
      'grid_title_num_words'     => esc_attr( angi_fn_opt( 'tc_grid_num_words') ),
      'grid_icons'               => esc_attr( angi_fn_opt( 'tc_grid_icons') ),
      'grid_expand_featured'     => esc_attr( angi_fn_opt( 'tc_grid_expand_featured') ),
      'show_thumb'               => esc_attr( angi_fn_opt( 'tc_post_list_show_thumb' ) ),
      'grid_bottom_border'       => esc_attr( angi_fn_opt( 'tc_grid_bottom_border') ),
      'grid_shadow'              => esc_attr( angi_fn_opt( 'tc_grid_shadow') ),
      'grid_hover_move'          => true,
      'grid_thumb_shape'         => esc_attr( angi_fn_opt( 'tc_grid_thumb_shape') ),
      'use_thumb_placeholder'    => esc_attr( angi_fn_opt( 'tc_post_list_thumb_placeholder' ) ),
      'excerpt_length'           => esc_attr( angi_fn_opt( 'tc_post_list_excerpt_length' ) ),
      'wrapped'                  => true,
      'masonry'                  => false,
      'contained'                => false,
      'title_in_caption_below'   => true,
      'content_wrapper_breadth'  => angi_fn_get_content_breadth(),
  ...
  */
  function angi_fn_extend_params( $model = array() ) {
    $model                              = parent::angi_fn_extend_params( $model );
    //layout dependency
    ...
    $this -> queried_id                 = angi_fn_get_id();
  ...
  */
  function angi_fn_reset_late_properties() {
    if ( angi_fn_is_loop_end() )
      //all post lists do this
      $this -> angi_fn_reset_text_hooks();
  }


  function angi_fn_get_grid_section_class() {
    $section_class = array( 'grid__section', sprintf( "cols-%s", $this -> angi_fn_get_section_cols() ) );
  ...
  function angi_fn_get_print_start_wrapper() {
    return $this -> wrapped && angi_fn_is_loop_start();
  }

  function angi_fn_get_print_end_wrapper() {
    return $this -> wrapped && angi_fn_is_loop_end();
  }


  function angi_fn_get_is_first_of_grid() {
    global $wp_query;
  ...
  function angi_fn_get_is_last_of_grid() {
    global $wp_query;
  ...
  function angi_fn_get_grid_cols() {
    if ( ! isset( $this -> grid_cols ) )
      $grid_cols = $this -> angi_fn_set_grid_cols( $this -> grid_columns, angi_fn_get_layout( $this -> queried_id , 'class' ) );
    else
      $grid_cols = $this -> grid_cols;

    return apply_filters( 'angi_get_grid_cols', $grid_cols );
  }



  /* returns articles wrapper section columns */
  public function angi_fn_get_section_cols() {
    return apply_filters( 'angi_grid_section_cols',
  ...
  */
  protected function angi_fn_force_current_post_expansion(){
    $is_expanded = $this->angi_fn_is_sticky_expanded();

    //set expanded sticky flag
    if ( ! isset( $this -> expanded_sticky ) )
      $this -> angi_fn_set_property( 'expanded_sticky', $is_expanded );
    return $is_expanded;
  }

  protected function angi_fn_is_sticky_expanded() {
    global $wp_query;
    return $this -> angi_fn_maybe_has_sticky_expanded() && 0 == $wp_query -> current_post && get_query_var( 'paged' ) < 2 && is_sticky();
  }
  ...
  *******************************/
  ...
  function angi_fn_get_grid_item() {
    $section_cols           = $this -> angi_fn_get_section_cols();

    $is_expanded            = $this -> angi_fn_force_current_post_expansion();

    $text                   = $this -> angi_fn_get_grid_item_text();

    //thumb
    $thumb_properties       = $this -> angi_fn_get_grid_item_thumb_properties( $section_cols );
    ...
    //figure class
    $figure_class           = $this -> angi_fn_get_grid_item_figure_class( $has_thumb, $section_cols, $is_expanded );

    //array
    $icon_visibility        = $this -> angi_fn_get_grid_item_icon_visibility();

    $title                  = $this -> angi_fn_get_grid_item_title( get_the_title(), $is_expanded );

    $has_title_in_caption   = $this -> angi_fn_grid_item_has_title_in_caption( $is_expanded );
    $title_in_caption_below = $this -> title_in_caption_below;

    $has_edit_above_thumb   = $this -> angi_fn_grid_item_has_edit_above_thumb( $is_expanded );

    $has_fade_expt          = $this -> angi_fn_grid_item_has_fade_expt( $is_expanded, $thumb_img );

    $article_selectors      = $this -> angi_fn_get_grid_item_article_selectors( $section_cols, $is_expanded );

    $use_thumb_placeholder  = $this -> use_thumb_placeholder;


    //various depending on whether is expanded
    $entry_summary_class    = $this -> angi_fn_get_grid_item_entry_summary_class($is_expanded);
    $gcont_class            = $this -> angi_fn_get_grid_item_gcont_class($is_expanded);
  ...
  function angi_fn_get_grid_item_entry_summary_class( $is_expanded ) {
    return $is_expanded ? 'angi-talign' : '';
  }

  function angi_fn_get_grid_item_gcont_class( $is_expanded ) {
    return ! $is_expanded ? 'angi-talign' : '';
  }

  function angi_fn_get_grid_item_text() {
  ...
  * has edit in caption
  */
  function angi_fn_grid_item_has_edit_above_thumb( $is_expanded ) {
    return $is_expanded;
  }
  ...
  */
  function angi_fn_grid_item_has_title_in_caption( $is_expanded ) {
    return $is_expanded;
  }
  ...
  * has fade expt
  */
  function angi_fn_grid_item_has_fade_expt( $is_expanded, $thumb_img ) {
  ...
  * @return string
  */
  function angi_fn_get_grid_item_title( $_title, $is_expanded ) {
  ...
  * thumb properties
  */
  function angi_fn_get_grid_item_thumb_properties( $section_cols ) {
      $has_thumb           = $this -> angi_fn_show_thumb();
      $thumb_img           = '';

      if ( $has_thumb ) {
          $thumb_model                   = angi_fn_get_thumbnail_model( array(
              'requested_size'              => $this -> angi_fn_get_thumb_size_name( $section_cols ),
              'filtered_thumb_size_name'    => $this -> angi_fn_get_filtered_thumb_size_name( $section_cols ),
        ...
        $thumb_img              = apply_filters( 'angi-grid-thumb-img', $thumb_model[ 'tc_thumb' ], angi_fn_get_id() );
      }
  ...
  * figure class
  */
  function angi_fn_get_grid_item_figure_class( $has_thumb, $section_cols, $is_expanded ) {
      ...
      $figure_class[]     = $this -> angi_fn_get_grid_figure_aspect_ratio_class( $section_cols );
  ...
  * @return array
  */
  function angi_fn_get_grid_item_icon_visibility() {
  ...
  function angi_fn_get_grid_item_article_selectors( $section_cols, $is_expanded ) {
    if ( apply_filters( 'angi_grid_add_expanded_class', $is_expanded ) )
    ...
    return angi_fn_get_the_post_list_article_selectors( $post_class, $id_suffix );
  ...
  */
  function angi_fn_get_thumb_size_name( $section_cols ){
    //layout dependency
  ...
  */
  function angi_fn_get_grid_figure_aspect_ratio_class( $section_cols ){
    //layout dependency
    return ( 1 == $section_cols && 'narrow' != $this->content_wrapper_breadth ) ? 'angi__r-wGOC' : 'angi__r-wGR';
  }
  ...
  */
  function angi_fn_get_filtered_thumb_size_name( $section_cols ){
    return ( 1 == $section_cols ) ? 'tc_ws_thumb_size' : 'tc_grid_size';
  }


  protected function angi_fn_show_thumb() {
  ...
  */
  protected function angi_fn_maybe_has_sticky_expanded(){
    global $wp_query;
    ...
    return apply_filters( 'angi_grid_expand_featured', $this -> grid_expand_featured );
  }
  ...
  */
  function angi_fn_get_element_class() {
    $_classes = array();
  ...
  */
  function angi_fn_set_grid_cols( $_col_nb, $_current_layout ) {
  ...
  */
  protected function angi_fn_get_grid_font_css( $_col_nb = '3' ) {
  ...
  */
  protected function angi_fn_get_grid_font_sizes( $_col_nb = '3', $_requested_media_size = null ) {
    $_col_media_matrix = apply_filters( 'angi_grid_font_matrix' , array(
  ...
  */
  protected function angi_fn_get_grid_font_ratios( $_size = 'xl' , $_sel = 'h' ) {
    $_ratios =  apply_filters( 'angi_get_grid_font_ratios' , array(
  ...
  */
  protected function angi_fn_grid_assign_css_rules_to_selectors( $_media_query, $_css_prop, $_col_nb ) {
    $_css = '';
    //Add one column font rules if there's a sticky post
    if ( $this -> angi_fn_maybe_has_sticky_expanded() || '1' == $_col_nb ) {
  ...
  */
  protected function angi_fn_grid_build_css_rules( $_size = 'xl', $_wot = 'h' ) {
    $_lh_ratio = apply_filters( 'angi_grid_line_height_ratio' , 1.55 ); //line-height / font-size
  ...
  */
  protected function angi_fn_get_grid_media_queries() {
    return apply_filters( 'angi_grid_media_queries' ,  array(
  ...
  */
  function angi_fn_user_options_style_cb( $_css ){
      ...
      $selector                      = '.grid-container__classic .angi__r-wGOC::before';
      ...
      */
      ...
      $media_queries      = $this -> angi_fn_get_grid_media_queries();
