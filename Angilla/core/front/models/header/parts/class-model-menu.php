<?php
class ANGI_menu_model_class extends ANGI_Model {
    ...
    protected $angi_menu_location; //can be sidenav, topbar, primary_navbar, secondary_navbar, mobile
    /**
    * @override
    ...
    */
    function angi_fn_get_preset_model() {
      ...
      $_preset = array(
          ...
          'menu_class'          => array(),
          'fallback_cb'         => angi_fn_isprevdem() ? array( $this, 'angi_fn_page_menu' ) : 'angi_fn_print_add_menu_button',
          'walker'              => ''
      );
    ...
    public function __construct( $model ) {
        ...
        //set properties depending on the angi_menu_location
        switch ( $this->angi_menu_location ) {

            case 'primary_navbar' :

                $_menu_position = esc_attr( angi_fn_opt( 'tc_menu_position') );
                ...
                $_model = array(
                    ...
                    'dropdown_type'       => $this->dropdown_type ? $this->dropdown_type : esc_attr( angi_fn_opt( 'tc_menu_type' ) ),
            ...
            case 'secondary_navbar' :
                ...
                $_menu_position = esc_attr( angi_fn_opt( 'tc_second_menu_position') );
            ...
            case 'sidenav' :
                ...
                if ( !$this->dropdown_type ) {
                    $_model[ 'dropdown_type' ] = 1 == esc_attr( angi_fn_opt( 'tc_side_menu_dropdown_on_click' ) ) ? 'click' : '';
                } else {
            ...
            case 'mobile' :
                ...
                $mobile_menu_opt = angi_fn_opt( 'tc_header_mobile_menu_layout' );
                ...
                if ( !$this->dropdown_type ) {
                    $_model[ 'dropdown_type' ] = 1 == esc_attr( angi_fn_opt( 'tc_header_mobile_menu_dropdown_on_click' ) ) ? 'click' : '';
                } else {
                    $_model[ 'dropdown_type' ] = $this->dropdown_type;
                }
    ...
    */
    /*
    * setup properties just before the rendering, to take in account params passed through angi_fn_render_template()
    */
    function angi_fn_setup_late_properties() {

        $walker     = ! angi_fn_has_location_menu( $this -> theme_location ) ? $this->walker : new ANGI_nav_walker( $this -> theme_location );
        ...
        $menu_class = angi_fn_stringify_array( array_merge( $menu_class, $this->def_menu_class ) );
        ...
        $_submenu_opening_class = 'click' == $this -> dropdown_type ? 'angi-open-on-click' : 'angi-open-on-hover';

        $_submenu_opening_class = ! $this->dropdown_type ? '' : $_submenu_opening_class;

        $element_class[]        = $_submenu_opening_class;

        $element_class          = angi_fn_stringify_array( $element_class );


        $this->angi_fn_update( compact( 'walker', 'menu_class', 'element_class' ) );
    }
