...
if ( have_posts() ) {
    do_action( '__loop_start' );
      ...
      //angi_fn_render_template(
          ...
          // )
          //array( 'model_args' => angi_fn_get_main_content_loop_item() )
      //);
      ...
      $loop_item_tmpl = apply_filters( 'angi_page_loop_item_template', 'content/singular/page_content');
      ...
      if ( angi_fn_is_list_of_posts() ) {
          ...
          if ( angi_fn_is_registered_or_possible('post_list') ) {
                $loop_item_tmpl = 'content/post-lists/post_list_alternate';
                $loop_item_model = array();
          
          } elseif ( angi_fn_is_registered_or_possible('post_list_plain') ) {
                $loop_item_tmpl = 'content/post-lists/post_list_plain';
                $loop_item_model = array();
                //$to_render = array( 'loop_item' => array('content/post-lists/post_list_plain' ));
          }
          $loop_item_tmpl = apply_filters( 'angi_post_list_loop_item_template', $loop_item_tmpl);
          $loop_item_model = apply_filters( 'angi_post_list_loop_item_model', $loop_item_model );

      } elseif ( is_single() ) {
          if ( angi_fn_is_registered_or_possible( 'attachment_image' ) ) {
                $loop_item_tmpl = apply_filters( 'angi_single_attachment_loop_item_template', 'content/singular/attachment_image_content');
          } else {
                $loop_item_tmpl = apply_filters( 'angi_single_loop_item_template', 'content/singular/post_content');  
          }          
      }

      $loop_item = apply_filters( "angi_main_content_loop_item", array(
      ...
      while ( have_posts() ) {
          ...
          //angi_fn_render_template takes 2 params :
