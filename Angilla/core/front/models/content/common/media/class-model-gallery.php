<?php
class ANGI_gallery_model_class extends ANGI_Model {
      ...
      */
      function angi_fn_setup_late_properties() {
            ...
            $this -> angi_fn__setup_the_gallery_items();
            $this -> angi_fn__setup_the_carousel_inner_attributes();
      }
      ...
      protected function angi_fn__set_raw_media() {
            ...
            $this -> angi_fn_set_property( 'media', $this->angi_fn__get_post_gallery() );
      ...
      protected function angi_fn__setup_the_gallery_items() {

            $this -> angi_fn_set_property( 'gallery_items', $this->angi_fn__get_the_gallery_items() );

      }


      protected function angi_fn__setup_the_carousel_inner_attributes() {

            $this -> angi_fn_set_property( 'carousel_inner_attributes', $this->angi_fn__get_the_carousel_inner_attributes() );

      }


      protected function angi_fn__get_the_gallery_items() {
            ...
            if ( angi_fn_is_checked( 'tc_slider_img_smart_load' ) ) {
                add_filter( 'wp_get_attachment_image_attributes', array( $this, 'angi_fn_setup_img_for_smartload'), 999 );
            }
            foreach ( array_keys( $raw_media ) as $id ) {
                  ...
                  if ( angi_fn_is_checked( 'tc_slider_img_smart_load' ) ) {
                      $gallery_items[] = angi_fn_parse_imgs( $img_html );//<- to prepare the img smartload without using the filter 'angi_thumb_html'  ( not declared if smartload not globally enabled )
                  } else {
                      $gallery_items[] = $img_html;
                  }
            }
            if ( angi_fn_is_checked( 'tc_slider_img_smart_load' ) ) {
                remove_filter( 'wp_get_attachment_image_attributes', array( $this, 'angi_fn_setup_img_for_smartload'), 999 );
            }
            return $gallery_items;

      }

      public function angi_fn__get_the_carousel_inner_attributes() {
            $atts = array();
      ...
      * // => That's why we need to deactivate the front js part with the flag and control it here afterwards
      ...
      function angi_fn_setup_img_for_smartload( $attr ) {
      ...
      protected function angi_fn__get_post_gallery() {
            ...
            if ( preg_match_all( '/' . get_shortcode_regex() . '/s', $post->post_content, $matches, PREG_SET_ORDER ) ) {
                  foreach ( $matches as $shortcode ) {
                        if ( 'gallery' === $shortcode[2] ) {
                              ...
                              $post_gallery = ANGI_gallery::$instance->angi_fn_angi_gallery( $post_gallery, $shortcode_attrs, '' );
                              break;
