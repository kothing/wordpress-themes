//extends api.ANGIDynModule
var ANGISocialModuleMths = ANGISocialModuleMths || {};
( function ( api, $, _ ) {
$.extend( ANGISocialModuleMths, {
      initialize: function( id, options ) {
              ...
              api.ANGIDynModule.prototype.initialize.call( module, id, options );

              //extend the module with new template Selectors
              $.extend( module, {
                    itemPreAddEl : '',/// 'angi-module-social-pre-add-view-content',
                    itemInputList : '',// 'angi-module-social-item-content',
                    modOptInputList : ''//angi-module-social-mod-opt'
              } );
              ...
              module.inputConstructor = api.ANGIInput.extend( module.ANGISocialsInputMths || {} );
              //EXTEND THE DEFAULT CONSTRUCTORS FOR MONOMODEL
              module.itemConstructor = api.ANGIItem.extend( module.ANGISocialsItem || {} );
      ...
      updateItemModel : function( item_instance, is_preItem ) {
              ...
              if ( ! is_preItem && item.angi_Input.has( 'social-color' ) )
                _new_color = item.angi_Input('social-color')();
              ...
              if ( is_preItem ) {
                    ...
                    item.angi_Input('title').set( _new_title );
                    //item.angi_Input('social-link').set( '' );
                    if ( item.angi_Input('social-color') ) { //optional
                      item.angi_Input('social-color').set( _new_color );
                    }
              }
      },
      ...
      getTitleFromIcon : function( icon ) {
              return api.ANGI_Helpers.capitalize( icon.replace('fa-', '').replace('envelope', 'email') );
      },
      ...
      ANGISocialsInputMths : {
              setupSelect : function() {
                    ...
                    _.each( socialList , function( icon_name, k ) {
                          ...
                          $( 'select[data-angitype="social-icon"]', input.container ).append( $('<option>', _attributes) );
                    });
                    ...
                    //fire select2
                    $( 'select[data-angitype="social-icon"]', input.container ).angiSelect2( {
            ...
            setupColorPicker : function( obj ) {
                    var input      = this,
                        item       = input.input_parent,
                        module     = input.module,
                        $el        = $( 'input[data-angitype="social-color"]', input.container );
      ...
      ANGISocialsItem : {
              ...
              ready : function() {
                    var item = this;
                    api.ANGIItem.prototype.ready.call( item );
              ...
              _buildTitle : function( title, icon, color ) {
                      ...
                      title = title || ( 'string' === typeof(icon) ? api.ANGI_Helpers.capitalize( icon.replace( 'fa-', '') ) : '' );
                      title = api.ANGI_Helpers.truncate(title, 20);
...
(function ( api, $ ) {
      ...
      api.angiModuleMap = api.angiModuleMap || {};
      $.extend( api.angiModuleMap, {
            angi_social_module : {
                  mthds : ANGISocialModuleMths,
                  crud : true,
                  name : 'Social Icons',
                  has_mod_opt : true
            },
